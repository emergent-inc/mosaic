...
func TestTmuxWaitForSignalPath(t *testing.T) {
	...
	if !strings.HasPrefix(path, "/tmp/mosaic-wait-for-") {
...
func TestTmuxDisplayReporterFormatFields(t *testing.T) {
	origHome := os.Getenv("HOME")
	origWorkspace := os.Getenv("MOSAIC_WORKSPACE_ID")
	origSurface := os.Getenv("MOSAIC_SURFACE_ID")
	origPane := os.Getenv("TMUX_PANE")
	os.Setenv("HOME", t.TempDir())
	os.Setenv("MOSAIC_WORKSPACE_ID", "workspace:1")
	os.Setenv("MOSAIC_SURFACE_ID", "surface:1")
	leaderPaneToken := "%" + tmuxStableNumericId("33333333-3333-4333-8333-333333333333")
	os.Setenv("TMUX_PANE", leaderPaneToken)
	defer func() {
		os.Setenv("HOME", origHome)
		if origWorkspace != "" {
			os.Setenv("MOSAIC_WORKSPACE_ID", origWorkspace)
		} else {
			os.Unsetenv("MOSAIC_WORKSPACE_ID")
		}
		if origSurface != "" {
			os.Setenv("MOSAIC_SURFACE_ID", origSurface)
		} else {
			os.Unsetenv("MOSAIC_SURFACE_ID")
		}
		if origPane != "" {
	...
	if values["session_name"] != "mosaic" {
		t.Fatalf("session_name = %q, want mosaic", values["session_name"])
	}
...
func TestConfigureAgentEnvironment(t *testing.T) {
	...
	envKeys := []string{
		"MOSAIC_CLAUDE_TEAMS_MOSAIC_BIN", "PATH", "TMUX", "TMUX_PANE",
		"TERM", "MOSAIC_SOCKET_PATH", "TERM_PROGRAM",
		"MOSAIC_WORKSPACE_ID", "MOSAIC_SURFACE_ID",
	...
	// Verify socket path
	if os.Getenv("MOSAIC_SOCKET_PATH") != "127.0.0.1:54321" {
		t.Errorf("MOSAIC_SOCKET_PATH = %q", os.Getenv("MOSAIC_SOCKET_PATH"))
	}
	...
	if os.Getenv("MOSAIC_WORKSPACE_ID") != "ws-abc" {
		t.Errorf("MOSAIC_WORKSPACE_ID = %q", os.Getenv("MOSAIC_WORKSPACE_ID"))
	}
	if os.Getenv("MOSAIC_SURFACE_ID") != "surf-789" {
		t.Errorf("MOSAIC_SURFACE_ID = %q", os.Getenv("MOSAIC_SURFACE_ID"))
	}
	// Verify extra env
