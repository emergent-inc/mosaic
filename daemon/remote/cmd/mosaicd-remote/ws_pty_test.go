...
// for https://github.com/emergent-inc/mosaic/issues/5185, where the failure
...
func TestWebSocketPTYRequiresSessionMatchAndConsumesLeaseOnce(t *testing.T) {
	...
	writeTestLease(t, leasePath, "mosaic-secret", "sess-good", true, time.Now().Add(time.Minute))
	conn := dialPTY(t, ctx, server.URL)
	sendAuth(t, ctx, conn, "mosaic-secret", "sess-other", 80, 24)
	_, _, err := conn.Read(ctx)
	if websocket.CloseStatus(err) != websocket.StatusPolicyViolation {
	...
	conn = dialPTY(t, ctx, server.URL)
	sendAuth(t, ctx, conn, "mosaic-secret", "sess-good", 100, 30)
...
func TestWebSocketPTYRunsShellOverBinaryFrames(t *testing.T) {
	...
	output := waitForBinaryContains(t, ctx, conn, "MOSAIC_WS_OK", 15*time.Second)
...
func TestWebSocketPTYReconnectKeepsSessionProcess(t *testing.T) {
	...
	if err := conn.Write(ctx, websocket.MessageBinary, []byte("MOSAIC_RECONNECT_MARKER=alive; export MOSAIC_RECONNECT_MARKER; printf 'first-ready\\n'\r")); err != nil {
...
func TestWebSocketPTYStressSessionCleanupAndBoundedScrollback(t *testing.T) {
	...
	for i := 0; i < 25; i++ {
		...
		waitForBinaryContainsLabel(t, ctx, conn, "stress session "+sessionID+" hold marker", "MOSAIC_HOLD", 10*time.Second)
...
func TestWebSocketPTYAnonymousAttachesAreIsolated(t *testing.T) {
	...
	readReady(t, ctx, a)
	if err := a.Write(ctx, websocket.MessageBinary, []byte("MOSAIC_ANON_MARK=one; export MOSAIC_ANON_MARK; printf 'A_READY\\n'\r")); err != nil {
...
func TestWebSocketPTYAnonymousSessionKeyCannotBeForged(t *testing.T) {
	...
	if err := anon.Write(ctx, websocket.MessageBinary, []byte("MOSAIC_FORGE_MARK=anon; export MOSAIC_FORGE_MARK; printf 'ANON_FORGE_READY\\n'\r")); err != nil {
...
func TestWebSocketPTYAttachmentWithoutSessionIDIsAnonymous(t *testing.T) {
	...
	readReady(t, ctx, a)
	if err := a.Write(ctx, websocket.MessageBinary, []byte("MOSAIC_NO_SESSION_MARK=one; export MOSAIC_NO_SESSION_MARK; printf 'NO_SESSION_A_READY\\n'\r")); err != nil {
...
func TestWebSocketPTYSeedsUTF8LocaleAndTerminalEnv(t *testing.T) {
	...
	command := "printf '%s\\n' \"$LANG|$LC_CTYPE|$LC_ALL|$TERM|$COLORTERM|$TERM_PROGRAM|$MOSAIC_REMOTE_TRANSPORT\"; locale charmap; exit\r"
