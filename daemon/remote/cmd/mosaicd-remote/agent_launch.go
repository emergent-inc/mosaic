...
const claudeNodeOptionsRestoreModuleScript = `const hadOriginalNodeOptions = process.env.MOSAIC_ORIGINAL_NODE_OPTIONS_PRESENT === "1";
if (hadOriginalNodeOptions) {
  process.env.NODE_OPTIONS = process.env.MOSAIC_ORIGINAL_NODE_OPTIONS ?? "";
} else {
  delete process.env.NODE_OPTIONS;
}
delete process.env.MOSAIC_ORIGINAL_NODE_OPTIONS;
delete process.env.MOSAIC_ORIGINAL_NODE_OPTIONS_PRESENT;
`

// runClaudeTeamsRelay implements `mosaic claude-teams` on the remote side.
...
func runClaudeTeamsRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "mosaic claude-teams: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	configureAgentEnvironment(agentConfig{
		...
		focused:        focused,
		tmuxPathPrefix: "mosaic-claude-teams",
		mosaicBinEnvVar:  "MOSAIC_CLAUDE_TEAMS_MOSAIC_BIN",
		termEnvVar:     "MOSAIC_CLAUDE_TEAMS_TERM",
	...
	if claudePath == "" {
		fmt.Fprintf(os.Stderr, "mosaic claude-teams: claude not found in PATH\n")
		return 1
	}
	argv := append([]string{claudePath}, launchArgs...)
	execErr := syscall.Exec(claudePath, argv, os.Environ())
	fmt.Fprintf(os.Stderr, "mosaic claude-teams: exec failed: %v\n", execErr)
	return 1
}

// runOMORelay implements `mosaic omo` on the remote side.
func runOMORelay(socketPath string, args []string, refreshAddr func() string) int {
	rc := &rpcContext{socketPath: socketPath, refreshAddr: refreshAddr}

	shimDir, err := createOMOShimDir()
	if err != nil {
		fmt.Fprintf(os.Stderr, "mosaic omo: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if opencodePath == "" {
		fmt.Fprintf(os.Stderr, "mosaic omo: opencode not found in PATH\n"+
	...
	if err := omoEnsurePlugin(originalPath); err != nil {
		fmt.Fprintf(os.Stderr, "mosaic omo: plugin setup: %v\n", err)
		return 1
	}
	...
	configureAgentEnvironment(agentConfig{
		...
		socketPath:     socketPath,
		focused:        focused,
		tmuxPathPrefix: "mosaic-omo",
		mosaicBinEnvVar:  "MOSAIC_OMO_MOSAIC_BIN",
		termEnvVar:     "MOSAIC_OMO_TERM",
		extraEnv:       map[string]string{},
	})
	...
	fmt.Fprintf(os.Stderr, "mosaic omo: exec failed: %v\n", execErr)
	return 1
}

// runOMXRelay implements `mosaic omx` on the remote side.
func runOMXRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "mosaic omx: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if omxPath == "" {
		fmt.Fprintf(os.Stderr, "mosaic omx: omx not found in PATH\n"+
	...
	configureAgentEnvironment(agentConfig{
		...
		socketPath:     socketPath,
		focused:        focused,
		tmuxPathPrefix: "mosaic-omx",
		mosaicBinEnvVar:  "MOSAIC_OMX_MOSAIC_BIN",
		termEnvVar:     "MOSAIC_OMX_TERM",
		extraEnv:       map[string]string{},
	})

	launchPath, launchArgv := resolveNodeScriptExec(omxPath, args, originalPath, shimDir)
	execErr := syscall.Exec(launchPath, launchArgv, os.Environ())
	fmt.Fprintf(os.Stderr, "mosaic omx: exec failed: %v\n", execErr)
	return 1
}

// runOMCRelay implements `mosaic omc` on the remote side.
func runOMCRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "mosaic omc: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if omcPath == "" {
		fmt.Fprintf(os.Stderr, "mosaic omc: omc not found in PATH\n"+
	...
	configureAgentEnvironment(agentConfig{
		...
		socketPath:     socketPath,
		focused:        focused,
		tmuxPathPrefix: "mosaic-omc",
		mosaicBinEnvVar:  "MOSAIC_OMC_MOSAIC_BIN",
		termEnvVar:     "MOSAIC_OMC_TERM",
		extraEnv:       map[string]string{},
	})
	...
	if restoreModulePath, err := ensureClaudeNodeOptionsRestoreModule(); err == nil {
		...
		fmt.Fprintf(os.Stderr, "mosaic omc: warning: failed to create NODE_OPTIONS restore module: %v\n", err)
	}

	launchPath, launchArgv := resolveNodeScriptExec(omcPath, args, originalPath, shimDir)
	execErr := syscall.Exec(launchPath, launchArgv, os.Environ())
	fmt.Fprintf(os.Stderr, "mosaic omc: exec failed: %v\n", execErr)
	return 1
}
...
set -euo pipefail
exec "${MOSAIC_CLAUDE_TEAMS_MOSAIC_BIN:-mosaic}" __tmux-compat "$@"
`
...
exec "${MOSAIC_OMO_MOSAIC_BIN:-mosaic}" __tmux-compat "$@"
`
...
exec "${MOSAIC_OMX_MOSAIC_BIN:-mosaic}" __tmux-compat "$@"
`
...
exec "${MOSAIC_OMC_MOSAIC_BIN:-mosaic}" __tmux-compat "$@"
`

const omoNotifierShimScript = `#!/usr/bin/env bash
# Intercept terminal-notifier calls and route through mosaic notify.
TITLE="" BODY=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
exec "${MOSAIC_OMO_MOSAIC_BIN:-mosaic}" notify --title "${TITLE:-OpenCode}" --body "${BODY:-}"
`
...
func createTmuxShimDir(dirName string, tmuxScript string) (string, error) {
	...
	dir := filepath.Join(home, ".mosaicterm", dirName)
...
func ensureClaudeNodeOptionsRestoreModule() (string, error) {
	dir := filepath.Join(os.TempDir(), "mosaic-claude-node-options")
	if err := os.MkdirAll(dir, 0755); err != nil {
		return "", err
	}
...
func configureClaudeNodeOptions(restoreModulePath string) {
	...
	if hadExisting {
		os.Setenv("MOSAIC_ORIGINAL_NODE_OPTIONS_PRESENT", "1")
		os.Setenv("MOSAIC_ORIGINAL_NODE_OPTIONS", existing)
	} else {
		os.Setenv("MOSAIC_ORIGINAL_NODE_OPTIONS_PRESENT", "0")
		os.Unsetenv("MOSAIC_ORIGINAL_NODE_OPTIONS")
	}
...
func configureAgentEnvironment(cfg agentConfig) {
	...
	os.Setenv(cfg.mosaicBinEnvVar, selfPath)
	...
	// Socket path
	os.Setenv("MOSAIC_SOCKET_PATH", cfg.socketPath)
	os.Unsetenv("MOSAIC_SOCKET")
	...
	if cfg.focused != nil {
		os.Setenv("MOSAIC_WORKSPACE_ID", cfg.focused.workspaceId)
		if cfg.focused.surfaceId != "" {
			os.Setenv("MOSAIC_SURFACE_ID", cfg.focused.surfaceId)
		}
	}
...
func omoShadowConfigDir() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".mosaicterm", "omo-config")
}
