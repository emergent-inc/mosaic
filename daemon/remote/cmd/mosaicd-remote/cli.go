...
// runCLI is the entry point for the "cli" subcommand (or busybox "mosaic" invocation).
func runCLI(args []string) int {
	socketPath := os.Getenv("MOSAIC_SOCKET_PATH")
	...
	for i := 0; i < len(args); i++ {
		...
		case "--socket":
			if i+1 >= len(args) {
				fmt.Fprintln(os.Stderr, "mosaic: --socket requires a path")
				return 2
			}
			socketPath = args[i+1]
			i++
...
doneFlags:
	...
	if socketPath == "" {
		fmt.Fprintln(os.Stderr, "mosaic: MOSAIC_SOCKET_PATH not set and --socket not provided")
		return 1
	}
	...
	if cmdName == "workspace" {
		...
		fmt.Fprintln(os.Stderr, "mosaic workspace: only the \"group\" subcommand is supported here. Use list-workspaces, new-workspace, close-workspace, or select-workspace for workspace operations.")
		return 2
	}
	...
	if !ok {
		fmt.Fprintf(os.Stderr, "mosaic: unknown command %q\n", cmdName)
		return 2
	}
	...
	default:
		fmt.Fprintf(os.Stderr, "mosaic: internal error: unknown protocol for %q\n", cmdName)
		return 1
	}
}
...
func runRPC(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "mosaic rpc: requires a method name")
		return 2
	}
	method := args[0]
	...
	if len(args) > 1 {
		if err := json.Unmarshal([]byte(args[1]), &params); err != nil {
			fmt.Fprintf(os.Stderr, "mosaic rpc: invalid JSON params: %v\n", err)
			return 2
		}
	}
...
// runWorkspaceGroupRelay handles "mosaic workspace group <sub>" (and the
// "workspace-group" alias) by mapping each subcommand to its
// workspace.group.* v2 method, mirroring the macOS mosaic CLI flags.
func runWorkspaceGroupRelay(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	...
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "mosaic workspace group: requires a subcommand (%s)\n", subcommandHint)
		return 2
	}
	sub := args[0]
	flagKeys, ok := workspaceGroupFlagKeys[sub]
	if !ok {
		fmt.Fprintf(os.Stderr, "mosaic workspace group: unknown subcommand %q (%s)\n", sub, subcommandHint)
		return 2
	}

	fail := func(format string, a ...any) int {
		fmt.Fprintf(os.Stderr, "mosaic workspace group %s: %s\n", sub, fmt.Sprintf(format, a...))
		return 2
	}
...
// runBrowserRelay handles "mosaic browser <subcommand>" by mapping to browser.* v2 methods.
func runBrowserRelay(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "mosaic browser: requires a subcommand (%s)\n", browserSubcommandHint())
		return 2
	}
	...
	if !ok {
		fmt.Fprintf(os.Stderr, "mosaic browser: unknown subcommand %q\n", sub)
		return 2
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "mosaic browser: %v\n", err)
		return 2
	}
	for _, key := range spec.flagKeys {
		if val, ok := parsed.flags[key]; ok {
...
func applyWorkspaceEnvFallback(params map[string]any) {
	...
	if envWs := os.Getenv("MOSAIC_WORKSPACE_ID"); envWs != "" {
...
func applySurfaceEnvFallback(params map[string]any) {
	...
	if envSf := os.Getenv("MOSAIC_SURFACE_ID"); envSf != "" {
...
// readSocketAddrFile reads the socket address from ~/.mosaic/socket_addr as a fallback
// when MOSAIC_SOCKET_PATH is not set. Written by the mosaic app after the relay establishes.
func readSocketAddrFile() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	data, err := os.ReadFile(filepath.Join(home, ".mosaic", "socket_addr"))
	if err != nil {
		return ""
	}
...
func readRelayAuthFile(socketPath string) *relayAuthState {
	if strings.Contains(socketPath, ":") && !strings.HasPrefix(socketPath, "/") {
		...
		data, err := os.ReadFile(filepath.Join(home, ".mosaic", "relay", port+".auth"))
		if err != nil {
			return nil
		}
...
func currentRelayAuth(socketPath string) *relayAuthState {
	relayID := strings.TrimSpace(os.Getenv("MOSAIC_RELAY_ID"))
	relayToken := strings.TrimSpace(os.Getenv("MOSAIC_RELAY_TOKEN"))
...
// dialSocket connects to the mosaic socket. If addr contains a colon and doesn't
...
func authenticateRelayConn(conn net.Conn, auth *relayAuthState) error {
	...
	if challenge.Protocol != "mosaic-relay-auth" || challenge.Version != 1 || challenge.RelayID != auth.RelayID || challenge.Nonce == "" {
...
func cliUsage() {
	fmt.Fprintln(os.Stderr, "Usage: mosaic [--socket <path>] [--json] <command> [args...]")
	...
	fmt.Fprintln(os.Stderr, "  browser <sub>             Browser commands through the local mosaic browser relay")
	fmt.Fprintln(os.Stderr, "  claude-teams [args...]     Launch Claude Code in teammate mode")
	fmt.Fprintln(os.Stderr, "  omo [args...]              Launch OpenCode with mosaic integration")
	fmt.Fprintln(os.Stderr, "  omx [args...]              Launch Oh My Codex with mosaic integration")
	fmt.Fprintln(os.Stderr, "  omc [args...]              Launch Oh My Claude Code with mosaic integration")
