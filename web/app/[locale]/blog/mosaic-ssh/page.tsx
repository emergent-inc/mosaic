...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: "blog.mosaicSsh" });
  return {
    title: t("metaTitle"),
    description: t("metaDescription"),
    keywords: [
      "mosaic", "SSH", "remote development", "terminal", "macOS",
    ...
    alternates: buildAlternates(locale, "/blog/mosaic-ssh"),
  };
}

export default function MosaicSshPage() {
  const t = useTranslations("blog.posts.mosaicSsh");
  const tc = useTranslations("common");

  return (
    <>
      <BlogSchema postKey="mosaicSsh" path="/blog/mosaic-ssh" datePublished="2026-03-30T00:00:00Z" />
      <div className="mb-8">
        <Link
          href="/blog"
      ...
      <video
        src="/blog/mosaic-ssh-image-upload.mp4"
        width={1824}
        height={1080}
        autoPlay
        loop
      ...
      <ul className="mt-4 space-y-1">
        ...
        <li><code>mosaic claude-teams</code> and <code>mosaic omo</code> work over SSH, spawning teammate panes locally while computation runs remote</li>
