...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: "blog.mosaicClaudeTeams" });
  return {
    title: t("metaTitle"),
    description: t("metaDescription"),
    keywords: [
      "mosaic", "Claude Code", "agent teams", "teammate mode", "tmux",
    ...
    alternates: buildAlternates(locale, "/blog/mosaic-claude-teams"),
  };
}

export default function MosaicClaudeTeamsPage() {
  const t = useTranslations("blog.posts.mosaicClaudeTeams");
  const tc = useTranslations("common");

  return (
    <>
      <BlogSchema postKey="mosaicClaudeTeams" path="/blog/mosaic-claude-teams" datePublished="2026-03-30T00:00:00Z" />
      <div className="mb-8">
        <Link
          href="/blog"
