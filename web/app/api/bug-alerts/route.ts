...
const BUG_ALERTS_SECRET_HEADER = "X-Mosaic-Bug-Alerts-Secret";
...
export async function POST(request: Request): Promise<Response> {
  return withApiRouteSpan(
    request,
    "/api/bug-alerts",
    { "mosaic.subsystem": "bug-alerts", "mosaic.bug_alert.operation": "notify" },
    async (span): Promise<Response> => {
      const secret = runtimeEnv("MOSAIC_BUG_ALERTS_SHARED_SECRET");
      ...
      setSpanAttributes(span, {
        "mosaic.bug_alert.event": parsed.alert.event,
        "mosaic.bug_alert.severity": parsed.alert.severity,
        "mosaic.bug_alert.source": parsed.alert.source,
        "mosaic.bug_alert.error_kind": parsed.alert.errorKind,
      });
      ...
      const webhookUrl = runtimeEnv("MOSAIC_BUG_ALERTS_WEBHOOK_URL");
      if (!webhookUrl) {
        return jsonResponse({ ok: true, notified: false });
      }
...
function runtimeEnv(name: "MOSAIC_BUG_ALERTS_SHARED_SECRET" | "MOSAIC_BUG_ALERTS_WEBHOOK_URL"): string | undefined {
