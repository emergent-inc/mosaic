...
describe("VM auth with mosaic native Clerk tokens", () => {
  test("accepts mosaic native access tokens and resolves Clerk user/team claims", async () => {
    ...
    const user = await verifyRequest(
      new Request("https://mosaic.test/api/vm?teamId=org_other", {
        headers: {
  ...
  test("rejects refresh tokens and malformed native tokens as request access credentials", async () => {
    ...
    const malformed = await verifyRequest(
      new Request("https://mosaic.test/api/vm", {
        headers: { authorization: "Bearer mosaicv1.not.valid" },
      }),
      { allowCookie: false }
    );
