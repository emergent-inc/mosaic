...
- Standard install is `npx skills add emergent-inc/cmux -g -y`. Omit `--skill` to install all cmux skills. Use repeated `--skill <name>` flags to install selected skills. Do not use `--all` to mean all skills, because that flag installs to every supported agent.
