...
agent lives in another mosaic instance), so the phone never sees it.

Data sources (read live over the tagged debug socket, no app rebuild):
  mosaic rpc chat.sessions.dump   -> the registry
  mosaic debug-terminals          -> current surfaces and their workspaces

Usage:  MOSAIC_TAG=<tag> scripts/mosaic-chat-debug.py [--live] [--all]
"""
import json
import os
import re
...
CLI = os.path.join(os.path.dirname(__file__), "mosaic-debug-cli.sh")


def cli(*args):
    if not os.environ.get("MOSAIC_TAG"):
        sys.exit("MOSAIC_TAG must be set, e.g. MOSAIC_TAG=asot scripts/mosaic-chat-debug.py")
    out = subprocess.run([CLI, *args], capture_output=True, text=True, env=dict(os.environ))
    if out.returncode != 0:
        sys.exit("mosaic-debug-cli.sh %s failed (rc=%d):\n%s"
...
def render(show_all):
    ...
    print("\033[1magent-chat host view\033[0m  tag=%s" % os.environ.get("MOSAIC_TAG", "?"))
