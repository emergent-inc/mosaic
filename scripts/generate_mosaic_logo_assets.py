...
def update_ios_logo_contents() -> None:
    contents_path = REPO / "ios" / "mosaic" / "Assets.xcassets" / "MosaicLogo.imageset" / "Contents.json"
    contents = {
        "images": [
            {
    ...
    for icon_json in [
        ...
        for group in contents.get("groups", []):
            for layer in group.get("layers", []):
                layer["image-name"] = "mosaic-logo-source.png"
                layer["name"] = "mosaic logo"
                layer["position"] = {
                    "scale": 1,
...
def write_icon_composer_assets() -> None:
    for assets_dir in [
        ...
        REPO / "design" / "mosaic.icon" / "Assets",
    ]:
        if assets_dir.exists():
            shutil.copyfile(DEFAULT_SOURCE, assets_dir / "mosaic-logo-source.png")
