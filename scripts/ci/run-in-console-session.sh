...
if [ -n "$console_user" ] && [ "$console_user" != "root" ] \
  ...
  forward=(PATH DEVELOPER_DIR GITHUB_WORKSPACE RUNNER_TEMP \
    MOSAIC_DERIVED_DATA_PATH MOSAIC_TAG MOSAIC_SKIP_ZIG_BUILD \
    MOSAIC_UNIT_TEST_TIMEOUT_SECONDS \
    MOSAIC_XCODEBUILD_NONINTERACTIVE_IDLE_TIMEOUT_SECONDS \
    MOSAIC_XCODEBUILD_NONINTERACTIVE_POST_TEST_TIMEOUT_SECONDS \
    MOSAIC_XCODEBUILD_NONINTERACTIVE_TIMEOUT_SECONDS \
    MOSAIC_APP_HOST_XCODEBUILD_ATTEMPTS)
  env_pairs=()
