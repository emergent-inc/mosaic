...
target `mosaic.json`. Preserve unrelated sections, then run `mosaic reload-config`
when available.

Prefer project-local `.mosaic/mosaic.json` for team workflows and global
`~/.config/mosaic/mosaic.json` for personal app preferences.
...
```json
{
  "controls": [
    ...
    { "id": "feed", "title": "Feed", "command": "mosaic feed tui --opentui", "height": 260 }
  ]
}
```
...
Use this when the user's normal environment is remote, or when local mosaic should
                    ...
                    "surfaces": [
                      {
                        "type": "terminal",
                        "name": "Markdown",
                        "command": "mosaic markdown open docs/README.md --direction right --focus false; exec ${SHELL:-/bin/zsh} -l"
                      }
                    ]
                  }
                }
