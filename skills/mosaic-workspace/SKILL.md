...
description: "Work inside the current mosaic workspace and terminal. Use for mosaic workspace, current workspace, caller surface, panes, surfaces, socket targeting, and non-interfering mosaic automation."
---

# mosaic Workspace

Use this skill when a task should be scoped to the mosaic workspace that invoked the agent. A workspace is the sidebar tab-like unit in mosaic. It contains split panes, and each pane contains one or more surfaces. A surface is the terminal or browser session the user interacts with.
...
Do not assume the visually focused mosaic workspace is the right target. An agent can be running in one workspace while the user is looking at another. Prefer the caller environment first:
...
Use `MOSAIC_WORKSPACE_ID` as the default workspace anchor and `MOSAIC_SURFACE_ID` as the default caller terminal/surface anchor. If those are missing, use `mosaic identify --json` and be explicit that you are using the currently focused mosaic context.
...
mosaic new-pane --workspace "${MOSAIC_WORKSPACE_ID}" --type browser --direction right --url "http://127.0.0.1:8765"
mosaic new-pane --workspace "${MOSAIC_WORKSPACE_ID}" --type terminal --direction down
```
...
Pass `--focus false` whenever the verb supports it. `move-surface --focus false` preserves the user's current attention. Other commands may grow the same flag over time (https://github.com/emergent-inc/mosaic/issues/1418, https://github.com/emergent-inc/mosaic/issues/2820).
...
```bash
mosaic identify --json
mosaic list-panes --workspace "${MOSAIC_WORKSPACE_ID:-}" --json
mosaic list-pane-surfaces --workspace "${MOSAIC_WORKSPACE_ID:-}" --json
```
...
- If the caller workspace already has a non-caller helper pane, add a new surface to that pane instead of creating another pane:
  ```bash
  mosaic new-surface --workspace "${MOSAIC_WORKSPACE_ID:-}" --pane pane:<helper> --type terminal --focus false
  ```
- If there is no helper pane, create exactly one right-side pane:
  ```bash
  mosaic new-pane --workspace "${MOSAIC_WORKSPACE_ID:-}" --type terminal --direction right --focus false
  ```
...
mosaic list-panes --workspace "${MOSAIC_WORKSPACE_ID:-}" --json
mosaic list-pane-surfaces --workspace "${MOSAIC_WORKSPACE_ID:-}" --json
mosaic list-panels --workspace "${MOSAIC_WORKSPACE_ID:-}" --json
```

Use `--id-format both` when logs or handoffs need stable UUIDs plus human refs:

```bash
mosaic --json --id-format both identify
```
...
mosaic new-workspace --name "debug auth" --cwd "$PWD"

# rename / close (only when explicitly requested)
mosaic rename-workspace --workspace "${MOSAIC_WORKSPACE_ID:-}" -- "build fix"
...
mosaic new-surface --workspace "${MOSAIC_WORKSPACE_ID:-}" --type terminal
...
```bash
mosaic move-surface --surface "${MOSAIC_SURFACE_ID}" --before surface:3
mosaic move-surface --surface "${MOSAIC_SURFACE_ID}" --after surface:3
mosaic move-surface --surface "${MOSAIC_SURFACE_ID}" --index 0
```

Move a surface to another existing pane. Pass `--focus false` to keep the user's current attention put:

```bash
mosaic move-surface --surface surface:240 --pane pane:172 --focus false
```

Split a surface off into a new pane:

```bash
mosaic drag-surface-to-split --surface surface:240 down
```

Known papercut: `drag-surface-to-split` currently routes through V1 and resolves the workspace via UI focus, so it can fail with `ERROR: Surface not found` when the caller's workspace is not the visually focused one. Tracked at https://github.com/emergent-inc/mosaic/issues/1901, related to https://github.com/emergent-inc/mosaic/issues/3189. Until that lands, prefer building the layout additively (see Non-Disruptive Automation above) over create-then-split.
...
```bash
mosaic set-status build "running" --workspace "${MOSAIC_WORKSPACE_ID:-}" --color "#ff9500"
mosaic set-progress 0.4 --label "Building" --workspace "${MOSAIC_WORKSPACE_ID:-}"
mosaic log --workspace "${MOSAIC_WORKSPACE_ID:-}" --level info -- "Started build"
...
For mosaic app/runtime changes in a mosaic source checkout, use tagged reloads from the active worktree. A tagged reload creates an isolated app name, bundle ID, debug socket, and DerivedData path.
...
Use the socket path provided by mosaic before falling back to defaults:

```bash
SOCK="${MOSAIC_SOCKET_PATH:-/tmp/mosaic.sock}"
```

Socket access can be off, restricted to mosaic-spawned processes, or allow all local processes. If a command cannot connect, inspect capabilities before changing settings:
...
- [../mosaic-browser/SKILL.md](../mosaic-browser/SKILL.md) covers browser surfaces with the same current-workspace rule.

## Rules

- Work in the current caller workspace by default.
- Use `MOSAIC_WORKSPACE_ID`, `MOSAIC_SURFACE_ID`, and `MOSAIC_SOCKET_PATH` before focused-window fallbacks.
...
- For app/runtime changes in a mosaic source checkout, reload with `./scripts/reload.sh --tag <tag>` from the worktree before dogfood handoff.
