...
This reference expands mosaic package extraction and refactor architecture rules.
...
- `MosaicAppearanceMath`
- `MosaicWorkspaceModel`
- `MosaicFooFormatting`
- `MosaicFooLogic`
- `MosaicFooState`
...
5. Executable: `mosaicApp` and `AppDelegate` as the composition root.
...
`@main` `mosaicApp` and `AppDelegate` stay in the executable target. Do not move extensions of executable-owned types down into lower packages. A lower package cannot extend a higher-owned type without creating the wrong dependency direction.
...
`mosaic.xcodeproj` lists package dependencies explicitly. Adding `Packages/MosaicFoo` means mirroring existing package entries:
...
App-target packages link into both `mosaic` and `mosaic-unit`, so tests can import and inject them. A package linked by the app but not `mosaic-unit` can make the app build pass while the test target fails.
