---
name: mosaic-browser
description: End-user browser automation with mosaic. Use when you need to open sites, interact with pages, wait for state changes, and extract data from mosaic browser surfaces.
---

# Browser Automation with mosaic

Use this skill for browser tasks inside mosaic webviews.
...
```bash
mosaic --json browser open https://example.com
# use returned surface ref, for example: surface:7

mosaic browser surface:7 get url
mosaic browser surface:7 wait --load-state complete --timeout-ms 15000
mosaic browser surface:7 snapshot --interactive
mosaic browser surface:7 fill e1 "hello"
mosaic --json browser surface:7 click e2 --snapshot-after
mosaic browser surface:7 snapshot --interactive
```
...
mosaic browser open https://example.com --workspace workspace:2 --window window:1 --json
```
...
mosaic supports wait patterns similar to agent-browser:

```bash
mosaic browser <surface> wait --selector "#ready" --timeout-ms 10000
mosaic browser <surface> wait --text "Success" --timeout-ms 10000
...
```bash
mosaic --json browser open https://example.com/signup
mosaic browser surface:7 get url
mosaic browser surface:7 wait --load-state complete --timeout-ms 15000
mosaic browser surface:7 snapshot --interactive
mosaic browser surface:7 fill e1 "Jane Doe"
mosaic browser surface:7 fill e2 "jane@example.com"
mosaic --json browser surface:7 click e3 --snapshot-after
mosaic browser surface:7 wait --url-contains "/welcome" --timeout-ms 15000
mosaic browser surface:7 snapshot --interactive
```
...
```bash
mosaic browser surface:7 fill e11 "" --snapshot-after --json
mosaic browser surface:7 get value e11 --json
```
...
mosaic browser surface:7 wait --load-state complete --timeout-ms 15000
mosaic browser surface:7 snapshot --interactive
mosaic --json browser surface:7 click e5 --snapshot-after
mosaic browser surface:7 snapshot --interactive
```
...
mosaic browser surface:7 get html body
```
