---
name: mosaic-custom-sidebar
description: "Build a custom mosaic sidebar from a plain-language request. Use when the user asks for a custom sidebar, a sidebar that shows their workspaces/tabs/PRs/clock, a vibe-coded sidebar, or anything involving files in ~/.config/mosaic/sidebars/. Covers authoring the interpreted SwiftUI-style file, enabling the beta flag, selecting it, and iterating with hot reload."
---

# mosaic Custom Sidebar

mosaic renders custom sidebars from a small SwiftUI-style file at runtime: no Xcode, no build step, no signing. The file hot-reloads on save, binds to live mosaic state (workspaces, tabs, git, PRs, clock), and can run real mosaic commands on tap.
...
```bash
mosaic docs sidebars
curl -fsSL https://raw.githubusercontent.com/emergent-inc/mosaic/main/docs/custom-sidebars.md
```
...
2. **Write a named file.** The name becomes the menu label; use short kebab-case:
   ```
   ~/.config/mosaic/sidebars/<name>.swift
   ```
...
3. **Validate and select it:**
   ```bash
   mosaic sidebar validate <name>   # parse/interpret check with real data shapes
   mosaic sidebar select <name>     # switch the sidebar to it
   ```
   The user can also pick it manually: right-click the sidebar toggle button.
4. **Iterate.** Saving the file hot-reloads the sidebar in place (`mosaic sidebar reload` forces it). Look at the result, fix what looks off, and verify rows show real data and taps do the right thing before declaring it done.

## Authoring rules

- **Default to live data.** Bind to the `workspaces` context instead of hard-coding text so the sidebar stays correct on its own.
- **Make it interactive by default.** Rows that represent something openable should run the matching `mosaic(...)` action on tap. A list that just displays text is rarely what they wanted.
...
SWIFT
mosaic sidebar validate mine && mosaic sidebar select mine
```
...
A button or `.onTapGesture` body calls `mosaic("<method>", param: value)`, dispatched through the same surface as the `mosaic` CLI. Common methods: `workspace.select` (`workspace_id`), `surface.focus` (`surface_id`), `workspace.reorder` (`workspace_id` + `index`). `openURL("https://...")` opens links. Discover the full command surface with `mosaic docs api`.
...
Not yet supported (write the natural Swift anyway; it degrades gracefully): `@State` and input controls (`TextField`, `Toggle`, `Slider`, `Picker`), custom `struct`/`View` definitions, navigation (`sheet`/`popover`), `AsyncImage`. Two-way editing does not work yet; taps that run `mosaic(...)` do.

## Troubleshooting

- Sidebar missing from the right-click picker: the beta flag is off, or the file is not directly under `~/.config/mosaic/sidebars/`.
- Blank or partial render: run `mosaic sidebar validate <name>`; errors show inline in the sidebar with the failing location. A broken save keeps the last working render on screen, so re-save after fixing.
- Rows not tappable: wrap the row in `Button(action: { mosaic(...) }) { ... }` or add `.onTapGesture { mosaic(...) }`.
