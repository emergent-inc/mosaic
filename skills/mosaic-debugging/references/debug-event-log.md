...
- tagged Debug app: `/tmp/mosaic-debug-<tag>.log`

`reload.sh` writes the current path to `/tmp/mosaic-last-debug-log-path`, so the most robust tail command is:
...
The package implementation lives in `Packages/macOS/MosaicDebugLog/Sources/MosaicDebugLog/DebugEventLog.swift`, and the app shim lives in `Sources/App/DebugLogging.swift`.

Call sites use:

```swift
#if DEBUG
mosaicDebugLog("focus.panel ...")
#endif
```
...
The debug logger has a 500-entry ring buffer. `MosaicDebugLog.DebugEventLog.shared.dump()` writes the full buffer to file. Use this when the interesting event occurred before you started tailing.
