...
```bash
tail -f "$(cat /tmp/mosaic-last-debug-log-path 2>/dev/null || echo /tmp/mosaic-debug.log)"
```

- Untagged Debug app: `/tmp/mosaic-debug.log`
- Tagged Debug app (`./scripts/reload.sh --tag <tag>`): `/tmp/mosaic-debug-<tag>.log`
- `reload.sh` writes the current path to `/tmp/mosaic-last-debug-log-path`
- `reload.sh` writes the selected dev CLI path to `/tmp/mosaic-last-cli-path`
- `reload.sh` updates `/tmp/mosaic-cli` and `$HOME/.local/bin/mosaic-dev` to that CLI
- Implementation: `Packages/macOS/MosaicDebugLog/Sources/MosaicDebugLog/DebugEventLog.swift`
- App shim: `Sources/App/DebugLogging.swift`
- Free function `mosaicDebugLog("message")` logs with timestamp and appends to file in real time from mosaic code
- The package implementation and app shim are `#if DEBUG`; all call sites must be wrapped in `#if DEBUG` / `#endif`
- 500-entry ring buffer; `MosaicDebugLog.DebugEventLog.shared.dump()` writes full buffer to file
...
- To add a debug toggle or visual option: create an `NSWindowController` subclass with a `shared` singleton, add it to the "Debug Windows" menu in `Sources/mosaicApp.swift`, and add a SwiftUI view with `@AppStorage` bindings for live changes.
