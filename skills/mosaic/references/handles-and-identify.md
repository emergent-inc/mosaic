...
```bash
mosaic identify --workspace workspace:2
mosaic identify --workspace workspace:2 --surface surface:8
```
...
```bash
mosaic --json identify                 # refs-first output
mosaic --json --id-format both identify
mosaic --json --id-format uuids identify
```
