...
/// Regression coverage for https://github.com/emergent-inc/mosaic/issues/5173.
///
/// The host renders sidebar views through an `any MosaicSidebarProvider`
/// existential (`MosaicExtensionSidebarSelection.provider(for:)?.render(snapshot:)`).
...
final class SidebarProviderExistentialDispatchTests: XCTestCase {
    ...
    func testSuperCompactRendersIdenticallyThroughExistential() {
        ...
        let existential: any MosaicSidebarProvider = concrete
    ...
    private static func snapshot(workspaceCount: Int) -> MosaicSidebarProviderSnapshot {
