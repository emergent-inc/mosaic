...
@testable import MosaicExtensionSidebarExamples
import XCTest
...
final class BrowserStackSidebarTests: XCTestCase {
    ...
    func testReconcilePreservesUserStateWhileApplyingSnapshotMembership() throws {
        ...
        let reconciled = try store.reconciledState(for: MosaicSidebarProviderSnapshot(
            sequence: 2,
    ...
    func testWindowScopedStateFallsBackToMatchingRelaunchScope() throws {
        ...
        preferredState.moveWorkspace(MosaicSidebarProviderWorkspaceMove(
            workspaceId: movedWorkspace.id,
            sourceSectionId: "loose",
            targetSectionId: "group:reading-list",
            targetIndex: 0
        ...
        let relaunchedSnapshot = MosaicSidebarProviderSnapshot(
            sequence: 2,
    ...
    func testBrowserStackRenderModelPreservesEmptyRequiredSections() {
        let snapshot = MosaicSidebarProviderSnapshot(sequence: 1, selectedWorkspaceId: nil, workspaces: [])
        let sections = [
    ...
    func testAsyncStateLoadNotifiesHostAndUpdatesRenderModel() throws {
        ...
        let snapshot = MosaicSidebarProviderSnapshot(
            sequence: 1,
            selectedWorkspaceId: nil,
    ...
    private func temporaryStateURL() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-browser-stack-\(UUID().uuidString)", isDirectory: true)
    ...
    private func snapshot(titles: [String], windowId: UUID? = nil) -> MosaicSidebarProviderSnapshot {
    ...
    private func workspace(title: String) -> MosaicSidebarProviderWorkspace {
