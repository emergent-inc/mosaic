...
public struct ProjectWorktreeSidebar: MosaicSidebarProvider {
    public let descriptor = MosaicSidebarProviderDescriptor(
        id: "com.example.mosaic.sidebar.project-worktrees",
    ...
    public func render(snapshot: MosaicSidebarProviderSnapshot) -> MosaicSidebarProviderRenderModel {
        var sections: [MosaicSidebarProviderSection] = []
        ...
        var grouped: [String: [MosaicSidebarProviderWorkspace]] = [:]
    ...
    private func branchSubtitle(_ workspace: MosaicSidebarProviderWorkspace) -> MosaicSidebarProviderText? {
        trimmed(workspace.branchSummary).map(MosaicSidebarProviderText.plain)
    }
}
