...
public struct BrowserStackSidebar: MosaicMutableSidebarProvider {
    public static let stateDidLoadNotification = Notification.Name("MosaicBrowserStackSidebarStateDidLoad")

    public let descriptor = MosaicSidebarProviderDescriptor(
        id: "com.example.mosaic.sidebar.browser-stack",
    ...
    public func handle(
        ...
        guard case .moveWorkspace(let move) = mutation else {
            return MosaicSidebarProviderCommandResult(ok: false)
        }
        stateCache.moveWorkspace(move, snapshot: snapshot)
        return MosaicSidebarProviderCommandResult(ok: true)
    }

    private func recentActivityText(_ workspace: MosaicSidebarProviderWorkspace) -> MosaicSidebarProviderText? {
        workspace.latestSubmittedAt.map { .relativeDate($0, style: .compact) }
    }

    private func browserIcon(_ workspace: MosaicSidebarProviderWorkspace) -> MosaicSidebarProviderIcon? {
        ...
        if title.contains("google") {
            return MosaicSidebarProviderIcon(
                text: "G",
...
private final class BrowserStackSidebarStateCache: @unchecked Sendable {
    ...
    private let persistenceQueue = DispatchQueue(label: "mosaic.browser-stack-sidebar.persistence")
    ...
    func state(for snapshot: MosaicSidebarProviderSnapshot) -> BrowserStackSidebarState {
    ...
    func moveWorkspace(
        _ move: MosaicSidebarProviderWorkspaceMove,
        snapshot: MosaicSidebarProviderSnapshot
    ) {
        let scopeKey = Self.scopeKey(for: snapshot)
    ...
    private func startLoadIfNeeded(scopeKey: String, snapshot: MosaicSidebarProviderSnapshot) {
    ...
    private static func scopeKey(for snapshot: MosaicSidebarProviderSnapshot) -> String {
...
public struct BrowserStackSidebarStore: Sendable {
    ...
    public func load(scopeKey: String, snapshot: MosaicSidebarProviderSnapshot) throws -> BrowserStackSidebarState {
        let url = scopedStateURL(scopeKey: scopeKey)
        if url != stateURL, FileManager.default.fileExists(atPath: url.path) {
            return try load(from: url)
        }
    ...
    private func loadBestScopedFallback(
        matching snapshot: MosaicSidebarProviderSnapshot,
    ...
    public func reconciledState(for snapshot: MosaicSidebarProviderSnapshot) throws -> BrowserStackSidebarState {
...
public struct BrowserStackSidebarState: Codable, Equatable, Sendable {
    ...
    public static func initial(snapshot: MosaicSidebarProviderSnapshot) -> BrowserStackSidebarState {
    ...
    public func reconciled(with snapshot: MosaicSidebarProviderSnapshot) -> BrowserStackSidebarState {
        let liveIds = Set(snapshot.workspaceIds)
        var seen = Set<UUID>()
    ...
    public mutating func moveWorkspace(_ move: MosaicSidebarProviderWorkspaceMove) {
