...
def _expect_error(label: str, fn, code_substr: str) -> None:
    try:
        ...
        if code_substr in text:
            return
        raise mosaicError(f"{label}: expected error containing {code_substr!r}, got: {text}")
    raise mosaicError(f"{label}: expected error containing {code_substr!r}, but call succeeded")
...
def _expect_error_contains(label: str, fn, *needles: str) -> None:
    try:
        ...
        if missing:
            raise mosaicError(f"{label}: missing expected substrings {missing!r} in error: {text}")
        return
    raise mosaicError(f"{label}: expected failure, but call succeeded")
...
def _wait_with_fallback(c: mosaic, surface_id: str, params: dict, pred, timeout_s: float, label: str) -> None:
...
<html>
  <head>
    <title>mosaic-browser-comprehensive-1</title>
    <style>
...
<!doctype html>
<html>
  <head><title>mosaic-browser-comprehensive-2</title></head>
  <body>
...
def main() -> int:
    ...
    with mosaic(SOCKET_PATH) as c:
        ...
        _wait_until(
            lambda: "mosaic-browser-comprehensive-1"
        ...
        _must(str(_value(out_text)) == "mosaic", f"Expected status text to be mosaic: {out_text}")
        ...
        _must(str(_value(name_val)) == "mosaic-v2", f"Expected typed suffix in input value: {name_val}")
        ...
        _must("mosaic-browser-comprehensive-1" in snapshot_text, f"Expected snapshot text for page1: {snap}")
        ...
        _wait_until(
            lambda: "mosaic-browser-comprehensive-2"
            in str((c._call("browser.get.title", {"surface_id": target}) or {}).get("title") or ""),
            timeout_s=3.0,
        ...
        _wait_with_fallback(
            c,
            target,
            {"url_contains": "mosaic-browser-comprehensive-1", "timeout_ms": 4000},
            lambda: "mosaic-browser-comprehensive-1" in str((c._call("browser.url.get", {"surface_id": target}) or {}).get("url") or ""),
            timeout_s=5.0,
        ...
        _wait_with_fallback(
            c,
            target,
            {"url_contains": "mosaic-browser-comprehensive-2", "timeout_ms": 4000},
            lambda: "mosaic-browser-comprehensive-2" in str((c._call("browser.url.get", {"surface_id": target}) or {}).get("url") or ""),
            timeout_s=5.0,
