...
def _rename_surface(client: mosaic, surface_id: str, title: str) -> None:
    client._call(
        "surface.action",
        {
...
def _current_surface_id(client: mosaic, workspace_id: str) -> str:
...
def main() -> int:
    with mosaic(SOCKET_PATH) as client:
        ...
        if len(rows) < 2:
            raise mosaicError(f"expected at least two surfaces after split: {payload}")
        ...
        if not left_surface_id:
            raise mosaicError(f"failed to resolve left surface id: {payload}")
        ...
        if not result_rows:
            raise mosaicError("switcher returned no rows for renamed surface query")
