...
def _focused_surface_id(client: mosaic) -> str:
    ...
    if not surface_id:
        raise mosaicError(f"Missing focused surface in identify payload: {ident}")
    return surface_id


def _run_remote_shell_probe(client: mosaic, surface_id: str, probe_label: str) -> str:
    token = f"__MOSAIC_REMOTE_SOCKET_{probe_label}_{secrets.token_hex(4)}__"
    client.send_surface(
        surface_id,
        (
            f"__mosaic_socket_path=\"${{MOSAIC_SOCKET_PATH:-}}\"; "
            f"printf '{token}:%s:__MOSAIC_REMOTE_SOCKET_END__\\n' \"$__mosaic_socket_path\"\n"
        ),
    )
    deadline = time.time() + 15.0
    last = ""
    pattern = re.compile(re.escape(token) + r":(.*?):__MOSAIC_REMOTE_SOCKET_END__")
    while time.time() < deadline:
        ...
        if matches:
            for candidate in reversed(matches):
                ...
                if cleaned and cleaned != "%s":
                    return cleaned
        time.sleep(0.15)
    raise mosaicError(f"Timed out waiting for socket token {token!r}: {last[-1200:]!r}")


def _assert_remote_socket_path(client: mosaic, surface_id: str, shortcut_name: str) -> None:
    socket_path = _run_remote_shell_probe(client, surface_id, shortcut_name)
    _must(
        socket_path.startswith("127.0.0.1:"),
        f"{shortcut_name} should keep the new terminal on the ssh relay, got MOSAIC_SOCKET_PATH={socket_path!r}",
    )


def _open_ssh_workspace(client: mosaic, cli: str, *, name: str) -> str:
...
def main() -> int:
    if not SSH_HOST:
        print("SKIP: set MOSAIC_SSH_TEST_HOST to run ssh shortcut inheritance regression")
        return 0
