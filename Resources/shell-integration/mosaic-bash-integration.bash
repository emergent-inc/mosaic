...
_MOSAIC_SEND_TOOL=""
_mosaic_detect_send_tool() {
    if command -v ncat >/dev/null 2>&1; then
        _MOSAIC_SEND_TOOL=ncat
    elif command -v socat >/dev/null 2>&1; then
        _MOSAIC_SEND_TOOL=socat
    elif command -v nc >/dev/null 2>&1; then
        _MOSAIC_SEND_TOOL=nc
    fi
}
# Detection deferred to after _mosaic_fix_path (end of file).

_mosaic_send() {
    local payload="$1"
    case "$_MOSAIC_SEND_TOOL" in
        ncat)
            printf '%s\n' "$payload" | ncat -w 1 -U "$MOSAIC_SOCKET_PATH" --send-only
            ;;
        socat)
            printf '%s\n' "$payload" | socat -T 1 - "UNIX-CONNECT:$MOSAIC_SOCKET_PATH" >/dev/null 2>&1
            ;;
        nc)
            if printf '%s\n' "$payload" | nc -N -U "$MOSAIC_SOCKET_PATH" >/dev/null 2>&1; then
                :
            else
                printf '%s\n' "$payload" | nc -w 1 -U "$MOSAIC_SOCKET_PATH" >/dev/null 2>&1 || true
            fi
            ;;
    esac
}
...
_mosaic_send_bg() {
    local payload="$1"
    if [[ "${_MOSAIC_IN_PREEXEC:-}" == "1" ]]; then
        {
            _mosaic_send "$payload"
        } >/dev/null 2>&1 &
        disown
        return 0
    fi
    _mosaic_detach_bg _mosaic_send "$payload"
}

_mosaic_start_tracked_bg() {
    local __pid_var="$1"
    shift
    local __pid_file="${TMPDIR:-/tmp}/mosaic-bg-pid-$$-${RANDOM:-0}"
    local __pid=""
    (
        "$@" >/dev/null 2>&1 &
        printf '%s\n' "$!" > "$__pid_file"
...
_mosaic_relay_rpc_bg() {
    ...
    _mosaic_detach_bg "$relay_cli" rpc "$method" "$params"
}
...
_mosaic_restore_scrollback_once
_MOSAIC_CLAUDE_WRAPPER="${_MOSAIC_CLAUDE_WRAPPER:-}"
_MOSAIC_GROK_WRAPPER="${_MOSAIC_GROK_WRAPPER:-}"
...
_MOSAIC_PWD_LAST_PWD="${_MOSAIC_PWD_LAST_PWD:-}"
_MOSAIC_GIT_LAST_PWD="${_MOSAIC_GIT_LAST_PWD:-}"
_MOSAIC_GIT_LAST_RUN="${_MOSAIC_GIT_LAST_RUN:-0}"
_MOSAIC_GIT_JOB_PID="${_MOSAIC_GIT_JOB_PID:-}"
_MOSAIC_GIT_JOB_STARTED_AT="${_MOSAIC_GIT_JOB_STARTED_AT:-0}"
_MOSAIC_GIT_HEAD_LAST_PWD="${_MOSAIC_GIT_HEAD_LAST_PWD:-}"
_MOSAIC_GIT_HEAD_PATH="${_MOSAIC_GIT_HEAD_PATH:-}"
_MOSAIC_GIT_HEAD_SIGNATURE="${_MOSAIC_GIT_HEAD_SIGNATURE:-}"
_MOSAIC_GIT_ACTIVE_PWD_FILE="${_MOSAIC_GIT_ACTIVE_PWD_FILE:-$(/usr/bin/mktemp "${TMPDIR:-/tmp}/mosaic-git-active-pwd.XXXXXX" 2>/dev/null || true)}"
_MOSAIC_PR_POLL_PID="${_MOSAIC_PR_POLL_PID:-}"
_MOSAIC_PR_POLL_PWD="${_MOSAIC_PR_POLL_PWD:-}"
_MOSAIC_PR_LAST_BRANCH="${_MOSAIC_PR_LAST_BRANCH:-}"
_MOSAIC_PR_NO_PR_BRANCH="${_MOSAIC_PR_NO_PR_BRANCH:-}"
_MOSAIC_PR_POLL_INTERVAL="${_MOSAIC_PR_POLL_INTERVAL:-45}"
_MOSAIC_PR_FORCE="${_MOSAIC_PR_FORCE:-0}"
_MOSAIC_PR_DEBUG="${_MOSAIC_PR_DEBUG:-0}"
_MOSAIC_ASYNC_JOB_TIMEOUT="${_MOSAIC_ASYNC_JOB_TIMEOUT:-20}"
_MOSAIC_LAST_PR_ACTION="${_MOSAIC_LAST_PR_ACTION:-}"
_MOSAIC_LAST_PR_TARGET="${_MOSAIC_LAST_PR_TARGET:-}"
_MOSAIC_PR_ACTION_HINT_FILE="${_MOSAIC_PR_ACTION_HINT_FILE:-${TMPDIR:-/tmp}/mosaic-pr-action-$$}"
_MOSAIC_BASH_HISTORY_LAST_FILE="${_MOSAIC_BASH_HISTORY_LAST_FILE:-${TMPDIR:-/tmp}/mosaic-history-last-$$}"

_MOSAIC_PORTS_LAST_RUN="${_MOSAIC_PORTS_LAST_RUN:-0}"
_MOSAIC_SHELL_ACTIVITY_LAST="${_MOSAIC_SHELL_ACTIVITY_LAST:-}"
_MOSAIC_TTY_NAME="${_MOSAIC_TTY_NAME:-}"
_MOSAIC_TTY_REPORTED="${_MOSAIC_TTY_REPORTED:-0}"
_MOSAIC_TMUX_PUSH_SIGNATURE="${_MOSAIC_TMUX_PUSH_SIGNATURE:-}"
_MOSAIC_TMUX_PULL_SIGNATURE="${_MOSAIC_TMUX_PULL_SIGNATURE:-}"
_MOSAIC_TMUX_SYNC_KEYS=(
    MOSAIC_BUNDLED_CLI_PATH
    MOSAIC_BUNDLE_ID
    MOSAICD_UNIX_PATH
...
_MOSAIC_TMUX_SURFACE_SCOPED_KEYS=(
    MOSAIC_PANEL_ID
    MOSAIC_SURFACE_ID
)
...
_mosaic_set_git_active_pwd() {
    ...
    printf '%s\n' "$active_pwd" >| "$_MOSAIC_GIT_ACTIVE_PWD_FILE" 2>/dev/null || true
}
...
_mosaic_clear_pr_for_panel() {
    ...
    _mosaic_send "clear_pr --tab=$MOSAIC_TAB_ID --panel=$MOSAIC_PANEL_ID"
}

_mosaic_clear_pr_command_hint_file() {
    [[ -n "${_MOSAIC_PR_ACTION_HINT_FILE:-}" ]] || return 0
    /bin/rm -f -- "$_MOSAIC_PR_ACTION_HINT_FILE" >/dev/null 2>&1 || true
}

_mosaic_store_pr_command_hint() {
    [[ -n "${_MOSAIC_PR_ACTION_HINT_FILE:-}" ]] || return 0
    if [[ -z "$_MOSAIC_LAST_PR_ACTION" ]]; then
        _mosaic_clear_pr_command_hint_file
        return 0
    fi

    local target="$_MOSAIC_LAST_PR_TARGET"
    target="${target//$'\n'/ }"
    target="${target//$'\r'/ }"
    target="${target//$'\t'/ }"
    printf '%s\t%s\n' "$_MOSAIC_LAST_PR_ACTION" "$target" > "$_MOSAIC_PR_ACTION_HINT_FILE" 2>/dev/null || true
}

_mosaic_load_pr_command_hint() {
    [[ -n "${_MOSAIC_PR_ACTION_HINT_FILE:-}" && -r "$_MOSAIC_PR_ACTION_HINT_FILE" ]] || return 0

    local action="" target=""
    IFS=$'\t' read -r action target < "$_MOSAIC_PR_ACTION_HINT_FILE" || true
    _mosaic_clear_pr_command_hint_file

    case "$action" in
        merge|close|reopen|create|checkout|ready|edit|view)
            _MOSAIC_LAST_PR_ACTION="$action"
            _MOSAIC_LAST_PR_TARGET="$target"
            ;;
    esac
}
...
_mosaic_pr_cache_prefix() {
    ...
    printf '%s\n' "/tmp/mosaic-pr-cache-${MOSAIC_PANEL_ID}"
}

_mosaic_pr_force_signal_path() {
    [[ -n "$MOSAIC_PANEL_ID" ]] || return 1
    printf '%s\n' "/tmp/mosaic-pr-force-${MOSAIC_PANEL_ID}"
}
...
_mosaic_halt_pr_poll_loop() {
    if [[ -n "$_MOSAIC_PR_POLL_PID" ]]; then
        ...
        kill -KILL -- -"$_MOSAIC_PR_POLL_PID" 2>/dev/null || true
    fi
    local signal_path=""
    signal_path="$(_mosaic_pr_force_signal_path 2>/dev/null || true)"
...
_mosaic_start_pr_poll_loop() {
    ...
    _MOSAIC_PR_POLL_PID=$!
    disown "$_MOSAIC_PR_POLL_PID" 2>/dev/null || disown
}

_mosaic_bash_cleanup() {
    _mosaic_stop_pr_poll_loop
...
_mosaic_bash_history_command() {
    local HISTTIMEFORMAT=
    local history_file="${TMPDIR:-/tmp}/mosaic-history-$$-${RANDOM:-0}"
    ...
    if [[ "$line" =~ ^[[:space:]]*([0-9]+)[[:space:]]+(.*)$ ]]; then
        ...
        if [[ -n "${_MOSAIC_BASH_HISTORY_LAST_FILE:-}" && -r "$_MOSAIC_BASH_HISTORY_LAST_FILE" ]]; then
            IFS= read -r last_number < "$_MOSAIC_BASH_HISTORY_LAST_FILE" || last_number=""
        fi
        [[ "$history_number" == "$last_number" ]] && return 1
        if [[ -n "${_MOSAIC_BASH_HISTORY_LAST_FILE:-}" ]]; then
            printf '%s\n' "$history_number" > "$_MOSAIC_BASH_HISTORY_LAST_FILE" 2>/dev/null || true
        fi
        printf '%s\n' "${BASH_REMATCH[2]}"
        return 0
    fi
...
_mosaic_bash_preexec_hook() {
    local cmd="${1:-}"
    local history_cmd=""
    history_cmd="$(_mosaic_bash_history_command 2>/dev/null || true)"
    if [[ -n "$history_cmd" ]]; then
        cmd="$history_cmd"
    fi
    _mosaic_preexec_command "$cmd"
}

_mosaic_bash_preexec_hook_subshell() {
    local _MOSAIC_IN_PREEXEC=1
    _mosaic_bash_preexec_hook "$@"
}
...
_mosaic_prompt_command() {
    ...
    if [[ -z "$_MOSAIC_TTY_NAME" ]]; then
        ...
        t="$(tty 2>/dev/null || true)"
        t="${t##*/}"
        [[ "$t" != "not a tty" ]] && _MOSAIC_TTY_NAME="$t"
    fi
    ...
    if (( ! mosaic_has_unix_socket )); then
        ...
        if (( now - _MOSAIC_PORTS_LAST_RUN >= 10 )); then
    ...
    if [[ "${MOSAIC_NO_GIT_WATCH:-}" != "1" && -n "$_MOSAIC_GIT_JOB_PID" ]] && kill -0 "$_MOSAIC_GIT_JOB_PID" 2>/dev/null; then
        if [[ "$pwd" != "$_MOSAIC_GIT_LAST_PWD" || "$git_head_changed" == "1" ]]; then
            kill "$_MOSAIC_GIT_JOB_PID" >/dev/null 2>&1 || true
            _MOSAIC_GIT_JOB_PID=""
            _MOSAIC_GIT_JOB_STARTED_AT=0
        fi
    ...
    if [[ "${MOSAIC_NO_GIT_WATCH:-}" != "1" ]] && { [[ -z "$_MOSAIC_GIT_JOB_PID" ]] || ! kill -0 "$_MOSAIC_GIT_JOB_PID" 2>/dev/null; }; then
        _MOSAIC_GIT_LAST_PWD="$pwd"
        _MOSAIC_GIT_LAST_RUN=$now
        _mosaic_start_tracked_bg _MOSAIC_GIT_JOB_PID _mosaic_report_git_branch_for_path "$pwd"
...
_mosaic_install_prompt_command() {
    [[ -n "${_MOSAIC_PROMPT_INSTALLED:-}" ]] && return 0
    _MOSAIC_PROMPT_INSTALLED=1
    ...
    if [[ "$decl" == "declare -a"* ]]; then
        ...
        for item in "${PROMPT_COMMAND[@]}"; do
            [[ "$item" == "_mosaic_prompt_command" ]] && existing=1 && break
        done
        if (( existing == 0 )); then
            PROMPT_COMMAND=("_mosaic_prompt_command" "${PROMPT_COMMAND[@]}")
        fi
    else
        case ";$PROMPT_COMMAND;" in
            ...
            *)
                if [[ -n "$PROMPT_COMMAND" ]]; then
                    ...
                    PROMPT_COMMAND="_mosaic_prompt_command"
                fi
                ;;
        esac
    fi
        ...
        if (( BASH_VERSINFO[0] > 5 || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] >= 3) )); then
            builtin readonly _MOSAIC_BASH_PS0='${ _mosaic_bash_preexec_hook; }'
        else
            builtin readonly _MOSAIC_BASH_PS0='$(_mosaic_bash_preexec_hook_subshell >/dev/null)'
        fi
        if [[ "$PS0" != *"${_MOSAIC_BASH_PS0}"* ]]; then
            PS0=$PS0"${_MOSAIC_BASH_PS0}"
        fi
    fi
}
