...
extension MosaicCLI {
    static let mosaicThemeOverrideBundleIdentifier = MosaicGhosttyConfigPathResolver.releaseBundleIdentifier
    static let mosaicThemesBlockStart = "# mosaic themes start"
    static let mosaicThemesBlockEnd = "# mosaic themes end"
    static let mosaicThemesReloadNotificationName = "com.mosaicterm.themes.reload-config"
    static let mosaicEnforcedTerminalThemeName = "Anysphere Dark"
    static let mosaicThemeChangesDisabledMessage = String(
        localized: "cli.themes.disabled",
        defaultValue: "Terminal theme is managed by mosaic and cannot be changed."
    )
    ...
    func runThemes(
        ...
        case "set":
            throw CLIError(message: Self.mosaicThemeChangesDisabledMessage)
        case "clear":
            if commandArgs.count > 1 {
        ...
        default:
            if subcommand.hasPrefix("-") {
                throw CLIError(message: "Unknown themes subcommand '\(subcommand)'. Run 'mosaic themes --help'.")
            }
    ...
    private func printThemesList(
        ...
        if jsonOutput {
            let currentPayload: [String: Any] = [
                "raw_value": Self.mosaicEnforcedTerminalThemeName,
                "light": Self.mosaicEnforcedTerminalThemeName,
                "dark": Self.mosaicEnforcedTerminalThemeName,
                "source_path": NSNull()
            ]
            let payload: [String: Any] = [
                "themes": themes.map { theme in
                    [
                        "name": theme,
                        "current_light": Self.mosaicEnforcedTerminalThemeName.caseInsensitiveCompare(theme) == .orderedSame,
                        "current_dark": Self.mosaicEnforcedTerminalThemeName.caseInsensitiveCompare(theme) == .orderedSame
                    ]
                },
                "current": currentPayload,
                "fixed_theme": Self.mosaicEnforcedTerminalThemeName,
                "managed": true
            ]
        ...
        print(
            String(
                ...
                defaultValue: "Current theme: Anysphere Dark (managed by mosaic)"
            )
        )
        print(Self.mosaicThemeChangesDisabledMessage)
        print("")
        ...
        for theme in themes {
            var badges: [String] = []
            if Self.mosaicEnforcedTerminalThemeName.caseInsensitiveCompare(theme) == .orderedSame {
                badges.append("light")
            }
            if Self.mosaicEnforcedTerminalThemeName.caseInsensitiveCompare(theme) == .orderedSame {
                badges.append("dark")
