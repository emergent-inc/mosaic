...
extension MosaicCLI {
    ...
    func resolvedSummarizerAgent(
        ...
        // Pure decision (unit-tested in MosaicSettings); the CLI only supplies the
    ...
    func summarizerBinaryAvailable(agent: String, env: [String: String]) -> Bool {
        switch agent {
        case "claude":
            let customPath = env["MOSAIC_CUSTOM_CLAUDE_PATH"]?
                .trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            if !customPath.isEmpty,
               FileManager.default.isExecutableFile(atPath: customPath),
               !isMosaicClaudeWrapper(at: customPath) {
                return true
            }
        ...
        default:
            guard let def = MosaicCLI.agentDef(named: agent) else { return false }
    ...
    func summarize(
        ...
        default:
            guard let def = MosaicCLI.agentDef(named: agent) else { return nil }
    ...
    private func summarizeWithCodex(
        ...
        summarizerEnv["MOSAIC_CODEX_HOOKS_DISABLED"] = "1"
        let outputFile = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-autoname-\(UUID().uuidString).txt")
        let workingDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-autoname-cwd-\(UUID().uuidString)", isDirectory: true)
