...
// `mosaic remotes` (alias `remote`): manage the team's device-registry routes so
...
extension MosaicCLI {
    static let remotesUsage = """
        Usage: mosaic remotes <list|add|remove> [options]

        Manage the remote Macs in your team's mosaic device registry. Added remotes
        appear in your iOS app's device list and the phone can attach to them.

          mosaic remotes list [--json]
              List your team's registered remotes (name, deviceId, routes, tag, last seen).

          mosaic remotes add <name> --route <host:port> [--route <host:port> ...] [--tag <tag>] [--json]
          ...
          mosaic remotes remove <name-or-deviceId> [--json]
              Remove a remote you registered from the device registry.

        Examples:
          mosaic remotes add my-studio --route 100.64.1.2:51001
          mosaic remotes add my-studio --route my-studio.tailnet.ts.net:51001 --tag stable
          mosaic remotes list --json
          mosaic remotes remove my-studio
        """
    ...
    func runRemotesCommand(commandArgs: [String], client: SocketClient, jsonOutput: Bool) throws {
        ...
        case "list", "ls":
            ...
            if remotes.isEmpty {
                print("No remotes. Add one: mosaic remotes add <name> --route <host:port>")
                return
            }
            ...
            guard !routeValues.isEmpty else {
                throw CLIError(message: """
                    remotes add requires at least one --route host:port.
                      ...
                      mosaic remotes add \(name) --route 100.64.1.2:51001
                    """)
            }
            ...
            guard let target = positionals.first, !target.isEmpty else {
                throw CLIError(message: """
                    remotes remove requires a name or deviceId.
                      ...
                      mosaic remotes remove <name-or-deviceId>

                    List remotes: mosaic remotes list
                    """)
            }
    ...
    /// link MosaicMobileCore); the app and server remain the authoritative check.
