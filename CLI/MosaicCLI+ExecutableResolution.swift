...
extension MosaicCLI {
    ...
    func isBundledProviderExecutable(at path: String) -> Bool {
        let candidate = URL(fileURLWithPath: path, isDirectory: false)
            .standardizedFileURL
            .path
        if isMosaicAppBundleResourceBinChild(candidate) {
            return true
        }
    ...
    func isMosaicClaudeWrapper(at path: String) -> Bool {
    ...
    func isMosaicClaudeCommandShim(at path: String) -> Bool {
        let candidate = URL(fileURLWithPath: path, isDirectory: false)
            .standardizedFileURL
            .path
        let environment = ProcessInfo.processInfo.environment
        let shimPaths = [
            environment["MOSAIC_CLAUDE_WRAPPER_SHIM"],
        ]
    ...
    func resolveClaudeExecutable(configuredCandidates: [String?], searchPath: String?) -> String? {
        for raw in configuredCandidates {
            ...
            guard FileManager.default.fileExists(atPath: trimmed, isDirectory: &isDirectory),
                  ...
                  !isMosaicClaudeCommandShim(at: trimmed),
                  !isMosaicClaudeWrapper(at: trimmed) else { continue }
    ...
    func resolveClaudeExecutable(searchPath: String?) -> String? {
        resolveExecutableInSearchPath(
            ...
            skip: { self.isMosaicClaudeCommandShim(at: $0) || self.isMosaicClaudeWrapper(at: $0) }
        )
    }
    ...
    /// in `MOSAIC_CLAUDE_TEAMS_SANDBOXED` so teammate respawns (which run as a separate
    /// `mosaic __tmux-compat` process and cannot see this argv) re-apply the same
    ...
    func claudeTeamsExtraEnvVars(commandArgs: [String]) -> [(key: String, value: String)] {
        ...
        if claudeTeamsHasDangerousSkipPermissions(commandArgs: commandArgs) {
            ...
            vars.append((key: "MOSAIC_CLAUDE_TEAMS_SANDBOXED", value: "1"))
        }
        return vars
    }
    ...
    /// The whole point of `mosaic claude-teams` is "just start a team." Claude Code's
    ...
    /// *what*?" — so a plain `mosaic claude-teams "make a demo team with 5 subagents"`
    ...
    /// restore command) so that stays canonical; restore re-invokes `mosaic
    ...
    var claudeTeamsTeamSpawnGuidance: String {
        """
        You are Claude Code running inside mosaic, started with `mosaic claude-teams`. \
    ...
    private func providerExecutableSearchDirectories(searchPath: String?) -> [String] {
        ...
        return directories.compactMap { rawDirectory in
            ...
            let standardized = URL(fileURLWithPath: trimmed, isDirectory: true)
                .standardizedFileURL
                .path
            guard !isMosaicAppBundleResourceBinDirectory(standardized) else { return nil }
    ...
    private func isMosaicAppBundleResourceBinDirectory(_ path: String) -> Bool {
    ...
    private func isMosaicAppBundleResourceBinChild(_ path: String) -> Bool {
        mosaicAppBundleResourceBinComponentIndex(path).map { index in
            URL(fileURLWithPath: path, isDirectory: false).standardizedFileURL.pathComponents.count > index + 4
        } ?? false
    }

    private func mosaicAppBundleResourceBinComponentIndex(_ path: String) -> Int? {
        let components = URL(fileURLWithPath: path).standardizedFileURL.pathComponents
        guard components.count >= 4 else { return nil }
        for index in components.indices {
            guard components[index].hasSuffix(".app"),
                  components[index].lowercased().contains("mosaic"),
