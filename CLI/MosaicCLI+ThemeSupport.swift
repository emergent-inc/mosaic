...
extension MosaicCLI {
    ...
    func themeDirectoryURLs() -> [URL] {
        ...
        for appSupportDirectory in MosaicApplicationSupportDirectories(environment: processEnv).userDirectories {
            appendIfExisting(
                appSupportDirectory
                    .appendingPathComponent(Self.mosaicThemeOverrideBundleIdentifier, isDirectory: true)
                    .appendingPathComponent("themes", isDirectory: true)
            )
        }
        appendIfExisting(
    ...
    func validatedThemeName(_ rawValue: String, availableThemes: [String]) throws -> String {
        ...
        throw CLIError(message: "Unknown theme '\(trimmed)'. Run 'mosaic themes' to list available themes.")
    }
    ...
    func themeConfigSearchURLs(targetBundleIdentifier: String) -> [URL] {
        ...
        for appSupportDirectory in MosaicApplicationSupportDirectories(
    ...
    func mosaicThemeOverrideConfigURL(targetBundleIdentifier: String) throws -> URL {
        guard let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
            throw CLIError(message: "Unable to resolve Application Support directory")
        }
        return MosaicGhosttyConfigPathResolver().editableConfigURL(
            currentBundleIdentifier: targetBundleIdentifier,
    ...
    func writeManagedThemeOverride(
        ...
        let fileManager = FileManager.default
        let configURL = try mosaicThemeOverrideConfigURL(targetBundleIdentifier: targetBundleIdentifier)
        ...
        let strippedContents = removingManagedThemeOverride(from: existingContents)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let block = """
        \(Self.mosaicThemesBlockStart)
        theme = \(rawThemeValue)
        \(Self.mosaicThemesBlockEnd)
        """
    ...
    func reloadThemesIfPossible(
        ...
        DistributedNotificationCenter.default().post(
            name: Notification.Name(Self.mosaicThemesReloadNotificationName),
            object: nil,
            userInfo: [
    ...
    func themeTargetBundleIdentifier(socketPath: String) -> String {
        bundleIdentifierForThemeReloadSocketPath(socketPath)
            ?? currentMosaicAppBundleIdentifier()
            ?? Self.mosaicThemeOverrideBundleIdentifier
    }
    ...
    private func bundleIdentifierForThemeReloadSocketPath(_ socketPath: String) -> String? {
        ...
        switch name {
        case "mosaic.sock":
            return Self.mosaicThemeOverrideBundleIdentifier
        case "mosaic-debug.sock":
            return "mosaic.com.emergent.app.debug"
        case "mosaic-nightly.sock":
            return "mosaic.com.emergent.app.nightly"
        case "mosaic-staging.sock":
            return "mosaic.com.emergent.app.staging"
        default:
            break
        }

        if name.range(of: #"^mosaic-\d+\.sock$"#, options: .regularExpression) != nil {
            return Self.mosaicThemeOverrideBundleIdentifier
        }

        if let slug = themeReloadSocketSlug(name, prefix: "mosaic-debug-", suffix: ".sock") {
            return "mosaic.com.emergent.app.debug.\(slug)"
        }
        if let slug = themeReloadSocketSlug(name, prefix: "mosaic-nightly-", suffix: ".sock") {
            return "mosaic.com.emergent.app.nightly.\(slug)"
        }
        if let slug = themeReloadSocketSlug(name, prefix: "mosaic-staging-", suffix: ".sock") {
    ...
    func currentMosaicAppBundleIdentifier() -> String? {
        if let bundleIdentifier = ProcessInfo.processInfo.environment["MOSAIC_BUNDLE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
