...
extension MosaicCLI {
    private static let ampExtensionMarker = "mosaic-amp-session-extension-marker"
    private static let ampExtensionFilename = "mosaic-session.ts"
    private static let ampExtensionSource = #"""
// mosaic-amp-session-extension-marker v2
// Bridges Amp session lifecycle events into mosaic's restorable session store
// AND reports live agent status (idle/thinking/tool calls/done/error) into
// the mosaic tab status bar.
// Installed by `mosaic hooks amp install` or `mosaic hooks setup`.
...
function hookEnvironment(cwd: string): NodeJS.ProcessEnv {
  ...
  if (!env.MOSAIC_AGENT_LAUNCH_ARGV_B64) {
    const argv = normalizedLaunchArgv();
    env.MOSAIC_AGENT_LAUNCH_KIND = "amp";
    env.MOSAIC_AGENT_LAUNCH_EXECUTABLE = argv[0] || resolveExecutable("amp");
    env.MOSAIC_AGENT_LAUNCH_ARGV_B64 = base64NulSeparated(argv);
    env.MOSAIC_AGENT_LAUNCH_CWD = cwd || process.cwd();
  }
  return env;
}
...
function sendHook(
  ...
  if (process.env.MOSAIC_AMP_HOOKS_DISABLED === "1") return;
  if (!process.env.MOSAIC_SURFACE_ID) return;
  if (!sessionId) return;
  ...
  const mosaic = process.env.MOSAIC_AMP_MOSAIC_BIN || "mosaic";
  try {
    const child = spawn(mosaic, ["hooks", "amp", subcommand], {
...
// Fires `mosaic set-status` / `mosaic clear-status` / `mosaic log` so the tab
...
// Short verbs shown in the mosaic status bar for each Amp tool.
...
// SF Symbol names rendered inside the mosaic status badge.
...
// Pin every mosaic call to the workspace this plugin process was launched in.
// mosaic sets MOSAIC_WORKSPACE_ID in every pane env, so this is stable across
// async callbacks. Without --workspace, mosaic defaults to whichever pane is
...
function workspaceArgs(): string[] {
  const ws = process.env.MOSAIC_WORKSPACE_ID;
  return ws ? ["--workspace", ws] : [];
}

// Sanitized environment for fire-and-forget mosaic status subprocesses.
// Strips Amp-provided secrets (`AMP_API_KEY`) so we never propagate them to
// every spawned `mosaic set-status` / `mosaic log` / `mosaic clear-status` child.
...
function runMosaic(args: string[]): void {
  if (process.env.MOSAIC_AMP_HOOKS_DISABLED === "1") return;
  if (!process.env.MOSAIC_SURFACE_ID) return;
  const mosaic = process.env.MOSAIC_AMP_MOSAIC_BIN || "mosaic";
  try {
    const child = spawn(mosaic, args, {
...
function clearStatus(): void {
  runMosaic(["clear-status", STATUS_KEY, ...workspaceArgs()]);
}
...
export default function (amp: PluginAPI) {
  ...
  // final status badge with "thinking". mosaic runs one Amp session per terminal
...
"""#
    ...
    func installAmpExtensionHooks(_ def: AgentHookDef) throws {
        ...
        if !existing.isEmpty, !existing.contains(Self.ampExtensionMarker) {
            throw CLIError(message: "\(extensionURL.path) exists and is not a mosaic plugin; leaving it alone")
        }
        if !skipConfirm {
    ...
    func uninstallAmpExtensionHooks(_ def: AgentHookDef) throws {
        ...
        guard fm.fileExists(atPath: extensionURL.path) else {
            print("No Amp mosaic plugin found at \(extensionURL.path)")
            return
        }
        let existing = (try? String(contentsOf: extensionURL, encoding: .utf8)) ?? ""
        guard existing.contains(Self.ampExtensionMarker) else {
            print("Refusing to remove \(extensionURL.path): missing mosaic marker")
            return
        }
        try fm.removeItem(at: extensionURL)
        print("Removed Amp mosaic plugin from \(extensionURL.path)")
    }
}
