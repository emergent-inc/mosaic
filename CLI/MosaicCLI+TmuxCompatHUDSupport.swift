...
extension MosaicCLI {
    ...
    func tmuxOMXHudConfigDisablesHud(cwd: String?) -> Bool {
        ...
        if tmuxBooleanValue(environment["OMX_HUD_ENABLED"]) == false
            || tmuxBooleanValue(environment["MOSAIC_OMX_HUD_ENABLED"]) == false
            || tmuxBooleanValue(environment["OMX_HUD_DISABLED"]) == true
            || tmuxBooleanValue(environment["MOSAIC_OMX_HUD_DISABLED"]) == true {
            return true
        }
    ...
    func tmuxCommandLooksLikeOMXHud(_ commandTokens: [String]) -> Bool {
        ...
        let environment = ProcessInfo.processInfo.environment
        let launchedThroughOMXShim = environment["MOSAIC_OMX_MOSAIC_BIN"] != nil
    ...
    func tmuxDebugDiagnosticsEnabled() -> Bool {
        let environment = ProcessInfo.processInfo.environment
        return tmuxBooleanValue(environment["MOSAIC_DEBUG"]) == true
            || tmuxBooleanValue(environment["MOSAIC_TMUX_DEBUG"]) == true
    }

    func tmuxWriteDebugDiagnostic(_ message: String) {
        guard tmuxDebugDiagnosticsEnabled(),
              let data = "[mosaic] \(message)\n".data(using: .utf8) else {
            return
        }
        cliWriteStderr(data)
    }
