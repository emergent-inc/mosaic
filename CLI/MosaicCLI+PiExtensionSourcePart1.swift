...
// mosaic-pi-session-extension-marker v2
// Bridges Pi session lifecycle, tool telemetry, notifications, and resume bindings into mosaic.
// Installed by `mosaic hooks pi install` or `mosaic hooks setup`.
...
function safeMosaicEnvKey(key: string): boolean {
  if (key.startsWith("MOSAIC_TEST_PI_")) return !secretLikeEnvKey(key);
  if (key.startsWith("MOSAIC_AGENT_LAUNCH_")) return !secretLikeEnvKey(key);
  if (key === "MOSAIC_AGENT_HOOK_STATE_DIR") return true;
  if (key === "MOSAIC_PI_MOSAIC_BIN" || key === "MOSAIC_PI_HOOKS_DISABLED") return true;
  if (key === "MOSAIC_SURFACE_ID" || key === "MOSAIC_WORKSPACE_ID" || key === "MOSAIC_WINDOW_ID") return true;
  if (key === "MOSAIC_PANE_ID" || key === "MOSAIC_TAB_ID" || key === "MOSAIC_PANEL_ID") return true;
  if (key === "MOSAIC_SOCKET" || key === "MOSAIC_SOCKET_PATH") return true;
  if (key === "MOSAIC_BUNDLE_ID" || key === "MOSAIC_BUNDLED_CLI_PATH") return true;
  if (key === "MOSAIC_CLI_SENTRY_DISABLED" || key === "MOSAIC_DEBUG_LOG") return true;
  return false;
}

function shouldPreserveEnvKey(key: string): boolean {
  if (safeMosaicEnvKey(key)) return true;
...
function hookEnvironment(cwd: string, includeSocketPassword = false): NodeJS.ProcessEnv {
  ...
  // Only mosaic CLI children need the socket credential; keep it out of the generic allowlist.
  if (includeSocketPassword) {
    const socketPassword = process.env.MOSAIC_SOCKET_PASSWORD;
    if (socketPassword) env.MOSAIC_SOCKET_PASSWORD = socketPassword;
  }
  if (!env.MOSAIC_AGENT_LAUNCH_ARGV_B64) {
    const argv = normalizedLaunchArgv();
    env.MOSAIC_AGENT_LAUNCH_KIND = "pi";
    env.MOSAIC_AGENT_LAUNCH_EXECUTABLE = argv[0] || resolveExecutable("pi");
...
function warn(ctx: ExtensionContext | null, message: string, details: Record<string, unknown> = {}): void {
  const payload = { source: "mosaic-pi-extension", level: "warning", message, ...details };
  try {
    console.warn(JSON.stringify(payload));
  } catch (_) {
    console.warn(`[mosaic-pi-extension] ${message}`);
  }
  const ui = (ctx as unknown as { ui?: { notify?: (message: string, type?: string) => void } } | null)?.ui;
  try {
    ui?.notify?.("mosaic Pi integration warning - check the terminal for details", "warning");
  } catch (_) {}
}

function mosaicExecutable(): string {
  return process.env.MOSAIC_PI_MOSAIC_BIN || "mosaic";
}

function runMosaic(args: string[], cwd: string, input?: string): CommandResult {
  try {
    const result = spawnSync(mosaicExecutable(), args, {
      input,
      encoding: "utf8",
