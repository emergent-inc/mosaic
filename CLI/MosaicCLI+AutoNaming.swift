...
// the bundled CLI target (which drives it from agent hooks) and the mosaic-unit
...
/// debounce shape proven by community prior art (emergent-inc/mosaic#2043)
...
/// that would recurse into mosaic hooks or the parent agent session while
...
struct AutoNamingEnvironmentPolicy: Sendable {
    /// Exact variables that mark a live agent session or mosaic terminal and
    ...
    func summarizerEnvironment(from env: [String: String]) -> [String: String] {
        env.filter { key, _ in
            if key.hasPrefix("MOSAIC_") { return false }
