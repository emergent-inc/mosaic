...
extension MosaicCLI {
    private static let ompExtensionMarker = "mosaic-omp-session-extension-marker"
    private static let ompExtensionFilename = "mosaic-omp-session.ts"
    private static let ompExtensionSource = #"""
// mosaic-omp-session-extension-marker v1
// Bridges OMP session lifecycle events into mosaic's restorable session store.
// Installed by `mosaic hooks omp install` or `mosaic hooks setup`.
...
function hookEnvironment(cwd: string): NodeJS.ProcessEnv {
  ...
  if (!env.MOSAIC_AGENT_LAUNCH_ARGV_B64) {
    ...
    env.MOSAIC_AGENT_LAUNCH_KIND = "omp";
    env.MOSAIC_AGENT_LAUNCH_EXECUTABLE = argv[0] || resolveExecutable("omp");
...
function hookInvocation(subcommand: string, ctx: ExtensionContext, extra: Record<string, unknown> = {}): HookInvocation | null {
  if (process.env.MOSAIC_OMP_HOOKS_DISABLED === "1") return null;
  if (!process.env.MOSAIC_SURFACE_ID) return null;
  ...
  const mosaic = process.env.MOSAIC_OMP_MOSAIC_BIN || "mosaic";
  return {
    mosaic,
    cwd,
...
async function sendHook(subcommand: string, ctx: ExtensionContext, extra: Record<string, unknown> = {}): Promise<void> {
  ...
  await new Promise<void>((resolve) => {
    ...
    try {
      const child = spawn(invocation.mosaic, ["hooks", "omp", subcommand], {
        env: invocation.env,
...
export default function mosaicOmpSessionExtension(api: ExtensionAPI) {
    ...
    func installOmpExtensionHooks(_ _: AgentHookDef) throws {
        ...
        if !existing.isEmpty, !existing.contains(Self.ompExtensionMarker) {
            throw CLIError(message: String.localizedStringWithFormat(
                String(
                    localized: "cli.hooks.omp.error.notMosaicExtension",
                    defaultValue: "%@ exists and is not a mosaic extension; leaving it alone"
                ),
                extensionURL.path
            ))
        }
    ...
    func uninstallOmpExtensionHooks(_ _: AgentHookDef) throws {
        ...
        guard fm.fileExists(atPath: extensionURL.path) else {
            print(String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "No OMP mosaic extension found at %@"
                ),
                extensionURL.path
            ))
            return
        ...
        guard existing.contains(Self.ompExtensionMarker) else {
            print(String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "Refusing to remove %@: missing mosaic marker"
                ),
                extensionURL.path
            ))
            return
        ...
        print(String.localizedStringWithFormat(
            String(
                ...
                defaultValue: "Removed OMP mosaic extension from %@"
            ),
            extensionURL.path
        ))
    }
