...
extension MosaicCLI {
    private static let piExtensionMarker = "mosaic-pi-session-extension-marker"
    private static let piExtensionFilename = "mosaic-session.ts"
    ...
    func installPiExtensionHooks(_ def: AgentHookDef) throws {
        ...
        if !existing.isEmpty, !existing.contains(Self.piExtensionMarker) {
            throw CLIError(message: String.localizedStringWithFormat(
                String(
                    localized: "cli.hooks.pi.error.notMosaicExtension",
    ...
    func uninstallPiExtensionHooks(_ def: AgentHookDef) throws {
        ...
        guard fm.fileExists(atPath: extensionURL.path) else {
            print(String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "No Pi mosaic extension found at %@"
                ),
                extensionURL.path
            ))
            return
        ...
        print(String.localizedStringWithFormat(
            String(
                ...
                defaultValue: "Removed Pi mosaic extension from %@"
            ),
            extensionURL.path
        ))
    }
