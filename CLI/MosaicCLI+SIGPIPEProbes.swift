...
extension MosaicCLI {
    ...
    private func sigpipeProbeExecutablePath() throws -> String {
        let candidate: String? = {
            if let explicit = ProcessInfo.processInfo.environment["MOSAIC_CLI_PATH"],
               !explicit.isEmpty {
                return explicit
            }
        ...
        guard let path = candidate,
            ...
            throw CLIError(message: "SIGPIPE probe could not resolve mosaic executable path")
        }
        return path
    }
    ...
    func runSIGPIPEProbe(commandArgs: [String]) throws {
        ...
        let inspectionURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-sigpipe-\(UUID().uuidString).json")
