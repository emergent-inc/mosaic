...
function sendHook(subcommand: string, ctx: ExtensionContext, extra: HookExtra = {}): boolean {
  if (process.env.MOSAIC_PI_HOOKS_DISABLED === "1") return true;
  if (!process.env.MOSAIC_SURFACE_ID) return true;
  ...
  const result = runMosaic(["hooks", "pi", subcommand], cwd, JSON.stringify(payload));
  if (!result.ok) {
    warn(ctx, "mosaic hook command failed", {
      subcommand,
...
function surfaceTargetArgs(): string[] | null {
  const surfaceId = firstString(process.env.MOSAIC_SURFACE_ID);
  if (!surfaceId) return null;
  const args: string[] = [];
  const workspaceId = firstString(process.env.MOSAIC_WORKSPACE_ID);
...
function ensureResumeBinding(ctx: ExtensionContext, sessionId: string, cwd: string): void {
  if (process.env.MOSAIC_PI_HOOKS_DISABLED === "1") return;
  ...
  const verified = parseJSONOutput(runMosaic(["--json", "surface", "resume", "get", ...target], cwd));
...
function sendFeed(eventName: "PreToolUse" | "PostToolUse", ctx: ExtensionContext, event: unknown, extra: HookExtra = {}): void {
  ...
  try {
    const child = spawn(mosaicExecutable(), ["hooks", "feed", "--source", "pi", "--event", eventName], {
...
export default function mosaicPiSessionExtension(pi: ExtensionAPI) {
  ...
  pi.on("agent_end", async (event, ctx) => {
    ...
    if (notificationRouted) stopPayload.mosaic_notification_routed = true;
