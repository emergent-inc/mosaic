...
extension MosaicCLI {
    ...
    /// mosaic subcommand they call and the codex hook timeout (ms). Lifecycle
    ...
    /// truth for `mosaic-codex-wrapper`'s injection, mirrored from the historic
    /// hand-rolled `mosaic_codex_add_hook` calls in the wrapper.
    static let codexWrapperInjectionEvents: [(agentEvent: String, mosaicSubcommand: String, timeoutMs: Int)] = [
    ...
    /// splice ahead of the user's args to enable + inject mosaic's fire-and-forget
    ...
    /// hook returns `{}` to codex instantly and backgrounds the real mosaic call.
    ...
    func emitCodexWrapperInjectArgs() throws {
        ...
        // invocations, so they are written once into a mosaic-owned dir (~/.mosaic/
        ...
        for event in Self.codexWrapperInjectionEvents {
            let ff = Self.codexFireAndForgetAgentHookShellCommand(
                "mosaic hooks codex \(event.mosaicSubcommand)", for: codexDef
            )
            let command: String
            if let scriptPath = hooksDir.flatMap({
                Self.writeCodexHookScript(subcommand: event.mosaicSubcommand, body: ff, in: $0)
            }), !scriptPath.contains("'''") {
                command = scriptPath
            } else {
                command = ff
    ...
    /// The mosaic-owned directory holding the generated codex hook scripts.
    /// `~/.mosaic/hooks` (NOT the user's `~/.codex`), created on demand. Returns
    ...
    static func writeCodexHookScript(subcommand: String, body: String, in dir: URL) -> String? {
        ...
        let url = dir.appendingPathComponent("mosaic-codex-hook-\(safeName).sh", isDirectory: false)
        let contents = "#!/bin/sh\n\(body)\n"
    ...
    static func codexFireAndForgetAgentHookShellCommand(_ command: String, for def: AgentHookDef) -> String {
        ...
        return [
            "mosaic_cli=\"${MOSAIC_BUNDLED_CLI_PATH:-}\"",
            "if [ -z \"$mosaic_cli\" ] || [ ! -x \"$mosaic_cli\" ]; then mosaic_cli=\"$(command -v mosaic 2>/dev/null || true)\"; fi",
            "agent_pid=\"${MOSAIC_CODEX_PID:-${PPID:-}}\"",
            "if [ -n \"$MOSAIC_SURFACE_ID\" ] && [ \"$\(def.disableEnvVar)\" != \"1\" ] && [ -n \"$mosaic_cli\" ]; then payload=\"$(mktemp \"${TMPDIR:-/tmp}/mosaic-codex-hook.XXXXXX\" 2>/dev/null || mktemp -t mosaic-codex-hook 2>/dev/null)\" || { echo '{}'; exit 0; }; cat >\"$payload\" || true; if [ -n \"${MOSAIC_SOCKET_PATH:-}\" ]; then MOSAIC_CODEX_PID=\"$agent_pid\" nohup sh -c '\(runner)' mosaic-codex-hook \"$payload\" \"$mosaic_cli\" --socket \"$MOSAIC_SOCKET_PATH\" \(routedArguments) >/dev/null 2>&1 & else MOSAIC_CODEX_PID=\"$agent_pid\" nohup sh -c '\(runner)' mosaic-codex-hook \"$payload\" \"$mosaic_cli\" \(routedArguments) >/dev/null 2>&1 & fi; echo '{}'; else echo '{}'; fi",
        ].joined(separator: "; ")
    }
}
