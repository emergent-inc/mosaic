...
extension MosaicCLI {
    ...
    private func buildMemoryPayload(
        ...
        do {
            ...
            throw CLIError(message: String(localized: "cli.memory.error.diagnosticsUnsupported", defaultValue: "mosaic memory requires a running mosaic build that supports memory diagnostics"))
        }
    }
    ...
    private func renderMemoryText(
        ...
        lines.append("  \(appName.isEmpty ? "mosaic" : appName) pid=\(appPID)")
