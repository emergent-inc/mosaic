...
final class DisplayResolutionRegressionUITests: XCTestCase {
    private let defaultDisplayHarnessManifestPath = "/tmp/mosaic-ui-test-display-harness.json"
    ...
    override func setUp() {
        ...
        let tempPrefix = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-ui-test-display-\(token)")
            .path
        launchTag = "ui-tests-display-resolution-\(token.prefix(8))"
        diagnosticsPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-ui-test-display-churn-\(token).json")
            .path
    ...
    private let prelaunchManifestPath = "/tmp/mosaic-ui-test-prelaunch.json"
    ...
    private func loadPrebuiltHelperBinaryPath(_ env: [String: String]) -> String? {
        guard let helperBinaryPath = env["MOSAIC_UI_TEST_DISPLAY_HELPER_BINARY_PATH"],
    ...
    private func loadExternalHarnessFromEnvironment(_ env: [String: String]) -> ExternalDisplayHarness? {
        guard let readyPath = env["MOSAIC_UI_TEST_DISPLAY_READY_PATH"], !readyPath.isEmpty,
              let displayIDPath = env["MOSAIC_UI_TEST_DISPLAY_ID_PATH"], !displayIDPath.isEmpty,
              let donePath = env["MOSAIC_UI_TEST_DISPLAY_DONE_PATH"], !donePath.isEmpty else {
            return nil
        }
        ...
        return ExternalDisplayHarness(
            ...
            startPath: env["MOSAIC_UI_TEST_DISPLAY_START_PATH"],
            donePath: donePath,
            logPath: env["MOSAIC_UI_TEST_DISPLAY_LOG_PATH"],
            helperBinaryPath: nil
        )
    }

    private func loadExternalHarnessFromManifest(_ env: [String: String]) -> ExternalDisplayHarness? {
        let manifestPath = env["MOSAIC_UI_TEST_DISPLAY_HARNESS_MANIFEST_PATH"] ?? defaultDisplayHarnessManifestPath
    ...
    private func launchAppProcess(targetDisplayID: String) throws {
        ...
        let logPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-ui-test-app-\(launchTag).log").path
    ...
    private func resolveAppBinaryPath() throws -> String {
        ...
        //   .../Build/Products/Debug/mosaicUITests-Runner.app/Contents/PlugIns/mosaicUITests.xctest
        // The app binary is at:
        //   .../Build/Products/Debug/Mosaic DEV.app/Contents/MacOS/Mosaic DEV
        let testBundle = Bundle(for: Self.self)
        let productsDir = testBundle.bundleURL
            .deletingLastPathComponent()  // -> .../Contents/PlugIns
            .deletingLastPathComponent()  // -> .../Contents
            .deletingLastPathComponent()  // -> .../mosaicUITests-Runner.app
            .deletingLastPathComponent()  // -> .../Debug
        let binaryPath = productsDir
            .appendingPathComponent("Mosaic DEV.app")
            .appendingPathComponent("Contents/MacOS/Mosaic DEV")
            .path
    ...
    private func launchEnvironment(targetDisplayID: String) -> [String: String] {
        [
            ...
            "MOSAIC_UI_TEST_DIAGNOSTICS_PATH": diagnosticsPath,
            "MOSAIC_UI_TEST_DISPLAY_RENDER_STATS": "1",
            "MOSAIC_UI_TEST_TARGET_DISPLAY_ID": targetDisplayID,
            "MOSAIC_TAG": launchTag,
        ]
    }
