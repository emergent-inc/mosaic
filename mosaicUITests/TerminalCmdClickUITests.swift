...
final class TerminalCmdClickUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        fixtureDirectoryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-ui-test-terminal-cmd-click-\(UUID().uuidString)", isDirectory: true)
    ...
    func testCmdClickPngOpensInMosaicFilePreviewWhenEnabled() throws {
        let app = launchApp(
            displayMode: .raw,
        ...
        XCTAssertEqual(
            ...
            "1",
            "Expected supported-file routing to create a mosaic file preview. result=\(result)"
        )
        XCTAssertTrue(
            waitForOpenCountToStay(0, timeout: 0.75),
            "Expected mosaic file preview routing to avoid the external opener. opened=\(loadCapturedOpenPaths())"
        )
    }
    ...
    func testCmdClickMarketingSkillMarkdownPathWithTrailingPeriodOpensMarkdownViewer() throws {
        ...
        let app = launchApp(
            ...
            captureOpenPaths: true,
            captureHoverDiagnostics: false,
            openMarkdownInMosaicViewer: true
        )
        defer { app.terminate() }
        ...
        XCTAssertEqual(
            ...
            "1",
            "Expected Markdown paths to open in the mosaic Markdown viewer. result=\(result)"
        )
        XCTAssertEqual(
            result["lastCommandOpenedInFilePreview"] as? String,
            "0",
        ...
        XCTAssertTrue(
            waitForOpenCountToStay(0, timeout: 0.75),
            "Expected mosaic Markdown routing to avoid the external opener. opened=\(loadCapturedOpenPaths())"
        )
    }
    ...
    func testCmdClickQuotedAbsoluteMarkdownPathWithTrailingPeriodOpensMarkdownViewer() throws {
        ...
        XCTAssertEqual(
            result["lastCommandOpenedInMarkdownViewer"] as? String,
            "1",
            "Expected absolute Markdown paths to open in the mosaic Markdown viewer. result=\(result)"
        )
        XCTAssertEqual(
            result["lastCommandOpenedInFilePreview"] as? String,
            "0",
    ...
    private func launchApp(
        ...
        openSupportedFilesInMosaic: Bool = false,
        openMarkdownInMosaicViewer: Bool? = nil,
        quicklookOverride: String? = nil,
        viewportOffsetDelta: Int? = nil
    ) -> XCUIApplication {
        let app = XCUIApplication()
        app.launchEnvironment["MOSAIC_TAG"] = "ui-test-terminal-cmd-click"
        app.launchEnvironment["MOSAIC_UI_TEST_MODE"] = "1"
        app.launchEnvironment["MOSAIC_UI_TEST_TERMINAL_CMD_CLICK_SETUP"] = "1"
        app.launchEnvironment["MOSAIC_UI_TEST_TERMINAL_CMD_CLICK_PATH"] = setupDataPath
        app.launchEnvironment["MOSAIC_UI_TEST_TERMINAL_CMD_CLICK_COMMAND_PATH"] = commandPath
        app.launchEnvironment["MOSAIC_UI_TEST_TERMINAL_CMD_CLICK_FIXTURE_DIR"] = fixtureDirectoryURL.path
        app.launchEnvironment["MOSAIC_UI_TEST_TERMINAL_CMD_CLICK_FILE_NAME"] = fileName
        app.launchEnvironment["MOSAIC_UI_TEST_TERMINAL_CMD_CLICK_DISPLAY_MODE"] = displayMode.rawValue
        app.launchEnvironment["MOSAIC_UI_TEST_TERMINAL_CMD_CLICK_LINE_FORMAT"] = lineFormat.rawValue
        app.launchEnvironment["MOSAIC_UI_TEST_OPEN_SUPPORTED_FILES_IN_MOSAIC"] = openSupportedFilesInMosaic ? "1" : "0"
        if !displaySuffix.isEmpty {
            app.launchEnvironment["MOSAIC_UI_TEST_TERMINAL_CMD_CLICK_DISPLAY_SUFFIX"] = displaySuffix
        }
        if displayAsAbsolutePath {
            app.launchEnvironment["MOSAIC_UI_TEST_TERMINAL_CMD_CLICK_DISPLAY_AS_ABSOLUTE_PATH"] = "1"
        }
        if let openMarkdownInMosaicViewer {
            app.launchEnvironment["MOSAIC_UI_TEST_OPEN_MARKDOWN_IN_MOSAIC_VIEWER"] = openMarkdownInMosaicViewer ? "1" : "0"
        }
        if !linePrefix.isEmpty {
            app.launchEnvironment["MOSAIC_UI_TEST_TERMINAL_CMD_CLICK_LINE_PREFIX"] = linePrefix
        }
        if !extraFileNames.isEmpty,
           let data = try? JSONSerialization.data(withJSONObject: extraFileNames, options: [.sortedKeys]),
           let json = String(data: data, encoding: .utf8) {
            app.launchEnvironment["MOSAIC_UI_TEST_TERMINAL_CMD_CLICK_EXTRA_FILE_NAMES_JSON"] = json
        }
        if captureOpenPaths {
            app.launchEnvironment["MOSAIC_UI_TEST_CAPTURE_OPEN_PATH"] = openCapturePath
        }
        if lineFormat == .osc8 {
            app.launchEnvironment["MOSAIC_UI_TEST_CAPTURE_OPEN_URL_PATH"] = openURLCapturePath
        }
        if captureHoverDiagnostics {
            app.launchEnvironment["MOSAIC_UI_TEST_CMD_HOVER_DIAGNOSTICS_PATH"] = hoverDiagnosticsPath
        }
        if let quicklookOverride {
            app.launchEnvironment["MOSAIC_UI_TEST_TERMINAL_CMD_CLICK_QUICKLOOK_OVERRIDE"] = quicklookOverride
        }
        if let viewportOffsetDelta {
            app.launchEnvironment["MOSAIC_UI_TEST_TERMINAL_CMD_CLICK_VIEWPORT_OFFSET_DELTA"] = String(viewportOffsetDelta)
        }
