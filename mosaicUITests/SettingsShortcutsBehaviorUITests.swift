...
/// `shortcuts.bindings` in mosaic.json (the JSON config store), and the
...
/// - `SettingsKeyboardShortcutsOpenSettingsFileButton` — open mosaic.json.
...
final class SettingsShortcutsBehaviorUITests: SettingsUITestCase {
    ...
    ///   - off: "Turn this on to show or hide all mosaic windows from any app."
    ///   - on:  "Press the shortcut from any app to show or hide all mosaic windows."
    ...
    func testEnableToggleSwapsSubtitleCopy() {
        ...
        let offSubtitle = "Turn this on to show or hide all mosaic windows from any app."
        let onSubtitle = "Press the shortcut from any app to show or hide all mosaic windows."
    ...
    /// starting state, so the test leaves mosaic.json clean.
...
//   chord shows/hides all mosaic windows from any app. That registration and
...
//   ("Open mosaic.json") — Calls SettingsHostActions.openConfigInExternalEditor(),
//   which runs NSWorkspace.shared.open(configFileURL) to hand the mosaic.json
...
//   the mosaic UI surface to assert against. Not e2e-testable from XCUITest;
