...
/// (`Packages/macOS/MosaicSettingsUI/.../Sections/BrowserSection.swift`).
///
...
/// `MOSAIC_UI_TEST_BROWSER_IMPORT_FIXTURE` env seam, so we can launch with a
...
final class SettingsBrowserBehaviorUITests: SettingsUITestCase {
    ...
    private func makeLaunchedAppWithImportFixture() -> XCUIApplication {
        ...
        app.launchEnvironment["MOSAIC_UI_TEST_BROWSER_IMPORT_FIXTURE"] =
    ...
    //   never writes (only the legacy `mosaicApp.browserEnabledBinding`
    ...
    //   mosaic without a warning prompt. The effect (warning prompt shown or
    ...
    //   only via dedicated MOSAIC_UI_TEST_BROWSER_IMPORT_HINT_* launch seams,
