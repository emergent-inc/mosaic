...
/// "Account + mosaic.json + Reset" group: the Account identity row, the
/// mosaic.json config-file/docs card, and the Reset All Settings button.
///
/// These three sidebar sections (`Account`, `mosaic.json`, `Reset`) are
...
/// control whose effect is observable purely inside the mosaic app surface
...
/// - TIER 2: mosaic.json "Open" button (opens the config file in an
///   external editor — leaves the mosaic process, no in-app element).
/// - TIER 3: Account Sign In / Sign Out (browser auth round trip),
///   mosaic.json "Open Docs" link (external browser).
///
/// The Account identity row and the mosaic.json card *layout* are still
...
final class SettingsAccountJSONResetBehaviorUITests: SettingsUITestCase {
    ...
    /// (TIER 3 below), which XCUITest cannot observe inside mosaic.
    ...
    /// The mosaic.json card renders its two action rows: the User config file
    /// row with an "Open" button (`SettingsJSONOpenButton`) and the
    /// Documentation row with an "Open Docs" link (`SettingsJSONDocsLink`).
    /// Presence guard only — both controls leave the mosaic process when
    ...
    func testSettingsJSONCardRendersOpenAndDocsControls() {
        ...
        navigate(window, to: "mosaic.json")
        ...
        let openButton = requireElement(
            ...
            timeout: 5.0,
            description: "mosaic.json Open button"
        )
        ...
        let docsLink = requireElement(
            ...
            timeout: 4.0,
            description: "mosaic.json Open Docs link"
        )
    ...
    // TIER 2 (needs runtime seam): mosaic.json "Open" button (SettingsJSONOpenButton)
    ...
    //   user's default editor for ~/.config/mosaic/mosaic.json in a *separate*
    //   application. mosaic opens no in-app window and changes no in-app element,
    //   so XCUITest (scoped to the mosaic process) cannot observe the effect.
    ...
    //   mosaic sign-in page in the system browser for an OAuth-style round trip;
    ...
    // TIER 3 (not e2e): mosaic.json "Open Docs" link (SettingsJSONDocsLink)
    //   — a SwiftUI `Link` to https://mosaic.inc/docs/configuration#mosaic-json that
    //   opens in the system browser. Cross-app navigation with no mosaic-side
