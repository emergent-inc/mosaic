...
final class MultiWindowNotificationsUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        dataPath = "/tmp/mosaic-ui-test-multi-window-notifs-\(UUID().uuidString).json"
        socketPath = "/tmp/mosaic-ui-test-socket-\(UUID().uuidString).sock"
    ...
    func testNotificationsRouteToCorrectWindow() {
        ...
        app.launchEnvironment["MOSAIC_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] = "1"
        app.launchEnvironment["MOSAIC_UI_TEST_MULTI_WINDOW_NOTIF_PATH"] = dataPath
        app.launchEnvironment["MOSAIC_TAG"] = launchTag
        ...
        XCTAssertTrue(
            ...
            "Expected mosaic to be foreground before sending notification shortcut. state=\(app.state.rawValue)"
        )
    ...
    func testNotificationsPopoverCanCloseViaShortcutAndEscape() {
        ...
        XCTAssertTrue(
            ensureAppForegroundForInteraction(app, timeout: 6.0),
            "Expected mosaic to be foreground before opening notifications popover. state=\(app.state.rawValue)"
        )
    ...
    func testEmptyNotificationsPopoverBlocksTerminalTyping() throws {
        ...
        app.launchEnvironment["MOSAIC_SOCKET_PATH"] = socketPath
        app.launchEnvironment["MOSAIC_SOCKET_MODE"] = "allowAll"
        app.launchEnvironment["MOSAIC_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["MOSAIC_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["MOSAIC_TAG"] = launchTag
        launchAllowingHeadlessBackgroundActivation(app)
        XCTAssertTrue(
            ensureAppRunningAfterLaunch(app, timeout: 12.0),
        ...
        XCTAssertTrue(
            ensureAppForegroundForInteraction(app, timeout: 6.0),
            "Expected mosaic to be foreground before opening empty notifications popover. state=\(app.state.rawValue)"
        )
        app.typeKey("i", modifierFlags: [.command])
        ...
        let marker = "mosaic_notif_block_\(UUID().uuidString.replacingOccurrences(of: "-", with: "").prefix(8))"
    ...
    func testNotifyCLIDoesNotStealFocusAcrossWindows() throws {
        ...
        app.launchEnvironment["MOSAIC_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["MOSAIC_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["MOSAIC_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] = "1"
        app.launchEnvironment["MOSAIC_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] = "1"
        app.launchEnvironment["MOSAIC_TAG"] = launchTag
        launchAllowingHeadlessBackgroundActivation(app)
        XCTAssertTrue(
            ensureAppRunningAfterLaunch(app, timeout: 12.0),
        ...
        let commandStatusPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-ui-test-notify-\(commandResultStem).status")
            .path
        let commandStdoutPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-ui-test-notify-\(commandResultStem).stdout")
            .path
        let commandStderrPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-ui-test-notify-\(commandResultStem).stderr")
            .path
        let commandScriptPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-ui-test-notify-\(commandResultStem).sh")
            .path
        defer {
        ...
        guard let bundledCLIPath = resolveMosaicCLIPaths(strategy: .bundledOnly).first else {
            XCTFail("Failed to locate bundled mosaic CLI for notify regression test")
            return
        }
        ...
        do {
            ...
            XCTFail(
                "Failed to write delayed bundled `mosaic notify` script. " +
        ...
        XCTAssertTrue(
            ensureAppForegroundForInteraction(app, timeout: 6.0),
            "Expected mosaic to be foreground before typing delayed notify command. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected mosaic to move to background before delayed notify command runs. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected delayed bundled `mosaic notify` command to finish without foregrounding mosaic. state=\(app.state.rawValue)"
        )
        ...
        // bundled `mosaic notify` command has finished while the app stayed
        ...
        XCTAssertFalse(
            ...
            "Expected mosaic to remain in background after bundled `mosaic notify`. state=\(app.state.rawValue) stderr=\(notifyStderr)"
        )
        guard notifyExitStatus == "0" else {
            XCTFail(
                "Expected bundled `mosaic notify` launched from the in-app shell to succeed. " +
    ...
    func testOpenNotificationCLISelectsWorkspaceSurfaceAndMarksRowRead() throws {
        ...
        let notifyResult = runMosaicCommand(
        ...
        let beforeToken = loadData()?["focusToken"]
        let openResult = runMosaicCommand(
            socketPath: socketPath,
            arguments: [
                "open-notification",
                "--id",
    ...
    func testNewWorkspaceCLIWindowFlagTargetsSpecificWindow() throws {
        ...
        app.launchEnvironment["MOSAIC_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["MOSAIC_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["MOSAIC_UI_TEST_WINDOW_ROUTE_CLI"] = "1"
        app.launchEnvironment["MOSAIC_UI_TEST_WINDOW_ROUTE_CLI_TITLE"] = title
        app.launchEnvironment["MOSAIC_TAG"] = launchTag
        launchAllowingHeadlessBackgroundActivation(app)
        XCTAssertTrue(
            ensureAppRunningAfterLaunch(app, timeout: 12.0),
        ...
        XCTAssertEqual(
            ...
            "1",
            "Expected bundled `mosaic --window` route test hook to finish. failure=\(setup["windowRouteFailure"] ?? "")"
        )
    ...
    private func waitForMosaicPing(timeout: TimeInterval) -> (stdout: String?, stderr: String?) {
        var lastStdout: String?
        var lastStderr: String?
        let didSucceed = waitForCondition(timeout: timeout) {
            let result = self.runMosaicCommand(
        ...
        let result = runMosaicCommand(socketPath: socketPath, arguments: ["ping"], responseTimeoutSeconds: 2.0)
        let stdout = result.stdout.isEmpty ? nil : result.stdout
        let stderr = result.stderr.isEmpty ? nil : result.stderr
    ...
    private func runMosaicCommand(
        socketPath: String,
        arguments: [String],
        responseTimeoutSeconds: Double = 3.0,
        cliStrategy: MosaicCLIStrategy = .any
    ) -> (terminationStatus: Int32, stdout: String, stderr: String) {
        var args = ["--socket", socketPath]
        args.append(contentsOf: arguments)
        var environment = ProcessInfo.processInfo.environment
        environment["MOSAICTERM_CLI_RESPONSE_TIMEOUT_SEC"] = String(responseTimeoutSeconds)

        let cliPaths = resolveMosaicCLIPaths(strategy: cliStrategy)
        if cliPaths.isEmpty, cliStrategy == .bundledOnly {
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to locate bundled mosaic CLI"
            )
        }

        var lastPermissionFailure: (terminationStatus: Int32, stdout: String, stderr: String)?
        for cliPath in cliPaths {
            let result = executeMosaicCommand(
        ...
        if cliStrategy == .bundledOnly {
            return lastPermissionFailure ?? (
                terminationStatus: -1,
                stdout: "",
                stderr: "Bundled mosaic CLI command failed without an executable path"
            )
        }

        let fallbackArgs = ["mosaic"] + args
        let fallbackResult = executeMosaicCommand(
    ...
    private enum MosaicCLIStrategy: Equatable {
        case any
        case bundledOnly
    }
    ...
    private func resolveMosaicCLIPaths(strategy: MosaicCLIStrategy) -> [String] {
        ...
        if strategy == .any {
            for key in ["MOSAIC_UI_TEST_CLI_PATH", "MOSAICTERM_CLI"] {
        ...
        candidates.append("/tmp/mosaic-\(launchTag)/Build/Products/Debug/Mosaic DEV.app/Contents/Resources/bin/mosaic")
        candidates.append("/tmp/mosaic-\(launchTag)/Build/Products/Debug/Mosaic.app/Contents/Resources/bin/mosaic")
        if strategy == .any {
            candidates.append("/tmp/mosaic-\(launchTag)/Build/Products/Debug/mosaic")
        }
    ...
    private func appendCLIPathCandidates(
        ...
        strategy: MosaicCLIStrategy,
        to candidates: inout [String]
    ) {
        candidates.append("\(productsDir)/Mosaic DEV.app/Contents/Resources/bin/mosaic")
        candidates.append("\(productsDir)/Mosaic.app/Contents/Resources/bin/mosaic")
        if strategy == .any {
            candidates.append("\(productsDir)/mosaic")
        }
        ...
        for entry in entries.sorted() where entry.hasSuffix(".app") {
            let cliPath = URL(fileURLWithPath: productsDir)
                ...
                .appendingPathComponent("Contents/Resources/bin/mosaic")
                .path
            candidates.append(cliPath)
        }
        if strategy == .any {
            for entry in entries.sorted() where entry == "mosaic" {
                let cliPath = URL(fileURLWithPath: productsDir)
                    .appendingPathComponent(entry)
                    .path
                candidates.append(cliPath)
    ...
    private func executeMosaicCommand(
        ...
        do {
            ...
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to run mosaic command: \(error.localizedDescription) (cliPath=\(executablePath))"
            )
        }
    ...
    private func resolveSocketPath(timeout: TimeInterval, requiredWorkspaceId: String? = nil) -> String? {
        ...
        _ = waitForCondition(timeout: timeout) {
            for candidate in primaryCandidates {
                ...
                // Primary candidate is the explicitly requested MOSAIC_SOCKET_PATH. If it responds,
    ...
    private func expectedSocketCandidates(includeGlobalFallback: Bool) -> [String] {
        ...
        let taggedDebugSocket = "/tmp/mosaic-debug-\(launchTag).sock"
        ...
        if includeGlobalFallback {
            ...
            candidates.append("/tmp/mosaic-debug.sock")
            candidates.append(stableSocketPath())
            candidates.append("/tmp/mosaic.sock")
        }
    ...
    private func stableSocketPath() -> String {
        // Mirrors MosaicStateDirectory.url() + mosaic.sock (non-TCC ~/.local/state/mosaic;
        // see https://github.com/emergent-inc/mosaic/issues/5146).
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".local/state/mosaic", isDirectory: true)
            .appendingPathComponent("mosaic.sock", isDirectory: false)
            .path
    }
    ...
    private func discoverTmpSocketCandidates(limit: Int) -> [String] {
        ...
        let matches = entries.filter { $0.hasPrefix("mosaic") && $0.hasSuffix(".sock") }
