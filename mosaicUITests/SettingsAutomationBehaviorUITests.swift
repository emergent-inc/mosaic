...
///
/// The Automation section (`AutomationSection` in `MosaicSettingsUI`) exposes:
...
/// env injection, which `rg` binary Find resolves, the `MOSAIC_PORT` range a new
...
final class SettingsAutomationBehaviorUITests: SettingsUITestCase {
    ...
    private enum L {
        ...
        static let modeMosaicOnly = "mosaic processes only"
    ...
    func testPasswordModeRevealsAndHidesSocketPasswordRow() {
        ...
        // Default (mosaicOnly): no password row.
        XCTAssertFalse(
            passwordTitle.waitForExistence(timeout: 1.0),
            "Socket Password row should be hidden in the default mosaic-only mode"
        )
        ...
        // Switch back to mosaic-only → row disappears again.
        XCTAssertTrue(selectMode(window, app, L.modeMosaicOnly), "could not select mosaic-only mode")
        XCTAssertTrue(
            poll(timeout: 4.0) { !passwordTitle.exists },
    ...
    //
    // TIER 2 (needs runtime seam): Socket Control Mode (mosaicOnly/automation/
    ...
    //   connection handler (consumed in AppDelegate/mosaicApp socket setup), not
    ...
    //   toggles — when enabled, mosaic injects session-tracking + notification
    //   hook env into newly launched `claude`/`cursor`/`gemini` processes
    //   inside mosaic terminals. The observable consequence (sidebar session
    ...
    //   these select which `claude` / `rg` executable mosaic resolves
    ...
    //   MOSAIC_PORT / MOSAIC_PORT_END environment range a new terminal inherits.
    //   The only true verification is reading the env of a freshly spawned
    //   terminal (e.g. `echo $MOSAIC_PORT`), which requires driving the Ghostty
