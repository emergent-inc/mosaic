...
final class CommandPaletteIdentifierClipboardUITests: XCTestCase {
    ...
    func testCmdShiftPOpenMosaicJSONOpensUserConfigFile() throws {
        let app = XCUIApplication()
        let capturePath = "/tmp/mosaic-ui-test-open-mosaic-json-\(UUID().uuidString).txt"
        ...
        app.launchArguments += ["-AppleLanguages", "(en)", "-AppleLocale", "en_US", "-menuBarOnly", "false"]
        app.launchEnvironment["MOSAIC_UI_TEST_MODE"] = "1"
        app.launchEnvironment["MOSAIC_UI_TEST_CAPTURE_OPEN_PATH"] = capturePath
        launchAndActivate(app)
        ...
        searchField.typeText("open mosaic json")

        let predicate = NSPredicate(
            format: "identifier BEGINSWITH %@ AND value == %@",
            "CommandPaletteResultRow.",
            "palette.openMosaicSettingsFile"
        )
        let row = app.descendants(matching: .any)
            .matching(predicate)
            .firstMatch
        XCTAssertTrue(row.waitForExistence(timeout: 5.0), "Expected row for Open mosaic.json")
        try? FileManager.default.removeItem(atPath: capturePath)
        row.click()
        ...
        let expectedPath = (loginHomeDirectoryPath() as NSString)
            .appendingPathComponent(".config/mosaic/mosaic.json")
