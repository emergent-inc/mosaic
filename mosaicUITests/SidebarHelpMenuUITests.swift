...
final class SidebarHelpMenuUITests: XCTestCase {
    ...
    func testTutorialVideoSeenFlagSuppressesAutomaticPresentation() {
        let app = XCUIApplication()
        app.launchArguments += ["-mosaicTutorialVideoSeen.v1", "YES"]
        app.launchEnvironment["MOSAIC_UI_TEST_MODE"] = "1"
        app.launchEnvironment["MOSAIC_UI_TEST_TUTORIAL_VIDEO_AUTO_SHOW"] = "1"
        launchAndActivate(app)
    ...
    func testHelpMenuCheckForUpdatesTriggersSidebarUpdatePill() {
        ...
        app.launchEnvironment["MOSAIC_UI_TEST_FEED_MODE"] = "available"
    ...
    func testHelpMenuSendFeedbackOpensComposerSheet() {
        ...
        XCTAssertTrue(
            app.staticTexts[
                "A human will read this! You can also reach us at contact@mosaic.inc."
...
final class FeedbackComposerShortcutUITests: XCTestCase {
    ...
    func testCmdOptionFWorksFromSettingsWindow() {
        let app = XCUIApplication()
        app.launchEnvironment["MOSAIC_UI_TEST_MODE"] = "1"
        app.launchEnvironment["MOSAIC_UI_TEST_SHOW_SETTINGS"] = "1"
        app.launch()
        app.activate()
...
final class CommandPaletteAllSurfacesUITests: XCTestCase {
    ...
    private let hiddenSurfaceToken = "mosaic-command-palette-hidden-surface"
    private let visibleSurfaceToken = "mosaic-command-palette-visible-surface"
    private let noMatchWorkspaceQuery = "mosaic-command-palette-no-match"

    override func setUp() {
        super.setUp()
        continueAfterFailure = false
        socketPath = "/tmp/mosaic-ui-test-command-palette-\(UUID().uuidString).sock"
    ...
    func testMinimalModeToggleKeepsSettingsWindowFocused() throws {
        let app = XCUIApplication()
        let diagnosticsPath = "/tmp/mosaic-ui-test-settings-focus-\(UUID().uuidString).json"
        ...
        let diagnostics = waitForDiagnostics(
            ...
            data["keyWindowIdentifier"] == "mosaic.settings" && data["settingsWindowIsKey"] == "1"
        }
    ...
    func testMenuBarOnlyToggleKeepsSettingsWindowFocused() throws {
        let app = XCUIApplication()
        let diagnosticsPath = "/tmp/mosaic-ui-test-menu-bar-only-focus-\(UUID().uuidString).json"
        try? FileManager.default.removeItem(atPath: diagnosticsPath)
