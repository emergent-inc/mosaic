...
import struct MosaicSettings.CommandPaletteSettingsStore
import enum MosaicSettings.ConfirmQuitMode
import struct MosaicSettings.SettingCatalog
import struct MosaicSettings.UserDefaultsSettingsClient
...
final class CommandPaletteRenameSelectionSettingsTests: XCTestCase {
    private let suiteName = "mosaic.tests.commandPaletteRenameSelection.\(UUID().uuidString)"
...
final class RightSidebarModeShortcutHintTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        KeyboardShortcutSettings.settingsFileStore = KeyboardShortcutSettingsFileStore(
            primaryPath: directoryURL.appendingPathComponent("mosaic.json", isDirectory: false).path,
            fallbackPath: nil,
            startWatching: false
        )
    ...
    func testAlwaysShowHintsIsOnlyEnabledForUITests() {
        ...
        XCTAssertTrue(
            ShortcutHintDebugSettings(
                environment: ["MOSAIC_UI_TEST_SHORTCUT_HINTS_ALWAYS_SHOW": "1"]
            ).alwaysShowHints
        )
    }
...
final class BuildFlavorTests: XCTestCase {
    ...
    func testDetectsNightlyFromBundleIdentifier() {
        XCTAssertEqual(
            BuildFlavor.detect(bundleName: "mosaic", bundleIdentifier: "mosaic.com.emergent.app.nightly"),
            .nightly
        )
    }
    ...
    func testDetectsStableByDefault() {
        XCTAssertEqual(
            BuildFlavor.detect(bundleName: "mosaic", bundleIdentifier: "mosaic.com.emergent.app"),
            .stable
        )
    }
}
