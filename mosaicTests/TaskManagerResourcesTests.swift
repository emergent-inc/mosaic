...
final class TaskManagerResourcesTests: XCTestCase {
    func testGrokCodingAgentDefinitionMatchesSymlinkLaunch() throws {
        let definition = try XCTUnwrap(MosaicTaskManagerCodingAgentDefinition.matchingDefinition(
            processName: "grok",
    ...
    func testParsesTypedIntPIDArrayFromSummaryPayload() {
        ...
        let resources = MosaicTaskManagerResources(payload)
    ...
    func testSortOrderSortsSiblingRowsByCPUWhilePreservingHierarchy() {
        ...
        let sortedRows = MosaicTaskManagerSortOrder.defaultOrder.sortedRows(rows)
    ...
    func testSortOrderUsesAscendingNameSortWhenNameColumnIsSelected() {
        ...
        let sortedRows = MosaicTaskManagerSortOrder(
            column: .name,
            direction: .ascending
        ).sortedRows(rows)
    ...
    func testSortOrderSortsMemoryAndProcessColumnsDescending() {
        ...
        let memorySortedRows = MosaicTaskManagerSortOrder(
            column: .memory,
            direction: .descending
        ).sortedRows(rows)
        let processSortedRows = MosaicTaskManagerSortOrder(
            column: .processes,
            direction: .descending
        ).sortedRows(rows)
    ...
    func testSortOrderTogglesCurrentColumnAndUsesMetricDefaultsForNewColumns() {
        let cpuDescending = MosaicTaskManagerSortOrder.defaultOrder
        ...
        XCTAssertEqual(cpuAscending, MosaicTaskManagerSortOrder(column: .cpu, direction: .ascending))
        XCTAssertEqual(memoryDescending, MosaicTaskManagerSortOrder(column: .memory, direction: .descending))
        XCTAssertEqual(nameAscending, MosaicTaskManagerSortOrder(column: .name, direction: .ascending))
    }

    func testSnapshotBuildsProgramAggregateRowsForRepeatedProcessNames() throws {
        let snapshot = MosaicTaskManagerSnapshot(
            rows: [
                taskManagerRow(
                    "node-101-a",
                    kind: .process,
    ...
    func testSnapshotParsesProgramTotalsFromTopPayloadWithoutProcessRows() throws {
        let snapshot = MosaicTaskManagerSnapshot(payload: [
    ...
    func testSnapshotOnlyCountsAsLoadedAfterSamplingOrRowsArrive() {
        XCTAssertFalse(MosaicTaskManagerSnapshot.empty.hasLoadedResourceUsage)

        let sampledSnapshot = MosaicTaskManagerSnapshot(
            rows: [],
            total: .zero,
    ...
    func testCodingAgentMatcherUsesSupportedAgentNamesAndLaunchMetadata() {
        XCTAssertEqual(
            MosaicTaskManagerCodingAgentDefinition.matchingDefinition(
                processName: "node",
                processPath: nil,
        ...
        XCTAssertTrue(MosaicTaskManagerCodingAgentDefinition.shouldReadArguments(
            processName: "2.1.140",
        ...
        XCTAssertNil(MosaicTaskManagerCodingAgentDefinition.matchingDefinition(
            processName: "2.1.140",
            processPath: nil,
            arguments: [],
            environment: [:]
        ...
        XCTAssertEqual(
            MosaicTaskManagerCodingAgentDefinition.matchingDefinition(
                ...
                environment: ["MOSAIC_AGENT_LAUNCH_KIND": "claudeTeams"]
            )?.id,
            "claude"
        )
        XCTAssertEqual(
            MosaicTaskManagerCodingAgentDefinition.matchingDefinition(
                ...
                environment: ["MOSAIC_AGENT_LAUNCH_KIND": "omo"]
            )?.id,
            "opencode"
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            MosaicTaskManagerCodingAgentDefinition.matchingDefinition(
                ...
                processPath: nil,
                arguments: ["node", "agent.js"],
                environment: ["MOSAIC_AGENT_LAUNCH_KIND": "omx"]
            )?.id,
            "codex"
        )
        XCTAssertNil(MosaicTaskManagerCodingAgentDefinition.matchingDefinition(
    ...
    private func resourceSummary() -> MosaicTopResourceSummary {
    ...
    private func taskManagerRow(
        _ id: String,
        kind: MosaicTaskManagerRow.Kind = .workspace,
        level: Int,
        title: String? = nil,
        cpuPercent: Double = 0,
