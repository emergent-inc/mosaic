...
struct CLIRemoteShellStartupPerformanceTests {
    ...
    func generatedSSHStartupDoesNotBlockOnRelayRPCWarmup() throws {
        ...
        environment["MOSAIC_BUNDLED_CLI_PATH"] = root.bin.appendingPathComponent("mosaic").path
        environment["MOSAIC_SOCKET_PATH"] = "/tmp/mosaic-test.sock"
        environment["MOSAIC_WORKSPACE_ID"] = "workspace-cli-perf"
        environment["MOSAIC_SURFACE_ID"] = "surface-cli-perf"
        environment["MOSAIC_FAKE_SHELL_MARKER"] = shellMarker.path
        environment["MOSAIC_FAKE_RELAY_RPC_GATE"] = relayRPCGate.path
    ...
    private func makeFakeRemoteShellRoot() throws -> FakeRemoteShellRoot {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("mosaic-cli-shell-perf-\(UUID().uuidString)")
        let home = root.appendingPathComponent("home")
        let bin = root.appendingPathComponent("bin")
        try FileManager.default.createDirectory(at: home, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: bin, withIntermediateDirectories: true)
        try writeExecutable(at: bin.appendingPathComponent("mosaic"), contents: """
        #!/bin/sh
        if [ "$1" = "rpc" ] && [ -n "${MOSAIC_FAKE_RELAY_RPC_GATE:-}" ]; then
          while [ ! -f "$MOSAIC_FAKE_RELAY_RPC_GATE" ]; do sleep 0.05; done
        fi
        exit 0
        """)
        ...
        #!/bin/sh
        if [ -n "${MOSAIC_FAKE_SHELL_MARKER:-}" ]; then printf started > "$MOSAIC_FAKE_SHELL_MARKER"; fi
        exit 0
        """)
    ...
    private func bundledCLIPath() throws -> String {
        ...
        let direct = appBundle.appendingPathComponent("Contents/Resources/bin/mosaic")
        ...
        throw NSError(domain: "mosaic.tests", code: 1, userInfo: [
            NSLocalizedDescriptionKey: "Bundled mosaic CLI not found in \(appBundle.path)",
        ])
    }
