...
struct BrowserWebContentProcessTests {
    private let recoveryURL = URL(string: "data:text/html,mosaic-recovery")!
    ...
    func browserPanelInstallsWebAuthnBridgeWithoutExposingNativeHandler() async throws {
        ...
        let result = try await webView.evaluateJavaScript(
            """
            ({
              bridge: window.__mosaicWebAuthnBridgeInstalled === true,
              handler: !!(
                window.webkit &&
                window.webkit.messageHandlers &&
                window.webkit.messageHandlers.mosaicWebAuthn &&
                typeof window.webkit.messageHandlers.mosaicWebAuthn.postMessage === "function"
              )
            })
            """
        ) as? [String: Bool]
