...
struct CLICodexHookTimeoutRegressionTests {
    ...
    @Test func codexHookInstallReplacesSynchronousBundledHook() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-sync-hook-\(UUID().uuidString)", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        try FileManager.default.createDirectory(at: codexHome, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let previousCommand = "mosaic_cli=\"${MOSAIC_BUNDLED_CLI_PATH:-}\"; if [ -z \"$mosaic_cli\" ] || [ ! -x \"$mosaic_cli\" ]; then mosaic_cli=\"$(command -v mosaic 2>/dev/null || true)\"; fi; if [ -n \"$MOSAIC_SURFACE_ID\" ] && [ \"$MOSAIC_CODEX_HOOKS_DISABLED\" != \"1\" ] && [ -n \"$mosaic_cli\" ]; then { if [ -n \"${MOSAIC_SOCKET_PATH:-}\" ]; then \"$mosaic_cli\" --socket \"$MOSAIC_SOCKET_PATH\" hooks codex prompt-submit; else \"$mosaic_cli\" hooks codex prompt-submit; fi; } || echo '{}'; else echo '{}'; fi"
        ...
        #expect(sessionStartCommands.allSatisfy { $0.contains("agent_pid=") && $0.contains("MOSAIC_CODEX_PID=") })
        ...
        #expect(promptCommands.allSatisfy { $0.contains("agent_pid=") && $0.contains("MOSAIC_CODEX_PID=") })
    ...
    @Test func codexInstalledHookReturnsBeforeSlowMosaicCommandFinishes() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-hook-async-\(UUID().uuidString)", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("mosaic", isDirectory: false)
        ...
        try makeExecutableShellFile(at: fakeCLI, lines: [
            "#!/bin/sh",
            "printf '%s\\n' \"$*\" > \"$MOSAIC_TEST_ARGS\"",
            "printf '%s\\n' \"$MOSAIC_CODEX_PID\" > \"$MOSAIC_TEST_PID\"",
            "cat > \"$MOSAIC_TEST_STDIN\"",
            "sleep 2",
            "printf done > \"$MOSAIC_TEST_DONE\"",
        ])
        ...
        let run = runProcess(
            ...
            environment: [
                ...
                "TMPDIR": root.path,
                "MOSAIC_SURFACE_ID": "surface-123",
                "MOSAIC_SOCKET_PATH": "/tmp/mosaic-test.sock",
                "MOSAIC_BUNDLED_CLI_PATH": fakeCLI.path,
                "MOSAIC_CODEX_PID": "4242",
                "MOSAIC_TEST_STDIN": capturedStdin.path,
                "MOSAIC_TEST_ARGS": capturedArgs.path,
                "MOSAIC_TEST_PID": capturedPID.path,
                "MOSAIC_TEST_DONE": doneFile.path,
            ],
        ...
        #expect(waitForFile(capturedArgs, containing: "--socket /tmp/mosaic-test.sock hooks codex prompt-submit", timeout: 1))
    ...
    @Test func codexPromptSubmitDoesNotReviveStoppedTurn() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-stale-prompt-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func codexSessionStartDoesNotOverwriteExistingTurnState() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-stale-start-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func codexSessionStartRefreshesCompletedPriorTurn() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-fresh-start-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func codexSessionStartDoesNotReviveCompletedTurnFromSamePID() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-same-pid-start-\(UUID().uuidString)", isDirectory: true)
    ...
    private func bindUnixSocket(at path: String) throws -> Int32 {
        ...
        guard fd >= 0 else {
            throw NSError(domain: "mosaic.tests", code: Int(errno))
        }
        ...
        guard utf8.count < maxPathLength else {
            Darwin.close(fd)
            throw NSError(domain: "mosaic.tests", code: Int(ENAMETOOLONG))
        }
        ...
        guard bindResult == 0, Darwin.listen(fd, 8) == 0 else {
            let code = errno
            Darwin.close(fd)
            throw NSError(domain: "mosaic.tests", code: Int(code))
        }
        return fd
    }
