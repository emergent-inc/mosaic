    ...
    func testWorkspaceSessionSnapshotPersistsAgentSessionWorkingDirectory() throws {
        ...
        let panel = try #require(
            workspace.newAgentSessionSurface(
                ...
                providerID: .codex,
                rendererKind: .react,
                workingDirectory: "/tmp/mosaic-agent-session-cwd",
                focus: true
            )
        )
        ...
        expectEqual(panelSnapshot.directory, "/tmp/mosaic-agent-session-cwd")
        expectEqual(panelSnapshot.agentSession?.workingDirectory, "/tmp/mosaic-agent-session-cwd")
    }
}
