...
/// Regression coverage for https://github.com/emergent-inc/mosaic/issues/5993:
/// mosaic ignored `macos-option-as-alt` left/right and captured Option before
...
/// mosaic sends. If mosaic maps both physical Option keys to the same generic
...
@Suite struct GhosttyOptionAsAltModsTests {
    ...
    @Test func rightOptionCarriesAltAndAltRightSideBit() {
        ...
        let mods = mosaicGhosttyModsFromFlags(modifierFlagsRawValue: raw)
    ...
    @Test func mouseModsNeverCarrySideBits() {
        ...
        let mods = mosaicGhosttyMouseModsFromFlags(modifierFlagsRawValue: raw)
        #expect(mods.rawValue & GHOSTTY_MODS_ALT.rawValue != 0)
        #expect(mods.rawValue & GHOSTTY_MODS_SHIFT.rawValue != 0)
        #expect(mods.rawValue & GHOSTTY_MODS_ALT_RIGHT.rawValue == 0)
        #expect(mods.rawValue & GHOSTTY_MODS_SHIFT_RIGHT.rawValue == 0)
    ...
    @Test func translationFlagsDropOptionWhenGhosttyStripsAlt() {
        ...
        let translated = mosaicTranslationModifierFlags(
            original: [.option],
