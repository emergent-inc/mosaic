...
/// and the managed `MOSAIC_*` variables can never be clobbered.
@Suite(.serialized)
@MainActor
struct WorkspaceEnvironmentTests {
    ...
    /// at `strdup`/Ghostty, so `"MOSAIC_SOCKET_PATH\0x"` would dodge the exact-match
    ...
    func sanitizedWorkspaceEnvironmentRejectsKeysThatTruncateAtTheCBoundary() {
        let result = Workspace.sanitizedWorkspaceEnvironment([
            "MOSAIC_SOCKET_PATH\u{0}x": "spoofed",  // NUL would truncate to MOSAIC_SOCKET_PATH
    ...
    // MARK: - Acceptance: managed MOSAIC_* variables cannot be clobbered
    ...
    @Test
    func workspaceEnvironmentCannotClobberProtectedMosaicVariables() {
        let merged = TerminalSurface.mergedStartupEnvironment(
            base: ["MOSAIC_WORKSPACE_ID": "real-id", "TERM": "xterm-ghostty"],
            protectedKeys: ["MOSAIC_WORKSPACE_ID", "TERM"],
            additionalEnvironment: [
                "MOSAIC_WORKSPACE_ID": "spoofed",   // must be ignored
                "TERM": "dumb",                    // must be ignored
                "AWS_PROFILE": "prod",             // must be applied
            ],
            initialEnvironmentOverrides: ["MOSAIC_WORKSPACE_ID": "also-spoofed"],
            ambientEnvironment: [:]
        )
        #expect(merged["MOSAIC_WORKSPACE_ID"] == "real-id")
    ...
    // MARK: - Config entry point (mosaic.json)

    @Test
    func mosaicWorkspaceDefinitionDecodesEnv() throws {
        let json = #"{"name":"Build","env":{"AWS_PROFILE":"prod","API_BASE":"https://api.example.com"}}"#
        let definition = try JSONDecoder().decode(MosaicWorkspaceDefinition.self, from: Data(json.utf8))
        #expect(definition.env == ["AWS_PROFILE": "prod", "API_BASE": "https://api.example.com"])
    }

    @Test
    func mosaicWorkspaceDefinitionEnvIsOptional() throws {
        let definition = try JSONDecoder().decode(MosaicWorkspaceDefinition.self, from: Data(#"{"name":"Build"}"#.utf8))
