...
struct ShellStartupMatrixTests {
    ...
    func zshStartupPreservesUserZdotdirAndLoadsGhosttyIntegration() throws {
        let bundled = try makeBundledIntegrationDir(files: [".zshenv": "# mosaic zsh bootstrap stub\n"])
        ...
        expectEqual(environment["MOSAIC_ZSH_ZDOTDIR"], "/Users/example/.zsh")
        expectEqual(environment["MOSAIC_LOAD_GHOSTTY_ZSH_INTEGRATION"], "1")
        expectTrue(protectedKeys.isSuperset(of: ["ZDOTDIR", "MOSAIC_ZSH_ZDOTDIR", "MOSAIC_LOAD_GHOSTTY_ZSH_INTEGRATION"]))
    }
    ...
    func zshStartupDoesNotPreserveGhosttyInjectedZdotdir() throws {
        ...
        expectEqual(environment["MOSAIC_ZSH_ZDOTDIR"], nil)
        expectFalse(protectedKeys.contains("MOSAIC_ZSH_ZDOTDIR"))
    }

    @Test
    func bashStartupUsesBundledPromptCommandAndHonorsGhosttyMode() {
        let integrationDir = "/Applications/mosaic.app/Contents/Resources/shell-integration"
        ...
        _ = TerminalSurface.applyManagedShellSpecificStartupEnvironment(
            ...
            protectedKeys: &protectedKeys,
            readFile: { path in
                expectEqual(path, "\(integrationDir)/mosaic-bash-bootstrap.bash")
                return """
                # comment

                export MOSAIC_TEST=1
                   # indented comment
                mosaic-test-command
                """
            }
        ...
        expectEqual(environment["MOSAIC_LOAD_GHOSTTY_BASH_INTEGRATION"], nil)
        expectEqual(environment["PROMPT_COMMAND"], "export MOSAIC_TEST=1\nmosaic-test-command")
        expectTrue(protectedKeys.contains("PROMPT_COMMAND"))
        expectFalse(protectedKeys.contains("MOSAIC_LOAD_GHOSTTY_BASH_INTEGRATION"))
    }

    @Test
    func fishStartupReturnsManagedCommandAndPreservesUserConfigHome() throws {
        let bundled = try makeBundledIntegrationDir(files: ["fish/config.fish": "# mosaic fish bootstrap stub\n"])
        defer { try? FileManager.default.removeItem(at: bundled.root) }
        let integrationDir = bundled.integrationDir
        ...
        expectEqual(environment["MOSAIC_FISH_INTEGRATION_FILE"], "\(integrationDir)/fish/config.fish")
        expectEqual(environment["MOSAIC_FISH_USER_CONFIG_ALREADY_LOADED"], "1")
    }
    ...
    func unsupportedLocalShellsKeepEnvironmentUnchanged(shell: String) {
        ...
        let command = TerminalSurface.applyManagedShellSpecificStartupEnvironment(
            shell: shell,
            integrationDir: "/Applications/mosaic.app/Contents/Resources/shell-integration",
            userGhosttyShellIntegrationMode: "detect",
            to: &environment,
            protectedKeys: &protectedKeys
        )
    ...
    @Test(arguments: [
        ...
        RemoteShellCase(name: "fish", expectedArgs: "-il --init-command source \"$MOSAIC_FISH_INTEGRATION_FILE\""),
    ...
    func generatedSshBootstrapHandlesShellMatrix(shellCase: RemoteShellCase) throws {
        ...
        expectTrue(result.capture.contains("PATH=\(result.home.path)/.mosaic/bin:"), result.capture)
        expectTrue(result.capture.contains("MOSAIC_SOCKET_PATH=127.0.0.1:64123"), result.capture)
        ...
        case "zsh":
            expectTrue(result.capture.contains("ZDOTDIR=\(result.home.path)/.mosaic/relay/64123.shell"), result.capture)
            expectTrue(result.capture.contains("MOSAIC_REAL_ZDOTDIR=\(result.home.path)/user-zdotdir"), result.capture)
        ...
        case "fish":
            expectTrue(result.capture.contains("MOSAIC_FISH_INTEGRATION_FILE=\(result.home.path)/.mosaic/relay/64123.shell/fish/config.fish"), result.capture)
            expectTrue(result.capture.contains("MOSAIC_FISH_USER_CONFIG_ALREADY_LOADED=1"), result.capture)
            expectTrue(result.capture.contains("FISH_HAS_MARKER=yes"), result.capture)
        default:
            expectTrue(result.capture.contains("MOSAIC_FISH_INTEGRATION_FILE=\n"), result.capture)
            expectTrue(result.capture.contains("MOSAIC_REAL_ZDOTDIR=\n"), result.capture)
        }
    }
    ...
    func generatedSshBootstrapDoesNotBlockOnRelayCliWarmup() throws {
        ...
        expectTrue(
            ...
            "mosaic ssh bootstrap waited for relay CLI warmup: \(formatSeconds(result.process.duration))"
        )
    }

    /// Regression for #6352: running Claude Code (or any full-screen TUI) inside
    /// a `mosaic ssh` remote workspace garbled the output because the remote
    ...
    func remoteTerminalSetupInstallsGhosttyTerminfoBeforeChoosingTerm() throws {
        ...
        guard fileManager.isExecutableFile(atPath: "/usr/bin/tic"),
            ...
            // cannot be exercised here. mosaic CI runners ship both binaries.
            return
        }

        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-terminfo-\(UUID().uuidString)")
        ...
        let terminfoSource = """
        xterm-ghostty|mosaic ghostty regression terminfo,
        ...
        let script = lines.joined(separator: "\n")
            + "\nprintf 'MOSAIC_TERMINFO_TEST_TERM=%s\\n' \"$TERM\"\n"
        ...
        expectTrue(
            result.stdout.contains("MOSAIC_TERMINFO_TEST_TERM=xterm-ghostty"),
    ...
    private func runGeneratedBootstrap(
        shellName: String,
        fakeMosaicDelay: TimeInterval? = nil,
        workspaceID: String? = nil,
        surfaceID: String? = nil,
        bootstrapTTY: String? = nil
    ) throws -> GeneratedBootstrapResult {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent("mosaic-shell-matrix-\(UUID().uuidString)")
        let home = root.appendingPathComponent("home")
        ...
        if let fakeMosaicDelay {
            try writeExecutableMosaicFile(at: bin.appendingPathComponent("mosaic"), delay: fakeMosaicDelay)
        }
        var script = RemoteInteractiveShellBootstrapBuilder.script(
            ...
            bundledZshIntegration: "mosaic_zsh_marker=1",
            bundledBashIntegration: "mosaic_bash_marker=1",
            bundledFishIntegration: "set -gx MOSAIC_FISH_MARKER 1"
        )
        if let workspaceID {
            script = script.replacingOccurrences(of: "__MOSAIC_WORKSPACE_ID__", with: workspaceID)
        }
        if let surfaceID {
            script = script.replacingOccurrences(of: "__MOSAIC_SURFACE_ID__", with: surfaceID)
        }
        var arguments = [
            "HOME=\(home.path)",
            ...
            "MOSAIC_CAPTURE_PATH=\(capturePath.path)",
        ]
        if let bootstrapTTY {
            arguments.append("MOSAIC_BOOTSTRAP_TTY=\(bootstrapTTY)")
        }
        arguments += [
            "/bin/sh",
            "-c",
    ...
    private func writeExecutableShellFile(at url: URL, capturePath: URL) throws {
        ...
        #!/bin/sh
        {
          printf 'ARGS=%s\\n' "$*"
          printf 'PATH=%s\\n' "$PATH"
          printf 'MOSAIC_SOCKET_PATH=%s\\n' "$MOSAIC_SOCKET_PATH"
          printf 'GHOSTTY_SHELL_FEATURES=%s\\n' "$GHOSTTY_SHELL_FEATURES"
          printf 'MOSAIC_SHELL_INTEGRATION_DIR=%s\\n' "$MOSAIC_SHELL_INTEGRATION_DIR"
          printf 'ZDOTDIR=%s\\n' "$ZDOTDIR"
          printf 'MOSAIC_REAL_ZDOTDIR=%s\\n' "$MOSAIC_REAL_ZDOTDIR"
          printf 'MOSAIC_FISH_INTEGRATION_FILE=%s\\n' "$MOSAIC_FISH_INTEGRATION_FILE"
          printf 'MOSAIC_FISH_USER_CONFIG_ALREADY_LOADED=%s\\n' "$MOSAIC_FISH_USER_CONFIG_ALREADY_LOADED"
          if [ -r "$MOSAIC_SHELL_INTEGRATION_DIR/mosaic-zsh-integration.zsh" ] && grep -q 'mosaic_zsh_marker=1' "$MOSAIC_SHELL_INTEGRATION_DIR/mosaic-zsh-integration.zsh"; then
            printf 'ZSH_INTEGRATION_HAS_MARKER=yes\\n'
          fi
          if [ -r "$MOSAIC_SHELL_INTEGRATION_DIR/mosaic-bash-integration.bash" ] && grep -q 'mosaic_bash_marker=1' "$MOSAIC_SHELL_INTEGRATION_DIR/mosaic-bash-integration.bash"; then
            printf 'BASH_INTEGRATION_HAS_MARKER=yes\\n'
          fi
          if [ -r "$MOSAIC_FISH_INTEGRATION_FILE" ] && grep -q 'MOSAIC_FISH_MARKER' "$MOSAIC_FISH_INTEGRATION_FILE"; then
    ...
    private func writeExecutableMosaicFile(at url: URL, delay: TimeInterval) throws {
        try """
        #!/bin/sh
        if [ "$1" = "rpc" ]; then
    ...
    private func makeBundledIntegrationDir(
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-shell-matrix-bundle-\(UUID().uuidString)")
