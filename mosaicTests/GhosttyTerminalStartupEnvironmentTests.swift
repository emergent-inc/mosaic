...
struct GhosttyTerminalStartupEnvironmentTests {
    @MainActor
    @Test
    func testTerminalSurfaceStartupEnvironmentIncludesMosaicContextValues() throws {
        ...
        let expectedContextValues = [
            ...
            "MOSAIC_SURFACE_ID": surface.id.uuidString,
            "MOSAIC_TAB_ID": workspaceId.uuidString,
            "MOSAIC_PANEL_ID": surface.id.uuidString,
        ]
        ...
        let socketPath = try #require(
            surface.startupEnvironmentValue("MOSAIC_SOCKET_PATH"),
            "MOSAIC_SOCKET_PATH should be present"
        )
    ...
    func testApplyManagedGitWatchEnvironmentDisablesShellGitWatch() {
        ...
        expectEqual(environment["MOSAIC_NO_GIT_WATCH"], "1")
        expectTrue(protectedKeys.contains("MOSAIC_NO_GIT_WATCH"))
    }
    ...
    func testApplyManagedGitWatchEnvironmentClearsInheritedOptOutWhenEnabled() {
        ...
        expectEqual(merged["MOSAIC_NO_GIT_WATCH"], "")
    }
    ...
    func testApplyManagedGitWatchEnvironmentDisablesShellPullRequestWatchWhenHidden() {
        ...
        expectEqual(environment["MOSAIC_NO_GIT_WATCH"], "")
        expectEqual(environment["MOSAIC_NO_PR_WATCH"], "1")
        expectTrue(protectedKeys.contains("MOSAIC_NO_GIT_WATCH"))
        expectTrue(protectedKeys.contains("MOSAIC_NO_PR_WATCH"))
    }
    ...
    func testInstallClaudeCommandShimCreatesExecutableOutsideBundleBin() throws {
        ...
        let bundleBin =
            root
            .appendingPathComponent("mosaic.app", isDirectory: true)
        ...
        let wrapperURL = bundleBin.appendingPathComponent("mosaic-claude-wrapper", isDirectory: false)
        try """
        #!/usr/bin/env bash
        set -euo pipefail
        {
            printf 'shim=%s\\n' "${MOSAIC_CLAUDE_WRAPPER_SHIM:-}"
            printf 'root=%s\\n' "${MOSAIC_CLAUDE_WRAPPER_SHIM_ROOT:-}"
            printf 'args=%s\\n' "$*"
        } > "$MOSAIC_TEST_LOG"
    ...
    func testClaudeCommandShimFallsBackToCurrentBundledWrapperWhenEmbeddedWrapperWasReaped() throws {
        ...
        let staleWrapperURL = oldBundleBin.appendingPathComponent("mosaic-claude-wrapper", isDirectory: false)
        try writeExecutable(staleWrapperURL, """
        #!/usr/bin/env bash
        printf 'stale %s\\n' "$*" > "$MOSAIC_TEST_LOG"
        """)

        let currentWrapperURL = currentBundleBin.appendingPathComponent("mosaic-claude-wrapper", isDirectory: false)
        try writeExecutable(currentWrapperURL, """
        #!/usr/bin/env bash
        printf 'current %s\\n' "$*" > "$MOSAIC_TEST_LOG"
        """)
        let currentCLIURL = currentBundleBin.appendingPathComponent("mosaic", isDirectory: false)
        ...
        process.environment = [
            "PATH": "/usr/bin:/bin",
            "MOSAIC_BUNDLED_CLI_PATH": currentCLIURL.path,
            "MOSAIC_TEST_LOG": logURL.path,
        ]
        try process.run()
        process.waitUntilExit()
    ...
    @Test
    func testClaudeCommandShimFallbackSkipsInheritedMosaicShimRoots() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "GhosttyTerminalStartupEnvironmentTests-\(UUID().uuidString)", isDirectory: true)
        let bundleBin =
    ...
    @Test
    func testMergedStartupEnvironmentAllowsSessionReplayAndInitialEnvMosaicKeys() {
        let replayPath = "/tmp/mosaic-replay-\(UUID().uuidString)"
        let merged = TerminalSurface.mergedStartupEnvironment(
            base: [
                "PATH": "/usr/bin",
                "MOSAIC_SURFACE_ID": "managed-surface",
            ],
            protectedKeys: ["PATH", "MOSAIC_SURFACE_ID"],
            additionalEnvironment: [
                SessionScrollbackReplayStore.environmentKey: replayPath
            ],
            initialEnvironmentOverrides: [
                "MOSAIC_INITIAL_ENV_TOKEN": "token-123"
            ]
        )

        expectEqual(merged[SessionScrollbackReplayStore.environmentKey], replayPath)
        expectEqual(merged["MOSAIC_INITIAL_ENV_TOKEN"], "token-123")
    }
    ...
    func testMergedStartupEnvironmentProtectsManagedKeysOnly() {
        let merged = TerminalSurface.mergedStartupEnvironment(
            ...
            protectedKeys: ["PATH", "MOSAIC_SURFACE_ID"],
            additionalEnvironment: [
                "MOSAIC_SURFACE_ID": "user-surface",
                "CUSTOM_FLAG": "1",
            ],
            initialEnvironmentOverrides: [
                "PATH": "/tmp/bin",
                "MOSAIC_SURFACE_ID": "override-surface",
            ]
        )

        expectEqual(merged["PATH"], "/usr/bin")
        expectEqual(merged["MOSAIC_SURFACE_ID"], "managed-surface")
    ...
    func testMergedStartupEnvironmentDoesNotDeriveHermesCodexBaseURLForGenericTerminals() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-hermes-codex-startup-\(UUID().uuidString)", isDirectory: true)
