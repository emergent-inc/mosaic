...
@Suite(.serialized) struct MosaicCLIErrorOutputRegressionTests {
    ...
    @Test func testCLIErrorPathDoesNotCrashWhenStderrIsClosed() throws {
        ...
        let result = runShell(
            "MOSAIC_CLI_SENTRY_DISABLED=1 \(shellSingleQuote(cliPath)) definitely-not-a-command 2>&-",
            timeout: 5
        )
    ...
    @Test func testAgentTeamsHelpDoesNotLaunchExternalAgentCLI() throws {
        ...
        for command in ["claude-teams", "codex-teams"] {
            ...
            XCTAssertTrue(result.stdout.contains("Usage: mosaic \(command)"), result.stdout)
    ...
    @Test func testBundledCLIInTaggedDebugAppPrefersItsOwnSocketWithoutEnvironmentOverride() throws {
        ...
        let taggedSocketPath = "/tmp/mosaic-debug-\(tagSlug).sock"
    ...
    @Test func testBundledCLIInTaggedDebugAppTreatsCaseVariantStableEnvSocketAsImplicitDefault() throws {
        ...
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        environment["MOSAICTERM_CLI_RESPONSE_TIMEOUT_SEC"] = "5"
        environment["MOSAIC_SOCKET_PATH"] = caseVariantStablePath
    ...
    @Test func testBundledCLIInTaggedDebugAppDoesNotFallBackToStableEnvSocketWhenTaggedSocketIsMissing() throws {
        ...
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        environment["MOSAICTERM_CLI_RESPONSE_TIMEOUT_SEC"] = "0.1"
        environment["MOSAIC_SOCKET_PATH"] = stableSocketURL.path
    ...
    @Test func testBundledCLIInTaggedDebugAppTreatsUserScopedStableEnvSocketAsImplicitDefault() throws {
        let cliPath = try bundledCLIPath()
        let fixedHomeURL = URL(fileURLWithPath: "/tmp/mosaic-cli-home-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: fixedHomeURL) }
        let stableSocketURL = fixedHomeURL
            .appendingPathComponent(".local/state/mosaic", isDirectory: true)
            .appendingPathComponent("mosaic-\(getuid()).sock", isDirectory: false)
        let stableSocketPath = stableSocketURL.path
        try FileManager.default.createDirectory(
            at: stableSocketURL.deletingLastPathComponent(),
            withIntermediateDirectories: true
        ...
        for alias in aliases {
            try autoreleasepool {
                ...
                environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
                environment["MOSAICTERM_CLI_RESPONSE_TIMEOUT_SEC"] = "5"
                environment["MOSAIC_SOCKET_PATH"] = alias
                environment["CFFIXED_USER_HOME"] = fixedHomeURL.path
    ...
    @Test func testBundledStableCLIPreservesLiveUserScopedStableEnvSocket() throws {
        ...
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        environment["MOSAICTERM_CLI_RESPONSE_TIMEOUT_SEC"] = "5"
        environment["MOSAIC_SOCKET_PATH"] = userScopedStableSocketPath
        environment["CFFIXED_USER_HOME"] = fixedHomeURL.path
    ...
    @Test func testBundledStableCLIFallsBackFromSymlinkedLegacyStableEnvSocket() throws {
        ...
        let defaultStableSocketPath = socketDirectoryURL
            .appendingPathComponent("mosaic.sock", isDirectory: false)
            .path
        let legacyStableSocketPath = "/tmp/mosaic.sock"
        let symlinkTargetSocketPath = "/tmp/mosaic-symlink-target-\(UUID().uuidString).sock"
        ...
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        environment["MOSAICTERM_CLI_RESPONSE_TIMEOUT_SEC"] = "5"
        environment["MOSAIC_SOCKET_PATH"] = legacyStableSocketPath
        environment["CFFIXED_USER_HOME"] = fixedHomeURL.path
    ...
    @Test func testThemesSetIsDisabledAndDoesNotWriteThemeOverride() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-themes-disabled-\(UUID().uuidString)", isDirectory: true)
        ...
        let socketPath = "/tmp/mosaic-theme-\(UUID().uuidString.prefix(8)).sock"
        ...
        environment["MOSAIC_BUNDLE_ID"] = bundleIdentifier
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        ...
        XCTAssertFalse(result.timedOut, result.stdout)
        XCTAssertNotEqual(result.status, 0, result.stdout)
        XCTAssertTrue(result.stdout.contains("Terminal theme is managed by mosaic and cannot be changed."), result.stdout)
    ...
    @Test func testThemesClearIsDisabledAndDoesNotContactRunningApp() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-themes-clear-disabled-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }

        let socketPath = "/tmp/mosaic-theme-clear-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(path: socketPath, response: "OK")
        defer { responder.stop() }
    ...
    @Test func testThemesListReportsCursorDarkAsManagedTheme() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-themes-list-managed-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    @Test func testBareInteractiveThemesDoesNotLaunchPicker() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-themes-picker-disabled-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    @Test func testBareInteractiveThemesListsManagedThemeWithoutSocket() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-themes-picker-list-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        XCTAssertFalse(result.timedOut, result.stdout)
        XCTAssertEqual(result.status, 0, result.stdout)
        XCTAssertTrue(result.stdout.contains("Terminal theme is managed by mosaic"), result.stdout)
    }

    @Test func testBrowserDownloadWaitUsesRequestedTimeoutForSocketResponse() throws {
        let cliPath = try bundledCLIPath()
        let socketPath = "/tmp/mosaic-dw-\(UUID().uuidString.prefix(8)).sock"
    ...
    @Test func testDotPathOpenBypassesProtectedSocketForExternalCLI() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-cli-external-open-\(UUID().uuidString)", isDirectory: true)
        ...
        let socketPath = "/tmp/mosaic-external-open-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(
            path: socketPath,
            response: "ERROR: Access denied — only processes started inside mosaic can connect"
        )
        defer { responder.stop() }
        ...
        environment["MOSAIC_SOCKET_PATH"] = socketPath
        environment["MOSAIC_SOCKET"] = "/tmp/mosaic-stale-\(UUID().uuidString.prefix(8)).sock"
        environment["MOSAIC_SOCKET_PASSWORD"] = "stale-password"
        environment["MOSAIC_SOCKET_ENABLE"] = "0"
        environment["MOSAIC_SOCKET_MODE"] = "off"
        environment["MOSAIC_ALLOW_SOCKET_OVERRIDE"] = "1"
        environment["MOSAIC_WORKSPACE_ID"] = "workspace:stale"
        environment["MOSAIC_PANEL_ID"] = "panel:stale"
        environment["MOSAIC_SURFACE_ID"] = "surface:stale"
        environment["MOSAIC_TAB_ID"] = "tab:stale"
        environment["MOSAIC_TAG"] = "keepme"
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        environment["MOSAIC_TEST_OPEN_TOOL_PATH"] = fakeOpenURL.path
        environment["MOSAIC_TEST_OPEN_LOG"] = openLogURL.path
        environment["MOSAIC_TEST_OPEN_ENV_LOG"] = openEnvLogURL.path
        ...
        XCTAssertTrue(openEnvironment.contains("MOSAIC_TAG=keepme"), openEnvironment.joined(separator: "\n"))
    }

    @Test func testBareRelativeDirectoryPathOpenBypassesProtectedSocketForExternalCLI() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-cli-bare-open-\(UUID().uuidString)", isDirectory: true)
        let workingDirectory = root.appendingPathComponent("project", isDirectory: true)
        try FileManager.default.createDirectory(at: workingDirectory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let socketPath = "/tmp/mosaic-bare-open-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(
            path: socketPath,
            response: "ERROR: Access denied — only processes started inside mosaic can connect"
        )
    ...
    @Test func testKnownCommandStillUsesSocketWhenMatchingBareRelativePathExists() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-cli-command-path-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(
        ...
        let socketPath = "/tmp/mosaic-command-path-\(UUID().uuidString.prefix(8)).sock"
    ...
    @Test func testCaseVariantBareRelativeDirectoryPathOpenBypassesProtectedSocket() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-cli-case-path-\(UUID().uuidString)", isDirectory: true)
        ...
        let socketPath = "/tmp/mosaic-case-open-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(
            path: socketPath,
            response: "ERROR: Access denied — only processes started inside mosaic can connect"
        )
    ...
    @Test func testExplicitSocketPathOpenUsesRequestedSocket() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-cli-explicit-open-\(UUID().uuidString)", isDirectory: true)
        let workingDirectory = root.appendingPathComponent("project", isDirectory: true)
        try FileManager.default.createDirectory(at: workingDirectory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let socketPath = "/tmp/mosaic-explicit-open-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(
            path: socketPath,
    ...
    /// via the canonical ``MosaicStateDirectory`` and pass the same home to the
    /// spawned CLI via `CFFIXED_USER_HOME`, so they never touch (or bind over) the
    /// developer's real `~/.local/state/mosaic` (issue #5146).
    private func makeTemporaryHome() throws -> URL {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-cli-home-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: home, withIntermediateDirectories: true)
        return home
    }

    /// The stable control-socket path under an injected (temp) home, resolved via
    /// the canonical ``MosaicStateDirectory`` so the test exercises the real layout.
    private func stableSocketURL(home: URL) throws -> URL {
        let directory = MosaicStateDirectory.url(homeDirectory: home)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        return directory.appendingPathComponent("mosaic.sock", isDirectory: false)
    }
    ...
    private func fakeTaggedBundledCLIPath(
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-cli-socket-\(UUID().uuidString)", isDirectory: true)
        ...
        let fakeCLIURL = binURL.appendingPathComponent("mosaic", isDirectory: false)
    ...
    private func fakeOpenScript() -> String {
        """
        #!/bin/sh
        : "${MOSAIC_TEST_OPEN_LOG:?}"
        : > "$MOSAIC_TEST_OPEN_LOG"
        printf 'fake open stdout should be suppressed\\n'
        printf 'fake open stderr should be suppressed\\n' >&2
        if [ -n "${MOSAIC_TEST_OPEN_ENV_LOG:-}" ]; then
          env | LC_ALL=C sort | grep '^MOSAIC_' > "$MOSAIC_TEST_OPEN_ENV_LOG" || :
        fi
        for arg in "$@"; do
          printf '%s\\n' "$arg" >> "$MOSAIC_TEST_OPEN_LOG"
        done
        exit 0
        """
    }
...
private final class UnixSocketResponder {
    ...
    private let queue = DispatchQueue(label: "com.mosaic.tests.unix-socket-responder")
