...
struct PostHogAnalyticsPropertiesTests {
    ...
    func superPropertiesIncludePlatformVersionAndBuild() {
        ...
        #expect(properties["platform"] as? String == "mosaicterm")
        #expect(properties["app_version"] as? String == "0.31.0")
        #expect(properties["app_build"] as? String == "230")
    }
    ...
    func propertiesOmitVersionFieldsWhenUnavailable() {
        ...
        #expect(superProperties["platform"] as? String == "mosaicterm")
    ...
    func flushPolicyIncludesDailyAndHourlyActiveEvents() {
        #expect(PostHogAnalytics.shouldFlushAfterCapture(event: "mosaic_daily_active"))
        #expect(PostHogAnalytics.shouldFlushAfterCapture(event: "mosaic_hourly_active"))
        #expect(PostHogAnalytics.shouldFlushAfterCapture(event: "mac_error_captured"))
        #expect(!PostHogAnalytics.shouldFlushAfterCapture(event: "mosaic_other_event"))
    }
    ...
    func nativeInteractionMetadataUsesSafeStableValues() {
        #expect(MacAnalyticsEvent.uiInteraction.rawValue == "mosaic_ui_interaction")
    ...
    func macBugAlertClientSendsSafeSummaryPayloadWhenEnabled() async throws {
        ...
        let client = MacBugAlertClient(
            endpoint: try #require(URL(string: "https://mosaic.test/api/bug-alerts")),
        ...
        #expect(request.request.url?.absoluteString == "https://mosaic.test/api/bug-alerts")
        #expect(request.request.httpMethod == "POST")
        #expect(request.request.value(forHTTPHeaderField: "X-Mosaic-Bug-Alerts-Secret") == "test-secret")
    ...
    func genericCaptureScrubsAndSendsMacEventsDirectlyToPostHog() async throws {
        let suiteName = "mosaic.posthog.analytics.tests.\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        defer {
            defaults.removePersistentDomain(forName: suiteName)
        }

        let capturedQueue = DispatchQueue(label: "com.mosaic.tests.posthog.generic.capture")
        ...
        let analytics = PostHogAnalytics.makeForTesting(
            workQueue: DispatchQueue(label: "com.mosaic.tests.posthog.generic.analytics"),
            didStart: true,
        ...
        #expect(captured.properties["platform"] as? String == "mosaicterm")
    }
    ...
    func activeEventCaptureFlushesBeforeShutdown() throws {
        ...
        let capturedQueue = DispatchQueue(label: "com.mosaic.tests.posthog.capture")
        var capturedEvents: [(event: String, properties: [String: Any])] = []
        let eventsCaptured = DispatchSemaphore(value: 0)
        let flushCalled = DispatchSemaphore(value: 0)
        let analytics = PostHogAnalytics.makeForTesting(
            workQueue: DispatchQueue(label: "com.mosaic.tests.posthog.analytics"),
            didStart: true,
            userDefaults: defaults,
            now: { fixedDate },
            capturePostHog: { event, properties in
        ...
        #expect(events.map(\.event) == ["mosaic_daily_active", "mosaic_hourly_active"])
...
private final class AnalyticsURLProtocolStubState: @unchecked Sendable {
    private let queue = DispatchQueue(label: "com.mosaic.tests.analytics.urlprotocol")
...
private final class AnalyticsURLProtocolStub: URLProtocol {
    ...
    override func startLoading() {
        ...
        let response = HTTPURLResponse(
            url: request.url ?? URL(string: "https://mosaic.test")!,
