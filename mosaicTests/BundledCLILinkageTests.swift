    ...
    static func bundledCLIURL(
        ...
        while let item = enumerator?.nextObject() as? URL {
            guard item.lastPathComponent == "mosaic",
                  item.path.contains(".app/Contents/Resources/bin/mosaic"),
                  fileManager.isExecutableFile(atPath: item.path) else { continue }
            return item
        }

        let message = "Bundled mosaic CLI not found at \(expectedCLIURL.path)"
...
final class BundledCLILinkageTests: XCTestCase {
    ...
    func testBundledCLIDoesNotDependOnPrivateRPathFrameworks() throws {
        ...
        XCTAssertEqual(
            ...
            [],
            "The bundled mosaic CLI is copied into Contents/Resources/bin as a standalone helper. Private @rpath framework dependencies abort in dyld before CLI code can run."
        )
    }
