...
struct BrowserHiddenWebViewDiscardMediaPlaybackTests {
    /// Regression coverage for https://github.com/emergent-inc/mosaic/issues/5409:
    ...
    /// so the memory bound from https://github.com/emergent-inc/mosaic/issues/4539
    /// is preserved.
...
final class BrowserHiddenWebViewDiscardManagerTests: XCTestCase {
    ...
    // Regression coverage for https://github.com/emergent-inc/mosaic/issues/5261:
...
final class BrowserPanelInitialNavigationTests: XCTestCase {
    ...
    func testDiffViewerURLIsNotPersistedForSessionRestore() throws {
        let schemeURL = try XCTUnwrap(URL(string: "\(MosaicDiffViewerURLSchemeHandler.scheme)://token/index.html"))
        ...
        let loopbackURL = try XCTUnwrap(URL(string: "http://127.0.0.1:49152/token/diff.html#mosaic-diff-viewer"))
        ...
        let aliasURL = try XCTUnwrap(URL(string: "http://mosaic-loopback.localtest.me:49152/token/diff.html#mosaic-diff-viewer"))
    ...
    func testDiffViewerURLIsNotRecordedInBrowserHistory() throws {
        let fileURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-browser-history-\(UUID().uuidString).json")
...
final class BrowserPanelDiffViewerSchemeTests: XCTestCase {
    ...
    func testDiffViewerSchemeRegistrationIsIdempotentForCopiedConfiguration() {
        ...
        XCTAssertNotNil(config.urlSchemeHandler(forURLScheme: MosaicDiffViewerURLSchemeHandler.scheme))
    }
    ...
    func testDiffViewerSchemeLoadsSameOriginModuleFromAllowlist() throws {
        ...
        try MosaicDiffViewerURLSchemeHandler.shared.register(
            token: token,
            files: [
        ...
        let allowedURL = try XCTUnwrap(URL(string: "\(MosaicDiffViewerURLSchemeHandler.scheme)://\(token)/index.html"))
        let allowedPatchURL = try XCTUnwrap(URL(string: "\(MosaicDiffViewerURLSchemeHandler.scheme)://\(token)/index.patch"))
        let blockedURL = try XCTUnwrap(URL(string: "\(MosaicDiffViewerURLSchemeHandler.scheme)://\(token)/not-allowed.html"))
        let queryURL = try XCTUnwrap(URL(string: "\(MosaicDiffViewerURLSchemeHandler.scheme)://\(token)/index.html?copy=1"))
        XCTAssertNotNil(MosaicDiffViewerURLSchemeHandler.shared.registeredFile(for: allowedURL))
        XCTAssertNotNil(MosaicDiffViewerURLSchemeHandler.shared.registeredFile(for: allowedPatchURL))
        XCTAssertNil(MosaicDiffViewerURLSchemeHandler.shared.registeredFile(for: blockedURL))
        XCTAssertNil(MosaicDiffViewerURLSchemeHandler.shared.registeredFile(for: queryURL))
    ...
    func testDiffViewerSchemeRejectsSymlinkEscapeFromTrustedRoot() throws {
        ...
        let temporaryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-diff-viewer-security-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertThrowsError(try MosaicDiffViewerURLSchemeHandler.shared.register(
            token: token,
            files: [
    ...
    func testEnsureReactGrabActiveRefreshesBridgeSessionTokenWhenAlreadyActive() async throws {
        ...
        _ = try await panel.evaluateJavaScript(
            ...
            window['\(panel.reactGrabBridgeSessionUpdaterName)'] = function(token) {
                window.__mosaicTestRoundTripToken = token;
                return true;
            };
            true;
            """
        ...
        let refreshedToken = try await panel.evaluateJavaScript("window.__mosaicTestRoundTripToken") as? String
...
final class BrowserWindowPortalLifecycleTests: XCTestCase {
    private final class TrackingPortalWebView: WKWebView {
        ...
        @objc(_enterInWindow)
        func mosaicUnitTestEnterInWindow() {
            reattachRenderingStateCount += 1
        }

        @objc(_endDeferringViewInWindowChangesSync)
        func mosaicUnitTestEndDeferringViewInWindowChangesSync() {
            reattachRenderingStateCount += 1
        }
    }
    ...
    // Regression guard for https://github.com/emergent-inc/mosaic/issues/5733.
    ...
    // Crash regression for https://github.com/emergent-inc/mosaic/issues/5733.
    //
    ...
    // both `mosaic_performKeyEquivalent` and `mosaic_makeFirstResponder` ->
    ...
    func testPortalSlotPinPreservesSideDockedInspectorManagedWebViewFrameOnRehost() {
        ...
        let webView = MosaicWebView(frame: NSRect(x: 0, y: 0, width: 132, height: 160), configuration: WKWebViewConfiguration())
...
final class OmnibarNativeTextFieldCaretTests: XCTestCase {
    ...
    private func installOmnibarField(
        in window: NSWindow,
        stringValue: String = "https://github.com/emergent-inc/mosaic"
    ...
    /// Regression for https://github.com/emergent-inc/mosaic/issues/5268: a single,
