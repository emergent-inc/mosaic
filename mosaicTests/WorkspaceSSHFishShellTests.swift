...
final class WorkspaceSSHFishShellTests: XCTestCase {
    ...
    func testSSHBootstrapStartupCommandPassesRemoteInstallScriptAsSingleSSHCommand() throws {
        ...
        let tempRoot = fileManager.temporaryDirectory.appendingPathComponent("mosaic-ssh-bootstrap-\(UUID().uuidString)")
        ...
        args = sys.argv[1:]
        with open(os.environ["MOSAIC_FAKE_SSH_LOG"], "a", encoding="utf-8") as handle:
        ...
        if local_command:
            ...
            subprocess.run([os.environ["MOSAIC_TEST_LOCAL_SHELL"], "-c", local_command], check=False, env=os.environ.copy())
        PY
        cat >/dev/null
        exit 0
        """
        ...
        startupEnvironment["MOSAIC_FAKE_SSH_LOG"] = fakeSSHLog.path
        startupEnvironment["MOSAIC_TEST_PYTHON3"] = python3Path
        startupEnvironment["MOSAIC_TEST_LOCAL_SHELL"] = fishExecutable
        startupEnvironment["MOSAIC_SOCKET_PATH"] = socketPath
        startupEnvironment["MOSAIC_WORKSPACE_ID"] = workspaceID
        startupEnvironment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        startupEnvironment["MOSAIC_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
        ...
        let destinationIndex = try XCTUnwrap(firstInvocation.lastIndex(of: "mosaic-macmini"))
        ...
        XCTAssertTrue(
            secondRemoteCommand.contains("mosaic_bootstrap_tty=") || secondRemoteCommand.contains("mosaic_bootstrap_tty\\\""),
