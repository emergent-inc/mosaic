...
@MainActor
@Suite(.serialized)
struct MosaicWebViewContextMenuLinkCaptureTests {
    ...
    func openLinkInDefaultBrowserOpensTheLinkUnderTheRightClick() async throws {
        ...
        MosaicWebView.contextMenuLinkCaptureAcceptsUntrustedEventsForTesting = true
        defer { MosaicWebView.contextMenuLinkCaptureAcceptsUntrustedEventsForTesting = false }
    ...
    func syntheticContextMenuEventCannotPlantDecoyLink() async throws {
        ...
        // point is identical because MosaicWebView (WKWebView) is flipped.
        let openedURL = try await openLinkInDefaultBrowser(
            webView,
            menuEventLocation: NSPoint(x: 60, y: 60),
            menuEventTimestamp: ProcessInfo.processInfo.systemUptime
    ...
    /// CSS (300,300)-(420,420).
    private func makeLoadedTwoLinkWebView() async throws -> MosaicWebView {
        _ = NSApplication.shared
        let webView = MosaicWebView(
            frame: NSRect(x: 0, y: 0, width: 800, height: 600),
            configuration: WKWebViewConfiguration()
