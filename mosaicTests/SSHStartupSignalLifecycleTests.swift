...
extension CLINotifyProcessIntegrationRegressionTests {
    func testSSHPaneCloseSignalDoesNotReportSessionEndToSharedTransport() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ssh-pane-close-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("mosaic")
        ...
        try writeShellFile(at: fakeCLI, lines: [
            "#!/bin/sh",
            "printf '%s\\n' \"$*\" >> \"${MOSAIC_TEST_SESSION_END_LOG}\"",
        ])
        try writeShellFile(at: fakeSSH, lines: [
            "#!/bin/sh",
            "kill -\"${MOSAIC_TEST_SIGNAL:?}\" \"${MOSAIC_SSH_STARTUP_PID:-$PPID}\"",
            "sleep 0.1",
            "exit 0",
        ])
        ...
        for signal in ["HUP", "INT", "TERM"] {
            ...
            environment["MOSAIC_BUNDLED_CLI_PATH"] = fakeCLI.path
            environment["MOSAIC_SOCKET_PATH"] = "/tmp/mosaic-debug-test.sock"
            environment["MOSAIC_WORKSPACE_ID"] = "11111111-1111-1111-1111-111111111111"
            environment["MOSAIC_SURFACE_ID"] = "22222222-2222-2222-2222-222222222222"
            environment["MOSAIC_TEST_SESSION_END_LOG"] = logFile.path
            environment["MOSAIC_TEST_SIGNAL"] = signal
    ...
    func testSSHPaneCloseSignalDoesNotTerminateWrappedSSHChild() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ssh-pane-close-child-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("mosaic")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        ...
        try writeShellFile(at: fakeSSH, lines: [
            "#!/bin/sh",
            "trap 'printf \"%s\\n\" child-hup >> \"${MOSAIC_TEST_CHILD_SIGNAL_LOG}\"; exit 0' HUP",
            "trap 'printf \"%s\\n\" child-int >> \"${MOSAIC_TEST_CHILD_SIGNAL_LOG}\"; exit 0' INT",
            "trap 'printf \"%s\\n\" child-term >> \"${MOSAIC_TEST_CHILD_SIGNAL_LOG}\"; exit 0' TERM",
            "printf '%s\\n' child-started >> \"${MOSAIC_TEST_CHILD_SIGNAL_LOG}\"",
            "kill -\"${MOSAIC_TEST_SIGNAL:?}\" \"${MOSAIC_SSH_STARTUP_PID:-$PPID}\"",
            "sleep 0.2",
            "printf '%s\\n' child-completed >> \"${MOSAIC_TEST_CHILD_SIGNAL_LOG}\"",
            "exit 0",
        ])
        try fileManager.setAttributes([.posixPermissions: 0o700], ofItemAtPath: fakeCLI.path)
        try fileManager.setAttributes([.posixPermissions: 0o700], ofItemAtPath: fakeSSH.path)
        ...
        for signal in ["HUP", "INT", "TERM"] {
            ...
            environment["MOSAIC_SURFACE_ID"] = "22222222-2222-2222-2222-222222222222"
            environment["MOSAIC_TEST_SESSION_END_LOG"] = logFile.path
            environment["MOSAIC_TEST_CHILD_SIGNAL_LOG"] = childSignalLog.path
            environment["MOSAIC_TEST_SIGNAL"] = signal
    ...
    func testSSHStartupRetriesTransientSSHExitBeforeReportingSessionEnd() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ssh-reconnect-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("mosaic")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        ...
        try writeShellFile(at: fakeSSH, lines: [
            "#!/bin/sh",
            "count=0",
            "if [ -r \"${MOSAIC_TEST_ATTEMPT_FILE}\" ]; then count=$(cat \"${MOSAIC_TEST_ATTEMPT_FILE}\"); fi",
            "count=$((count + 1))",
            "printf '%s\\n' \"$count\" > \"${MOSAIC_TEST_ATTEMPT_FILE}\"",
        ...
        environment["MOSAIC_SURFACE_ID"] = "22222222-2222-2222-2222-222222222222"
        environment["MOSAIC_TEST_SESSION_END_LOG"] = logFile.path
        environment["MOSAIC_TEST_ATTEMPT_FILE"] = attemptFile.path
        environment["MOSAIC_SSH_RECONNECT_DELAY_SECONDS"] = "0"
        ...
        // The regular `mosaic ssh` bootstrap path runs one SSH command to install
    ...
    func testSSHStartupRemovesStaleMosaicControlSocketBeforeLaunchingPaneSSH() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ssh-stale-control-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("mosaic")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh.log")
        let staleControlPath = URL(fileURLWithPath: "/tmp", isDirectory: true)
            .appendingPathComponent("mosaic-ssh-\(getuid())-\(UUID().uuidString.prefix(8)).sock")
        ...
        try writeShellFile(at: fakeSSH, lines: [
            ...
            "  previous_arg=\"$arg\"",
            "done",
            "if [ -e \"${MOSAIC_TEST_CONTROL_PATH}\" ]; then",
            "  printf 'ControlSocket %s already exists, disabling multiplexing\\n' \"${MOSAIC_TEST_CONTROL_PATH}\" >&2",
            "  exit 99",
            "fi",
            "exit 0",
        ])
        ...
        environment["MOSAIC_WORKSPACE_ID"] = "11111111-1111-1111-1111-111111111111"
        environment["MOSAIC_SURFACE_ID"] = "22222222-2222-2222-2222-222222222222"
        environment["MOSAIC_TEST_CONTROL_PATH"] = staleControlPath.path
        environment["MOSAIC_TEST_SESSION_END_LOG"] = root.appendingPathComponent("ssh-session-end.log").path
        environment["MOSAIC_TEST_SSH_LOG"] = logFile.path
        environment["MOSAIC_SSH_RECONNECT_DELAY_SECONDS"] = "0"
    ...
    func testSSHStartupStopsAtConfiguredReconnectLimit() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ssh-retry-limit-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("mosaic")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        let attemptFile = root.appendingPathComponent("ssh-attempts.txt")
        ...
        environment["MOSAIC_TEST_ATTEMPT_FILE"] = attemptFile.path
        environment["MOSAIC_SSH_RECONNECT_DELAY_SECONDS"] = "0"
        environment["MOSAIC_SSH_RECONNECT_LIMIT"] = "2"
    ...
    func testSSHStartupDoesNotRetryNonTransientSSHExit() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ssh-no-retry-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("mosaic")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        let attemptFile = root.appendingPathComponent("ssh-attempts.txt")
    ...
    func testSSHSignalDerivedChildExitReportsSessionEnd() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ssh-child-signal-exit-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("mosaic")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
    ...
    func testSSHSignalDuringReconnectDelayDoesNotStartAnotherSSH() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ssh-signal-during-reconnect-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("mosaic")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        let attemptFile = root.appendingPathComponent("ssh-attempts.txt")
        ...
        try writeShellFile(at: fakeSSH, lines: [
            ...
            "printf '%s\\n' \"$count\" > \"${MOSAIC_TEST_ATTEMPT_FILE}\"",
            "if [ \"$count\" -eq 1 ]; then",
            "  ( sleep 0.2; kill -TERM \"${MOSAIC_SSH_STARTUP_PID:?}\" ) &",
            "  exit 255",
            "fi",
            "exit 0",
        ])
        ...
        environment["MOSAIC_TEST_SESSION_END_LOG"] = logFile.path
        environment["MOSAIC_TEST_ATTEMPT_FILE"] = attemptFile.path
        environment["MOSAIC_SSH_RECONNECT_DELAY_SECONDS"] = "2"
        environment["MOSAIC_SSH_RECONNECT_LIMIT"] = "2"
    ...
    func testSSHStartupPrintsFinalErrorBannerWhenStderrIsCaptured() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ssh-error-banner-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("mosaic")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 1, result.stderr)
        XCTAssertTrue(result.stderr.contains("[mosaic] ssh exited with status 1."), result.stderr)
        XCTAssertTrue(result.stderr.contains("[mosaic] press Enter to close this pane."), result.stderr)
    }

    func testSSHStartupForwardsStdinToBackgroundedSSH() throws {
        // Regression test for mosaic ssh sessions where output flowed back from
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ssh-stdin-forward-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("mosaic")
        let fakeSSH = root.appendingPathComponent("ssh")
        ...
        environment["MOSAIC_WORKSPACE_ID"] = "11111111-1111-1111-1111-111111111111"
        environment["MOSAIC_SURFACE_ID"] = "22222222-2222-2222-2222-222222222222"
        environment["MOSAIC_TEST_SESSION_END_LOG"] = sessionEndLog.path
        environment["MOSAIC_TEST_STDIN_LOG"] = stdinCapture.path
        environment["MOSAIC_SSH_RECONNECT_DELAY_SECONDS"] = "0"
    ...
    private func generatedSSHStartupCommand(
        ...
        arguments.append("mosaic-macmini")
