...
struct TerminalClearScreenKeepScrollbackTests {
    ...
    func remappedDefaultShortcutDoesNotTriggerStaleMenuSuppression() throws {
        ...
        withTemporaryShortcut(action: .clearScreenKeepScrollback, shortcut: .unbound) {
            #expect(!appDelegate.shouldSuppressStaleMosaicMenuShortcut(event: event))
        }
    }

    @Test
    func clearScreenKeepScrollbackDeliversFormFeedToForegroundProgram() throws {
        let readyMarker = "MOSAIC_CLEAR_READY_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        let captureMarker = "MOSAIC_CLEAR_HEX_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        ...
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-clear-keep-scrollback-\(UUID().uuidString).py")
        let script = """
        import os
        import select
        import sys
