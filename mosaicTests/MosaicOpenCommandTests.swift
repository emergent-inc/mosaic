...
final class MosaicOpenCommandTests: XCTestCase {
    ...
    func testDiffCommandGeneratesCodeViewAndOpensBrowserSplit() throws {
        ...
        try FileManager.default.createDirectory(at: mosaicConfigURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: mosaicAppSupportConfigURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        ...
        try ghosttyConfigContents.write(to: mosaicAppSupportConfigURL, atomically: true, encoding: .utf8)
        try """
        {
          "shortcuts": {
            "bindings": {
        ...
        """.write(to: mosaicConfigURL, atomically: true, encoding: .utf8)
        try """
        ...
        let serverHandled = startMockServer(listenerFD: listenerFD, state: state) { line in
            ...
            guard method == "browser.open_split",
                  ...
                  viewerURL.fragment == "mosaic-diff-viewer" else {
                return Self.v2Response(id: id, ok: false, error: ["code": "unexpected", "message": method])
            }
            return Self.v2Response(
                id: id,
        ...
        XCTAssertEqual(viewerURL.fragment, "mosaic-diff-viewer")
        ...
        XCTAssertTrue(html.contains("<script id=\"mosaic-diff-viewer-config\" type=\"application/json\">") && html.contains("background: transparent;"), html)
        XCTAssertTrue(html.contains("<div id=\"root\"></div>"), html)
        XCTAssertTrue(html.contains("<script type=\"module\" src=\"./assets/mosaic-diff-viewer-app/main.mjs\"></script>"), html)
        ...
        let appAssetDirectory = viewerFileURL.deletingLastPathComponent()
            .appendingPathComponent("assets", isDirectory: true)
            .appendingPathComponent("mosaic-diff-viewer-app", isDirectory: true)
        ...
        XCTAssertTrue(html.contains("\"dark\":\"mosaic-ghostty-dark-"), html)
        XCTAssertTrue(html.contains("\"light\":\"mosaic-ghostty-light-"), html)
        ...
        XCTAssertTrue(files.contains { file in
            file["request_path"] as? String == "/assets/mosaic-diff-viewer-app/main.mjs" &&
        ...
        try darkOnlyConfigContents.write(to: mosaicAppSupportConfigURL, atomically: true, encoding: .utf8)
    ...
    func testDiffCommandUsesTaggedSocketAppAssetsAndServer() throws {
        ...
        let socketPath = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("mosaic-debug-\(tag).sock", isDirectory: false)
            .path
        unlink(socketPath)
        let listenerFD = try bindUnixSocket(at: socketPath)
        let rootURL = FileManager.default.temporaryDirectory
        ...
        try writeTestDiffViewerAssets(
            ...
            appMain: "export const mosaicTaggedSocketAssetMarker = 'target-\(tag)';\n"
        )
        try """
        ...
        let appEntry = try XCTUnwrap(files.first { file in
            (file["request_path"] as? String)?.hasSuffix("/assets/mosaic-diff-viewer-app/main.mjs") == true
        })
        ...
        XCTAssertTrue(appMain.contains("mosaicTaggedSocketAssetMarker = 'target-\(tag)'"), appMain)
    ...
    func testDiffCommandLinksOriginalDiffshubPRURL() throws {
        ...
        let result = try runDiffCLIAndReadHTML(
            ...
            environmentOverrides: ["MOSAIC_DIFF_VIEWER_STREAM_REMOTE": "1"],
    ...
    func testDiffViewerServerBoundsDeferredWaitRequests() throws {
        ...
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-diff-viewer-wait-\(UUID().uuidString)", isDirectory: true)
        ...
        try """
        <!doctype html>
        <html data-mosaic-diff-pending="true">
        ...
        environment["MOSAIC_DIFF_VIEWER_WAIT_TIMEOUT_SECONDS"] = "0.05"
        ...
        let url = try XCTUnwrap(URL(string: "http://127.0.0.1:\(port)/__mosaic_diff_viewer_wait/\(token)/pending.html"))
        // The bounded deferred wait is the unit under test. With
        // MOSAIC_DIFF_VIEWER_WAIT_TIMEOUT_SECONDS=0.05 the server must give up on the
        ...
        XCTAssertFalse(body.contains("data-mosaic-diff-pending=\"true\""), body)
    ...
    func testDiffCommandFallsBackToNonEmptyGitSourceForSelector() throws {
        ...
        try runGit(["config", "user.name", "mosaic tests"], in: repoURL)
        try runGit(["config", "user.email", "mosaic@example.invalid"], in: repoURL)
        try "one\n".write(to: fileURL, atomically: true, encoding: .utf8)
    ...
    func testDiffCommandShowsFriendlyEmptyStateForLastTurnWithoutBaseline() throws {
        ...
        let result = try runDiffCLIAndReadHTML(
            ...
            environmentOverrides: [
                "MOSAIC_AGENT_HOOK_STATE_DIR": rootURL.appendingPathComponent("hook-state", isDirectory: true).path,
                "MOSAIC_WORKSPACE_ID": UUID().uuidString.lowercased(),
                "MOSAIC_SURFACE_ID": UUID().uuidString.lowercased()
            ],
            currentDirectoryURL: repoURL,
            readPatchSidecar: false
        )
    ...
    func testDiffCommandShowsFriendlyEmptyStateForEmptyLastTurnDiff() throws {
        ...
        let result = try runDiffCLIAndReadHTML(
            ...
            arguments: ["diff", "--last-turn"],
            environmentOverrides: [
                "MOSAIC_AGENT_HOOK_STATE_DIR": stateURL.path,
    ...
    func testDiffCommandSupportsGitSourcesAndSurfaceScopedLastTurn() throws {
        ...
        try runGit(["config", "user.name", "mosaic tests"], in: siblingRepoURL)
        try runGit(["config", "user.email", "mosaic@example.invalid"], in: siblingRepoURL)
        ...
        let homeStateURL = homeURL.appendingPathComponent(".mosaicterm", isDirectory: true)
    ...
    func testAgentTurnDiffBaselineStoresUntrackedSnapshotsOutsideGit() throws {
        ...
        let untrackedRefs = try runGitStdout(
            ["for-each-ref", "--format=%(refname)", "refs/mosaic/last-turn/untracked"],
            in: repoURL
        )
    ...
    func testDiffCommandOpensPendingViewerBeforeGitDiffCompletes() throws {
        ...
        if [ "${1:-}" = "rev-parse" ] && [ "${2:-}" = "--show-toplevel" ]; then
          printf '%s\\n' "$MOSAIC_FAKE_GIT_REPO_ROOT"
          exit 0
        fi
        if [ "${1:-}" = "rev-parse" ] && [ "${2:-}" = "--verify" ]; then
          : > "$MOSAIC_FAKE_GIT_STARTED"
          while [ ! -f "$MOSAIC_FAKE_GIT_RELEASE" ]; do
            sleep 0.05
          done
          exit 1
        fi
        if [ "${1:-}" = "diff" ] && [ "${2:-}" = "--cached" ]; then
          : > "$MOSAIC_FAKE_GIT_ALTERNATE_STARTED"
          while [ ! -f "$MOSAIC_FAKE_GIT_RELEASE_ALTERNATE" ]; do
            sleep 0.05
          done
          exit 0
        fi
        ...
        environment["MOSAIC_FAKE_GIT_REPO_ROOT"] = repoURL.path
        environment["MOSAIC_FAKE_GIT_STARTED"] = diffStartedURL.path
        environment["MOSAIC_FAKE_GIT_RELEASE"] = releaseDiffURL.path
        environment["MOSAIC_FAKE_GIT_ALTERNATE_STARTED"] = alternateStartedURL.path
        environment["MOSAIC_FAKE_GIT_RELEASE_ALTERNATE"] = releaseAlternateURL.path
        process.executableURL = URL(fileURLWithPath: cliPath)
        ...
        XCTAssertTrue(pendingHTML.contains("data-mosaic-diff-pending=\"true\""), pendingHTML)
        ...
        XCTAssertTrue(waitUntil(timeout: 5) {
            ...
            return html.contains("data-mosaic-diff-redirect=")
        ...
        XCTAssertFalse(html.contains("data-mosaic-diff-pending=\"true\""), html)
    ...
    func testTopCommandOutputsWindowLevelProcessRows() throws {
        ...
        XCTAssertEqual(outputLines(result.stdout), [
            ...
            "2.0\t2000\t1\tprocess\t4129\twindow:1\tmosaic",
        ])
    }
    ...
    func testTopCommandSortsFlatWindowProcessesAndWorkspacesTogetherByMemory() throws {
        ...
        XCTAssertEqual(outputLines(result.stdout), [
            "2.0\t2000\t2\twindow\twindow:1\ttotal\t",
            "4.0\t10000\t1\tprocess\t4129\twindow:1\tmosaic",
            "1.0\t1000\t1\tworkspace\tworkspace:low\twindow:1\t",
        ])
    }
    ...
    private static func diffViewerRedirectURL(from html: String) -> String? {
        let marker = "data-mosaic-diff-redirect=\""
    ...
    private func diffViewerConfig(from html: String) throws -> [String: Any] {
        let marker = "<script id=\"mosaic-diff-viewer-config\" type=\"application/json\">"
    ...
    private func runGit(_ arguments: [String], in directory: URL) throws {
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        if result.status != 0 {
            throw NSError(domain: "MosaicOpenCommandTests.git", code: Int(result.status), userInfo: [NSLocalizedDescriptionKey: result.stderr])
        }
    }
    ...
    private func writeTestDiffViewerAssets(resourcesURL: URL, appMain: String) throws {
        ...
        try "self.mosaicWorkerFixture = true;\n".write(
    ...
    private func readLine(from handle: FileHandle, timeout: TimeInterval) throws -> String {
        ...
        guard finished.wait(timeout: .now() + timeout) == .success else {
            throw NSError(domain: "mosaic.tests", code: 1, userInfo: [
    ...
    private func fetchData(from url: URL, timeout: TimeInterval) throws -> (data: Data, statusCode: Int) {
        ...
        guard finished.wait(timeout: .now() + timeout) == .success else {
            ...
            throw NSError(domain: "mosaic.tests", code: 2, userInfo: [
