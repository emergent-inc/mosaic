...
final class CLINotifyProcessIntegrationRegressionTests: XCTestCase {
    ...
    // https://github.com/emergent-inc/mosaic/issues/6606
    //
    ...
    func testCodexPromptSubmitDoesNotRefreshTerminalLastTurnDiffBaseline() throws {
        ...
        _ = try runGit(["config", "user.name", "mosaic tests"])
        _ = try runGit(["config", "user.email", "mosaic@example.invalid"])
    ...
    // MARK: - Forked conversation restore (https://github.com/emergent-inc/mosaic/issues/5908)
    //
    ...
    private func runClaudeHookWithoutServer(
        ...
        var environment = [
            ...
            "MOSAIC_SOCKET_PATH": context.socketPath,
            "MOSAIC_WORKSPACE_ID": context.workspaceId,
            "MOSAIC_SURFACE_ID": context.surfaceId,
            "MOSAIC_CLAUDE_HOOK_STATE_PATH": context.root.appendingPathComponent("claude-hook-sessions.json").path,
            "MOSAIC_CLI_SENTRY_DISABLED": "1",
            "MOSAIC_CLAUDE_HOOK_SENTRY_DISABLED": "1",
        ]
    ...
    func testClaudeForkSessionStartWithoutSurfaceIdentityDoesNotRegisterPIDOnFallbackPane() throws {
        ...
        environment["MOSAIC_SURFACE_ID"] = ""
        environment["MOSAIC_CLAUDE_PID"] = "12345"
        let result = runClaudeHookListingSurfaces(
            context: context,
            surfaceIds: [parentSurfaceId, context.surfaceId],
            arguments: ["hooks", "claude", "session-start"],
    ...
    func testClaudeLegacyStoreBackfillsPaneBoundaryFromWorkspaceActiveSlot() throws {
        ...
        let paneBPrompt = runClaudeHookWithoutServer(
            ...
            extraEnvironment: ["MOSAIC_SURFACE_ID": paneB]
        )
        ...
        let lateStop = runClaudeHookWithoutServer(
            ...
            extraEnvironment: ["MOSAIC_SURFACE_ID": paneA]
        )
    ...
    func testClaudeForkedSessionPromptSubmitRecordsWhileParentTurnActive() throws {
        ...
        XCTAssertEqual(
            ...
            true,
            "The forked session must become restorable so a mosaic restart resumes the fork, not the parent"
        )
    ...
    func testClaudeForkedSessionPromptSubmitRecordsWithSurfaceRefForm() throws {
        ...
        let result = runClaudeHookListingSurfaces(
            ...
            arguments: ["hooks", "claude", "prompt-submit"],
            standardInput: #"{"session_id":"\#(forkedSessionId)","turn_id":"fork-turn-1","cwd":"\#(context.root.path)","hook_event_name":"UserPromptSubmit","prompt":"diverge here"}"#,
            extraEnvironment: ["MOSAIC_SURFACE_ID": "surface:2"]
        )
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
    ...
    func testClaudeStaleStopFromClosedPaneStaysStaleWhenSurfaceResolutionFallsBack() throws {
        ...
        let result = runClaudeHookListingSurfaces(
            ...
            extraEnvironment: ["MOSAIC_SURFACE_ID": closedSurfaceId]
        )
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
    ...
    func testClaudeStaleStopStaysStaleAfterAnotherPaneBecomesWorkspaceActive() throws {
        ...
        func runHook(_ subcommand: String, stdin: String, surface: String) -> ProcessRunResult {
            runClaudeHookWithoutServer(
                ...
                standardInput: stdin,
                extraEnvironment: ["MOSAIC_SURFACE_ID": surface]
            )
        }
    ...
    func testClaudeParentPaneStopAppliesAfterForkedSessionPromoted() throws {
        ...
        let result = runClaudeHookListingSurfaces(
            ...
            extraEnvironment: ["MOSAIC_SURFACE_ID": parentSurfaceId]
        )
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
    ...
    func testClaudePromptSubmitResumeBindingPersistsSafeAuthSelectionValues() throws {
        ...
        let launchEnvironment = [
            ...
            "MOSAIC_AGENT_LAUNCH_EXECUTABLE": "/usr/local/bin/claude",
            "MOSAIC_AGENT_LAUNCH_CWD": context.root.path,
            "MOSAIC_AGENT_LAUNCH_ARGV_B64": base64NULSeparated([
        ...
        XCTAssertEqual(environment["MOSAIC_PRESERVE_CLAUDE_AUTH_SELECTION_ENV"] as? String, "1")
        XCTAssertEqual(
            environment["MOSAIC_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS"] as? String,
    ...
    func testManagedCodexSubagentStopDoesNotReplaceResumeBinding() throws {
        ...
        let result = runCodexHook(
            ...
            extraEnvironment: codexLaunchEnvironment(context: context, sessionId: sessionId).merging([
                "MOSAIC_AGENT_MANAGED_SUBAGENT": "1",
                "MOSAIC_CODEX_TEAMS_THREAD_ID": "child-thread",
                "MOSAIC_CODEX_TEAMS_PARENT_THREAD_ID": "root-thread",
                "MOSAIC_CODEX_TEAMS_DEPTH": "1",
    ...
    func testRightSidebarInvalidCommandValidatesBeforeTargetResolution() throws {
        let cliPath = try bundledCLIPath()
        let missingSocketPath = "/tmp/mosaic-test-missing-\(UUID().uuidString).sock"
        var environment = ProcessInfo.processInfo.environment
        environment["MOSAIC_SOCKET_PATH"] = missingSocketPath
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
    ...
    func testSSHForwardAgentRepeatedOptionUsesLastValue() throws {
        let run = try runMockedSSH(
            ...
            environmentOverrides: [
                "SSH_AUTH_SOCK": "/tmp/mosaic-test-agent-\(UUID().uuidString).sock",
            ]
        )
        let createParams = try XCTUnwrap(params(for: "workspace.create", in: run.requests))
        let configureParams = try XCTUnwrap(params(for: "workspace.remote.configure", in: run.requests))
    ...
    func testSSHForwardAgentTildeSocketPathExpandsSocketPath() throws {
        let homeURL = try makeTemporaryDirectory(prefix: "mosaic-ssh-home")
        let tildeSocketPath = "~/.ssh/mosaic-test-agent.sock"
        let expandedSocketURL = homeURL.appendingPathComponent(".ssh/mosaic-test-agent.sock")
    ...
    private func assertSSHPersistentPTYUsesReusableForegroundAuthControlConnection(
        ...
        XCTAssertTrue(initialScript.contains("MOSAIC_WORKSPACE_ID"), initialScript)
        XCTAssertTrue(initialScript.contains("MOSAIC_SURFACE_ID"), initialScript)
        XCTAssertTrue(
        ...
        XCTAssertTrue(
            initialScript.contains("ssh-$mosaic_ssh_pty_workspace_id-$mosaic_ssh_pty_surface_id"),
            initialScript
        )
        XCTAssertTrue(initialScript.contains("254|255"), initialScript)
        assertSSHPTYAttachOmitsSurfaceArgument(initialScript)
        XCTAssertTrue(
            initialScript.contains("--workspace \"$mosaic_ssh_pty_workspace_id\""),
            initialScript
        )
        XCTAssertEqual(
        ...
        XCTAssertTrue(terminalStartupScript.contains("MOSAIC_WORKSPACE_ID"), terminalStartupScript)
        XCTAssertTrue(terminalStartupScript.contains("MOSAIC_SURFACE_ID"), terminalStartupScript)
        XCTAssertTrue(
        ...
        XCTAssertTrue(
            terminalStartupScript.contains("ssh-$mosaic_ssh_pty_workspace_id-$mosaic_ssh_pty_surface_id"),
            terminalStartupScript
        )
        ...
        XCTAssertTrue(
            terminalStartupScript.contains("--workspace \"$mosaic_ssh_pty_workspace_id\""),
            terminalStartupScript
        )
        XCTAssertEqual(
    ...
    func testSSHPersistentPTYFallsBackWhenForegroundAuthCannotBeReused() throws {
        let cases: [(name: String, arguments: [String])] = [
            ...
            ("local-command", ["--ssh-option", "LocalCommand=echo mosaic-test"]),
    ...
    func testSSHPTYAttachWithoutSurfaceDoesNotSendLocalAttachEnd() throws {
        ...
        environment["MOSAIC_SOCKET_PATH"] = socketPath
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        environment.removeValue(forKey: "MOSAIC_SURFACE_ID")
    ...
    func testSSHPTYAttachWaitUsesCurrentTerminalSizeForBridgeHandshake() throws {
        ...
        guard openpty(&masterFD, &slaveFD, nil, nil, nil) == 0 else {
            throw NSError(domain: "mosaic.tests", code: Int(errno), userInfo: [
    ...
    func testSSHSessionAttachCreatesSurfaceWithPersistedPTYSessionID() throws {
        ...
        let serverHandled = startMockServer(listenerFD: listenerFD, state: state) { line in
            ...
            XCTAssertTrue(initialCommand.contains("MOSAIC_WORKSPACE_ID"), initialCommand)
            XCTAssertTrue(initialCommand.contains("MOSAIC_SURFACE_ID"), initialCommand)
    ...
    func testNotifyWithUUIDSurfaceDoesNotRequireCallerWorkspaceOrWindow() throws {
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["MOSAIC_SOCKET_PATH"] = socketPath
        environment["MOSAIC_WORKSPACE_ID"] = callerWorkspace
        environment["MOSAIC_SURFACE_ID"] = callerSurface
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
    ...
    func testCodexPromptSubmitRebindsRestoredSessionToCurrentCallerSurface() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-rebind-\(UUID().uuidString)", isDirectory: true)
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["MOSAIC_SOCKET_PATH"] = socketPath
        environment["MOSAIC_WORKSPACE_ID"] = currentWorkspaceId
        environment["MOSAIC_SURFACE_ID"] = currentSurfaceId
    ...
    func testCodexPromptSubmitWithForeignMosaicEnvDoesNotFallbackToSelectedWorkspace() throws {
        let cliPath = try bundledCLIPath()
        let socketPath = makeSocketPath("codex-foreign-env")
        let listenerFD = try bindUnixSocket(at: socketPath)
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-foreign-env-\(UUID().uuidString)", isDirectory: true)
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["MOSAIC_SOCKET_PATH"] = socketPath
        environment["MOSAIC_WORKSPACE_ID"] = foreignWorkspaceId
        environment["MOSAIC_SURFACE_ID"] = foreignSurfaceId
        environment["MOSAIC_AGENT_HOOK_STATE_DIR"] = root.path
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        environment["CODEX_HOME"] = root.appendingPathComponent("codex-home", isDirectory: true).path
        ...
        XCTAssertFalse(
            ...
            "Foreign mosaic env must not mutate the selected workspace, saw \(state.commands)"
        )
    }

    func testCodexPromptSubmitWithForeignMosaicEnvIgnoresStaleMappedSession() throws {
        let cliPath = try bundledCLIPath()
        let socketPath = makeSocketPath("codex-foreign-mapped")
        let listenerFD = try bindUnixSocket(at: socketPath)
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-foreign-mapped-\(UUID().uuidString)", isDirectory: true)
        let foreignWorkspaceId = "11111111-1111-1111-1111-111111111111"
        let foreignSurfaceId = "22222222-2222-2222-2222-222222222222"
        let selectedWorkspaceId = "33333333-3333-3333-3333-333333333333"
        let selectedSurfaceId = "44444444-4444-4444-4444-444444444444"
        ...
        XCTAssertFalse(
            state.commands.contains { $0.contains("set_status codex Running") || $0.contains("notify_target_async") },
            "Foreign mosaic env must not reuse stale mapped sessions, saw \(state.commands)"
        )
    }
    ...
    func testCodexPromptSubmitWithInvalidSurfaceDoesNotFallbackToFocusedSurface() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-invalid-surface-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "33333333-3333-3333-3333-333333333333"
    ...
    func testCodexPromptSubmitWithInvalidMappedWorkspaceDoesNotFallbackToSelectedWorkspace() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-invalid-mapped-\(UUID().uuidString)", isDirectory: true)
        let staleWorkspaceId = "11111111-1111-1111-1111-111111111111"
        let staleSurfaceId = "22222222-2222-2222-2222-222222222222"
        let selectedWorkspaceId = "33333333-3333-3333-3333-333333333333"
        ...
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        environment["CODEX_HOME"] = root.appendingPathComponent("codex-home", isDirectory: true).path
        environment.removeValue(forKey: "MOSAIC_WORKSPACE_ID")
        environment.removeValue(forKey: "MOSAIC_SURFACE_ID")
    ...
    func testCodexTeamsForkPromptPublishesResumeBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-teams-resume-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "33333333-3333-3333-3333-333333333333"
        ...
        environment["MOSAIC_AGENT_HOOK_STATE_DIR"] = root.path
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        environment["MOSAIC_AGENT_LAUNCH_KIND"] = "codexTeams"
        environment["MOSAIC_AGENT_LAUNCH_EXECUTABLE"] = "/usr/local/bin/mosaic"
        ...
        XCTAssertEqual(
            ...
            "cd -- '\(root.path)' 2>/dev/null || [ ! -d '\(root.path)' ] && '/usr/local/bin/mosaic' 'codex-teams' 'resume' '\(sessionId)' '--model' 'gpt-5.4' '--sandbox' 'danger-full-access'"
        )
    }
    ...
    func testAgentPromptClearsSurfaceResumeBindingWhenResumeCommandUnavailable() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-agent-resume-unavailable-\(UUID().uuidString)", isDirectory: true)
            ...
            "sessions": [
                sessionId: [
                    ...
                    "launchCommand": [
                        "launcher": "omx",
                        "executablePath": "/usr/local/bin/mosaic",
                        "arguments": ["/usr/local/bin/mosaic", "omx", "hud"],
                        "workingDirectory": root.path,
                        "capturedAt": now,
                        "source": "test",
                    ],
    ...
    func testGenericAgentSessionEndClearsMatchingSurfaceResumeBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-agent-resume-clear-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer {
            Darwin.close(listenerFD)
            unlink(socketPath)
    ...
    func testSurfaceResumeSetCLIDoesNotScopeExplicitSurfaceToEnvWorkspace() throws {
        ...
        environment["MOSAIC_SOCKET_PATH"] = socketPath
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        environment["MOSAIC_WORKSPACE_ID"] = staleWorkspaceId
        environment["MOSAIC_SURFACE_ID"] = "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb"
    ...
    private func agentLaunchEnvironment(
        ...
        [
            "MOSAIC_AGENT_LAUNCH_KIND": kind,
            "MOSAIC_AGENT_LAUNCH_EXECUTABLE": executable,
            "MOSAIC_AGENT_LAUNCH_CWD": context.root.path,
            "MOSAIC_AGENT_LAUNCH_ARGV_B64": base64NULSeparated(arguments ?? [executable]),
        ]
    }
    ...
    private func runAgentHook(
        ...
        var environment = [
            ...
            "MOSAIC_WORKSPACE_ID": context.workspaceId,
            "MOSAIC_SURFACE_ID": context.surfaceId,
            "MOSAIC_AGENT_HOOK_STATE_DIR": context.root.path,
            "MOSAIC_CLI_SENTRY_DISABLED": "1",
        ]
    ...
    private func makeClaudeHookContext(name: String) throws -> ClaudeHookContext {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-\(name)-\(UUID().uuidString)", isDirectory: true)
    ...
    func testBrowserImportUsesInteractiveDialogOutsideCodingAgent() throws {
        ...
        environment["MOSAIC_SOCKET_PATH"] = socketPath
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        environment.removeValue(forKey: "MOSAIC_AGENT_LAUNCH_KIND")
    ...
    private func runMockedSSH(
        ...
        var environment = ProcessInfo.processInfo.environment
        for key in Array(environment.keys) where key.hasPrefix("MOSAIC_") {
    ...
    private func makeExistingAgentSocketPath() throws -> String {
        let directory = try makeTemporaryDirectory(prefix: "mosaic-agent")
...
extension CLINotifyProcessIntegrationRegressionTests {
    ...
    // stamped the focused surface (A), desyncing MOSAIC_SURFACE_ID from MOSAIC_PANEL_ID and jumbling codex
    // into the wrong surface on reload.
    func testTmuxCompatEnvStampsLaunchSurfaceNotFocusedPane() throws {
        let cliPath = try bundledCLIPath()
        let tmpDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-spawn-id-e2e-\(UUID().uuidString)", isDirectory: true)
        ...
        let result = runProcess(
            ...
            environment: [
                "MOSAIC_SOCKET_PATH": socketPath,
                "MOSAIC_WORKSPACE_ID": launchWorkspace,
                "MOSAIC_SURFACE_ID": launchSurface,
                "MOSAIC_PANEL_ID": launchSurface,
                "MOSAIC_TAB_ID": launchTab,
                "HOME": tmpDir.path,
        ...
        XCTAssertTrue(
            result.stdout.contains("MOSAIC_SURFACE_ID=\(launchSurface)"),
            "launcher must stamp the LAUNCH surface; stdout:\n\(result.stdout)\nstderr:\n\(result.stderr)"
        )
        XCTAssertFalse(
            result.stdout.contains("MOSAIC_SURFACE_ID=\(focusedSurface)"),
            "launcher must NOT stamp the focused surface; stdout:\n\(result.stdout)"
        )
        XCTAssertTrue(result.stdout.contains("MOSAIC_WORKSPACE_ID=\(launchWorkspace)"), result.stdout)
        // Matched-pair invariant: SURFACE == PANEL (the desync is exactly the bug). The surface-scoped
        // tab id passes through untouched.
        XCTAssertTrue(result.stdout.contains("MOSAIC_PANEL_ID=\(launchSurface)"), result.stdout)
        XCTAssertTrue(result.stdout.contains("MOSAIC_TAB_ID=\(launchTab)"), result.stdout)
    }
}
