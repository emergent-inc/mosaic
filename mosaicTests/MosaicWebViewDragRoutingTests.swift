...
private var mosaicUnitTestWKWebViewDragLifecycleOverrideInstalled = false
private var mosaicUnitTestWKWebViewDragLifecycleEvents: [String]?

extension WKWebView {
    @objc func mosaicUnitTest_draggingEntered(_ sender: any NSDraggingInfo) -> NSDragOperation {
        if mosaicUnitTestWKWebViewDragLifecycleEvents != nil {
            mosaicUnitTestWKWebViewDragLifecycleEvents?.append("entered")
            return .copy
        }
        return mosaicUnitTest_draggingEntered(sender)
    }

    @objc func mosaicUnitTest_draggingUpdated(_ sender: any NSDraggingInfo) -> NSDragOperation {
        if mosaicUnitTestWKWebViewDragLifecycleEvents != nil {
            mosaicUnitTestWKWebViewDragLifecycleEvents?.append("updated")
            return .copy
        }
        return mosaicUnitTest_draggingUpdated(sender)
    }

    @objc func mosaicUnitTest_prepareForDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        if mosaicUnitTestWKWebViewDragLifecycleEvents != nil {
            mosaicUnitTestWKWebViewDragLifecycleEvents?.append("prepare")
            return true
        }
        return mosaicUnitTest_prepareForDragOperation(sender)
    }

    @objc func mosaicUnitTest_performDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        if mosaicUnitTestWKWebViewDragLifecycleEvents != nil {
            mosaicUnitTestWKWebViewDragLifecycleEvents?.append("perform")
            return true
        }
        return mosaicUnitTest_performDragOperation(sender)
    }

    @objc func mosaicUnitTest_concludeDragOperation(_ sender: (any NSDraggingInfo)?) {
        if mosaicUnitTestWKWebViewDragLifecycleEvents != nil {
            mosaicUnitTestWKWebViewDragLifecycleEvents?.append("conclude")
            return
        }
        mosaicUnitTest_concludeDragOperation(sender)
    }
}

private func installMosaicUnitTestWKWebViewDragLifecycleOverride() {
    guard !mosaicUnitTestWKWebViewDragLifecycleOverrideInstalled else { return }
    ...
    swizzle(
        ...
        #selector(WKWebView.mosaicUnitTest_draggingEntered(_:))
    )
    swizzle(
        #selector(NSView.draggingUpdated(_:)),
        #selector(WKWebView.mosaicUnitTest_draggingUpdated(_:))
    )
    swizzle(
        #selector(NSView.prepareForDragOperation(_:)),
        #selector(WKWebView.mosaicUnitTest_prepareForDragOperation(_:))
    )
    swizzle(
        #selector(NSView.performDragOperation(_:)),
        #selector(WKWebView.mosaicUnitTest_performDragOperation(_:))
    )
    swizzle(
        #selector(NSView.concludeDragOperation(_:)),
        #selector(WKWebView.mosaicUnitTest_concludeDragOperation(_:))
    )

    mosaicUnitTestWKWebViewDragLifecycleOverrideInstalled = true
}
...
@MainActor
final class MosaicWebViewDragRoutingTests: XCTestCase {
    override func setUp() {
        super.setUp()
        installMosaicUnitTestWKWebViewDragLifecycleOverride()
        mosaicUnitTestWKWebViewDragLifecycleEvents = nil
    }
    ...
    func testRejectsInternalPaneDragEvenWhenFilePromiseTypesArePresent() {
        XCTAssertTrue(
            MosaicWebView.shouldRejectInternalPaneDrag([
    ...
    func testAllowsRegularExternalFileImageAndURLDrops() {
        ...
        for pasteboardTypes in externalPayloads {
            XCTAssertFalse(
                MosaicWebView.shouldRejectInternalPaneDrag(pasteboardTypes),
    ...
    func testWebsiteDragPayloadReachesWebKitDragLifecycle() {
        let pasteboard = NSPasteboard(name: NSPasteboard.Name("mosaic.web-drag.\(UUID().uuidString)"))
        ...
        mosaicUnitTestWKWebViewDragLifecycleEvents = []
    ...
    func testInternalPaneDragDoesNotReachWebKitDragLifecycle() {
        let pasteboard = NSPasteboard(name: NSPasteboard.Name("mosaic.internal-drag.\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        XCTAssertEqual(mosaicUnitTestWKWebViewDragLifecycleEvents, [])
    }
}
...
final class BrowserScreenshotPipelineTests: XCTestCase {
    ...
    func testFullPageSnapshotWritesPNGAndTIFFToPasteboard() async throws {
        let pasteboard = NSPasteboard(name: .init("mosaic-browser-screenshot-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testScreenshotPasteboardWriterClearsExistingContentsBeforeWriting() async throws {
        let pasteboard = NSPasteboard(name: .init("mosaic-browser-screenshot-existing-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testSectionSnapshotCropsSelectionBeforeWritingPasteboard() async throws {
        let pasteboard = NSPasteboard(name: .init("mosaic-browser-screenshot-section-\(UUID().uuidString)"))
        pasteboard.clearContents()
