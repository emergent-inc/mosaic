...
final class MosaicSSHURLRequestTests: XCTestCase {
    deinit {}
    ...
    func testCommandPreviewIncludesSocketPathWhenProvided() throws {
        ...
        case .success(.some(let request)):
            XCTAssertEqual(
                request.cliPreview(socketPath: "/tmp/mosaic-urlcmd.sock"),
                "mosaic --socket /tmp/mosaic-urlcmd.sock ssh --name \"Dev SSH\" dev.example.com"
            )
        case .success(nil):
            XCTFail("Expected SSH URL request")
        case .failure(let error):
    ...
    func testParsesStableNightlyAndDevSchemes() throws {
        for scheme in ["mosaic", "mosaic-nightly", "mosaic-dev"] {
            ...
            switch MosaicSSHURLRequest.parse(url, supportedSchemes: MosaicSSHURLRequest.supportedSchemes) {
            case .success(.some(let request)):
                XCTAssertEqual(request.destination, "dev.example.com")
            case .success(nil):
    ...
    func testDefaultParserIgnoresOtherProductSchemes() throws {
        let inactiveScheme = try XCTUnwrap(MosaicSSHURLRequest.supportedSchemes.first {
    ...
    func testAcceptsTextURLAtMaxLength() throws {
        let text = String(repeating: "a", count: MosaicTextURLRequest.maxTextLength)
        let url = try XCTUnwrap(textURL(host: "prompt", queryItems: [
        ...
        switch MosaicTextURLRequest.parse(url) {
        case .success(.some(let request)):
            XCTAssertEqual(request.text.count, MosaicTextURLRequest.maxTextLength)
        case .success(nil):
            XCTFail("Expected prompt URL request")
        case .failure(let error):
            XCTFail("Unexpected parse error: \(error)")
    ...
    func testRejectsTextURLExceedingMaxLength() throws {
        let text = String(repeating: "a", count: MosaicTextURLRequest.maxTextLength + 1)
        let url = try XCTUnwrap(textURL(host: "prompt", queryItems: [
            URLQueryItem(name: "text", value: text)
        ]))

        switch MosaicTextURLRequest.parse(url) {
        case .failure(.textTooLong(maxLength: MosaicTextURLRequest.maxTextLength)):
            break
        default:
    ...
    private func parsedOptional(_ url: URL) throws -> MosaicSSHURLRequest? {
        switch MosaicSSHURLRequest.parse(url) {
...
final class MosaicNavigationURLRequestTests: XCTestCase {
    private let supportedScheme = "mosaic-test"
    ...
    func testGeneratedLinksRoundTrip() throws {
        let workspaceURL = try XCTUnwrap(URL(string: MosaicNavigationURLRequest.workspaceLink(workspaceId: workspaceId, scheme: supportedScheme)))
        let paneURL = try XCTUnwrap(URL(string: MosaicNavigationURLRequest.paneLink(workspaceId: workspaceId, paneId: paneId, scheme: supportedScheme)))
        let surfaceURL = try XCTUnwrap(URL(string: MosaicNavigationURLRequest.surfaceLink(workspaceId: workspaceId, surfaceId: surfaceId, scheme: supportedScheme)))
    ...
    func testIgnoresOtherMosaicRoutesAndInactiveSchemes() throws {
        ...
        let inactiveURL = try XCTUnwrap(URL(string: "mosaic-other://workspace/\(workspaceId.uuidString)"))
    ...
    func testRejectsQueryFragmentAuthorityAndExtraPathComponents() throws {
        ...
        for rawURL in cases {
            let url = try XCTUnwrap(URL(string: rawURL))
            switch MosaicNavigationURLRequest.parse(url, supportedSchemes: [supportedScheme]) {
    ...
    private func parsedOptional(_ url: URL) throws -> MosaicNavigationURLRequest? {
        switch MosaicNavigationURLRequest.parse(url, supportedSchemes: [supportedScheme]) {
        case .success(let request):
            return request
        case .failure(let error):
    ...
    private func parsedTarget(_ url: URL) throws -> MosaicNavigationURLRequest.Target {
