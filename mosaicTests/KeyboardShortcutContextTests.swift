...
// Line ~253 compares MosaicSettings.ShortcutAction.defaultStroke, so the
// package stroke is the intended type here (unlike StoredShortcut above).
private typealias ShortcutStroke = MosaicSettings.ShortcutStroke
...
final class KeyboardShortcutContextTests: XCTestCase {
    ...
    func testNewBrowserWorkspaceSettingsPackageActionStaysAligned() {
        guard let settingsAction = ShortcutAction(
            ...
            XCTFail("Expected MosaicSettings.ShortcutAction for newBrowserWorkspace")
            return
        }
    ...
    // still match from those keys. See https://github.com/emergent-inc/mosaic/pull/5163.
    ...
    func testSettingsFileWhenClauseSupportsContextComparisons() throws {
        ...
        // A boolean key combined with an integer comparison, parsed from mosaic.json.
        let workspaceWhen = KeyboardShortcutSettings.effectiveWhenClause(for: .selectWorkspaceByNumber)
    ...
    private func makeTemporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-shortcut-context-\(UUID().uuidString)", isDirectory: true)
