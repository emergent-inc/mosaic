...
struct AgentExecutableResolverTests {
    ...
    func testResolvesExecutableInsideAnotherAppBundleResourceBin() throws {
        ...
        let mosaicResources = root
            .appendingPathComponent("mosaic.app/Contents/Resources", isDirectory: true)
        try FileManager.default.createDirectory(at: otherAppBin, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: mosaicResources, withIntermediateDirectories: true)
        ...
        let resolver = AgentExecutableResolver(
            ...
            bundleResourceURL: mosaicResources,
            includeStandardSearchDirectories: false
        )
    ...
    @Test
    func testSkipsOlderMosaicAppBundleResourceBin() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "AgentExecutableResolverTests-\(UUID().uuidString)", isDirectory: true)
        let oldMosaicBin = root
            .appendingPathComponent("mosaic DEV old.app/Contents/Resources/bin", isDirectory: true)
        let userBin = root.appendingPathComponent("user-bin", isDirectory: true)
        let mosaicResources = root
            .appendingPathComponent("mosaic.app/Contents/Resources", isDirectory: true)
        try FileManager.default.createDirectory(at: oldMosaicBin, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: userBin, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: mosaicResources, withIntermediateDirectories: true)
        let bundledExecutable = oldMosaicBin.appendingPathComponent("codex")
        ...
        let resolver = AgentExecutableResolver(
            environment: ["PATH": "\(oldMosaicBin.path):\(userBin.path)", "HOME": root.path],
            bundleResourceURL: mosaicResources,
            includeStandardSearchDirectories: false
        )
    ...
    func testIgnoresOtherAppBundleResourceBinEntriesFromPath() throws {
        ...
        let otherAppResourceBin =
            root
            .appendingPathComponent("Older mosaic.app", isDirectory: true)
            .appendingPathComponent("Contents", isDirectory: true)
            .appendingPathComponent("Resources", isDirectory: true)
            .appendingPathComponent("bin", isDirectory: true)
    ...
    @Test
    func testSkipsMosaicClaudeCommandShim() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "AgentExecutableResolverTests-\(UUID().uuidString)", isDirectory: true)
        ...
        let resolver = AgentExecutableResolver(
            environment: [
                ...
                "HOME": root.path,
                "MOSAIC_CLAUDE_WRAPPER_SHIM": shimClaude.path,
                "MOSAIC_CLAUDE_WRAPPER_SHIM_ROOT": shimBin.path,
            ],
            bundleResourceURL: root.appendingPathComponent("Resources", isDirectory: true)
        )
    ...
    func testLaunchEnvironmentKeepsPWDInSyncWithWorkingDirectory() {
        ...
        let environment = plan.environment(
            overridingWorkingDirectory: "/tmp/mosaic-agent-session/../mosaic-agent-session")

        expectEqual(environment["PATH"], "/bin")
        expectEqual(environment["PWD"], "/tmp/mosaic-agent-session")
    }
    ...
    func testOpenCodeLaunchEnvironmentPreservesExplicitLoopbackPassword() {
        let plan = AgentSessionLaunchPlan(
            ...
            arguments: AgentSessionProviderID.opencode.launchArguments,
            environment: [
                "OPENCODE_SERVER_USERNAME": "mosaic",
        ...
        expectEqual(environment["OPENCODE_SERVER_USERNAME"], "mosaic")
