...
final class TabManagerSessionSnapshotTests: XCTestCase {
    ...
    private func reserveRemoteRestoreSocket() -> String {
        ...
        let requestedPath = "/tmp/mosaic-restore-\(UUID().uuidString).sock"
    ...
    func testClosedItemHistoryPersistsRecordsWithoutSharedCapacityLimit() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-closed-history-\(UUID().uuidString)", isDirectory: true)
    ...
    func testClosedItemHistoryAsyncLoadMergesEarlyMutation() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-closed-history-merge-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testClosedItemHistoryFlushPendingSavesPersistsLatestRecords() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-closed-history-flush-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testClosedItemHistoryAsyncLoadReplaysQueuedPanelWorkspaceRemap() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-closed-history-remap-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testSessionSnapshotIncludesRemoteWorkspacesForRestore() throws {
        ...
        XCTAssertEqual(remoteSnapshot.remote?.destination, "mosaic-macmini")
    }
    ...
    func testSessionSnapshotSkipsTemporaryDiffViewerBrowserPanels() throws {
        ...
        let url = try XCTUnwrap(URL(string: "\(MosaicDiffViewerURLSchemeHandler.scheme)://token/index.html"))
        _ = try XCTUnwrap(
    ...
    func testSessionSnapshotRestoresSSHWorkspaceDescriptor() throws {
        ...
        let originalAgentSocketPath = "/tmp/mosaic-original-restore-agent.sock"
        let restoredAgentSocketPath = "/tmp/mosaic-current-restore-agent-\(UUID().uuidString).sock"
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/mosaic-restore-test.sock",
        ...
        let snapshotURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-ssh-session-restore-\(UUID().uuidString).json")
        ...
        let store = SessionSnapshotRepository<AppSessionSnapshot>(
            ...
            bundleIdentifier: "com.mosaicterm.tests"
        )
    ...
    func testSessionSnapshotRestoreOmitsSSHAgentEnvironmentWhenSocketUnavailable() throws {
        ...
        let originalAgentSocketPath = "/tmp/mosaic-original-missing-agent.sock"
        let previousAgentSocketPath = getenv("SSH_AUTH_SOCK").map { String(cString: $0) }
        defer {
            if let previousAgentSocketPath {
                setenv("SSH_AUTH_SOCK", previousAgentSocketPath, 1)
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/mosaic-missing-agent-test.sock",
            terminalStartupCommand: "ssh dev@example.com",
            agentSocketPath: originalAgentSocketPath
        )
        remoteWorkspace.configureRemoteConnection(configuration, autoConnect: false)
    ...
    func testSessionSnapshotRestoresPersistentSSHPTYSessionAfterRelaunch() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/mosaic-persist-test.sock",
        ...
        let snapshotURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-ssh-pty-session-restore-\(UUID().uuidString).json")
        defer { try? FileManager.default.removeItem(at: snapshotURL) }
        let snapshot = AppSessionSnapshot(
            version: SessionSnapshotSchema.currentVersion,
            createdAt: Date().timeIntervalSince1970,
        ...
        XCTAssertTrue(expectedScrollback.contains("mosaic perf synthetic scrollback"), expectedScrollback)
        ...
        XCTAssertTrue(
            restoredWorkspace.remoteConfiguration?.sshOptions.contains("ControlPath=/tmp/mosaic-ssh-\(getuid())-64003-%C") == true
        )
        ...
        XCTAssertNil(restoredWorkspace.terminalPanel(for: restoredPanelId)?.requestedWorkingDirectory)
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("export MOSAIC_SOCKET_PATH=127.0.0.1:64003"),
            restoredDefaultRemoteCommand
        )
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("export PATH=\"$HOME/.mosaic/bin:$PATH\""),
            restoredDefaultRemoteCommand
        )
        XCTAssertTrue(restoredDefaultRemoteCommand.contains("MOSAIC_SHELL_INTEGRATION_DIR"), restoredDefaultRemoteCommand)
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("mosaic_workspace_id='__MOSAIC_WORKSPACE_ID__'"),
            restoredDefaultRemoteCommand
        )
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("'__MOSAIC_''WORKSPACE_ID__'"),
            restoredDefaultRemoteCommand
        )
        XCTAssertFalse(
            restoredDefaultRemoteCommand.contains("[ -n '__MOSAIC_WORKSPACE_ID__' ]"),
            restoredDefaultRemoteCommand
        )
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("mosaic_surface_id='__MOSAIC_SURFACE_ID__'"),
            restoredDefaultRemoteCommand
        )
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("'__MOSAIC_''SURFACE_ID__'"),
            restoredDefaultRemoteCommand
        )
        XCTAssertFalse(
            restoredDefaultRemoteCommand.contains("[ -n '__MOSAIC_SURFACE_ID__' ]"),
            restoredDefaultRemoteCommand
        )
        let substitutedRestoredDefaultRemoteCommand = restoredDefaultRemoteCommand
            .replacingOccurrences(of: "__MOSAIC_WORKSPACE_ID__", with: restoredWorkspace.id.uuidString)
            .replacingOccurrences(of: "__MOSAIC_SURFACE_ID__", with: restoredPanelId.uuidString)
        XCTAssertTrue(
            substitutedRestoredDefaultRemoteCommand.contains(
                "mosaic_workspace_id='\(restoredWorkspace.id.uuidString)'"
            ),
            substitutedRestoredDefaultRemoteCommand
        )
        XCTAssertTrue(
            substitutedRestoredDefaultRemoteCommand.contains("mosaic_surface_id='\(restoredPanelId.uuidString)'"),
            substitutedRestoredDefaultRemoteCommand
        )
        XCTAssertFalse(
            substitutedRestoredDefaultRemoteCommand.contains("MOSAIC_WORKSPACE_ID=__MOSAIC_WORKSPACE_ID__"),
            substitutedRestoredDefaultRemoteCommand
        )
        XCTAssertFalse(
            substitutedRestoredDefaultRemoteCommand.contains("MOSAIC_SURFACE_ID=__MOSAIC_SURFACE_ID__"),
            substitutedRestoredDefaultRemoteCommand
        )
        ...
        XCTAssertTrue(restoredInitialCommand.contains("MOSAIC_SURFACE_ID"), restoredInitialCommand)
    ...
    func testSessionSnapshotRestoresSplitPersistentSSHPTYWithoutDefaultAttachScaffold() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "c", count: 64),
            localSocketPath: "/tmp/mosaic-persist-split.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: persistentDaemonSlot
        )
    ...
    func testPersistentSSHPTYRestoreRewritesStaleRemoteRelayContextIDs() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/mosaic-relay-alias.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: persistentDaemonSlot
        )
        ...
        let refreshedRelayConfiguration = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "ControlMaster=auto",
                "ControlPersist=600",
                "ControlPath=/tmp/mosaic-ssh-\(getuid())-64006-%C",
                "StrictHostKeyChecking=accept-new",
            ],
            localProxyPort: nil,
            relayPort: 64006,
            ...
            relayToken: String(repeating: "c", count: 64),
            localSocketPath: "/tmp/mosaic-relay-alias-refreshed.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
    ...
    func testPersistentSSHPTYRestoreRewritesMovedSourceWorkspaceContextID() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "d", count: 64),
            localSocketPath: "/tmp/mosaic-relay-moved-alias.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: persistentDaemonSlot
        )
    ...
    func testPersistentSSHPTYReattachRewritesStaleRemoteRelayContextIDs() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "b", count: 64),
            localSocketPath: "/tmp/mosaic-relay-reattach-alias.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: persistentDaemonSlot
        )
    ...
    func testPersistentSSHPTYRestoreFallsBackToSnapshotPanelDefaultSessionIDWhenActiveMarkerExists() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "f", count: 64),
            localSocketPath: "/tmp/mosaic-legacy-persist.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: persistentDaemonSlot
        )
    ...
    func testPersistentSSHPTYRestoreDoesNotReattachEndedSnapshotPanel() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/mosaic-ended-persist.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
    ...
    func testPersistentSSHPTYRestorePreservesLocalTerminalWorkingDirectory() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/mosaic-local-terminal.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
        ...
        remoteWorkspace.configureRemoteConnection(configuration, autoConnect: false)
        let paneId = try XCTUnwrap(remoteWorkspace.bonsplitController.allPaneIds.first)
        let localDirectory = "/tmp/mosaic-local-terminal"
    ...
    func testSessionSnapshotFallsBackWhenPersistentSSHPTYRestoreHasNoSocketPath() throws {
        ...
        remoteWorkspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: String(repeating: "f", count: 64),
                localSocketPath: "/tmp/mosaic-no-socket.sock",
                terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
                preserveAfterTerminalExit: true,
    ...
    func testSessionSnapshotFallsBackFromSkipBootstrapPersistentSSHPTYWithoutDaemonBridge() throws {
        ...
        let snapshotURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-ssh-pty-durable-restore-\(UUID().uuidString).json")
        defer { try? FileManager.default.removeItem(at: snapshotURL) }
        let snapshot = AppSessionSnapshot(
            version: SessionSnapshotSchema.currentVersion,
            createdAt: Date().timeIntervalSince1970,
    ...
    func testSessionRemoteWorkspaceSnapshotRequiresPersistentDaemonSlotForPTYRestore() throws {
        ...
        let configuration = try XCTUnwrap(snapshot.workspaceConfiguration(localSocketPath: "/tmp/mosaic-restore.sock"))
    ...
    func testSessionRemoteWorkspaceSnapshotStripsTransientControlOptionsWhenPreservedRestoreFallsBack() throws {
        let snapshot = SessionRemoteWorkspaceSnapshot(
            ...
            sshOptions: [
                ...
                "ControlMaster=auto",
                "ControlPersist=600",
                "ControlPath=/tmp/mosaic-ssh-501-64003-%C",
            ],
            preserveAfterTerminalExit: true,
            skipDaemonBootstrap: nil,
            relayPort: 64003,
    ...
    /// Regression for https://github.com/emergent-inc/mosaic/issues/5931 — a restored
    ...
    func testRestoredTerminalPaneHeaderTitleSyncsToBonsplitTab() throws {
        ...
        let restoredTitle = "~/projects/mosaic"
