...
/// workspace (the `MOSAIC_WORKSPACE_ID` the app injects into every terminal surface)
...
/// `--workspace "${MOSAIC_WORKSPACE_ID:-}"` expands to an empty value whenever a
...
struct CLICallerWorkspaceDefaultTests {
    ...
    /// dangerous case where `--workspace "${MOSAIC_WORKSPACE_ID:-}"` expands to an empty
    ...
    /// visibly retarget there. Pass `callerWorkspaceId: nil` to omit `MOSAIC_WORKSPACE_ID`.
    ...
    private func cliEnvironment(socketPath: String, callerWorkspaceId: String?) -> [String: String] {
        ...
        environment.removeValue(forKey: "MOSAIC_WINDOW_ID")
        if let callerWorkspaceId {
            environment["MOSAIC_WORKSPACE_ID"] = callerWorkspaceId
        } else {
