...
struct TitlebarInteractiveControlTests {
    ...
    @Test func nativeTitlebarGapUsesExplicitWindowDragPath() {
        ...
        defer { window.orderOut(nil) }
        window.identifier = NSUserInterfaceItemIdentifier("mosaic.main.titlebar-gap-test")
    ...
    @Test func attachedRightLogoAccessoryUsesExplicitWindowDragPath() {
        ...
        defer { window.orderOut(nil) }
        window.identifier = NSUserInterfaceItemIdentifier("mosaic.main.titlebar-control-test")
        window.titleVisibility = .hidden
        window.titlebarAppearsTransparent = true
        window.contentView = NSView(frame: window.contentRect(forFrameRect: window.frame))
        configureMosaicMainWindowDragBehavior(window)
