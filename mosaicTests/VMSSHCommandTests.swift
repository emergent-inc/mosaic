...
extension CLINotifyProcessIntegrationRegressionTests {
    func testVMSSHOpensManagedWorkspaceThroughSharedSSHPath() throws {
        ...
        XCTAssertEqual(result.stdout, "OK workspace=\(workspaceRef) target=mosaic@gateway.freestyle.sh state=connecting\n")
        ...
        XCTAssertEqual(configureParams["destination"] as? String, "mosaic@gateway.freestyle.sh")
    ...
    func testSSHCommandGlobalWindowOverridesCallerEnvironment() throws {
        ...
        environment["MOSAIC_SURFACE_ID"] = callerSurfaceID
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        environment["MOSAIC_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    func testVMSSHInfoRemainsPrintOnly() throws {
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertTrue(result.stdout.contains("ssh mosaic@gateway.freestyle.sh -p 2222"), result.stdout)
