...
private extension GhosttyNSView {
    @objc func mosaicUnitTest_interpretKeyEvents(_ eventArray: [NSEvent]) {
        if let hook = cjkIMEInterpretKeyEventsHook, hook(self, eventArray) {
            return
        }
        mosaicUnitTest_interpretKeyEvents(eventArray)
    }

    @objc func mosaicUnitTest_paste(_ sender: Any?) {
        ghosttyPasteActionHook?(self, sender)
        mosaicUnitTest_paste(sender)
    }

    @objc func mosaicUnitTest_pasteAsPlainText(_ sender: Any?) {
        ghosttyPasteAsPlainTextActionHook?(self, sender)
        mosaicUnitTest_pasteAsPlainText(sender)
    }
}
...
func installCJKIMEInterpretKeyEventsSwizzle() {
    ...
    let swizzledSelector = #selector(GhosttyNSView.mosaicUnitTest_interpretKeyEvents(_:))
...
private func installGhosttyPasteActionSwizzle() {
    ...
    let swizzledSelector = #selector(GhosttyNSView.mosaicUnitTest_paste(_:))
    ...
    let plainTextSwizzledSelector = #selector(GhosttyNSView.mosaicUnitTest_pasteAsPlainText(_:))
...
// MARK: - Space release regression (Codex hold-to-talk in mosaic)
...
final class GhosttyKeyEquivalentRegressionTests: XCTestCase {
    ...
    private func mosaicZshTerminalKeyboardResetSequence() throws -> Data {
        let repoRoot = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let integrationPath = repoRoot
            .appendingPathComponent("Resources/shell-integration/mosaic-zsh-integration.zsh")
            .path
        ...
        process.arguments = [
            ...
            if (( $+functions[_mosaic_reset_terminal_keyboard_protocols] )); then
              _mosaic_reset_terminal_keyboard_protocols
            fi
            """
        ]
        process.environment = [
            "MOSAIC_TEST_FORCE_KEYBOARD_RESET": "1"
        ]
        let output = Pipe()
    ...
    func testStaleKittyKeyboardAfterClearHistoryDoesNotEncodePlainLetterAsCSIU() throws {
        let captureReadyMarker = "MOSAIC_KBD_READY_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        let captureMarker = "MOSAIC_KBD_HEX_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-kbd-capture-\(UUID().uuidString).py")
        let script = """
        import os
        import select
        import sys
        ...
        let keyboardResetData = try mosaicZshTerminalKeyboardResetSequence()
        XCTAssertEqual(
            keyboardResetData,
            Data("\u{1B}[>m\u{1B}[<8u\u{1B}[?9l\u{1B}[?1000l\u{1B}[?1002l\u{1B}[?1003l\u{1B}[?1005l\u{1B}[?1006l\u{1B}[?1004l\u{1B}[?2004l\u{1B}[?2026l".utf8),
            "mosaicZshTerminalKeyboardResetSequence must reset keyboard, mouse, and focus reporting state"
        )
