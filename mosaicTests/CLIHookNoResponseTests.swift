...
struct CLIHookNoResponseTests {
    ...
    @Test func nonActionableFeedHooksDoNotWaitForSocketResponseAcrossAgents() throws {
        let cases = [
            FeedHookCase(source: "codex", event: "PreToolUse", toolName: "apply_patch", pidKey: "MOSAIC_CODEX_PID"),
            FeedHookCase(source: "gemini", event: "PreToolUse", toolName: "read", pidKey: "MOSAIC_GEMINI_PID"),
            FeedHookCase(source: "kiro", event: "postToolUse", toolName: "fs_write", pidKey: "MOSAIC_KIRO_PID"),
            FeedHookCase(source: "hermes-agent", event: "pre_tool_call", toolName: "terminal", pidKey: "MOSAIC_HERMES_AGENT_PID"),
            FeedHookCase(source: "antigravity", event: "PostToolUse", toolName: "run_command", pidKey: "MOSAIC_ANTIGRAVITY_PID"),
        ]
        ...
        for testCase in cases {
            ...
            let root = FileManager.default.temporaryDirectory
                .appendingPathComponent("mosaic-feed-no-reply-\(testCase.source)-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func genericLifecycleFeedTelemetryDoesNotWaitForSocketResponse() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-kiro-lifecycle-no-response-\(UUID().uuidString)", isDirectory: true)
        ...
        let result = Self.runProcess(
            ...
            environment: [
                ...
                "MOSAIC_AGENT_LAUNCH_KIND": "kiro",
                "MOSAIC_AGENT_LAUNCH_EXECUTABLE": "/Users/example/.cargo/bin/kiro-cli",
                "MOSAIC_AGENT_LAUNCH_ARGV_B64": Self.base64NULSeparated([
                ...
                "MOSAIC_AGENT_LAUNCH_CWD": root.path,
                "MOSAIC_CLI_SENTRY_DISABLED": "1",
                "MOSAIC_SOCKET_PASSWORD": "test-password",
            ],
    ...
    @Test func nonActionableFeedHookDoesNotBlockWhenAcceptedSocketStopsReading() throws {
        ...
        let listenerFD = try Self.bindUnixSocket(at: socketPath, backlog: 1)
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-feed-no-read-\(UUID().uuidString)", isDirectory: true)
    ...
    private static func bundledCLIPath() throws -> String {
        ...
        while let item = enumerator?.nextObject() as? URL {
            guard item.lastPathComponent == "mosaic",
                  item.path.contains(".app/Contents/Resources/bin/mosaic") else {
                continue
            }
            return item.path
        }
        ...
        throw NSError(domain: "mosaic.tests", code: 1, userInfo: [
            NSLocalizedDescriptionKey: "Bundled mosaic CLI not found in \(appBundleURL.path)",
        ])
    }
    ...
    private static func runProcess(
        ...
        if let standardInput {
            let url = FileManager.default.temporaryDirectory
                .appendingPathComponent("mosaic-test-stdin-\(UUID().uuidString).json")
            do {
    ...
    private static func posixError(_ message: String) -> NSError {
        NSError(domain: "mosaic.tests", code: Int(errno), userInfo: [
