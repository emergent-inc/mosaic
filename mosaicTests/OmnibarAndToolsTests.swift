...
final class FinderServicePathResolverTests: XCTestCase {
    private func withTemporaryDirectory<T>(_ body: (URL) throws -> T) throws -> T {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "mosaic-finder-service-tests-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testOrderedUniqueDirectoriesUsesParentForFilesAndDedupes() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/mosaic-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/mosaic-services/project/README.md", isDirectory: false),
            URL(fileURLWithPath: "/tmp/mosaic-services/../mosaic-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/mosaic-services/other", isDirectory: true),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/tmp/mosaic-services/project",
                "/tmp/mosaic-services/other",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesPreservesFirstSeenOrder() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/mosaic-services/b", isDirectory: true),
            URL(fileURLWithPath: "/tmp/mosaic-services/a/file.txt", isDirectory: false),
            URL(fileURLWithPath: "/tmp/mosaic-services/a", isDirectory: true),
            URL(fileURLWithPath: "/tmp/mosaic-services/b/file.txt", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/tmp/mosaic-services/b",
                "/tmp/mosaic-services/a",
            ]
        )
    }

    func testOrderedUniqueDirectoriesSkipsBundleAndEmbeddedPathsWhenExcludingBundleRoot() {
        let bundleURL = URL(fileURLWithPath: "/Applications/Tools/../mosaic.app", isDirectory: true)
        let input: [URL] = [
            bundleURL,
            URL(fileURLWithPath: "/Applications/mosaic.app/Contents/MacOS/mosaic", isDirectory: false),
            URL(fileURLWithPath: "/Applications/mosaic.app/Contents/Resources/bin/mosaic", isDirectory: false),
            URL(fileURLWithPath: "/Users/tester/Projects/mosaic", isDirectory: true),
            URL(fileURLWithPath: "/Users/tester/Projects/mosaic/README.md", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/Users/tester/Projects/mosaic",
            ]
        )
    }

    func testOrderedUniqueDirectoriesExclusionDoesNotFilterSiblingPaths() {
        let bundleURL = URL(fileURLWithPath: "/Applications/mosaic.app", isDirectory: true)
        let input: [URL] = [
            URL(fileURLWithPath: "/Applications/mosaic.app backup/project", isDirectory: true),
            URL(fileURLWithPath: "/Applications/mosaic.app.beta/project/file.txt", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/Applications/mosaic.app backup/project",
                "/Applications/mosaic.app.beta/project",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesResolvesSymlinksOnlyForExcludedRootComparison() throws {
        try withTemporaryDirectory { root in
            ...
            let actualBundle = applicationsDirectory.appendingPathComponent("mosaic.app", isDirectory: true)
            let actualBinary = actualBundle.appendingPathComponent("Contents/MacOS/mosaic", isDirectory: false)
            ...
            let directories = FinderServicePathResolver.orderedUniqueDirectories(
                from: [
                    aliasApplications.appendingPathComponent("mosaic.app", isDirectory: true),
                    aliasApplications.appendingPathComponent("mosaic.app/Contents/MacOS/mosaic", isDirectory: false),
                    aliasWorkspace,
                ],
...
final class VSCodeServeWebURLBuilderTests: XCTestCase {
    ...
    func testOpenFolderURLAppendsFolderQueryWhilePreservingToken() {
        ...
        let url = VSCodeServeWebURLBuilder.openFolderURL(
            ...
            directoryPath: "/Users/tester/Projects/mosaic"
        )

        let components = URLComponents(url: url!, resolvingAgainstBaseURL: false)
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "tkn" })?.value, "test-token")
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "folder" })?.value, "/Users/tester/Projects/mosaic")
    }
...
final class BrowserOmnibarFieldEditorResolutionTests: XCTestCase {
    func testPanelIdResolutionUsesLiveOmnibarFieldWhenFieldEditorResponderChainIsStale() {
        ...
        let staleWebView = MosaicWebView(frame: NSRect(x: 0, y: 0, width: 420, height: 80), configuration: WKWebViewConfiguration())
