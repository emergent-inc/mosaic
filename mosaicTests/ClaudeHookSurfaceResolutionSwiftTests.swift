...
struct ClaudeHookSurfaceResolutionSwiftTests {
    @Test func claudeSessionStartOverridesLeakedEnvSurfaceWithTTYBinding() throws {
        ...
        let environment = [
            ...
            "MOSAIC_SURFACE_ID": leakedSurfaceId,
            "MOSAIC_CLI_TTY_NAME": ttyName,
            "MOSAIC_CLAUDE_PID": "42424",
            ...
            "MOSAIC_AGENT_LAUNCH_ARGV_B64": base64NULSeparated(["/usr/local/bin/claude"]),
        ]
        ...
        #expect(
            ...
            "Claude must persist the agent TTY surface, not the leaked ambient MOSAIC_SURFACE_ID; params=\(request)"
        )
        #expect(
    ...
    @Test func claudeSessionStartOverridesLeakedEnvWorkspaceAndSurfaceWithTTYBinding() throws {
        ...
        let environment = [
            ...
            "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
            "MOSAIC_SOCKET_PATH": context.socketPath,
            "MOSAIC_WORKSPACE_ID": leakedWorkspaceId,
            "MOSAIC_SURFACE_ID": leakedSurfaceId,
            "MOSAIC_CLI_TTY_NAME": ttyName,
            "MOSAIC_CLAUDE_HOOK_STATE_PATH": context.root.appendingPathComponent("claude-hook-sessions.json").path,
            "MOSAIC_CLI_SENTRY_DISABLED": "1",
        ...
        #expect(
            ...
            "Claude must persist the agent TTY workspace, not the leaked ambient MOSAIC_WORKSPACE_ID; params=\(request)"
        )
        #expect(
            request["surface_id"] as? String == ttySurfaceId,
            "Claude must persist the agent TTY surface, not the leaked ambient MOSAIC_SURFACE_ID; params=\(request)"
    ...
    @Test func claudeSessionStartOverridesLeakedEnvWithClaudePIDBindingWhenTTYMissing() throws {
        ...
        let environment = [
            ...
            "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
            "MOSAIC_SOCKET_PATH": context.socketPath,
            "MOSAIC_SOCKET_PASSWORD": socketPassword,
            "MOSAIC_WORKSPACE_ID": leakedWorkspaceId,
            "MOSAIC_SURFACE_ID": leakedSurfaceId,
            "MOSAIC_CLAUDE_PID": "\(claudePID)",
            "MOSAIC_CLAUDE_HOOK_STATE_PATH": context.root.appendingPathComponent("claude-hook-sessions.json").path,
            "MOSAIC_CLI_SENTRY_DISABLED": "1",
            "MOSAIC_CLAUDE_HOOK_SENTRY_DISABLED": "1",
            "MOSAIC_AGENT_LAUNCH_KIND": "claude",
        ...
        #expect(
            ...
            "Claude PID binding must beat leaked ambient MOSAIC_WORKSPACE_ID when no TTY marker exists; params=\(request)"
        )
        #expect(
            request["surface_id"] as? String == pidSurfaceId,
            "Claude PID binding must beat leaked ambient MOSAIC_SURFACE_ID when no TTY marker exists; params=\(request)"
        )
        #expect(
    ...
    @Test func claudeSessionStartIgnoresStaleTTYWorkspaceWhenBoundSurfaceIsGone() throws {
        ...
        #expect(
            ...
            "Stale TTY workspace must not beat valid ambient MOSAIC_WORKSPACE_ID; params=\(request)"
        )
        #expect(
    ...
    func claudeHookEnvironment(context: ClaudeHookContext, surfaceId: String, ttyName: String, storeURL: URL) -> [String: String] {
        [
            ...
            "MOSAIC_CLI_TTY_NAME": ttyName,
            "MOSAIC_CLAUDE_HOOK_STATE_PATH": storeURL.path,
            "MOSAIC_CLI_SENTRY_DISABLED": "1",
            "MOSAIC_CLAUDE_HOOK_SENTRY_DISABLED": "1",
        ]
    }
    ...
    private func assertPromptSubmitRoutes(
        ...
        #expect(request["surface_id"] as? String != ambientSurfaceId, "Raw ambient MOSAIC_SURFACE_ID must not win")
        #expect(
            context.state.snapshot().contains {
                $0.hasPrefix("set_status claude_code Running ")
