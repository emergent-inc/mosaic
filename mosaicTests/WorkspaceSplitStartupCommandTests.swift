...
private func waitForWorkspaceSplitView(
    ...
    XCTFail(
        "Timed out waiting for rendered mosaic.json split ratio \(expectedDividerPosition); last ratio: \(lastRatioDescription)",
        file: file,
        line: line
    )
...
final class WorkspaceSplitStartupCommandTests: XCTestCase {
    func testCustomLayoutSplitRatioSurvivesInitialBonsplitViewLayout() throws {
        ...
        let layout = MosaicLayoutNode.split(MosaicSplitDefinition(
            ...
            children: [
                ...
                .pane(MosaicPaneDefinition(surfaces: [
                    MosaicSurfaceDefinition(type: .terminal, name: "Right")
                ]))
            ]
        ))
        ...
        XCTAssertEqual(
            ...
            accuracy: 0.000_1,
            "mosaic.json split ratio should be applied to the Bonsplit model before rendering"
        )
        ...
        XCTAssertEqual(
            ...
            expectedDividerPosition,
            accuracy: 0.000_1,
            "Bonsplit initial view layout should not rewrite the mosaic.json split ratio back to 0.5"
        )
    }
    ...
    func testTabManagerSplitCarriesRequestedWorkingDirectoryAndStartupCommand() {
        ...
        let requestedDirectory = "/tmp/mosaic-split-startup-\(UUID().uuidString)"
        let startupCommand = "/tmp/mosaic-tmux-command-\(UUID().uuidString).sh"
    ...
    func testNewTerminalSurfaceCarriesRequestedWorkingDirectoryAndStartupCommand() {
        ...
        let requestedDirectory = "/tmp/mosaic-surface-startup-\(UUID().uuidString)"
        let startupCommand = "/tmp/mosaic-surface-command-\(UUID().uuidString).sh"
        let tmuxStartCommand = "node /opt/oh-my-codex/dist/omx.js hud --watch"
    ...
    func testRespawnTerminalSurfacePreservesPaneTabAndSurfaceIdentity() throws {
        ...
        let requestedDirectory = "/tmp/mosaic-respawn-\(UUID().uuidString)"
        let startupEnvironment = [
            "MOSAIC_OMO_SUBAGENT": "1",
        ...
        XCTAssertTrue(
            ...
            "Respawn should replace the registered terminal surface for the existing mosaic surface id"
        )
    }
    ...
    func testSessionRestoreRelaunchesOMXHudTmuxStartCommand() throws {
        let workspace = Workspace()
        let sourcePanelId = try XCTUnwrap(workspace.focusedPanelId)
        let requestedDirectory = "/tmp/mosaic-hud-restore-\(UUID().uuidString)"
        let originalStartupScript = "/tmp/mosaic-tmux-command-\(UUID().uuidString).sh"
        ...
        XCTAssertTrue(restoredStartupScript.contains("mosaic-session-terminal-command"))
    ...
    func testSessionSnapshotDoesNotPersistGenericTmuxStartCommand() throws {
        ...
        let panel = try XCTUnwrap(workspace.newTerminalSurface(
            inPane: paneId,
            focus: false,
            initialCommand: "/tmp/mosaic-command-\(UUID().uuidString).sh",
