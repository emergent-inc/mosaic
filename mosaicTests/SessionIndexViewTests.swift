...
final class SessionIndexViewTests: XCTestCase {
    ...
    func testClaudeResumeCommandPinsSnapshotConfigDirectory() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-session-index-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertEqual(
            entry.resumeCommand,
            posixShWrappedForTest("env CLAUDE_CONFIG_DIR='\(configDir.path)' MOSAIC_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1 MOSAIC_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=CLAUDE_CONFIG_DIR \"$([ -x \"${MOSAIC_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$MOSAIC_CLAUDE_WRAPPER_SHIM\" || printf claude)\" --resume claude-session-123")
        )
    }
    ...
    // Regression for https://github.com/emergent-inc/mosaic/issues/5262.
    // mosaic captures Codex's *internal* sandbox-policy `type`, which is a superset of
    ...
    func testCodexResumeCommandReproducesBypassFlagForDisabledSandbox() {
        ...
        // The codex resume now routes the codex executable through the mosaic codex
    ...
    func testCodexSQLSearchMatchesRolloutTranscriptContent() async throws {
        ...
        {"timestamp":"2026-05-01T09:01:00.000Z","type":"response_item","payload":{"type":"function_call","name":"exec_command","arguments":"{\\"cmd\\":\\"tail -n 80 /tmp/mosaic-debug-task-activation-performance-harness.log\\"}","call_id":"call_1"}}
        """
        ...
        let outcome = await SessionIndexStore.loadCodexEntriesForTesting(
            ...
            needle: "/tmp/mosaic-debug-task-activation-performance-harness.log",
            offset: 0,
            limit: 10,
    ...
    // Regression for https://github.com/emergent-inc/mosaic/issues/6302.
    ...
    /// (fish/csh/tcsh). https://github.com/emergent-inc/mosaic/issues/5639
