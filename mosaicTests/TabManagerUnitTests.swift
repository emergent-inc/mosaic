    ...
    func testPaneCloseOnLastRemotePanelKeepsWorkspaceDisconnected() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: nil,
                localSocketPath: "/tmp/mosaic-debug-test.sock",
                terminalStartupCommand: "mosaic remote websocket"
            ),
            autoConnect: false
        )
    ...
    func testChildExitAfterPersistentAttachEndKeepsExitedSurfaceVisible() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayID: String(repeating: "a", count: 16),
                relayToken: String(repeating: "b", count: 64),
                localSocketPath: "/tmp/mosaic-debug-attach-end-test.sock",
                terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
                preserveAfterTerminalExit: true,
    ...
    func testChildExitOnSplitPersistentRemotePanelKeepsExitedSurfaceVisibleAndClearsOnlyThatPTYState() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayID: String(repeating: "a", count: 16),
                relayToken: String(repeating: "b", count: 64),
                localSocketPath: "/tmp/mosaic-debug-split-test.sock",
                terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
                preserveAfterTerminalExit: true,
...
final class TabManagerPullRequestProbeTests: XCTestCase {
    ...
    // Pure pull-request selection/policy tests moved to the MosaicGit package
    // (MosaicGitTests.PullRequestProbeServiceTests) with the extraction.
    ...
    func testTrackedWorkspaceGitMetadataPollCandidatesIncludeMainAndMasterPanels() throws {
        ...
        let staleURL = try XCTUnwrap(URL(string: "https://github.com/emergent-inc/mosaic/pull/371"))
    ...
    func testSameDirectoryInitialGitMetadataProbesShareOneSnapshotRead() async throws {
        ...
        let directoryURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "mosaic-git-coalesced-probes-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testTrackedWorkspaceGitMetadataPollCandidatesExcludeDirectoriesWithoutResolvedGitMetadata() throws {
        ...
        let directoryURL = fileManager.temporaryDirectory.appendingPathComponent(
            "mosaic-git-nonrepo-candidate-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testInheritedBackgroundWorkspaceFetchesGitBranchWithoutSelection() throws {
        ...
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent(
            "mosaic-git-inherited-background-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testPeriodicWorkspaceGitMetadataRefreshUpdatesMainWorkspaceAfterCheckoutToFeatureBranch() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent("mosaic-git-main-refresh-\(UUID().uuidString)")
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: repoURL) }
    ...
    func testPeriodicWorkspaceGitMetadataRefreshRestoresClearedBranchForStaleTerminal() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent(
            "mosaic-git-stale-branch-refresh-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: repoURL) }
    ...
    // MosaicProcessTests.resolvesCommandViaFallbackDirectoryOutsidePath when the
    // command runner was extracted into the MosaicProcess package.

    func testPeriodicWorkspaceGitMetadataRefreshClearsStalePullRequestAfterBranchReset() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent("mosaic-git-refresh-\(UUID().uuidString)")
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: repoURL) }
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 1052,
            label: "PR",
            url: try XCTUnwrap(URL(string: "https://github.com/emergent-inc/mosaic/pull/1052")),
            status: .open,
...
final class TabManagerCloseCurrentPanelTests: XCTestCase {
    ...
    func testCloseCurrentPanelHonorsWarnBeforeClosingTabDisabledFromMosaicJSON() throws {
    ...
    func testCloseCurrentPanelHonorsWarnBeforeClosingTabEnabledFromMosaicJSON() throws {
        try assertCloseCurrentPanelConfirmation(
    ...
    func testTabCloseButtonWarningHonorsMosaicJSON() throws {
    ...
    func testHideTabCloseButtonHonorsMosaicJSON() throws {
...
final class TabManagerSurfaceCreationTests: XCTestCase {
    ...
    func testNewBrowserSurfaceCanSelectBackgroundPaneWithoutTakingFocus() {
        ...
        guard let workspace = manager.selectedWorkspace,
              ...
              let url = URL(string: "file:///tmp/mosaic-diff.html") else {
            XCTFail("Expected split setup to succeed")
            return
        }
    ...
    func testDuplicateBrowserPreservesDiffViewerChromeAndProxyBypass() throws {
        ...
        let url = try XCTUnwrap(URL(string: "http://127.0.0.1:49152/token/diff.html#mosaic-diff-viewer"))
...
final class TabManagerEqualizeSplitsTests: XCTestCase {
    func testEqualizeSplitsKeepsMultiTabPaneAndBrowserAtHalfWidth() {
        ...
        workspace.applyCustomLayout(
            .split(MosaicSplitDefinition(
                direction: .horizontal,
                split: 0.2,
                children: [
                    .pane(MosaicPaneDefinition(surfaces: [
                        MosaicSurfaceDefinition(type: .terminal, name: "Terminal A"),
                        MosaicSurfaceDefinition(type: .terminal, name: "Terminal B")
                    ])),
                    .pane(MosaicPaneDefinition(surfaces: [
                        MosaicSurfaceDefinition(type: .browser, name: "Browser", url: "https://example.com")
                    ]))
                ]
            )),
    ...
    func testEqualizeSplitsBalancesThreeSameAxisSiblingPanesIntoThirds() {
        ...
        workspace.applyCustomLayout(
            .split(MosaicSplitDefinition(
                ...
                children: [
                    .pane(MosaicPaneDefinition(surfaces: [
                        MosaicSurfaceDefinition(type: .terminal, name: "Left")
                    ])),
                    .split(MosaicSplitDefinition(
                        direction: .horizontal,
                        split: 0.8,
                        children: [
                            .pane(MosaicPaneDefinition(surfaces: [
                                MosaicSurfaceDefinition(type: .terminal, name: "Middle")
                            ])),
                            .pane(MosaicPaneDefinition(surfaces: [
                                MosaicSurfaceDefinition(type: .browser, name: "Right", url: "https://example.com")
                            ]))
                        ]
                    ))
                ]
            ...
            .split(MosaicSplitDefinition(
                ...
                children: [
                    .split(MosaicSplitDefinition(
                        ...
                        children: [
                            .pane(MosaicPaneDefinition(surfaces: [
                                MosaicSurfaceDefinition(type: .terminal, name: "Top Terminal")
                            ])),
                            .pane(MosaicPaneDefinition(surfaces: [
                                MosaicSurfaceDefinition(type: .terminal, name: "Bottom Terminal")
                            ]))
                        ]
                    )),
                    .pane(MosaicPaneDefinition(surfaces: [
                    ...
                    .split(MosaicSplitDefinition(
                        ...
                        children: [
                            .split(MosaicSplitDefinition(
                                ...
                                children: [
                                    .pane(MosaicPaneDefinition(surfaces: [
                                        MosaicSurfaceDefinition(type: .terminal, name: "Top Left")
                                    ])),
                                    .pane(MosaicPaneDefinition(surfaces: [
                                        MosaicSurfaceDefinition(type: .terminal, name: "Top Right")
                                    ]))
                                ]
                            )),
                            .pane(MosaicPaneDefinition(surfaces: [
                                MosaicSurfaceDefinition(type: .terminal, name: "Bottom")
                            ]))
                        ]
                    )),
                    .pane(MosaicPaneDefinition(surfaces: [
...
final class TabManagerReopenClosedBrowserFocusTests: XCTestCase {
    ...
    func testTemporaryDiffViewerTabCloseDoesNotStageRestoreSnapshot() throws {
        let workspace = Workspace()
        let diffViewerURL = try XCTUnwrap(URL(string: "http://127.0.0.1:49152/token/diff.html#mosaic-diff-viewer"))
        guard let paneId = workspace.bonsplitController.focusedPaneId,
