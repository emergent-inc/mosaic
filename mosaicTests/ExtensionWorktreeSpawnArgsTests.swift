...
/// Regression coverage for https://github.com/emergent-inc/mosaic/issues/5032.
///
...
struct ExtensionWorktreeSpawnArgsTests {
    private func makeResult(setupCommand: String) -> MosaicExtensionWorktreeCreationResult {
        MosaicExtensionWorktreeCreationResult(
            worktreePath: "/tmp/project/.mosaic/worktrees/mosaic-sidebar-123",
            workspaceTitle: "mosaic-sidebar-123",
    ...
    func setupCommandRunsAsShellInput() {
        ...
        // interactive shell, matching the `mosaic new-workspace --cwd` contract.
    ...
    func worktreePathIsWorkingDirectory() {
        ...
        #expect(args.workingDirectory == "/tmp/project/.mosaic/worktrees/mosaic-sidebar-123")
        #expect(args.inheritWorkingDirectory == false)
        #expect(args.title == "mosaic-sidebar-123")
    }
