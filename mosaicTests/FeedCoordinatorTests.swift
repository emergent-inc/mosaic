...
struct FeedCoordinatorTests {
    @Test func codexTeamsResolvesExplicitWorkingDirectoryFlags() {
        let base = "/tmp/mosaic-base"

        #expect(
            CodexTeamsApprovalBridge.resolvedWorkingDirectory(
                commandArgs: ["-C", "child", "prompt"],
                baseDirectory: base
            ) == "/tmp/mosaic-base/child"
        )
        #expect(
            CodexTeamsApprovalBridge.resolvedWorkingDirectory(
                commandArgs: ["--cwd=/tmp/mosaic-review", "--cd", "/tmp/mosaic-final"],
                baseDirectory: base
            ) == "/tmp/mosaic-final"
        )
        #expect(
    ...
    @Test func codexTeamsValidatesExplicitWorkingDirectoryExists() throws {
        let existing = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-teams-cwd-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func codexAppServerApprovalBuildsActionableFeedEvent() throws {
        let event = CodexTeamsApprovalBridge.feedEvent(
            ...
            params: [
                ...
                "command": "touch /tmp/mosaic-security-review",
                "cwd": "/tmp/project",
                ...
                "commandActions": [[
                    "type": "write",
                    "path": "/tmp/mosaic-security-review",
        ...
        #expect(toolInput["command"] as? String == "touch /tmp/mosaic-security-review")
