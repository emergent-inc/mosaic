...
final class SessionPersistenceTests: XCTestCase {
    ...
    private func sessionStore(
        bundleIdentifier: String? = "com.mosaicterm.tests",
    ...
    func testWorkspaceSessionSnapshotRestoresMarkdownPanel() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-session-markdown-\(UUID().uuidString)", isDirectory: true)
    ...
    func testSaveAndLoadRoundTripWithCustomSnapshotPath() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-session-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testLoadReopenSessionSnapshotRequiresPreviousSnapshotFile() throws {
        ...
        let bundleIdentifier = "dev.mosaic.tests.\(UUID().uuidString)"
    ...
    func testRestorePolicySkipsWhenLaunchHasExplicitArguments() {
        let shouldRestore = SessionRestorePolicy.shouldAttemptRestore(
            arguments: ["/Applications/mosaic.app/Contents/MacOS/mosaic", "--window", "window:1"],
            environment: [:]
        )
    ...
    func testRestorePolicyAllowsFinderStyleLaunchArgumentsOnly() {
        let shouldRestore = SessionRestorePolicy.shouldAttemptRestore(
            arguments: ["/Applications/mosaic.app/Contents/MacOS/mosaic", "-psn_0_12345"],
            environment: [:]
        )
    ...
    func testRestorePolicySkipsWhenRunningUnderXCTest() {
        let shouldRestore = SessionRestorePolicy.shouldAttemptRestore(
            arguments: ["/Applications/mosaic.app/Contents/MacOS/mosaic"],
    ...
    func testScrollbackReplayEnvironmentWritesReplayFile() {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-scrollback-replay-\(UUID().uuidString)", isDirectory: true)
        try? FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    // Regression for https://github.com/emergent-inc/mosaic/issues/5165.
    //
    ...
    func testNormalizedExportedScreenPathAcceptsAbsoluteAndFileURL() {
        XCTAssertEqual(
            TerminalController.normalizedExportedScreenPath("/tmp/mosaic-screen.txt"),
            "/tmp/mosaic-screen.txt"
        )
        XCTAssertEqual(
            TerminalController.normalizedExportedScreenPath(" file:///tmp/mosaic-screen.txt "),
            "/tmp/mosaic-screen.txt"
        )
    }
    ...
    func testShouldRemoveExportedScreenDirectoryOnlyWithinTemporaryRoot() {
        let tempRoot = URL(fileURLWithPath: "/tmp")
            .appendingPathComponent("mosaic-export-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeRestorableAgentIndex(
        ...
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-agent-hook-store-\(UUID().uuidString)", isDirectory: true)
...
final class SocketListenerAcceptPolicyTests: XCTestCase {
    func testClaudeResumeCommandRoutesThroughWrapperInsteadOfCapturedRealBinary() {
        ...
        // (MOSAIC_AGENT_LAUNCH_EXECUTABLE). Resuming with it directly bypasses
        // mosaic's `claude` wrapper, which is what injects the hooks, so resumed
        // sessions silently lost SessionStart/Stop/Notification. Resume must use
        // the bare `claude` wrapper. https://github.com/emergent-inc/mosaic/issues/5427
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .claude,
            sessionId: "claude-session-123",
            workingDirectory: "/tmp/mosaic project",
        ...
        XCTAssertEqual(
            ...
            "cd -- '/tmp/mosaic project' 2>/dev/null || [ ! -d '/tmp/mosaic project' ] && /bin/sh -c "
                + shellQuotedForTest("'env' 'CLAUDE_CONFIG_DIR=/tmp/claude config' 'MOSAIC_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1' 'MOSAIC_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=CLAUDE_CONFIG_DIR' \"$([ -x \"${MOSAIC_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$MOSAIC_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' 'claude-session-123' '--model' 'sonnet' '--permission-mode' 'auto'")
        )
    ...
    func testClaudeForkCommandRoutesThroughWrapperInsteadOfCapturedRealBinary() throws {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                arguments: [
                    ...
                    "sonnet",
                    "--settings",
                    #"{"hooks":{"SessionStart":[{"hooks":[{"command":"mosaic claude-hook session-start"}]}]}}"#,
                    "--session-id",
                    "old-session"
                ],
                workingDirectory: "/tmp/mosaic project",
        ...
        // https://github.com/emergent-inc/mosaic/issues/5427
        ...
        XCTAssertFalse(command.contains("mosaic claude-hook session-start"), command)
        XCTAssertFalse(command.contains("old-session"), command)
    }

    /// Regression for https://github.com/emergent-inc/mosaic/issues/5639.
    ///
    ...
    /// where mosaic's shell integration (the `claude()` function + per-surface PATH shim)
    ...
    /// user's *real* binary, the mosaic wrapper was bypassed, no hook `--settings` was
    ...
    /// and asserts the launched invocation routed through the mosaic wrapper (its injected
    ...
    func testClaudeResumeCommandExecutesThroughWrapperInsideLoginShellLauncher() throws {
        ...
        let sandbox = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-5639-\(UUID().uuidString)", isDirectory: true)
        let shimDir = sandbox.appendingPathComponent("mosaic-cli-shims", isDirectory: true)
        ...
        let wrapperURL = sandbox.appendingPathComponent("mosaic-claude-wrapper", isDirectory: false)
        ...
        // Stand-in for mosaic-claude-wrapper: re-inject the hook --settings on --resume
        ...
        for arg in "$@"; do
          if [[ "$arg" == "--resume" || "$arg" == "-r" ]]; then
            set -- --settings MOSAIC_HOOKS_JSON "$@"
            break
          fi
        done
        printf 'wrapper %s\\n' "$*" > \(shellQuotedForTest(recordURL.path))
        """)
        // Per-surface shim that mosaic installs and points MOSAIC_CLAUDE_WRAPPER_SHIM at.
        ...
        // MOSAIC_CLAUDE_WRAPPER_SHIM is a managed terminal env var inherited by the -lic shell.
        environment["MOSAIC_CLAUDE_WRAPPER_SHIM"] = shimURL.path
        ...
        XCTAssertTrue(
            ...
            "Resumed claude must route through the mosaic wrapper (which injects the hook --settings). Recorded invocation: \(recorded.isEmpty ? "<none>" : recorded)"
        )
        XCTAssertTrue(
            recorded.hasPrefix("wrapper "),
            "Resume must exec the mosaic wrapper, not the user's real claude binary. Recorded invocation: \(recorded.isEmpty ? "<none>" : recorded)"
        )
    }
    ...
    /// `"$_mosaic_resume_shell" -c <command>` for its `csh|tcsh` branch), and the
    ...
    /// under `zsh -lic`. https://github.com/emergent-inc/mosaic/issues/5639
    func testClaudeResumeCommandExecutesThroughWrapperInsideTcshLauncher() throws {
        ...
        XCTAssertTrue(
            recorded.hasPrefix("wrapper "),
            "tcsh-dispatched resume must parse and exec the mosaic wrapper. Recorded invocation: \(recorded.isEmpty ? "<none>" : recorded)"
        )
        XCTAssertTrue(
            recorded.contains("--settings"),
    ...
    /// is a shipped mosaic integration (`Resources/shell-integration/fish/`). The restore
    /// launcher's `*)` branch dispatches `"$_mosaic_resume_shell" -c <command>` for fish
    ...
    /// (`cd … || [ ! -d … ] && …`, https://github.com/emergent-inc/mosaic/issues/6285) is
    /// exercised too — older fish rejects the POSIX `{ …; }` grouping the guard used to
    /// emit. https://github.com/emergent-inc/mosaic/issues/5639
    func testClaudeResumeCommandExecutesThroughWrapperInsideFishLauncher() throws {
        ...
        XCTAssertTrue(
            recorded.hasPrefix("wrapper "),
            "fish-dispatched resume must parse and exec the mosaic wrapper. Recorded invocation: \(recorded.isEmpty ? "<none>" : recorded)"
        )
        XCTAssertTrue(
            recorded.contains("--settings"),
    ...
    /// Regression for the stale-shim fallback: `MOSAIC_CLAUDE_WRAPPER_SHIM` can outlive
    ...
    func testClaudeResumeCommandFallsBackToPathClaudeWhenShimFileIsStale() throws {
        ...
        let recorded = try runClaudeResumeCommand(
            ...
            environmentOverrides: [
                ...
                "MOSAIC_CLAUDE_WRAPPER_SHIM": reapedShimPath
            ]
        )
        XCTAssertTrue(
    ...
    /// restore-launcher regression tests: a recording stand-in for `mosaic-claude-wrapper`
    /// (re-injects the hook `--settings` on `--resume`, as the real wrapper does), the
    /// per-surface shim that `MOSAIC_CLAUDE_WRAPPER_SHIM` points at, and a hook-less "real"
    ...
    /// profile sourcing, `MOSAIC_CLAUDE_WRAPPER_SHIM` from the managed terminal env).
    private func runClaudeResumeCommand(
        ...
        var environment = [
            ...
            "MOSAIC_CLAUDE_WRAPPER_SHIM": sandbox.shimURL.path
        ]
    ...
    func testRestorableAgentResumeStartupInputEscapesNonAsciiWorkingDirectoryAsAsciiShellInput() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-agent-resume-\(UUID().uuidString)", isDirectory: true)
        let cwdURL = root
        ...
        // (https://github.com/emergent-inc/mosaic/issues/5639) exceed the inline
    ...
    func testSessionEntryClaudeResumeCommandChangesToSessionCwdBeforeResume() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-claude-resume-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testRestorableAgentStartupInputUsesLauncherScriptWhenCommandExceedsTerminalInputBudget() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-agent-resume-test-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testClaudeResumeCommandPreservesDangerouslySkipPermissionsAndObservedEnvironment() {
        ...
        XCTAssertEqual(
            ...
            "cd -- '/Users/lawrence/fun' 2>/dev/null || [ ! -d '/Users/lawrence/fun' ] && /bin/sh -c "
                + shellQuotedForTest("'env' 'CLAUDE_CONFIG_DIR=/Users/lawrence/.codex-accounts/claude/_p1775010019397' 'MOSAIC_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1' 'MOSAIC_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=CLAUDE_CONFIG_DIR' \"$([ -x \"${MOSAIC_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$MOSAIC_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' '24ec0052-450c-4914-b1dd-2ee80d4bc84b' '--dangerously-load-development-channels' 'server:custom-dev-channel' '--dangerously-skip-permissions'")
        )
    }
    ...
    func testCodexResumeCommandDropsStartupImagesAndPlacesSessionBeforeFlags() {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            workingDirectory: "/Users/lawrence/fun/mosaicterm-hq",
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "codex",
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/Users/lawrence/fun/mosaicterm-hq' 2>/dev/null || [ ! -d '/Users/lawrence/fun/mosaicterm-hq' ] && '/Users/lawrence/.bun/bin/codex' 'resume' '019e2bb9-5544-7201-a517-d77bb00d724f' '--yolo' '--model' 'gpt-5.4'"
        )
    }
    ...
    func testCodexTeamsResumeCommandUsesWrapperSubcommand() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/Users/example/repo' 2>/dev/null || [ ! -d '/Users/example/repo' ] && 'env' 'CODEX_HOME=/tmp/codex home' '/usr/local/bin/mosaic' 'codex-teams' 'resume' '019dad34-d218-7943-b81a-eddac5c87951' '--model' 'gpt-5.4' '--sandbox' 'danger-full-access'"
        )
    }
    ...
    func testCodexTeamsResumeCommandDropsOriginalForkTarget() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/Users/example/repo' 2>/dev/null || [ ! -d '/Users/example/repo' ] && 'env' 'CODEX_HOME=/tmp/codex home' '/usr/local/bin/mosaic' 'codex-teams' 'resume' '019dad34-d218-7943-b81a-eddac5c87952' '--model' 'gpt-5.4' '--sandbox' 'danger-full-access'"
        )
    }
    ...
    func testForkCommandsUseVerifiedAgentForkSyntaxAndPreserveContext() {
        ...
        XCTAssertEqual(
            claude.forkCommand,
            "cd -- '/Users/lawrence/fun' 2>/dev/null || [ ! -d '/Users/lawrence/fun' ] && /bin/sh -c "
                + shellQuotedForTest("'env' 'CLAUDE_CONFIG_DIR=/Users/lawrence/.codex-accounts/claude/_p1775010019397' 'MOSAIC_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1' 'MOSAIC_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=CLAUDE_CONFIG_DIR' \"$([ -x \"${MOSAIC_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$MOSAIC_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' '24ec0052-450c-4914-b1dd-2ee80d4bc84b' '--fork-session' '--dangerously-load-development-channels' 'server:custom-dev-channel' '--dangerously-skip-permissions'")
        )
        XCTAssertEqual(
            claudeFork.forkCommand,
            "cd -- '/Users/lawrence/fun' 2>/dev/null || [ ! -d '/Users/lawrence/fun' ] && /bin/sh -c "
                + shellQuotedForTest("'env' 'CLAUDE_CONFIG_DIR=/Users/lawrence/.codex-accounts/claude/_p1775010019397' 'MOSAIC_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1' 'MOSAIC_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=CLAUDE_CONFIG_DIR' \"$([ -x \"${MOSAIC_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$MOSAIC_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' 'claude-fork-child' '--fork-session' '--model' 'sonnet' '--dangerously-skip-permissions'")
        )
        XCTAssertEqual(
            codex.forkCommand,
        ...
        XCTAssertEqual(
            ...
            "cd -- '/Users/example/repo' 2>/dev/null || [ ! -d '/Users/example/repo' ] && 'env' 'CODEX_HOME=/tmp/codex home' '/usr/local/bin/mosaic' 'codex-teams' 'fork' 'codex-teams-session' '--model' 'gpt-5.4' '--sandbox' 'danger-full-access'"
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            ...
            "cd -- '/tmp/opencode repo' 2>/dev/null || [ ! -d '/tmp/opencode repo' ] && 'env' 'OPENCODE_CONFIG_DIR=/tmp/opencode config' '/usr/local/bin/mosaic' 'omo' '--session' 'opencode-session-123' '--fork' '--model' 'anthropic/claude-sonnet-4-6' '/tmp/opencode repo'"
        )
        XCTAssertEqual(
            omoOpenCodeFork.forkCommand,
            "cd -- '/tmp/opencode repo' 2>/dev/null || [ ! -d '/tmp/opencode repo' ] && 'env' 'OPENCODE_CONFIG_DIR=/tmp/opencode config' '/usr/local/bin/mosaic' 'omo' '--session' 'opencode-child-session' '--fork' '--model' 'anthropic/claude-sonnet-4-6' '/tmp/opencode repo'"
        )
    ...
    func testOpenCodeForkSupportProbesFromLaunchWorkingDirectory() async throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-opencode-probe-\(UUID().uuidString)", isDirectory: true)
    ...
    func testOpenCodeForkSupportSkipsLocalProbeForRemoteLikeContext() async {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            workingDirectory: "/remote/mosaic/project-\(UUID().uuidString)",
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "opencode",
    ...
    func testAgentForkSupportRejectsRemoteForksThatNeedLauncherScript() async {
        let longPath = "/Users/mosaic/" + String(repeating: "nested-project-", count: 120)
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .codex,
            sessionId: "019dad34-d218-7943-b81a-eddac5c87951",
            workingDirectory: "/Users/mosaic/project",
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "codex",
                executablePath: "/Users/example/.bun/bin/codex",
                arguments: [
    ...
    func testOpenCodeForkSupportRejectsMissingLocalExecutable() async throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-opencode-missing-executable-\(UUID().uuidString)", isDirectory: true)
        defer { try? fileManager.removeItem(at: root) }
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
    ...
    func testOpenCodeForkSupportCachesUnsupportedVersion() async throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-opencode-probe-cache-\(UUID().uuidString)", isDirectory: true)
        let executable = root.appendingPathComponent("opencode", isDirectory: false)
    ...
    func testProcessDetectedOpenCodeResolvesBareExecutableWithCapturedPath() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-opencode-path-\(UUID().uuidString)", isDirectory: true)
    ...
    func testProcessDetectedOpenCodeWorkingDirectoryUsesProjectPositional() {
        ...
        XCTAssertEqual(
            RestorableAgentSessionIndex.openCodeWorkingDirectoryForProcess(
                ...
                environment: ["MOSAIC_AGENT_LAUNCH_CWD": "/tmp/hook-cwd", "PWD": "/tmp/shell-cwd"]
            ),
            "/tmp/hook-cwd"
        )
    }
    ...
    func testProcessDetectedOpenCodeLaunchArgumentsPreserveSafeForkContext() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-opencode-argv-\(UUID().uuidString)", isDirectory: true)
        let bin = root.appendingPathComponent("bin", isDirectory: true)
        let executable = bin.appendingPathComponent("opencode", isDirectory: false)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testClaudeTeamsResumeCommandPreservesRemoteControlLauncher() {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                executablePath: "/Applications/mosaic.app/Contents/Resources/bin/mosaic",
                arguments: [
                ...
                workingDirectory: "/tmp/team repo",
                environment: [
                    "MOSAIC_CUSTOM_CLAUDE_PATH": "/opt/Claude Code/bin/claude",
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/tmp/team repo' 2>/dev/null || [ ! -d '/tmp/team repo' ] && 'env' 'MOSAIC_CUSTOM_CLAUDE_PATH=/opt/Claude Code/bin/claude' '/Applications/mosaic.app/Contents/Resources/bin/mosaic' 'claude-teams' '--resume' 'claude-team-session' '--teammate-mode' 'auto' '--model' 'sonnet' '--remote-control-session-name-prefix' 'mosaic-team' '--permission-mode' 'auto'"
        )
    }
    ...
    func testClaudeResumeCommandHandlesOptionalDebugValueAndFilteredEnvironment() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "/bin/sh -c " + shellQuotedForTest("'env' 'NODE_OPTIONS=--max-old-space-size=4096' \"$([ -x \"${MOSAIC_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$MOSAIC_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' 'claude-session-debug' '--debug' 'api,mcp' '--model' 'sonnet'")
        )
    }
    ...
    func testResumeCommandPreservesSafeProviderEnvironmentValuesOnly() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "/bin/sh -c " + shellQuotedForTest("'env' 'ANTHROPIC_BASE_URL=https://api.example.test' 'ANTHROPIC_MODEL=' 'MOSAIC_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1' 'MOSAIC_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=ANTHROPIC_BASE_URL,ANTHROPIC_MODEL' \"$([ -x \"${MOSAIC_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$MOSAIC_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' 'claude-session-env'")
        )
        XCTAssertFalse(snapshot.resumeCommand?.contains("ANTHROPIC_AUTH_TOKEN") ?? true)
    }

    func testClaudeResumeCommandStripsStaleMosaicNodeOptionsRestoreModule() {
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .claude,
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                workingDirectory: nil,
                environment: [
                    "NODE_OPTIONS": "--require=/tmp/mosaic-claude-node-options/restore-node-options.cjs --max-old-space-size=4096 --trace-warnings"
                ],
                capturedAt: nil,
                source: nil
            )
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "/bin/sh -c " + shellQuotedForTest("'env' 'NODE_OPTIONS=--trace-warnings' \"$([ -x \"${MOSAIC_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$MOSAIC_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' 'claude-session-node-options' '--model' 'sonnet'")
        )
    }

    func testClaudeResumeCommandDropsEmptyStaleMosaicNodeOptionsEnvironment() {
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .claude,
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                workingDirectory: nil,
                environment: [
                    "NODE_OPTIONS": "--require /tmp/mosaic-claude-node-options/restore-node-options.cjs --max-old-space-size 4096"
                ],
                capturedAt: nil,
                source: nil
            )
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "/bin/sh -c " + shellQuotedForTest("\"$([ -x \"${MOSAIC_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$MOSAIC_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' 'claude-session-empty-node-options' '--model' 'sonnet'")
        )
    }
    ...
    func testOpenCodeWrapperResumeCommandAndUnsupportedOhMyLaunchers() {
        ...
        let omx = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "omx",
                executablePath: "/usr/local/bin/mosaic",
                arguments: ["/usr/local/bin/mosaic", "omx", "team"],
                workingDirectory: nil,
                environment: nil,
                capturedAt: nil,
                source: nil
        ...
        let omc = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "omc",
                executablePath: "/usr/local/bin/mosaic",
                arguments: ["/usr/local/bin/mosaic", "omc", "team"],
                workingDirectory: nil,
                environment: nil,
                capturedAt: nil,
                source: nil
        ...
        XCTAssertEqual(
            omo.resumeCommand,
            "cd -- '/tmp/opencode repo' 2>/dev/null || [ ! -d '/tmp/opencode repo' ] && 'env' 'OPENCODE_CONFIG_DIR=/tmp/opencode config' '/usr/local/bin/mosaic' 'omo' '--session' 'opencode-session-123' '--model' 'anthropic/claude-sonnet-4-6' '/tmp/opencode repo'"
        )
        XCTAssertEqual(
    ...
    func testRestorableAgentIndexLoadsLaunchCommandFromHookStore() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-agent-hook-store-\(UUID().uuidString)", isDirectory: true)
        let storeDir = home.appendingPathComponent(".mosaicterm", isDirectory: true)
    ...
    func testAntigravityProcessDetectionDoesNotTreatTrailingFlagAsConversationID() throws {
        ...
        let processSnapshot = MosaicTopProcessSnapshot(
            processes: [
                MosaicTopProcessInfo(
                    pid: processId,
                    parentPID: 1,
                    name: "agy",
                    path: "/usr/local/bin/agy",
                    ttyDevice: nil,
                    mosaicWorkspaceID: workspaceId,
                    mosaicSurfaceID: panelId,
                    mosaicAttributionReason: "mosaic-test",
                    processGroupID: nil,
        ...
        let registry = MosaicVaultAgentRegistry(registrations: [.builtInAntigravity])
...
extension SessionPersistenceTests {
    ...
    // Regression for https://github.com/emergent-inc/mosaic/issues/6285: the cwd guard runs
    ...
    // A binding persisted by an older mosaic with the legacy braced guard must be
    ...
    func testAgentHookSurfaceResumeStartupInputRunsWhenSavedWorkingDirectoryWasDeleted() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-surface-resume-missing-cwd-\(UUID().uuidString)", isDirectory: true)
        let bin = root.appendingPathComponent("bin", isDirectory: true)
        ...
        try """
        #!/bin/zsh
        print -r -- "$PWD|$*" > "$MOSAIC_FAKE_CODEX_OUTPUT"
        ...
        environment["MOSAIC_FAKE_CODEX_OUTPUT"] = outputURL.path
        process.environment = environment
        let stderr = Pipe()
    ...
    func testSurfaceResumeBindingStartupInputUsesLauncherScriptWhenLong() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-surface-resume-test-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testSurfaceResumeApprovalMissingRecordResetsStalePromptPolicy() throws {
        ...
        let effectiveBinding = SurfaceResumeApprovalStore.applyingStoredApproval(
            to: binding,
            fileURL: URL(fileURLWithPath: "/tmp/mosaic-missing-\(UUID().uuidString).json"),
    ...
    func testSurfaceResumeApprovalWritesRecordsIntoMosaicJSON() throws {
        let settingsURL = try makeSurfaceResumeApprovalMosaicSettingsURL()
        let secret = Data("approval-secret".utf8)
        let initialSettings = """
        {
          "$schema": "https://raw.githubusercontent.com/emergent-inc/mosaic/main/web/data/mosaic.schema.json",
          // keep root comment
          "schemaVersion": 1,
          "terminal": {
    ...
    func testSurfaceResumeApprovalWritesNonUTF8MosaicJSON() throws {
        let settingsURL = try makeSurfaceResumeApprovalMosaicSettingsURL()
        let secret = Data("approval-secret".utf8)
        let initialSettings = """
        {
    ...
    func testSurfaceResumeApprovalMigratesLegacyRecordsIntoMosaicJSON() throws {
        let settingsURL = try makeSurfaceResumeApprovalMosaicSettingsURL()
    ...
    func testSurfaceResumeApprovalDoesNotOverwriteInvalidMosaicJSON() throws {
        let settingsURL = try makeSurfaceResumeApprovalMosaicSettingsURL()
        let legacyURL = settingsURL.deletingLastPathComponent()
            .appendingPathComponent("resume-commands.json", isDirectory: false)
        let secret = Data("approval-secret".utf8)
    ...
    func testHermesAgentHookSurfaceResumeBootstrapsSubrouterAndRewritesStaleCodexProvider() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-hermes-surface-resume-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeSurfaceResumeApprovalStoreURL() throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-surface-resume-approvals-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        addTeardownBlock {
    ...
    private func makeSurfaceResumeApprovalMosaicSettingsURL() throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-surface-resume-settings-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        addTeardownBlock {
            try? FileManager.default.removeItem(at: root)
        }
        return root.appendingPathComponent("mosaic.json", isDirectory: false)
    }
    ...
    func testRestoreRunsSurfaceResumeBindingFromBindingCwd() throws {
        ...
        let bindingCwd = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-surface-binding-cwd-\(UUID().uuidString)", isDirectory: true)
    ...
    func testRestoreDoesNotPassDeletedAgentHookCwdToTerminalRuntime() throws {
        try withAutoResumeAgentSessionsEnabled {
            ...
            let missingCwd = FileManager.default.temporaryDirectory
                .appendingPathComponent("mosaic-deleted-agent-hook-cwd-\(UUID().uuidString)", isDirectory: true)
                .appendingPathComponent("repo", isDirectory: true)
            let bindingIndex = SurfaceResumeBindingIndex(bindingsByPanel: [
                SurfaceResumeBindingIndex.PanelKey(workspaceId: source.id, panelId: sourcePanelId): SurfaceResumeBindingSnapshot(
                    name: "Codex",
            ...
            // The guard is the fish-safe, brace-free form (https://github.com/emergent-inc/mosaic/issues/6285):
    ...
    func testRestorePreservesUnmountedVolumeCwdBindingsWhenInitialReportsAreScrambled() throws {
        try withAutoResumeAgentSessionsEnabled {
            ...
            let volumeName = "mosaic-issue-5278-\(UUID().uuidString)"
    ...
    func testMarkdownFileLinkResolverPrefersCurrentMarkdownDirectory() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-markdown-link-resolver-\(UUID().uuidString)", isDirectory: true)
    ...
    func testMarkdownFileLinkResolverFallsBackToProcessWorkingDirectory() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-markdown-link-resolver-cwd-\(UUID().uuidString)", isDirectory: true)
        let docs = root.appendingPathComponent("docs", isDirectory: true)
        let openedFile = docs.appendingPathComponent("index.md")
    ...
    func testMarkdownFileLinkResolverRejectsMissingAndNonMarkdownFiles() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-markdown-link-resolver-reject-\(UUID().uuidString)", isDirectory: true)
