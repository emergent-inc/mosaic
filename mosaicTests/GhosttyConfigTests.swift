...
// The app target still declares legacy duplicates of these MosaicSettings
// value types; with MosaicSettings imported unconditionally the names are
...
final class GhosttyConfigTests: XCTestCase {
    ...
    func testLaunchGhosttyResourcesPreferCurrentBundleOverInheritedEnvironment() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-launch-resources-\(UUID().uuidString)")
        ...
        let resolved = mosaicApp.resolvedGhosttyResourcesDirectory(
    ...
    func testLaunchGhosttyResourcesKeepInheritedEnvironmentWhenBundleHasNoResources() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-launch-resource-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLaunchGhosttyResourcesKeepInheritedEnvironmentWhenBundleLacksThemes() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-launch-incomplete-resource-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLaunchGhosttyResourcesUseIncompleteBundleOnlyAsLastFallback() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-launch-incomplete-resource-last-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testThemeSearchPathsIncludeXDGDataDirsThemes() {
        let pathA = "/tmp/mosaic-theme-a"
        let pathB = "/tmp/mosaic-theme-b"
    ...
    func testThemeSearchPathsIncludeMosaicUserThemesDirectory() {
        let paths = GhosttyConfig.themeSearchPaths(
    ...
    func testThemeSearchPathsIncludeMosaicUserThemesDirectoryFromFixedHome() {
        let fixedHome = "/tmp/mosaic-fixed-home-\(UUID().uuidString)"
        let paths = GhosttyConfig.themeSearchPaths(
            forThemeName: "Zag Light",
    ...
    func testThemesListIncludesMosaicUserThemesDirectory() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-user-theme-list-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testMosaicDefaultThemeConfigContentsSkipsInvalidUTF8Candidate() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-managed-theme-search-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let contents = GhosttyConfig.mosaicDefaultThemeConfigContents(
            preferredColorScheme: .light,
    ...
    func testLoadReadsSymlinkedGhosttyConfigFile() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-config-symlink-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadEnforcesCursorDarkOverRecursiveColorConfigFile() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-config-recursive-background-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        XCTAssertEqual(loaded.theme, GhosttyConfig.mosaicEnforcedThemeName)
    ...
    func testLoadDoesNotReparseTopLevelConfigReferencedByConfigFile() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-config-top-level-cycle-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadAllowsRecursiveConfigFileToReloadTopLevelConfigAsFinalOverride() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-config-top-level-reload-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadReadsOptionalQuotedConfigFilePath() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-config-optional-quoted-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadIgnoresLegacyAppSupportConfigWhenConfigGhosttyIsNonEmpty() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-app-support-legacy-skip-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadUsesLegacyAppSupportConfigWhenConfigGhosttyIsEmpty() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-app-support-legacy-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadEnforcesCursorDarkAfterUserThemeAndCursorColorOverride() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-theme-order-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadThemeReadsAbsoluteThemeFilePath() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-absolute-theme-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadThemeResolvesPairedThemeValueByColorScheme() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-theme-pair-\(UUID().uuidString)")
    ...
    func testLoadThemeResolvesBuiltinAliasFromGhosttyResourcesDir() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-themes-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedLightAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-solarized-light-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedDarkAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-ghostty-solarized-dark-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testUnparsedAppearanceFallbackIgnoresNativeLegacyBaselineWhenCurrentConfigExists() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-native-legacy-baseline-\(UUID().uuidString)")
    ...
    func testUnparsedAppearanceFallbackKeepsNativeLegacyBaselineWhenCurrentConfigIsMissingOrEmpty() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-native-legacy-baseline-empty-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }
    ...
    func testRuntimeColorSchemeForMosaicSingleThemeReloadKeepsResolvedSchemeDuringConfigLoad() {
        XCTAssertEqual(
            GhosttyApp.runtimeColorSchemeForConfigLoad(
                ...
                mosaicThemeValue: "light:3024 Day,dark:3024 Day"
            ),
            .light
        )
        XCTAssertEqual(
    ...
    func testRuntimeColorSchemeForPairedThemeReloadUsesAppearanceDuringConfigLoad() {
        XCTAssertEqual(
            GhosttyApp.runtimeColorSchemeForConfigLoad(
                ...
                requestedColorScheme: .dark,
                effectiveTerminalColorScheme: .light,
                mosaicThemeValue: "light:3024 Day,dark:3024 Night"
            ),
            .dark
        )
        XCTAssertEqual(
    ...
    func testClaudeCodeIntegrationDefaultsToEnabledWhenUnset() {
        let suiteName = "mosaic.tests.claude-hooks.\(UUID().uuidString)"
    ...
    func testKiroIntegrationDefaultsToEnabledWithStandardNotificationsWhenUnset() {
        let suiteName = "mosaic.tests.kiro-hooks.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated user defaults suite")
            return
        }
    ...
    func testSubagentNotificationSuppressionDefaultsToEnabledWhenUnset() {
        let suiteName = "mosaic.tests.subagent-notifications.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated user defaults suite")
            return
        }
    ...
    func testTelemetryDefaultsToEnabledWhenUnset() {
        let suiteName = "mosaic.tests.telemetry.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated user defaults suite")
            return
        }
    ...
    private func runCLI(
        ...
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
...
// mosaicShouldApplyWindowGlass) were lifted out of the app target into
// MosaicWorkspaceWindow's WindowBackgroundPolicy by the window-chrome tranche, and
// equivalent coverage now lives in
// Packages/macOS/MosaicWorkspaceWindow/Tests/MosaicWorkspaceWindowTests/WindowBackgroundPolicyTests.swift.
// The stale app-side test was left referencing the removed symbols, which broke
// the mosaicTests compile on the Swift 6 depot toolchain.
...
final class WorkspaceRemoteDaemonManifestTests: XCTestCase {
    func testParsesEmbeddedRemoteDaemonManifestJSON() throws {
        let manifestJSON = """
        {
          ...
          "appVersion": "0.62.0",
          "releaseTag": "v0.62.0",
          "releaseURL": "https://github.com/emergent-inc/mosaic/releases/tag/v0.62.0",
          "checksumsAssetName": "mosaicd-remote-checksums.txt",
          "checksumsURL": "https://github.com/emergent-inc/mosaic/releases/download/v0.62.0/mosaicd-remote-checksums.txt",
          "entries": [
            {
              "goOS": "linux",
              "goArch": "amd64",
              "assetName": "mosaicd-remote-linux-amd64",
              "downloadURL": "https://github.com/emergent-inc/mosaic/releases/download/v0.62.0/mosaicd-remote-linux-amd64",
              "sha256": "abc123"
            }
          ]
        }
        ...
        XCTAssertEqual(manifest?.entry(goOS: "linux", goArch: "amd64")?.assetName, "mosaicd-remote-linux-amd64")
    }
    ...
    func testRemoteDaemonCachePathIsVersionedByPlatform() throws {
        ...
        XCTAssertTrue(url.path.contains("/.local/state/mosaic/remote-daemons/0.62.0/linux-arm64/"))
        XCTAssertEqual(url.lastPathComponent, "mosaicd-remote")
    }
}
...
final class RemoteLoopbackHTTPRequestRewriterTests: XCTestCase {
    func testRewritesLoopbackAliasHostHeadersToLocalhost() {
        let original = Data(
            (
                "GET /demo HTTP/1.1\r\n" +
                "Host: mosaic-loopback.localtest.me:3000\r\n" +
                "Origin: http://mosaic-loopback.localtest.me:3000\r\n" +
                "Referer: http://mosaic-loopback.localtest.me:3000/app\r\n" +
                "\r\n"
            ).utf8
        )
        ...
        let rewritten = RemoteLoopbackHTTPRequestRewriter.rewriteIfNeeded(
            data: original,
            aliasHost: "mosaic-loopback.localtest.me"
        )
        ...
        XCTAssertFalse(text.contains("mosaic-loopback.localtest.me"))
    }
    ...
    func testRewritesLoopbackSubdomainAliasHostHeadersToOriginalLocalhostSubdomain() {
        let original = Data(
            (
                "GET /demo HTTP/1.1\r\n" +
                "Host: api.mosaic-loopback.localtest.me:3000\r\n" +
                "Origin: http://api.mosaic-loopback.localtest.me:3000\r\n" +
                "Referer: http://api.mosaic-loopback.localtest.me:3000/app\r\n" +
                "\r\n"
            ).utf8
        )
        ...
        XCTAssertFalse(text.contains("api.mosaic-loopback.localtest.me"))
    }

    func testRewritesAbsoluteFormRequestLineForLoopbackAlias() {
        let original = Data(
            (
                "GET http://mosaic-loopback.localtest.me:3000/demo HTTP/1.1\r\n" +
                "Host: mosaic-loopback.localtest.me:3000\r\n" +
                "\r\n"
            ).utf8
        )
    ...
    func testRewritesLoopbackResponseHeadersBackToAlias() {
        ...
        let text = String(decoding: rewritten, as: UTF8.self)
        XCTAssertTrue(text.contains("Location: http://mosaic-loopback.localtest.me:3000/login"))
        XCTAssertTrue(text.contains("Access-Control-Allow-Origin: http://mosaic-loopback.localtest.me:3000"))
        XCTAssertTrue(text.contains("Set-Cookie: sid=1; Domain=mosaic-loopback.localtest.me; Path=/"))
    }
    ...
    func testRewritesLoopbackSubdomainResponseHeadersBackToAliasSubdomain() {
        ...
        let text = String(decoding: rewritten, as: UTF8.self)
        XCTAssertTrue(text.contains("Location: http://api.mosaic-loopback.localtest.me:3000/login"))
        XCTAssertTrue(text.contains("Access-Control-Allow-Origin: http://api.mosaic-loopback.localtest.me:3000"))
        XCTAssertTrue(text.contains("Set-Cookie: sid=1; Domain=api.mosaic-loopback.localtest.me; Path=/"))
    }
    ...
    func testRewritesLeadingDotLoopbackCookieDomainsBackToAliasDomains() {
        ...
        let text = String(decoding: rewritten, as: UTF8.self)
        XCTAssertTrue(text.contains("Set-Cookie: root=1; Domain=.mosaic-loopback.localtest.me; Path=/"))
        XCTAssertTrue(text.contains("Set-Cookie: api=1; Domain=.api.mosaic-loopback.localtest.me; Path=/"))
    }
}
...
final class BrowserPanelWebViewLifecycleTests: XCTestCase {
    func testHiddenDiscardPolicyReadsUserDefaults() throws {
        let suiteName = "mosaic.browserHiddenDiscardPolicyTests.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        defer {
            defaults.removePersistentDomain(forName: suiteName)
        }
        let hasEnabledEnvironmentOverride =
            ProcessInfo.processInfo.environment["MOSAIC_BROWSER_HIDDEN_WEBVIEW_DISCARD_ENABLED"] != nil
        let hasDelayEnvironmentOverride =
            ProcessInfo.processInfo.environment["MOSAIC_BROWSER_HIDDEN_WEBVIEW_DISCARD_DELAY_SECONDS"] != nil
...
final class BrowserDefaultsNormalizationTests: XCTestCase {
    ...
    func testNormalizeRewritesOutOfRangeAndLegacyValues() throws {
        let suiteName = "mosaic.browserDefaultsNormalizationTests.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
    ...
    func testNormalizePreservesValidValues() throws {
        ...
        // `MosaicSettings`, which declares a same-named enum.
...
final class BrowserNewTabNavigationSeedTests: XCTestCase {
    func testPreservesOriginalRequestHeadersMethodBodyAndBypassHost() throws {
        ...
        request.setValue("keep-me", forHTTPHeaderField: "X-Mosaic-Test")
        ...
        XCTAssertEqual(
            seed.initialRequest.value(forHTTPHeaderField: "X-Mosaic-Test"),
            "keep-me"
        )
...
final class BrowserPanelRemoteStoreTests: XCTestCase {
    ...
    func testRemoteWorkspaceDefersInitialNavigationUntilProxyEndpointIsReady() {
        ...
        XCTAssertEqual(panel.preferredURLStringForOmnibar(), url.absoluteString)
        XCTAssertEqual(panel.webView.url?.host, "mosaic-loopback.localtest.me")
    }
    ...
    func testRemoteWorkspacePreservesLocalhostSubdomainWhenAliasingLoopbackURL() {
        ...
        XCTAssertEqual(panel.preferredURLStringForOmnibar(), url.absoluteString)
        XCTAssertEqual(panel.webView.url?.host, "api.mosaic-loopback.localtest.me")
    }
    ...
    func testRemoteWorkspaceRuntimeBridgeAliasesMultipleLoopbackPortsFromSamePage() async throws {
        ...
        let baseURL = try XCTUnwrap(URL(string: "http://mosaic-loopback.localtest.me:3000/"))
        ...
        let result = try await panel.evaluateJavaScript(
            """
            (() => {
              const rewrite = window.__mosaicRewriteRemoteLoopbackURL;
        ...
        XCTAssertEqual(
            result,
            #"["http://mosaic-loopback.localtest.me:3000/frontend","http://mosaic-loopback.localtest.me:8000/api","http://api.mosaic-loopback.localtest.me:8000/v1","ws://mosaic-loopback.localtest.me:5173/hmr","wss://localhost:5173/hmr","https://localhost:9443/secure"]"#
        )
    }
    ...
    func testBrowserMoveIntoRemoteWorkspaceRebuildsWebsiteDataStoreScope() throws {
        ...
        destination.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/mosaic-store-dest.sock",
                terminalStartupCommand: "ssh mosaic-macmini"
            ),
            autoConnect: false
        )
    ...
    func testBrowserMoveOutOfRemoteWorkspaceRestoresDefaultWebsiteDataStore() throws {
        ...
        source.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/mosaic-store-source.sock",
                terminalStartupCommand: "ssh mosaic-macmini"
            ),
            autoConnect: false
        )
    ...
    func testNewTerminalSurfaceStaysRemoteWhileBrowserPanelsKeepWorkspaceRemote() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayID: "relay-test",
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/mosaic-test.sock",
            terminalStartupCommand: "ssh mosaic-macmini"
        )
...
final class WorkspaceRemoteConfigurationTransportKeyTests: XCTestCase {
    func testProxyBrokerTransportKeyIgnoresControlPath() {
        let first = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/mosaic-ssh-501-64000-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64000,
            relayID: "relay-a",
            relayToken: "token-a",
            localSocketPath: "/tmp/mosaic-a.sock",
            terminalStartupCommand: "ssh mosaic-macmini"
        )
        let second = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/mosaic-ssh-501-64001-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64001,
            relayID: "relay-b",
            relayToken: "token-b",
            localSocketPath: "/tmp/mosaic-b.sock",
            terminalStartupCommand: "ssh mosaic-macmini"
        )
    ...
    func testProxyBrokerTransportKeyIgnoresEphemeralAgentSocketPath() {
        let first = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/mosaic-a.sock",
            terminalStartupCommand: "ssh mosaic-macmini",
            agentSocketPath: "/tmp/mosaic-agent-a.sock"
        )
        let second = WorkspaceRemoteConfiguration(
            destination: "mosaic-macmini",
            port: 22,
            ...
            localSocketPath: "/tmp/mosaic-b.sock",
            terminalStartupCommand: "ssh mosaic-macmini",
            agentSocketPath: "/tmp/mosaic-agent-b.sock"
        )
...
final class WorkspaceRemoteSSHCleanupTests: XCTestCase {
    func testOrphanedMosaicRemoteSSHPIDsMatchesOnlyParentOneRelayAndDaemonTransports() {
        let psOutput = """
          101 1 /usr/bin/ssh -N -T -S none -o ControlPath=/tmp/mosaic-ssh-501-56080-%C -R 127.0.0.1:56080:127.0.0.1:64048 mosaic-macmini
          102 1 /usr/bin/ssh -T -S none -o RequestTTY=no mosaic-macmini sh -c 'exec .mosaic/bin/mosaicd-remote/0.63.1/darwin-arm64/mosaicd-remote serve --stdio'
          107 1 /usr/bin/ssh -T -S none -o RequestTTY=no mosaic-macmini sh -c 'exec .mosaic/bin/mosaicd-remote/0.63.1/darwin-arm64/mosaicd-remote 'serve' '--stdio' '--persistent' '--slot' 'ssh-test''
          103 999 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56081:127.0.0.1:64049 mosaic-macmini
          104 1 /usr/bin/ssh -tt mosaic-macmini
          105 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56082:127.0.0.1:64050 other-host
          106 1 /usr/bin/ssh -T -S none mosaic-macmini /bin/sh
        """

        XCTAssertEqual(
            RemoteSessionCoordinator.orphanedMosaicRemoteSSHPIDs(
                psOutput: psOutput,
                destination: "mosaic-macmini"
            ),
            [101, 102, 107]
    ...
    func testOrphanedMosaicRemoteSSHPIDsCanRestrictCleanupToSpecificRelayPort() {
        let psOutput = """
          201 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56080:127.0.0.1:64048 mosaic-macmini
          202 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56081:127.0.0.1:64049 mosaic-macmini
          203 1 /usr/bin/ssh -T -S none -o RequestTTY=no mosaic-macmini sh -c 'exec .mosaic/bin/mosaicd-remote/0.63.1/darwin-arm64/mosaicd-remote serve --stdio'
          204 1 /usr/bin/ssh -T -S none -o RequestTTY=no mosaic-macmini sh -c 'exec .mosaic/bin/mosaicd-remote/0.63.1/darwin-arm64/mosaicd-remote 'serve' '--stdio' '--persistent' '--slot' 'ssh-test''
          205 1 /usr/bin/ssh -T -S none -o RequestTTY=no mosaic-macmini sh -c 'exec .mosaic/bin/mosaicd-remote/0.63.1/darwin-arm64/mosaicd-remote 'serve' '--stdio' '--persistent' '--slot' 'ssh-other''
        """
    ...
    func testOrphanedMosaicRemoteSSHPIDsMatchesBackslashEscapedPersistentDaemonSlot() {
        let psOutput = """
          211 1 /usr/bin/ssh -T -S none -o RequestTTY=no mosaic-macmini sh -c 'exec '\''.mosaic/bin/mosaicd-remote/0.63.1/darwin-arm64/mosaicd-remote'\'' '\''serve'\'' '\''--stdio'\'' '\''--persistent'\'' '\''--slot'\'' '\''ssh-test'\'''
          212 1 /usr/bin/ssh -T -S none -o RequestTTY=no mosaic-macmini sh -c 'exec '\''.mosaic/bin/mosaicd-remote/0.63.1/darwin-arm64/mosaicd-remote'\'' '\''serve'\'' '\''--stdio'\'' '\''--persistent'\'' '\''--slot'\'' '\''ssh-other'\'''
        """
    ...
    func testOrphanedMosaicRemoteSSHPIDsMatchesEqualsQuotedPersistentDaemonSlot() {
        let psOutput = """
          221 1 /usr/bin/ssh -T -S none -o RequestTTY=no mosaic-macmini sh -c 'exec .mosaic/bin/mosaicd-remote serve --stdio --persistent --slot='ssh-test''
          222 1 /usr/bin/ssh -T -S none -o RequestTTY=no mosaic-macmini sh -c 'exec .mosaic/bin/mosaicd-remote serve --stdio --persistent --slot="ssh-other"'
        """
    ...
    func testOrphanedMosaicRemoteSSHPIDsWithSlotAndNoRelayKeepsGenericCleanup() {
        let psOutput = """
          301 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56080:127.0.0.1:64048 mosaic-macmini
          302 1 /usr/bin/ssh -T -S none -o RequestTTY=no mosaic-macmini sh -c 'exec .mosaic/bin/mosaicd-remote/0.63.1/darwin-arm64/mosaicd-remote serve --stdio'
          303 1 /usr/bin/ssh -T -S none -o RequestTTY=no mosaic-macmini sh -c 'exec .mosaic/bin/mosaicd-remote/0.63.1/darwin-arm64/mosaicd-remote 'serve' '--stdio' '--persistent' '--slot' 'ssh-test''
          304 1 /usr/bin/ssh -T -S none -o RequestTTY=no mosaic-macmini sh -c 'exec .mosaic/bin/mosaicd-remote/0.63.1/darwin-arm64/mosaicd-remote 'serve' '--stdio' '--persistent' '--slot' 'ssh-other''
          305 1 /usr/bin/ssh -T -S none -o RequestTTY=no other-host sh -c 'exec .mosaic/bin/mosaicd-remote/0.63.1/darwin-arm64/mosaicd-remote serve --stdio'
        """
...
final class SocketControlSettingsTests: XCTestCase {
    func testMigrateModeSupportsExpandedSocketModes() {
        ...
        XCTAssertEqual(SocketControlSettings.migrateMode("mosaicOnly"), .mosaicOnly)
    ...
    func testSocketModePermissions() {
        ...
        XCTAssertEqual(SocketControlMode.mosaicOnly.socketFilePermissions, 0o600)
    ...
    func testInvalidEnvSocketModeDoesNotOverrideUserMode() {
        XCTAssertNil(
            SocketControlSettings.envOverrideMode(
                environment: ["MOSAIC_SOCKET_MODE": "definitely-not-a-mode"]
            )
        )
        XCTAssertEqual(
    ...
    func testStableReleaseIgnoresAmbientSocketOverrideByDefault() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "MOSAIC_TAG": "stray-tag",
                "MOSAIC_SOCKET_PATH": "/tmp/mosaic-debug-issue-153-tmux-compat.sock",
            ],
    ...
    func testTaggedDebugLaunchUsesTagDefaultWhenNoOverrideIsProvided() {
        ...
        XCTAssertEqual(path, "/tmp/mosaic-debug-my-tag.sock")
    }
    ...
    func testTaggedDebugLaunchStillHonorsSocketOverride() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "MOSAIC_TAG": "my-tag",
                "MOSAIC_SOCKET_PATH": "/tmp/mosaic-debug-forced.sock",
            ],
            bundleIdentifier: "mosaic.com.emergent.app.debug",
            isDebugBuild: true
        )

        XCTAssertEqual(path, "/tmp/mosaic-debug-forced.sock")
    }
    ...
    func testNightlyReleaseUsesDedicatedDefaultAndIgnoresAmbientSocketOverride() {
        ...
        XCTAssertEqual(path, "/tmp/mosaic-nightly.sock")
    }

    func testTaggedDebugBundleKeepsMatchingSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "MOSAIC_SOCKET_PATH": "/tmp/mosaic-debug-my-tag.sock",
            ],
    ...
    func testTaggedDebugBundleIgnoresSocketOverrideInheritedFromDifferentMosaicBundle() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "MOSAIC_BUNDLE_ID": "mosaic.com.emergent.app.nightly",
                "MOSAIC_SOCKET_PATH": "/tmp/mosaic-nightly.sock",
            ],
            bundleIdentifier: "mosaic.com.emergent.app.debug.issue.4355.mosaic.themes.set.state.dependent",
            isDebugBuild: true
        )

        XCTAssertEqual(path, "/tmp/mosaic-debug-issue-4355-mosaic-themes-set-state-dependent.sock")
    }
    ...
    func testTaggedDebugBundleIgnoresMismatchedInheritedSocketOverride() {
        ...
        XCTAssertEqual(path, "/tmp/mosaic-debug-fix-grok-notifications.sock")
    }
    ...
    func testTaggedDebugBundleRefusesStableSocketOverrideEvenWithOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "MOSAIC_SOCKET_PATH": SocketControlSettings.stableDefaultSocketPath,
                "MOSAIC_ALLOW_SOCKET_OVERRIDE": "1",
            ],
        ...
        XCTAssertEqual(path, "/tmp/mosaic-debug-sockguard.sock")
    }
    ...
    func testTaggedDebugBundleRefusesUserScopedStableSocketOverrideEvenWithOptInFlag() {
        let aliases = [
            ...
            "/private/tmp/mosaic-501.sock",
        ]

        for alias in aliases {
            let path = SocketControlSettings.socketPath(
                environment: [
                    "MOSAIC_SOCKET_PATH": alias,
                    "MOSAIC_ALLOW_SOCKET_OVERRIDE": "1",
                ],
                bundleIdentifier: "mosaic.com.emergent.app.debug.sockguard",
                isDebugBuild: false,
            ...
            XCTAssertEqual(path, "/tmp/mosaic-debug-sockguard.sock", alias)
        }
    }
    ...
    func testTaggedDebugBundleRefusesCanonicalLegacyStableSocketAliasEvenWithOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "MOSAIC_SOCKET_PATH": "/private/tmp/mosaic.sock",
                "MOSAIC_ALLOW_SOCKET_OVERRIDE": "1",
            ],
            bundleIdentifier: "mosaic.com.emergent.app.debug.sockguard",
            isDebugBuild: false
    ...
    func testTaggedDebugBundleRefusesCaseVariantStableSocketAliasesEvenWithOptInFlag() {
        let aliases = [
            ...
            "/private/tmp/MOSAIC.sock",
            SocketControlSettings.userScopedStableSocketPath(currentUserID: 501)
                .replacingOccurrences(of: "mosaic-501.sock", with: "MOSAIC-501.sock"),
            SocketControlSettings.legacyUserScopedStableSocketPath(currentUserID: 501)
                .replacingOccurrences(of: "mosaic-501.sock", with: "MOSAIC-501.sock"),
        ]
    ...
    func testTaggedDebugBundleRefusesLeafSymlinkToStableSocketEvenWithOptInFlag() throws {
        let alias = "/tmp/mosaic-stable-alias-\(UUID().uuidString).sock"
    ...
    func testTaggedDebugBundleRefusesExcessiveSymlinkChainEvenWithOptInFlag() throws {
        let root = "/tmp/mosaic-stable-chain-\(UUID().uuidString)"
    ...
    func testStagingBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "MOSAIC_SOCKET_PATH": "/tmp/mosaic-staging-my-tag.sock",
            ],
            bundleIdentifier: "mosaic.com.emergent.app.staging.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/mosaic-staging-my-tag.sock")
    }
    ...
    func testInitialStableLaunchTreatsPrivateTmpLegacyStableAliasAsStablePath() {
        let path = SocketControlSettings.initialSocketPathBeforeListenerStart(
            preferredPath: "/private/tmp/mosaic.sock",
            bundleIdentifier: "mosaic.com.emergent.app",
            isDebugBuild: false,
            currentUserID: 501,
            probeStableDefaultPathEntry: { socketPath in
                XCTAssertEqual(socketPath, "/private/tmp/mosaic.sock")
    ...
    func testInitialSocketPathDoesNotProbeForTaggedDebugBuild() {
        let debugPath = "/tmp/mosaic-debug-tag.sock"
        let path = SocketControlSettings.initialSocketPathBeforeListenerStart(
    ...
    func testUntaggedDebugBundleAllowedWithLaunchTag() {
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: ["MOSAIC_TAG": "tests-v1"],
                bundleIdentifier: "mosaic.com.emergent.app.debug",
                isDebugBuild: true
            )
        )
    ...
    func testXCUITestLaunchEnvironmentIgnoresLaunchTagGate() {
        ...
        // The app receives MOSAIC_UI_TEST_* vars via XCUIApplication.launchEnvironment.
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: ["MOSAIC_UI_TEST_MODE": "1"],
                bundleIdentifier: "mosaic.com.emergent.app.debug",
                isDebugBuild: true
            )
        )
...
final class UITestLaunchManifestTests: XCTestCase {
    func testManifestPathReadsArgumentValue() {
        XCTAssertEqual(
            UITestLaunchManifest.manifestPath(
                from: ["mosaic", "-mosaicUITestLaunchManifest", "/tmp/mosaic-ui-test-launch.json"]
            ),
            "/tmp/mosaic-ui-test-launch.json"
        )
    }
    ...
    func testManifestPathReturnsNilWithoutValue() {
        XCTAssertNil(
            UITestLaunchManifest.manifestPath(
                from: ["mosaic", "-mosaicUITestLaunchManifest"]
            )
        )
    }
    ...
    func testApplyIfPresentDecodesEnvironmentPayload() {
        let payload = """
        {"environment":{"MOSAIC_TAG":"ui-tests-display","MOSAIC_SOCKET_PATH":"/tmp/mosaic-ui-tests.sock"}}
        """.data(using: .utf8)!
        var applied: [String: String] = [:]

        UITestLaunchManifest.applyIfPresent(
            arguments: ["mosaic", UITestLaunchManifest.argumentName, "/tmp/mosaic-ui-test-launch.json"],
        ...
        XCTAssertEqual(applied["MOSAIC_TAG"], "ui-tests-display")
        XCTAssertEqual(applied["MOSAIC_SOCKET_PATH"], "/tmp/mosaic-ui-tests.sock")
    }
}
...
final class GhosttyMouseFocusTests: XCTestCase {
    ...
    private func withTempConfig(
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-cjk-\(UUID().uuidString)")
    ...
    func testUserConfigContainsCJKCodepointMapReturnsFalseForMissingFiles() {
        let path = NSTemporaryDirectory() + "mosaic-nonexistent-\(UUID().uuidString)/config"
        XCTAssertFalse(
    ...
    func testUserConfigContainsCJKCodepointMapFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-cjk-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapFollowsRelativeIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-cjk-rel-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesOptionalInclude() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-cjk-opt-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesCyclicIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-cjk-cycle-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-cjk-font-family-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainIgnoresDuplicateFamilies() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-cjk-font-family-duplicate-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainMatchesGhosttyIncludeLoadOrder() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-cjk-font-family-order-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainRespectsConfigFileReset() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-cjk-font-family-config-file-reset-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testLoadedCJKScanPathsIncludesNativeGhosttyAppSupportWhenTaggedConfigExists() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-cjk-app-support-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }

        let taggedDir = appSupport.appendingPathComponent("com.example.mosaic-dev", isDirectory: true)
        ...
        let paths = GhosttyApp.loadedCJKScanPaths(
            currentBundleIdentifier: "com.example.mosaic-dev",
            appSupportDirectory: appSupport
        )
    ...
    func testShouldApplyManagedDefaultAppearanceScansNativeGhosttyAppSupport() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-appearance-app-support-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }
    ...
    func testLoadedGhosttyConfigScanPathsSkipsNativeLegacyConfigWhenCurrentConfigIsNonEmpty() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-appearance-app-support-current-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }
    ...
    // Regression: https://github.com/emergent-inc/mosaic/issues/3459
    // `mosaic themes set` always encodes the selection with conditional
    ...
    // stay at the white defaults), so mosaic must inject the resolved plain theme
    ...
    // Regression: https://github.com/emergent-inc/mosaic/issues/3459
    // `mosaic themes set --light X` encodes `theme = light:X`, which ghostty treats
    // as conditional config. mosaic injects the resolved plain theme for the
    ...
    func testShouldApplyManagedDefaultAppearanceFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-theme-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testShouldApplyManagedDefaultAppearancePreservesQuotedQuestionMarkConfigFile() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-theme-quoted-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testShouldApplyManagedDefaultAppearanceProcessesIncludeQueuedAfterReset() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-test-theme-reset-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testStartupAppearanceFreshInstallPreviewUsesEnforcedTheme() {
        ...
        XCTAssertEqual(config.theme, GhosttyConfig.mosaicEnforcedThemeName)
...
final class ZshShellIntegrationHandoffTests: XCTestCase {
    func testGhosttyPromptHooksLoadWhenMosaicRequestsZshIntegration() throws {
        let output = try runInteractiveZsh(mosaicLoadGhosttyIntegration: true)
    ...
    func testGhosttyPromptHooksDoNotLoadWithoutMosaicHandoffFlag() throws {
        let output = try runInteractiveZsh(mosaicLoadGhosttyIntegration: false)
    ...
    func testGhosttySemanticPatchRetriesAfterDeferredInitCreatesLiveHooks() throws {
        let output = try runInteractiveZsh(
            mosaicLoadGhosttyIntegration: true,
            mosaicLoadShellIntegration: true,
            command: """
    ...
    func testShellIntegrationWinchGuardDoesNotPrintSpacerLineOnResize() throws {
        let output = try runInteractiveZsh(
            mosaicLoadGhosttyIntegration: false,
            mosaicLoadShellIntegration: true,
            command: """
            print -r -- BEFORE
            TRAPWINCH
    ...
    func testShellIntegrationPreservesStartupTermForThemeSelectionBeforeRestoringManagedTerm() throws {
        let output = try runPromptInteractiveZsh(
            ...
            mosaicLoadShellIntegration: true,
            command: """
            print -r -- "CMD=$TERM|${MOSAIC_ZSH_RESTORE_TERM-unset}" >> "$MOSAIC_TEST_OUTPUT"
            """,
            userZshRCContents: """
            export MOSAIC_STARTUP_THEME_TERM="$TERM"
            if [[ $TERM = (*256color|*rxvt*) ]]; then
              export MOSAIC_STARTUP_THEME_BRANCH=extended
            else
              export MOSAIC_STARTUP_THEME_BRANCH=basic
            fi

            mosaic_test_ready() {
              [[ -e "$MOSAIC_TEST_READY" ]] && return 0
              print -r -- "PRE=$MOSAIC_STARTUP_THEME_TERM|$MOSAIC_STARTUP_THEME_BRANCH|$TERM|${MOSAIC_ZSH_RESTORE_TERM-unset}" > "$MOSAIC_TEST_OUTPUT"
              : > "$MOSAIC_TEST_READY"
              precmd_functions=(${precmd_functions:#mosaic_test_ready})
            }
            precmd_functions+=(mosaic_test_ready)
            """
        )
    ...
    func testShellIntegrationDoesNotSpoofManagedTermForInteractiveCommandMode() throws {
        let output = try runInteractiveZsh(
            ...
            mosaicLoadShellIntegration: true,
            command: """
            print -r -- "$MOSAIC_STARTUP_TERM|$TERM|${MOSAIC_ZSH_RESTORE_TERM-unset}"
            """,
            userZshRCContents: """
            export MOSAIC_STARTUP_TERM="$TERM"
            """
        )
    ...
    func testShellIntegrationDoesNotSpoofManagedTermWhenIntegrationDisabled() throws {
        let output = try runInteractiveZsh(
            mosaicLoadGhosttyIntegration: false,
            mosaicLoadShellIntegration: false,
            command: """
            print -r -- "$MOSAIC_STARTUP_TERM|$TERM|${MOSAIC_ZSH_RESTORE_TERM-unset}"
            """,
            userZshRCContents: """
    ...
    func testShellIntegrationDoesNotSpoofManagedTermWhenUserZshEnvDisablesIntegration() throws {
        let output = try runInteractiveZsh(
            ...
            mosaicLoadShellIntegration: true,
            command: """
            print -r -- "$MOSAIC_STARTUP_TERM|$TERM|${MOSAIC_ZSH_RESTORE_TERM-unset}|${MOSAIC_SHELL_INTEGRATION:-unset}"
            """,
            userZshEnvContents: """
            export MOSAIC_SHELL_INTEGRATION=0
            """,
            userZshRCContents: """
            export MOSAIC_STARTUP_TERM="$TERM"
            """
    ...
    func testShellIntegrationNormalizesClaudeConfigDirAfterUserZshrc() throws {
        let output = try runPromptInteractiveZsh(
            ...
            mosaicLoadShellIntegration: true,
            command: """
            print -r -- "CMD=$CLAUDE_CONFIG_DIR" >> "$MOSAIC_TEST_OUTPUT"
            """,
            userZshRCContents: """
            ...
            mosaic_test_ready() {
              [[ -e "$MOSAIC_TEST_READY" ]] && return 0
              print -r -- "PRE=$CLAUDE_CONFIG_DIR" > "$MOSAIC_TEST_OUTPUT"
              : > "$MOSAIC_TEST_READY"
              precmd_functions=(${precmd_functions:#mosaic_test_ready})
            }
            precmd_functions+=(mosaic_test_ready)
    ...
    func testShellIntegrationDoesNotRegisterPromptTimeTermRestoreHooks() throws {
        ...
        XCTAssertEqual(
            output,
            "_mosaic_precmd,_mosaic_fix_path",
            output
        )
    }
    ...
    func testShellIntegrationRestoresManagedTermDuringPreexec() throws {
        let output = try runInteractiveZsh(
            ...
            mosaicLoadShellIntegration: true,
            command: """
            _mosaic_preexec 'echo $TERM'
            print -r -- "$TERM|${MOSAIC_ZSH_RESTORE_TERM-unset}"
            """,
            extraEnvironment: [
                "TERM": "xterm-ghostty",
                "MOSAIC_ZSH_RESTORE_TERM": "xterm-256color",
            ]
        )
    ...
    func testShellIntegrationPublishesOnlyWorkspaceScopedMosaicEnvironmentToTmuxServerAutomatically() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-zsh-tmux-publish-\(UUID().uuidString)")
        ...
        _ = try runInteractiveZsh(
            mosaicLoadGhosttyIntegration: false,
            mosaicLoadShellIntegration: true,
            command: "_mosaic_preexec tmux; print -r -- READY",
            extraEnvironment: [
                "PATH": "\(binDir.path):/usr/bin:/bin:/usr/sbin:/sbin",
                "MOSAIC_SOCKET_PATH": "/tmp/mosaic-current.sock",
                "MOSAIC_TAG": "feat-tmux-notification-attention-state",
                "MOSAIC_WORKSPACE_ID": "11111111-1111-1111-1111-111111111111",
                "MOSAIC_SURFACE_ID": "22222222-2222-2222-2222-222222222222",
                "MOSAIC_TAB_ID": "11111111-1111-1111-1111-111111111111",
                "MOSAIC_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )

        let log = (try? String(contentsOf: logPath, encoding: .utf8)) ?? ""
        XCTAssertTrue(log.contains("set-environment -g MOSAIC_TAG feat-tmux-notification-attention-state"), log)
        XCTAssertTrue(log.contains("set-environment -g MOSAIC_SOCKET_PATH /tmp/mosaic-current.sock"), log)
        XCTAssertTrue(log.contains("set-environment -g MOSAIC_WORKSPACE_ID 11111111-1111-1111-1111-111111111111"), log)
        XCTAssertFalse(log.contains("set-environment -g MOSAIC_SURFACE_ID"), log)
        XCTAssertFalse(log.contains("set-environment -g MOSAIC_PANEL_ID"), log)
    }

    func testShellIntegrationClearsStaleSurfaceScopedTmuxEnvironmentAutomatically() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-zsh-tmux-clear-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("tmux.log", isDirectory: false)
        ...
        try writeExecutableScript(
            ...
            #!/bin/sh
            if [ "$1" = "show-environment" ] && [ "$2" = "-g" ]; then
              printf '%s\\n' 'MOSAIC_SURFACE_ID=99999999-9999-9999-9999-999999999999'
              printf '%s\\n' 'MOSAIC_PANEL_ID=99999999-9999-9999-9999-999999999999'
              exit 0
            fi
            printf '%s\\n' "$*" >> "\(logPath.path)"
            exit 0
        ...
        let log = (try? String(contentsOf: logPath, encoding: .utf8)) ?? ""
        XCTAssertTrue(log.contains("set-environment -gu MOSAIC_SURFACE_ID"), log)
        XCTAssertTrue(log.contains("set-environment -gu MOSAIC_PANEL_ID"), log)
    }

    func testShellIntegrationRefreshesWorkspaceScopedMosaicEnvironmentFromTmuxWithoutOverwritingSurfaceScope() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-zsh-tmux-refresh-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        ...
        let output = try runInteractiveZsh(
            mosaicLoadGhosttyIntegration: false,
            mosaicLoadShellIntegration: true,
            command: "_mosaic_precmd; print -r -- \"$MOSAIC_TAG|$MOSAIC_SOCKET_PATH|$MOSAIC_WORKSPACE_ID|$MOSAIC_SURFACE_ID|$MOSAIC_PANEL_ID\"",
            extraEnvironment: [
                "PATH": "\(binDir.path):/usr/bin:/bin:/usr/sbin:/sbin",
                "TMUX": "/tmp/tmux-stale,123,0",
                "MOSAIC_SOCKET_PATH": "/tmp/mosaic-stale.sock",
                "MOSAIC_TAG": "feat-tmux-integration-experiments",
                "MOSAIC_WORKSPACE_ID": "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA",
                "MOSAIC_SURFACE_ID": "22222222-2222-2222-2222-222222222222",
                "MOSAIC_TAB_ID": "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA",
                "MOSAIC_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )
        ...
        XCTAssertEqual(
            output,
            "feat-tmux-notification-attention-state|/tmp/mosaic-current.sock|11111111-1111-1111-1111-111111111111|22222222-2222-2222-2222-222222222222|22222222-2222-2222-2222-222222222222"
        )
    }
    ...
    func testShellIntegrationReportsTTYFromTmuxWithoutUsingPanelScope() throws {
        let output = try runInteractiveZsh(
            ...
            mosaicLoadShellIntegration: true,
            command: """
            _MOSAIC_TTY_NAME=ttys999
            print -r -- "$(_mosaic_report_tty_payload)"
            """,
            extraEnvironment: [
    ...
    func testShellIntegrationRelayReportTTYUsesWorkspaceIDInZsh() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-zsh-relay-report-tty-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
    ...
    func testShellIntegrationRelayPortsKickOmitsSurfaceIDUntilAvailableInZsh() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-zsh-relay-kick-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        let output = try runInteractiveZsh(
            ...
            command: """
            : > "\(logPath.path)"
            _mosaic_ports_kick_via_relay refresh
            repeat 20; do
    ...
    func testShellIntegrationRelayPromptRefreshUsesRefreshReasonInZsh() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-zsh-relay-precmd-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
    ...
    func testShellIntegrationRelayReportTTYUsesWorkspaceIDInBash() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-bash-relay-report-tty-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
    ...
    func testShellIntegrationRelayPreexecWorksBeforeSurfaceIDExistsInBash() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-bash-relay-preexec-no-surface-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        let result = try runInteractiveBash(
            ...
            command: """
            : > "\(logPath.path)"
            _MOSAIC_TTY_NAME=ttys889
            _MOSAIC_TTY_REPORTED=0
            _mosaic_preexec_command "python3 -m http.server 8899"
    ...
    func testShellIntegrationRelayPromptRefreshUsesRefreshReasonInBashWithoutPromptNoise() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-bash-relay-prompt-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        XCTAssertFalse(result.stderr.contains("_mosaic_report_tmux_state"), result.stderr)
        XCTAssertTrue(
    ...
    func testBashNoGitWatchSkipsHeadTrackingAndPRClear() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-bash-no-git-watch-\(UUID().uuidString)")
        ...
        let socketPath = root.appendingPathComponent("mosaic-test.sock", isDirectory: false)
        ...
        let result = try runInteractiveBash(
            ...
            _MOSAIC_GIT_HEAD_PATH="$PWD/.git/HEAD"
            _MOSAIC_GIT_HEAD_SIGNATURE="$(_mosaic_git_head_signature "$_MOSAIC_GIT_HEAD_PATH")"
            printf '%s\\n' 'ref: refs/heads/old-cleared' > "$_MOSAIC_GIT_HEAD_PATH"
            cd "\(repoB.path)"
            _MOSAIC_PWD_LAST_PWD="$PWD"
            _MOSAIC_LAST_PR_ACTION="checkout"
            _MOSAIC_LAST_PR_TARGET="feature"
            _mosaic_prompt_command
            ...
            """,
            extraEnvironment: [
                "MOSAIC_NO_GIT_WATCH": "1",
                "MOSAIC_SOCKET_PATH": socketPath.path,
                "MOSAIC_TAB_ID": "22222222-2222-2222-2222-222222222222",
                "MOSAIC_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )
    ...
    func testZshNoGitWatchSkipsHeadTrackingAndPRClear() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-zsh-no-git-watch-\(UUID().uuidString)")
        let repoA = root.appendingPathComponent("repo-a", isDirectory: true)
        let repoB = root.appendingPathComponent("repo-b", isDirectory: true)
        let logPath = root.appendingPathComponent("send.log", isDirectory: false)
        let socketPath = root.appendingPathComponent("mosaic-test.sock", isDirectory: false)
    ...
    func testZshNoPullRequestWatchSkipsLegacyGhPRProbe() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-zsh-no-pr-watch-\(UUID().uuidString)")
        ...
        try writeExecutableScript(
            ...
            contents: """
            #!/bin/sh
            printf invoked > "$MOSAIC_GH_MARKER"
            printf '2746\\tOPEN\\thttps://github.com/emergent-inc/mosaic/pull/2746\\n'
            """
        )
        let socketFD = try bindUnixSocket(at: socketPath.path)
        defer {
        ...
        let output = try runInteractiveZsh(
            ...
            _mosaic_send() { :; }
            _mosaic_send_bg() { :; }
            _mosaic_report_pr_for_path "\(repoURL.path)" || true
            [[ -e "\(markerURL.path)" ]] && print MARKER=1 || print MARKER=0
            """,
            extraEnvironment: [
                "MOSAIC_NO_PR_WATCH": "1",
                "MOSAIC_GH_MARKER": markerURL.path,
                "MOSAIC_SOCKET_PATH": socketPath.path,
    ...
    func testBashNoPullRequestWatchSkipsLegacyGhPRProbe() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-bash-no-pr-watch-\(UUID().uuidString)")
        let repoURL = root.appendingPathComponent("repo", isDirectory: true)
        let fakeBinURL = root.appendingPathComponent("fake-bin", isDirectory: true)
        let markerURL = root.appendingPathComponent("gh-pr-invoked", isDirectory: false)
        let socketPath = root.appendingPathComponent("mosaic-test.sock", isDirectory: false)
    ...
    func testZshPromptResetsTerminalKeyboardProtocols() throws {
        let output = try runInteractiveZsh(
            ...
            extraEnvironment: [
                "MOSAIC_SOCKET_PATH": "127.0.0.1:64011",
                "MOSAIC_BUNDLED_CLI_PATH": "/usr/bin/true",
                "MOSAIC_WORKSPACE_ID": "11111111-1111-1111-1111-111111111111",
                "MOSAIC_TAB_ID": "22222222-2222-2222-2222-222222222222",
                "MOSAIC_PANEL_ID": "22222222-2222-2222-2222-222222222222",
                "MOSAIC_TEST_FORCE_KITTY_RESET": "1",
            ]
        )
    ...
    private func runInteractiveZsh(mosaicLoadGhosttyIntegration: Bool) throws -> String {
        try runInteractiveZsh(
            mosaicLoadGhosttyIntegration: mosaicLoadGhosttyIntegration,
            mosaicLoadShellIntegration: false,
    ...
    private func runInteractiveZsh(
        mosaicLoadGhosttyIntegration: Bool,
        mosaicLoadShellIntegration: Bool,
        command: String,
        ...
        process.environment = [
            ...
            "MOSAIC_SHELL_INTEGRATION": "0",
            "GHOSTTY_RESOURCES_DIR": ghosttyResources.path,
        ]
        if mosaicLoadGhosttyIntegration {
            process.environment?["MOSAIC_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        }
        if mosaicLoadShellIntegration {
            process.environment?["MOSAIC_SHELL_INTEGRATION"] = "1"
            process.environment?["MOSAIC_SHELL_INTEGRATION_DIR"] = mosaicZdotdir.path
            process.environment?["MOSAIC_SOCKET_PATH"] = root.appendingPathComponent("mosaic-test.sock").path
            process.environment?["MOSAIC_TAB_ID"] = "tab-test"
            process.environment?["MOSAIC_PANEL_ID"] = "panel-test"
        }
    ...
    private func runPromptInteractiveZsh(
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-zsh-prompt-integration-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        process.environment = [
            ...
            "MOSAIC_SHELL_INTEGRATION": "0",
            "GHOSTTY_RESOURCES_DIR": ghosttyResources.path,
            "MOSAIC_TEST_READY": readyPath.path,
            "MOSAIC_TEST_OUTPUT": outputPath.path,
        ]
        if mosaicLoadGhosttyIntegration {
            process.environment?["MOSAIC_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        }
    ...
    private func runInteractiveBash(
        ...
        let rcfileContents: String = {
            guard mosaicLoadShellIntegration else { return ":\n" }
            return """
...
final class BrowserInstallDetectorTests: XCTestCase {
    ...
    private func makeTemporaryHome() -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent("mosaic-browser-detect-\(UUID().uuidString)")
    }
