...
/// Behavior tests for the remote-tmux SSH auth path that backs `mosaic ssh-tmux`:
...
@Suite struct RemoteTmuxAuthTests {
    ...
    @Test func controlModeArgumentsUseRemoteTmuxResolverAfterDestinationGuard() throws {
        ...
        #expect(remoteCommand.hasSuffix("'mosaic-remote-tmux' '-CC' 'attach-session' '-t' 'work session'"))
    }
    ...
    @Test @MainActor func layoutChangePrunesRemovedPaneDiagnosticState() {
        ...
        connection.handleMessageForTesting(.subscriptionChanged(name: "mosaic_reflow_4", value: "0|zsh"))
        connection.handleMessageForTesting(.subscriptionChanged(name: "mosaic_reflow_5", value: "1|vim"))
    ...
    @Test func pastePaneCommandsProtectOptionLookingText() throws {
        ...
        #expect(commands.setBuffer == "set-buffer -b mosaic-paste-7 -- '-n not-an-option'")
        #expect(commands.pasteBuffer == "paste-buffer -p -d -b mosaic-paste-7 -t %7")
    }
    ...
    // MARK: - Interactive auth invocation (what `mosaic ssh-tmux` runs in the tty)
