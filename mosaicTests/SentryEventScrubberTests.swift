...
@Suite struct SentryEventScrubberTests {
    ...
    @Test func scrubsContextDictionaries() {
        let event = Event()
        // Mirrors scope.setContext(value:key:): mosaic puts cwd / path data here.
        event.context = [
            "ui": ["cwd": "/Users/lawrence/dev/mosaic", "action": "open"],
            "auth": ["token": "abcdef0123456789secretvalue"],
        ]

        let scrubbed = scrubber.scrub(event)
        #expect(scrubbed.context?["ui"]?["cwd"] as? String == "/Users/<redacted>/dev/mosaic")
    ...
    @Test func scrubsStackFramePathsContextAndVarsButKeepsSymbols() {
        let event = Event()
        let frame = Frame()
        frame.fileName = "/Users/buildbot/work/mosaic/Sources/AppDelegate.swift"
        frame.function = "applicationDidFinishLaunching(_:)"
        frame.package = "/Users/buildbot/Library/Developer/Xcode/DerivedData/mosaic/Build/mosaic.app"
        ...
        #expect(outFrame?.fileName == "/Users/<redacted>/work/mosaic/Sources/AppDelegate.swift")
    ...
    @Test func scrubsDebugMetaCodeFilePaths() {
        let event = Event()
        let image = DebugMeta()
        image.codeFile = "/Users/lawrence/Library/Developer/Xcode/DerivedData/mosaic/mosaic.app/mosaic"
