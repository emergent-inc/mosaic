...
import struct MosaicSettings.IntegrationsCatalogSection
import enum MosaicSettings.KiroNotificationLevel
...
final class KeyboardShortcutSettingsFileStoreTests: XCTestCase {
    ...
    func testSettingsFileStoreUsesLegacyFallbackWhenCanonicalConfigHasNoSetting() throws {
        ...
        try writeSettingsFile(
            """
            {
              "$schema": "https://raw.githubusercontent.com/emergent-inc/mosaic/main/web/data/mosaic-settings.schema.json",
              "shortcuts": {
    ...
    @MainActor
    func testReloadConfigurationMenuActionReloadsRegisteredMosaicConfigStore() throws {
#if DEBUG
        let directoryURL = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directoryURL) }
        ...
        XCTAssertNotNil(mosaicConfigStore.resolvedAction(id: "first"))
        XCTAssertNil(mosaicConfigStore.resolvedAction(id: "second"))
        ...
        let appMenuItem = NSMenuItem(title: "mosaic", action: nil, keyEquivalent: "")
        let appMenu = NSMenu(title: "mosaic")
        ...
        XCTAssertEqual(
            ...
            NSUserInterfaceItemIdentifier("com.mosaic.reloadConfiguration")
        )
        ...
        XCTAssertNil(mosaicConfigStore.resolvedAction(id: "first"))
        XCTAssertNotNil(mosaicConfigStore.resolvedAction(id: "second"))
#else
    ...
    func testBootstrapCreatesCommentedTemplateWhenPrimaryAndFallbackAreMissing() throws {
        ...
        let settingsFileURL = directoryURL
            .appendingPathComponent(".config/mosaic", isDirectory: true)
        ...
        XCTAssertTrue(contents.contains(#""$schema": "https://raw.githubusercontent.com/emergent-inc/mosaic/main/web/data/mosaic.schema.json""#))
    ...
    func testSettingsFileURLForEditingPrefersInvalidPrimaryForRepair() throws {
        ...
        let primaryURL = directoryURL.appendingPathComponent("primary/mosaic.json", isDirectory: false)
        let fallbackURL = directoryURL.appendingPathComponent("fallback/mosaic.json", isDirectory: false)
...
final class StoredShortcutMatchingTests: XCTestCase {
    ...
    func testSystemWideHotkeyNormalizationReportsMosaicActionConflictByRecordedPhysicalKey() {
        KeyboardShortcutSettings.resetAll()
        defer { KeyboardShortcutSettings.resetAll() }
...
final class WorkspaceCreationWorkingDirectoryInheritanceTests: XCTestCase {
    ...
    func testNewWorkspaceInheritsSourceWorkingDirectoryByDefault() throws {
        try withWorkspaceWorkingDirectoryInheritanceSetting(nil) {
            let sourceCwd = "/tmp/mosaic-source-\(UUID().uuidString)"
            let manager = TabManager(
    ...
    func testExplicitWorkspaceWorkingDirectoryWinsWhenInheritanceIsDisabled() throws {
        try withWorkspaceWorkingDirectoryInheritanceSetting(false) {
            let sourceCwd = "/tmp/mosaic-source-\(UUID().uuidString)"
            let explicitCwd = "/tmp/mosaic-explicit-\(UUID().uuidString)"
            let manager = TabManager(
                initialWorkingDirectory: sourceCwd,
                autoWelcomeIfNeeded: false
            )
    ...
    func testDetachedWorkspaceTransferDirectoryWinsWhenInheritanceIsDisabled() throws {
        try withWorkspaceWorkingDirectoryInheritanceSetting(false) {
            let sourceCwd = "/tmp/mosaic-source-\(UUID().uuidString)"
            let transferCwd = "/tmp/mosaic-detached-\(UUID().uuidString)"
            let manager = TabManager(
                initialWorkingDirectory: sourceCwd,
                autoWelcomeIfNeeded: false
            )
    ...
    func testDetachedWorkspaceDoesNotPersistProcessDetectedResumeBinding() throws {
        let manager = TabManager(
            initialWorkingDirectory: "/tmp/mosaic-source-\(UUID().uuidString)",
            autoWelcomeIfNeeded: false
        )
        let source = try XCTUnwrap(manager.selectedWorkspace)
...
final class WorkspaceCreationConfigSanitizationTests: XCTestCase {
    private final class UnsafeConfigSnapshotTabManager: TabManager {
        private var injectedConfig: MosaicSurfaceConfigTemplate?
        var capturedConfigTemplate: MosaicSurfaceConfigTemplate?

        func installInjectedConfig(fontSize: Float) {
            var config = MosaicSurfaceConfigTemplate()
            config.fontSize = fontSize
            config.workingDirectory = "/tmp/mosaic-workspace-snapshot"
            config.command = "echo snapshot"
            config.environmentVariables = ["MOSAIC_INHERITED_ENV": "1"]
...
final class WorkspaceReorderTests: XCTestCase {
    ...
    func testMoveTabToTopPublishesWorkspaceReorderedEvent() throws {
        MosaicEventBus.shared.resetForTesting()
        defer { MosaicEventBus.shared.resetForTesting() }
        ...
        let event = try XCTUnwrap(MosaicEventBus.shared.retainedSnapshot().last)
...
final class WorkspaceSplitWorkingDirectoryTests: XCTestCase {
    ...
    func testNewTerminalSplitFallsBackToRequestedWorkingDirectoryWhenReportedDirectoryIsStale() {
        ...
        let requestedDirectory = "/tmp/mosaic-requested-split-cwd-\(UUID().uuidString)"
...
final class WorkspacePanelGitBranchTests: XCTestCase {
    ...
    func testForkAgentConversationInRemoteWorkspaceUsesRemoteStartupCommand() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/mosaic-fork-remote.sock",
        ...
        XCTAssertEqual(forkPanel.surface.debugInitialCommand(), "ssh mosaic-macmini")
        XCTAssertNil(forkPanel.requestedWorkingDirectory)
        XCTAssertEqual(workspace.panelDirectories[forkPanel.id], "/Users/mosaic/project")
        XCTAssertEqual(forkPanel.surface.initialInput, snapshot.forkCommand.map { $0 + "\n" })
    ...
    func testForkAgentConversationInRemoteWorkspaceUsesFallbackDirectoryInForkCommand() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/mosaic-fork-fallback-remote.sock",
                terminalStartupCommand: "ssh mosaic-macmini"
            ),
            autoConnect: false
        )
        workspace.currentDirectory = "/Users/mosaic/fallback repo"
        let sourcePanelId = try XCTUnwrap(workspace.focusedPanelId)
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .codex,
            sessionId: "019dad34-d218-7943-b81a-eddac5c87951",
        ...
        XCTAssertEqual(forkPanel.surface.debugInitialCommand(), "ssh mosaic-macmini")
        XCTAssertNil(forkPanel.requestedWorkingDirectory)
        XCTAssertEqual(workspace.panelDirectories[forkPanel.id], "/Users/mosaic/fallback repo")
        XCTAssertEqual(
            forkPanel.surface.initialInput,
            "cd -- '/Users/mosaic/fallback repo' 2>/dev/null || [ ! -d '/Users/mosaic/fallback repo' ] && '/Users/example/.bun/bin/codex' 'fork' '019dad34-d218-7943-b81a-eddac5c87951'\n"
        )
    }
    ...
    func testSessionIndexRemoteSplitDoesNotInjectRemoteStartupCommand() throws {
        ...
        let hookStateRoot = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-session-drop-hook-state-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: hookStateRoot, withIntermediateDirectories: true)
        let previousHookStateDir = getenv("MOSAIC_AGENT_HOOK_STATE_DIR").map { String(cString: $0) }
        setenv("MOSAIC_AGENT_HOOK_STATE_DIR", hookStateRoot.path, 1)
        defer {
            if let previousHookStateDir {
                setenv("MOSAIC_AGENT_HOOK_STATE_DIR", previousHookStateDir, 1)
            } else {
                unsetenv("MOSAIC_AGENT_HOOK_STATE_DIR")
            }
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/mosaic-session-drop-remote.sock",
                terminalStartupCommand: "ssh mosaic-macmini"
            ),
            autoConnect: false
        )
    ...
    func testForkAgentWorkspaceLaunchInRemoteWorkspacePreservesRemoteContext() throws {
        let workspace = Workspace()
        let agentSocketPath = "/tmp/mosaic-fork-agent.sock"
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                destination: "mosaic-macmini",
                port: 2222,
                identityFile: "/Users/example/.ssh/mosaic",
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/mosaic-fork-remote.sock",
                terminalStartupCommand: "ssh -p 2222 -i /Users/example/.ssh/mosaic -o ServerAliveInterval=30 -o ForwardAgent=yes -tt mosaic-macmini",
        ...
        XCTAssertEqual(launch.workingDirectory, "/Users/mosaic/project")
        XCTAssertNil(launch.terminalWorkingDirectory)
        XCTAssertEqual(
            launch.initialTerminalCommand,
            "ssh -p 2222 -i /Users/example/.ssh/mosaic -o ServerAliveInterval=30 -o ForwardAgent=yes -tt mosaic-macmini"
        )
        ...
        XCTAssertEqual(launch.remoteConfiguration?.destination, "mosaic-macmini")
        XCTAssertEqual(launch.remoteConfiguration?.port, 2222)
        XCTAssertEqual(launch.remoteConfiguration?.identityFile, "/Users/example/.ssh/mosaic")
    ...
    func testForkAgentWorkspaceLaunchFromPersistentSSHPTYDoesNotReuseParentRelayOrDaemonSlot() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/mosaic-fork-persistent.sock",
        ...
        XCTAssertEqual(
            startupCommand,
            "ssh -p 2222 -i /Users/example/.ssh/mosaic -tt mosaic-macmini"
        )
    }
    ...
    func testForkAgentWorkspaceLaunchInRemoteWorkspaceUsesFallbackDirectoryInForkCommand() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/mosaic-workspace-fallback-remote.sock",
                terminalStartupCommand: "ssh mosaic-macmini"
            ),
            autoConnect: false
        )
        ...
        XCTAssertEqual(launch.workingDirectory, "/Users/mosaic/fallback repo")
        XCTAssertNil(launch.terminalWorkingDirectory)
        XCTAssertEqual(launch.initialTerminalCommand, "ssh -tt mosaic-macmini")
        XCTAssertEqual(
    ...
    func testForkAgentConversationInRemoteConfiguredLocalWorkspaceAllowsLauncherScript() throws {
        ...
        XCTAssertEqual(forkPanel.requestedWorkingDirectory, "/Users/mosaic/project")
        ...
        XCTAssertEqual(launch.terminalWorkingDirectory, "/Users/mosaic/project")
        XCTAssertNil(launch.initialTerminalCommand)
        XCTAssertFalse(launch.autoConnectRemoteConfiguration)
        XCTAssertNil(launch.remoteConfiguration)
    ...
    func testForkAgentConversationFromLocalTerminalInRemoteWorkspaceStaysLocal() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/mosaic-fork-local-remote.sock",
                terminalStartupCommand: "ssh mosaic-macmini"
            ),
            autoConnect: false
        )
    ...
    func testUpdatingFocusedPanelPullRequestWithSameStateDoesNotRepublishWorkspace() {
        ...
        let pullRequestURL = URL(string: "https://github.com/emergent-inc/mosaic/pull/2388")!
    ...
    func testSidebarPullRequestsTrackFocusedPanelOnly() {
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 1629,
            label: "PR",
            url: URL(string: "https://github.com/emergent-inc/mosaic/pull/1629")!,
            status: .open
        )
    ...
    func testSidebarDerivedCollectionsMatchWhenUsingPrecomputedPanelOrder() {
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 101,
            label: "PR",
            url: URL(string: "https://github.com/emergent-inc/mosaic/pull/101")!,
            status: .open
        )
        workspace.updatePanelPullRequest(
            ...
            number: 18,
            label: "MR",
            url: URL(string: "https://gitlab.com/emergentinc/mosaic/-/merge_requests/18")!,
            status: .merged
        )
    ...
    func testForkConversationDefaultSettingFallsBackToRight() throws {
        let suiteName = "mosaic.forkConversationDefault.\(UUID().uuidString)"
...
final class MosaicVisibleBrandingTests: XCTestCase {
    func testHelpMenuResourcesUseMosaicDestinations() {
        ...
        XCTAssertEqual(MosaicHelpResource.githubIssues.url, MosaicBranding.githubIssuesURL)
        XCTAssertEqual(MosaicHelpResource.discord.url.absoluteString, "https://discord.gg/zmWHDeZffZ")
    }
    ...
    func testRebrandedRuntimeLabelsUseMosaic() {
        ...
        XCTAssertEqual(SocketControlMode.mosaicOnly.displayName, "mosaic processes only")
        XCTAssertEqual(
            SocketControlMode.mosaicOnly.description,
...
final class ExtensionWorktreePrototypeTests: XCTestCase {
    func testPipeOutputCollectorDrainsBufferedOutputOnFinish() async throws {
        let pipe = Pipe()
        let collector = MosaicExtensionPipeOutputCollector(fileHandle: pipe.fileHandleForReading)
    ...
    func testCreateWorktreeKeepsMosaicDirectoryLocallyIgnored() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-worktree-prototype-\(UUID().uuidString)", isDirectory: true)
        ...
        _ = try runGit([
            ...
            "-c", "user.email=mosaic@example.invalid",
            "commit",
            "-m",
            "initial"
        ], in: projectRoot)

        let result = try await MosaicExtensionWorktreePrototype.createWorktree(projectRootPath: projectRoot.path)

        XCTAssertTrue(FileManager.default.fileExists(atPath: result.worktreePath))
        XCTAssertTrue(result.workspaceTitle.hasPrefix("mosaic-sidebar-"))
