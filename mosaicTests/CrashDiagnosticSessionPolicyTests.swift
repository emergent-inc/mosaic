...
struct CrashDiagnosticSessionPolicyTests {
    @Test
    func terminalDefaultFileOpenIgnoresGhosttyCrashReportsInMosaicCrashDirectory() {
        let crashReport = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".local/state/mosaic/crash/mosaic.ghosttycrash", isDirectory: false)
    ...
    @Test
    func terminalDefaultFileOpenIgnoresSymlinkedGhosttyCrashReportsInMosaicCrashDirectory() throws {
        let crashReport = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".local/state/mosaic/crash/mosaic.ghosttycrash", isDirectory: false)
        let symlink = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-symlinked-crash-\(UUID().uuidString).ghosttycrash", isDirectory: false)
    ...
    func appDelegateSessionSnapshotDropsCrashDiagnosticWindow() throws {
        ...
        let projectDirectory = "/tmp/mosaic-project"
        ...
        let crashDirectory = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".local/state/mosaic/crash", isDirectory: true)
            .path
    ...
    func sessionSnapshotDropsEmptyCrashDiagnosticWorkspace() {
        ...
        let pruned = SessionPersistencePolicy.pruningMosaicCrashDiagnosticWindows(from: snapshot)
    ...
    func sessionSnapshotPruningDoesNotResolveSymlinkedCrashDirectories() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-crash-storage-symlink-\(UUID().uuidString)", isDirectory: true)
        ...
        #expect(!SessionPersistencePolicy.isMosaicCrashStoragePath(
            symlink.path,
        ...
        #expect(!SessionPersistencePolicy.isMosaicCrashDiagnosticWindow(
            window,
            homeDirectory: homeDirectory,
            environment: [:]
        ))
