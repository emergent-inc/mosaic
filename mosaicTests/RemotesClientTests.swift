...
/// Tests the pure request-shaping for `mosaic remotes`: host:port parsing, the
...
@Suite struct RemotesClientTests {
    ...
    // Note: `MosaicCLI.sanitizeForTerminal` (used by `remotes list`) lives in the
    // CLI executable target, which `mosaicTests` does not link, so it cannot be
    // unit-tested here (referencing `MosaicCLI` breaks the test-target compile).
