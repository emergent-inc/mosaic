...
final class MosaicConfigDecodingTests: XCTestCase {

    private func decode(_ json: String) throws -> MosaicConfigFile {
        let data = json.data(using: .utf8)!
        return try JSONDecoder().decode(MosaicConfigFile.self, from: data)
    }

    private func resolvedActions(
        from config: MosaicConfigFile,
        sourcePath: String? = nil
    ) -> [String: MosaicResolvedConfigAction] {
        Dictionary(
            uniqueKeysWithValues: config.actions.compactMap { id, definition in
                MosaicResolvedConfigAction.fromDefinition(
                    id: id,
    ...
    func testSurfaceTabBarActionReferenceUsesActionSourcePath() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-config-store-\(UUID().uuidString)", isDirectory: true)
        ...
        let globalConfigURL = globalDirectory.appendingPathComponent("mosaic.json")
        let localConfigURL = localDirectory.appendingPathComponent("mosaic.json")
        let globalJSON = """
        {
          "actions": {
    ...
    func testGlobalSVGIconAllowsNamespaceAndInternalGradient() throws {
        ...
        let configPath = root.appendingPathComponent("mosaic.json").path
        ...
        let icon = MosaicButtonIcon.imagePath("icons/codex.svg")
        XCTAssertEqual(
            icon.bonsplitIcon(
    ...
    func testProjectLocalSVGIconRejectsExternalReferences() throws {
        ...
        let globalConfigPath = globalDirectory.appendingPathComponent("mosaic.json").path
        let projectConfigPath = projectDirectory.appendingPathComponent("mosaic.json").path
        ...
        let icon = MosaicButtonIcon.imagePath("icons/bad.svg")
        XCTAssertEqual(
            icon.bonsplitIcon(
    ...
    func testUntrustedProjectLocalIconUsesLockPlaceholder() throws {
        ...
        let icon = MosaicButtonIcon.imagePath("icons/safe.svg")
        XCTAssertEqual(
            icon.bonsplitIcon(
                configSourcePath: projectConfigPath,
                globalConfigPath: globalConfigPath,
    ...
    func testConfigChangesRequireExplicitLoadByDefault() async throws {
        ...
        let store = MosaicConfigStore(globalConfigPath: configURL.path)
        store.loadAll()
        XCTAssertNotNil(store.resolvedAction(id: "first"))
        XCTAssertNil(store.resolvedAction(id: "second"))

        let didAutoReload = expectation(description: "mosaic.json should not hot reload")
    ...
    @MainActor
    func testConfigStoreParsesGlobalMosaicJSONCSettingsAndActionSections() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "mosaic-config-store-\(UUID().uuidString)",
            isDirectory: true
        )
        ...
        let data = """
        {
          // mosaic-owned app settings share the global mosaic.json file.
          "app": {
            "appearance": "dark",
          },
          "actions": {
    ...
    func testLocalWatcherDetectsFirstCanonicalConfigAfterDirectoryCreation() async throws {
        ...
        let configDirectory = projectDirectory.appendingPathComponent(".mosaic", isDirectory: true)
        let configURL = configDirectory.appendingPathComponent("mosaic.json", isDirectory: false)
        let store = MosaicConfigStore(
            globalConfigPath: root.appendingPathComponent("missing-global.json").path,
            localConfigPath: configURL.path,
        ...
        let loaded = expectation(description: "created local mosaic config is loaded")
        ...
        // watcher) to observe the .mosaic directory creation, re-arm onto the new
        // mosaic.json, reload, and republish loadedActions. vnode notification +
    ...
    @MainActor
    func testLocalWatcherDetectsFirstLegacyConfigWhenMosaicDirectoryExists() async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "mosaic-config-store-\(UUID().uuidString)",
            isDirectory: true
        )
        ...
        let canonicalConfigURL = configDirectory.appendingPathComponent("mosaic.json", isDirectory: false)
        let legacyConfigURL = projectDirectory.appendingPathComponent("mosaic.json", isDirectory: false)
        let store = MosaicConfigStore(
            globalConfigPath: root.appendingPathComponent("missing-global.json").path,
        ...
        let loaded = expectation(description: "created legacy mosaic config is loaded")
        loaded.assertForOverFulfill = false
        var cancellable: AnyCancellable?
        cancellable = store.$loadedActions.dropFirst().sink { actions in
        ...
        // The store observes the legacy mosaic.json write via a DispatchSource vnode
    ...
    func testGlobalNewWorkspaceActionUsesLocalActionOverride() throws {
        ...
        let globalConfigURL = root.appendingPathComponent("global-mosaic.json")
        let localConfigURL = root.appendingPathComponent("local-mosaic.json")
        try """
        {
          "actions": {
    ...
    func testNotificationHooksAppendThroughConfigHierarchy() throws {
        ...
        let globalDirectory = root.appendingPathComponent("global", isDirectory: true)
        let projectDirectory = root.appendingPathComponent("project", isDirectory: true)
        let parentConfigDirectory = projectDirectory.appendingPathComponent(".mosaic", isDirectory: true)
        let childDirectory = projectDirectory.appendingPathComponent("child", isDirectory: true)
        let childConfigDirectory = childDirectory.appendingPathComponent(".mosaic", isDirectory: true)
        try FileManager.default.createDirectory(at: globalDirectory, withIntermediateDirectories: true)
        ...
        let globalConfigURL = globalDirectory.appendingPathComponent("mosaic.json")
        let parentConfigURL = parentConfigDirectory.appendingPathComponent("mosaic.json")
        let childConfigURL = childConfigDirectory.appendingPathComponent("mosaic.json")
        try """
        {
          "notifications": {
    ...
    func testNotificationHooksIncludeExplicitLocalConfigOutsideDiscoveredHierarchy() throws {
        ...
        let globalConfigURL = globalDirectory.appendingPathComponent("mosaic.json")
        let explicitConfigURL = explicitDirectory.appendingPathComponent("custom-mosaic.json")
        try """
        {
          "notifications": {
            "hooks": [{ "id": "global", "command": "cat" }]
    ...
    func testSurfaceTabBarWorkspaceCommandButtonRoundTrips() throws {
        let original = MosaicSurfaceTabBarButton(
            id: "new-dev",
        ...
        let decoded = try JSONDecoder().decode(MosaicSurfaceTabBarButton.self, from: data)
...
final class MosaicCommandIdentityTests: XCTestCase {

    func testCommandIdIsDeterministic() {
        let cmd = MosaicCommandDefinition(name: "Run tests", command: "test")
        XCTAssertEqual(cmd.id, "mosaic.config.command.Run%20tests")
    }

    func testCommandIdEncodesSpecialCharacters() {
        let cmd = MosaicCommandDefinition(name: "build & deploy", command: "make")
        XCTAssertTrue(cmd.id.hasPrefix("mosaic.config.command."))
        XCTAssertFalse(cmd.id.contains("&"))
        XCTAssertFalse(cmd.id.contains(" "))
    }

    func testCommandIdIsUniqueForDifferentNames() {
        let cmd1 = MosaicCommandDefinition(name: "build", command: "make build")
        let cmd2 = MosaicCommandDefinition(name: "test", command: "make test")
        XCTAssertNotEqual(cmd1.id, cmd2.id)
    }

    func testCommandIdDoesNotCollideWithBuiltinPrefix() {
        let cmd = MosaicCommandDefinition(name: "palette.newWorkspace", command: "echo")
        XCTAssertTrue(cmd.id.hasPrefix("mosaic.config.command."))
...
@MainActor
final class MosaicConfigWorkspaceCommandExecutionTests: XCTestCase {
    ...
    func testWorkspaceCommandCreatesNewWorkspaceByDefaultWhenNameAlreadyExists() {
        ...
        let command = MosaicCommandDefinition(
            name: "Dev command",
            workspace: MosaicWorkspaceDefinition(name: "Dev")
        )

        XCTAssertTrue(MosaicConfigExecutor.execute(
            command: command,
            tabManager: manager,
            baseCwd: NSTemporaryDirectory(),
            configSourcePath: nil,
            globalConfigPath: "/tmp/mosaic-test-global-config.json"
        ))
...
final class MosaicSplitDefinitionTests: XCTestCase {
    ...
    func testSplitOrientationVertical() {
        let split = MosaicSplitDefinition(direction: .vertical, split: nil, children: [])
...
@MainActor
final class MosaicConfigCwdResolutionTests: XCTestCase {

    private let baseCwd = "/Users/test/project"

    func testNilCwdReturnsBase() {
        XCTAssertEqual(
            MosaicConfigStore.resolveCwd(nil, relativeTo: baseCwd),
            baseCwd
        )
    }
    ...
    func testEmptyCwdReturnsBase() {
        XCTAssertEqual(
            MosaicConfigStore.resolveCwd("", relativeTo: baseCwd),
            baseCwd
        )
    }
    ...
    func testAbsolutePathReturnedAsIs() {
        XCTAssertEqual(
            MosaicConfigStore.resolveCwd("/tmp/other", relativeTo: baseCwd),
            "/tmp/other"
        )
    }
    ...
    func testRelativePathJoinedToBase() {
        XCTAssertEqual(
            MosaicConfigStore.resolveCwd("backend/src", relativeTo: baseCwd),
    ...
    func testTildeSlashExpandsToHomePlusPath() {
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        XCTAssertEqual(
            MosaicConfigStore.resolveCwd("~/Documents/work", relativeTo: baseCwd),
    ...
    func testSingleSubdirectory() {
        XCTAssertEqual(
            MosaicConfigStore.resolveCwd("src", relativeTo: baseCwd),
...
final class MosaicLayoutEncodingTests: XCTestCase {

    func testPaneNodeRoundTrips() throws {
        let original = MosaicLayoutNode.pane(MosaicPaneDefinition(surfaces: [
            MosaicSurfaceDefinition(type: .terminal, name: "shell")
        ]))
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(MosaicLayoutNode.self, from: data)
    ...
    func testSplitNodeRoundTrips() throws {
        let original = MosaicLayoutNode.split(MosaicSplitDefinition(
            direction: .vertical,
            split: 0.7,
            children: [
                .pane(MosaicPaneDefinition(surfaces: [MosaicSurfaceDefinition(type: .terminal)])),
                .pane(MosaicPaneDefinition(surfaces: [MosaicSurfaceDefinition(type: .browser, url: "http://localhost")]))
            ]
        ))
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(MosaicLayoutNode.self, from: data)
