...
extension CLINotifyProcessIntegrationRegressionTests {
    ...
    func testGenericHookAgentsPersistSanitizedLaunchCommandsForSessionRestore() throws {
        let scenarios: [GenericHookPersistenceScenario] = [
            ...
            GenericHookPersistenceScenario(
                ...
                executable: "/tmp/mosaic-agent-upstreams/copilot-install/bin/copilot",
                launchArguments: [
                    "/tmp/mosaic-agent-upstreams/copilot-install/bin/copilot",
                    "--model",
                    "gpt-5.4",
                    "--resume=old-session",
                    "--allow-all-tools",
    ...
    func testAntigravityStopAndNotificationsUseGenericNotificationPath() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-antigravity-notification-\(UUID().uuidString)", isDirectory: true)
        ...
        let environment: [String: String] = [
            ...
            "PWD": root.path,
            "MOSAIC_SOCKET_PATH": socketPath,
            "MOSAIC_WORKSPACE_ID": workspaceId,
            "MOSAIC_SURFACE_ID": surfaceId,
            "MOSAIC_AGENT_HOOK_STATE_DIR": root.path,
        ...
        XCTAssertTrue(
            ...
            "Expected Antigravity permission notifications to publish through mosaic, saw \(permissionCommands)"
        )
        XCTAssertTrue(
        ...
        XCTAssertTrue(
            ...
            "Expected Antigravity Stop errors to publish through mosaic, saw \(stopErrorCommands)"
        )
        XCTAssertTrue(
        ...
        XCTAssertTrue(
            ...
            "Expected Antigravity error notifications to publish through mosaic, saw \(errorCommands)"
        )
        XCTAssertTrue(
    ...
    func testHermesAgentNotificationsUseShellHookExtraPayload() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-hermes-notification-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testHermesAgentSessionEndIsTurnBoundaryButFinalizeTearsDown() throws {
        ...
        // separate `on_session_finalize` hook once at genuine teardown. mosaic maps the
        ...
        // See https://github.com/emergent-inc/mosaic/issues/5000.
        let cliPath = try bundledCLIPath()
        let socketPath = makeSocketPath("hermes-session-end")
        let listenerFD = try bindUnixSocket(at: socketPath)
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-hermes-session-end-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testAntigravityHookInstallUsesNativeHooksJSONShape() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-antigravity-hook-install-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        ...
        let result = runProcess(
            ...
            environment: [
                "HOME": root.path,
                "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
                "MOSAIC_BUNDLED_CLI_PATH": root.path,
                "MOSAIC_CLI_SENTRY_DISABLED": "1",
            ],
            timeout: 5
        )
        ...
        let mosaicGroup = try XCTUnwrap(json["mosaic"] as? [String: Any])
        let allCommands = mosaicGroup.values
        ...
        XCTAssertTrue(
            allCommands.allSatisfy { $0.contains("mosaic-antigravity-hook-v2") },
        ...
        XCTAssertFalse(
            ...
            "Directory-valued MOSAIC_BUNDLED_CLI_PATH must not be embedded as a hook executable, saw \(allCommands)"
        )
        XCTAssertFalse(
            allCommands.contains { $0.contains(#"[ -n "$MOSAIC_SURFACE_ID" ]"#) },
            "Antigravity hooks must still dispatch when agy does not preserve MOSAIC_SURFACE_ID, saw \(allCommands)"
        )

        let preToolUse = try XCTUnwrap(mosaicGroup["PreToolUse"] as? [[String: Any]])
        ...
        let stop = try XCTUnwrap(mosaicGroup["Stop"] as? [[String: Any]])
        XCTAssertTrue(
            stop.contains {
        ...
        XCTAssertNotNil(mosaicGroup["SessionStart"])
        XCTAssertNotNil(mosaicGroup["SessionEnd"])
        XCTAssertNotNil(mosaicGroup["turn-completion"])
        XCTAssertNotNil(mosaicGroup["Notification"])
        XCTAssertNotNil(mosaicGroup["PostToolUse"])
    }

    func testKiroHookInstallUsesAgentConfigShapeAndPreservesDenyExit() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-kiro-hook-install-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertTrue(
            result.stdout.contains("kiro-cli chat --agent mosaic"),
            "Expected Kiro install to print the --agent mosaic activation hint, saw: \(result.stdout)"
        )
        ...
        let json = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: hookURL)) as? [String: Any])
        XCTAssertEqual(json["name"] as? String, "mosaic")
        XCTAssertNil(json["version"], "Kiro agent configs should not receive Cursor's hooks version field")
        XCTAssertEqual(
            json["tools"] as? [String], ["*"],
            "Kiro mosaic agent must grant the full tool set so `--agent mosaic` can run tools and fire preToolUse hooks"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected Kiro preToolUse feed hook to preserve mosaic's exit status for deny decisions, saw \(preToolUse)"
        )
    ...
    func testKiroFeedDenyUsesPreToolUseExitCodeTwo() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-kiro-feed-deny-\(UUID().uuidString)", isDirectory: true)
        ...
        let result = runProcess(
            ...
            environment: [
                ...
                "MOSAIC_WORKSPACE_ID": workspaceId,
                "MOSAIC_SURFACE_ID": surfaceId,
                "MOSAIC_KIRO_PID": "525252",
                "MOSAIC_KIRO_NOTIFICATION_LEVEL": "standard",
                "MOSAIC_CLI_SENTRY_DISABLED": "1",
            ],
        ...
        XCTAssertTrue(result.stderr.contains("User denied permission via mosaic Feed."), result.stderr)
    ...
    func testKiroFeedAllowModesProceedAndUnknownModeDenies() throws {
        func runKiroDecision(mode: String) throws -> ProcessRunResult {
            ...
            let state = MockSocketServerState()
            let root = FileManager.default.temporaryDirectory
                .appendingPathComponent("mosaic-kiro-feed-mode-\(UUID().uuidString)", isDirectory: true)
            try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
            defer {
                Darwin.close(listenerFD)
                unlink(socketPath)
            ...
            let result = runProcess(
                ...
                environment: [
                    ...
                    "PWD": root.path,
                    "MOSAIC_SOCKET_PATH": socketPath,
                    "MOSAIC_WORKSPACE_ID": "33333333-3333-3333-3333-333333333333",
                    "MOSAIC_SURFACE_ID": "44444444-4444-4444-4444-444444444444",
                    "MOSAIC_KIRO_PID": "525252",
                    "MOSAIC_KIRO_NOTIFICATION_LEVEL": "standard",
                    "MOSAIC_CLI_SENTRY_DISABLED": "1",
                ],
    ...
    func testKiroStandardLevelSuppressesReadOnlyToolFeedEvents() throws {
        func feedPushCount(forTool tool: String) throws -> Int {
            ...
            let state = MockSocketServerState()
            let root = FileManager.default.temporaryDirectory
                .appendingPathComponent("mosaic-kiro-suppress-\(UUID().uuidString)", isDirectory: true)
            try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
            defer {
                Darwin.close(listenerFD)
                unlink(socketPath)
    ...
    func testLowercaseGenericFeedToolsStayTelemetryOutsideKiro() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-generic-lowercase-feed-tool-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "33333333-3333-3333-3333-333333333333"
        let surfaceId = "44444444-4444-4444-4444-444444444444"
    ...
    func testAntigravityFeedHookMissingSessionIdUsesStableFallback() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-antigravity-feed-stable-session-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "33333333-3333-3333-3333-333333333333"
        let surfaceId = "44444444-4444-4444-4444-444444444444"
    ...
    func testGrokNotificationHookUsesPayloadMessageAndStopDoesNotSendGenericNotification() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-grok-notification-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testGrokStopFallbackCompletionsFireForTwoConcurrentThreads() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-grok-two-threads-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceIds = [
        ...
        func runGrokHook(_ subcommand: String, input: String, surfaceId: String) -> ProcessRunResult {
            ...
            environment["MOSAIC_SURFACE_ID"] = surfaceId
            let result = runProcess(
                executablePath: cliPath,
                arguments: ["hooks", "grok", subcommand],
                environment: environment,
    ...
    func testGrokStopNotificationFallsBackWhenTranscriptCwdIsUnavailable() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-grok-stop-without-cwd-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testGrokNotificationStillFiresOnRepeatedPromptWhenFeedTelemetryDoesNotReply() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-grok-repeat-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testGrokSessionEndDoesNotDropRoutingForLaterChatMessages() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-grok-turns-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
        ...
        for index in 1...2 {
            ...
            XCTAssertTrue(
                ...
                "Expected Grok prompt \(index) to reuse the saved target without MOSAIC env, saw \(promptCommands)"
            )
            XCTAssertTrue(
    ...
    func testGrokCompletionDoesNotResetStatusWhileSiblingSessionRuns() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-grok-sibling-status-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
    ...
    func testGrokCompletionResetsStatusWhenSiblingRunningRecordHasDeadPID() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-grok-stale-sibling-status-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        ...
        let environment: [String: String] = [
            ...
            "MOSAIC_CLI_SENTRY_DISABLED": "1",
            "MOSAIC_WORKSPACE_ID": workspaceId,
            "MOSAIC_SURFACE_ID": completingSurfaceId,
        ]
    ...
    func testGrokHookInstallRoutesNotificationEventToNotificationSubcommand() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-grok-hook-install-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
            ...
            "hooks": [
                "PostToolUse": [
                    [
                        "hooks": [
                            [
                                "command": "[ -n \"$MOSAIC_SURFACE_ID\" ] && [ \"$MOSAIC_GROK_HOOKS_DISABLED\" != \"1\" ] && command -v mosaic >/dev/null 2>&1 && mosaic hooks feed --source grok --event PostToolUse || echo '{}'",
                                "timeout": 120,
                                "type": "command",
                            ],
                        ],
                ...
                "Stop": [
                    [
                        "hooks": [
                            [
                                "command": "[ -n \"$MOSAIC_SURFACE_ID\" ] && [ \"$MOSAIC_GROK_HOOKS_DISABLED\" != \"1\" ] && command -v mosaic >/dev/null 2>&1 && mosaic hooks grok stop || echo '{}'",
                                "timeout": 5,
                                "type": "command",
                            ],
                        ],
        ...
        let hookURL = root
            .appendingPathComponent(".grok", isDirectory: true)
            .appendingPathComponent("hooks", isDirectory: true)
            .appendingPathComponent("mosaic-session.json", isDirectory: false)
        let json = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: hookURL)) as? [String: Any])
        let hooks = try XCTUnwrap(json["hooks"] as? [String: Any])
        ...
        XCTAssertTrue(
            notificationCommands.contains { $0.contains("mosaic hooks grok notification") },
            "Expected Grok Notification to dispatch to the notification handler, saw \(notificationCommands)"
        )
        XCTAssertFalse(
            notificationCommands.contains { $0.contains("mosaic hooks grok stop") },
        ...
        XCTAssertFalse(
            allCommands.contains { $0.contains("[ -n \"$MOSAIC_SURFACE_ID\" ]") },
            "Grok strips MOSAIC_* from hook subprocesses, so installed commands must not gate on MOSAIC_SURFACE_ID. Saw \(allCommands)"
        )
        XCTAssertFalse(
            allCommands.contains { $0.contains("$MOSAIC_") },
            "Grok treats $VAR references as required hook environment, so installed commands must avoid MOSAIC variable interpolation. Saw \(allCommands)"
        )
        XCTAssertFalse(
            FileManager.default.fileExists(atPath: legacyHookURL.path),
            "Expected setup to remove legacy mosaic-owned Grok hook file"
        )
    }

    func testGrokHookInstallPinsInstallingCLIAndSocketWithoutMosaicInterpolation() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-grok-hook-pin-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let pinnedCLI = root.appendingPathComponent("mosaic pinned dev cli", isDirectory: false)
        ...
        let socketPath = "/tmp/mosaic-debug-grok-pin.sock"
        let result = runProcess(
            executablePath: cliPath,
            arguments: ["hooks", "grok", "install", "--yes"],
            environment: [
                "HOME": root.path,
                "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
                "MOSAIC_BUNDLED_CLI_PATH": pinnedCLI.path,
                "MOSAIC_SOCKET_PATH": socketPath,
                "MOSAIC_CLI_SENTRY_DISABLED": "1",
            ],
            timeout: 5
        ...
        XCTAssertFalse(allCommands.isEmpty)
        XCTAssertTrue(
            allCommands.allSatisfy { $0.contains("mosaic-grok-hook-v2") },
        ...
        XCTAssertFalse(
            allCommands.contains { $0.contains("$MOSAIC_") },
            "Grok hook commands must not depend on MOSAIC environment interpolation, saw \(allCommands)"
        )
    }
    ...
    func testGrokHookInstallPreservesUserWrappedLegacyCommands() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-grok-hook-preserve-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let preservedCommand = "bash -lc 'mosaic hooks grok notification && ~/bin/after-grok'"
        let legacyHookJSON: [String: Any] = [
            "hooks": [
                "Notification": [
                    [
                        "hooks": [
                            ...
                            [
                                "command": "[ \"$MOSAIC_GROK_HOOKS_DISABLED\" != \"1\" ] && command -v mosaic >/dev/null 2>&1 && mosaic hooks grok notification || echo '{}'",
                                "timeout": 10,
                                "type": "command",
                            ],
                        ],
        ...
        XCTAssertFalse(
            commands.contains { $0.hasPrefix("[ \"$MOSAIC_GROK_HOOKS_DISABLED\"") },
            "Expected setup to remove only exact mosaic-owned legacy commands, saw \(commands)"
        )
    }
    ...
    func testGrokHookInstallPreservesLegacyFileMetadataWhenPruningOwnedHooks() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-grok-hook-metadata-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testCodexHookInstallPrefersLaunchingAppBundledCLI() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-hook-install-\(UUID().uuidString)", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        try FileManager.default.createDirectory(at: codexHome, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let previousBundledHookCommand = "mosaic_cli=\"${MOSAIC_BUNDLED_CLI_PATH:-}\"; if [ -z \"$mosaic_cli\" ] || [ ! -x \"$mosaic_cli\" ]; then mosaic_cli=\"$(command -v mosaic 2>/dev/null || true)\"; fi; [ -n \"$MOSAIC_SURFACE_ID\" ] && [ \"$MOSAIC_CODEX_HOOKS_DISABLED\" != \"1\" ] && [ -n \"$mosaic_cli\" ] && \"$mosaic_cli\" hooks codex prompt-submit || echo '{}'"
        let legacyHookJSON: [String: Any] = [
            "hooks": [
                "UserPromptSubmit": [
                    [
        ...
        XCTAssertTrue(
            commandBodies.contains {
                $0.contains("MOSAIC_BUNDLED_CLI_PATH")
                    && $0.contains("\"$mosaic_cli\" --socket \"$MOSAIC_SOCKET_PATH\" hooks codex prompt-submit")
            },
            "Codex hooks should route through the launching app's bundled CLI, saw \(commandBodies)"
        )
        XCTAssertFalse(
            commandBodies.contains { $0.contains("command -v mosaic >/dev/null 2>&1 && mosaic hooks codex") },
            "Codex hooks must not use the reload-global mosaic shim directly, saw \(commandBodies)"
        )
        XCTAssertFalse(
            commandBodies.contains { $0 == previousBundledHookCommand },
            "Codex setup should replace bundled-CLI hooks that did not pin MOSAIC_SOCKET_PATH, saw \(commandBodies)"
        )
        XCTAssertEqual(
            allCommands.filter { $0.contains("hooks codex prompt-submit") }.count,
            1,
            "Codex setup should collapse duplicate mosaic-owned prompt hooks to one entry, saw \(allCommands)"
        )
    }
    ...
    func testGrokHookInstallRejectsFileAtHooksDirectory() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-grok-hook-file-dir-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertTrue(
            result.stderr.contains("mosaic could not create the hooks directory: a file exists at \(hooksPath.path); remove or rename the conflicting file and re-run `mosaic hooks setup`"),
            result.stderr
        )
        XCTAssertFalse(
    ...
    func runGenericHookPersistenceScenario(_ scenario: GenericHookPersistenceScenario) throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-\(scenario.agent)-hook-\(UUID().uuidString)", isDirectory: true)
        ...
        var environment: [String: String] = [
            ...
            "MOSAIC_SURFACE_ID": surfaceId,
            "MOSAIC_AGENT_HOOK_STATE_DIR": root.path,
            "MOSAIC_AGENT_LAUNCH_KIND": scenario.agent,
            "MOSAIC_AGENT_LAUNCH_EXECUTABLE": scenario.executable,
            "MOSAIC_AGENT_LAUNCH_ARGV_B64": base64NULSeparated(scenario.launchArguments),
            "MOSAIC_AGENT_LAUNCH_CWD": workspace.path,
            "MOSAIC_CLI_SENTRY_DISABLED": "1",
        ]
        ...
        if scenario.agent == "kiro" {
            ...
            XCTAssertEqual(
                ...
                "cd -- '\(workspace.path)' 2>/dev/null || [ ! -d '\(workspace.path)' ] && '\(scenario.executable)' 'chat' '--resume-id' '\(scenario.sessionId)' '--agent' 'mosaic' '--trust-tools' 'fs_read,fs_write'"
            )
    ...
    /// G2 (https://github.com/emergent-inc/mosaic/issues/5350): plain `codex` under the subrouter account
    /// manager points CODEX_HOME at ~/.codex-accounts/<account>, not ~/.codex. When the launch argv
    /// can't be captured (no MOSAIC_AGENT_LAUNCH_ARGV_B64 and an exited PID), the session record used to
    ...
    func testCodexHookPreservesCodexHomeWhenLaunchCommandUnavailable() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-home-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
        ...
        environment["MOSAIC_WORKSPACE_ID"] = workspaceId
        environment["MOSAIC_SURFACE_ID"] = surfaceId
        environment["MOSAIC_CLI_TTY_NAME"] = ttyName
        environment["MOSAIC_AGENT_HOOK_STATE_DIR"] = root.path
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        environment["CODEX_HOME"] = codexHome
        for key in ["MOSAIC_AGENT_LAUNCH_KIND", "MOSAIC_AGENT_LAUNCH_EXECUTABLE", "MOSAIC_AGENT_LAUNCH_ARGV_B64", "MOSAIC_AGENT_LAUNCH_CWD"] {
    ...
    /// G3 (https://github.com/emergent-inc/mosaic/issues/5333): the codex surface jumble. MOSAIC_SURFACE_ID
    ...
    func testCodexHookOverridesLeakedEnvSurfaceWithProcessTTYBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-surface-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let leakedSurfaceId = "22222222-2222-2222-2222-222222222222"   // env MOSAIC_SURFACE_ID (wrong)
        ...
        environment["MOSAIC_SOCKET_PATH"] = socketPath
        environment["MOSAIC_WORKSPACE_ID"] = workspaceId
        environment["MOSAIC_SURFACE_ID"] = leakedSurfaceId
        environment["MOSAIC_CLI_TTY_NAME"] = ttyName
        environment["MOSAIC_AGENT_HOOK_STATE_DIR"] = root.path
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        environment["MOSAIC_AGENT_LAUNCH_KIND"] = "codex"
        environment["MOSAIC_AGENT_LAUNCH_EXECUTABLE"] = "/usr/local/bin/codex"
        environment["MOSAIC_AGENT_LAUNCH_CWD"] = root.path
        environment["MOSAIC_AGENT_LAUNCH_ARGV_B64"] = base64NULSeparated(["/usr/local/bin/codex"])
        ...
        XCTAssertEqual(
            ...
            "PID/TTY ground truth must override the leaked env MOSAIC_SURFACE_ID; params=\(params)"
        )
    }

    /// G3 stale-env variant (https://github.com/emergent-inc/mosaic/issues/5333): when the ambient
    /// MOSAIC_SURFACE_ID is stale/invalid (the surface was closed, or belongs to another workspace) it no
    ...
    func testCodexHookRecoversFromStaleEnvSurfaceViaProcessTTYBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-stale-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let staleSurfaceId = "22222222-2222-2222-2222-222222222222"   // env MOSAIC_SURFACE_ID, no longer exists
        ...
        environment["MOSAIC_SOCKET_PATH"] = socketPath
        environment["MOSAIC_WORKSPACE_ID"] = workspaceId
        environment["MOSAIC_SURFACE_ID"] = staleSurfaceId
        environment["MOSAIC_CLI_TTY_NAME"] = ttyName
        environment["MOSAIC_AGENT_HOOK_STATE_DIR"] = root.path
        environment["MOSAIC_CLI_SENTRY_DISABLED"] = "1"
        environment["MOSAIC_AGENT_LAUNCH_KIND"] = "codex"
        ...
        XCTAssertEqual(
            params["surface_id"] as? String, ttySurfaceId,
            "a stale ambient MOSAIC_SURFACE_ID must fall through to the TTY pane; params=\(params)"
        )
    }
    ...
    func testCodexHookDoesNotPersistEnvOnlyRecordForNonRestorableExec() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-codex-exec-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
        ...
        environment["MOSAIC_AGENT_LAUNCH_ARGV_B64"] = base64NULSeparated(["/usr/local/bin/codex", "exec", "do a one-shot task"])
