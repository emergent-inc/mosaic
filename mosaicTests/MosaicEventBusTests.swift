...
final class MosaicEventBusTests: XCTestCase {
    ...
    func testSubscribeReplaysEventsAfterSequenceAndReportsAck() throws {
        let bus = MosaicEventBus(retainedEventLimit: 4)
        bus.publish(
    ...
    func testSubscribeReportsGapWhenCursorFallsOutOfRetention() throws {
        let bus = MosaicEventBus(retainedEventLimit: 2)
    ...
    func testSubscriptionFiltersLiveEventsByCategory() {
        let bus = MosaicEventBus(retainedEventLimit: 8)
    ...
    func testSlowSubscriptionClosesWhenPendingQueueIsFull() {
        let bus = MosaicEventBus(retainedEventLimit: 8, maxPendingEventsPerSubscription: 2)
    ...
    func testEventEncodingIsSingleLineJSON() throws {
        ...
        let line = try XCTUnwrap(MosaicEventBus.encodeLine(event))
    ...
    func testEncodingPreservesZeroAndOneNumbers() throws {
        let line = try XCTUnwrap(MosaicEventBus.encodeLine([
    ...
    func testStrictSequenceParsingRejectsBooleanAndFloatFrames() throws {
        XCTAssertEqual(MosaicEventBus.int64(NSNumber(value: Int64(42))), 42)
        XCTAssertEqual(MosaicEventBus.int64("42"), 42)
        XCTAssertNil(MosaicEventBus.int64(true))
        XCTAssertNil(MosaicEventBus.int64(NSNumber(value: 1.5)))
    }

    func testOversizedEventPayloadIsTruncatedBeforeRetention() throws {
        let bus = MosaicEventBus(retainedEventLimit: 4, maxEventLineBytes: 1_024)
    ...
    func testWorkspaceReorderSocketMapperDoesNotDuplicateLifecycleEvent() throws {
        ...
        let snapshot = MosaicEventBus.shared.subscribe(
            afterSequence: nil,
            names: ["workspace.reordered"],
            categories: []
        )
        defer {
            MosaicEventBus.shared.unsubscribe(snapshot.subscription)
            MosaicEventBus.shared.resetForTesting()
        }
        ...
        MosaicSocketEventMapper.publish(command: command, response: response)
    ...
    func testPublishV2ReadTextResponseDoesNotAccumulateOnLongLivedThread() throws {
        ...
        XCTAssertTrue(MosaicEventBus.shared.retainedSnapshot().isEmpty)
    }
    ...
    func testBulkNotificationClearPublishesClearedWithoutRemovedDuplicates() throws {
        ...
        let events = MosaicEventBus.shared.retainedSnapshot()
    ...
    func testNotificationSocketParamsRedactTextFields() throws {
        let redacted = MosaicSocketEventMapper.redactedNotificationParams([
            "title": "Secret title",
    ...
    func testV1NotifySurfacePublishesSurfaceIdWithoutWorkspaceId() throws {
        ...
        MosaicSocketEventMapper.publish(command: "notify_surface \(surfaceId.uuidString) done", response: "OK")
    ...
    func testV1MapperIgnoresNonSuccessResponses() throws {
        ...
        MosaicSocketEventMapper.publish(command: "notify title", response: "OKAY")
        MosaicSocketEventMapper.publish(command: "notify title", response: "queued")
        MosaicSocketEventMapper.publish(command: "notify title", response: "ERROR: failed")
    ...
    func testWorkstreamPayloadRedactsSensitiveFields() throws {
        ...
        let payload = MosaicEventBus.workstreamPayload(event)
        ...
        let line = try XCTUnwrap(MosaicEventBus.encodeLine(["payload": payload]))
        XCTAssertFalse(line.contains("secret"))
    }

    func testPublishAppendsDurableEventLog() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-event-log-\(UUID().uuidString)", isDirectory: true)
        let logURL = directory.appendingPathComponent("events.jsonl")
        let bus = MosaicEventBus(retainedEventLimit: 4, eventLogURL: logURL)
    ...
    func testDurableEventLogDropsOldestPendingLinesUnderBackpressure() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-event-log-backpressure-\(UUID().uuidString)", isDirectory: true)
        let logURL = directory.appendingPathComponent("events.jsonl")
        let bus = MosaicEventBus(
    ...
    func testDurableEventLogRotatesAtByteLimit() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-event-log-rotation-\(UUID().uuidString)", isDirectory: true)
        let logURL = directory.appendingPathComponent("events.jsonl")
        let bus = MosaicEventBus(
