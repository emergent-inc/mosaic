...
final class MosaicTopSnapshotScopeTests: XCTestCase {
    ...
    func testWindowRollupMatchesPSForApplicationProcessTree() throws {
        ...
        let snapshot = MosaicTopProcessSnapshot.capture(includeProcessDetails: false)
    ...
    func testApplicationProcessTreeIsExposedAtWindowLevel() throws {
        ...
        let snapshot = MosaicTopProcessSnapshot.capture(includeProcessDetails: true)
        var windows: [[String: Any]] = [[
            "kind": "window",
            "id": UUID().uuidString,
            "index": 0,
    ...
    func testSamplePayloadDescribesPhysicalFootprintFallbackSource() {
        let sample = MosaicTopProcessSnapshot.capture(includeProcessDetails: false).samplePayload()

        XCTAssertEqual(
            sample["memory_source"] as? String,
            MosaicTopProcessMemorySource.physicalFootprint.rawValue
        )
        XCTAssertEqual(
            sample["memory_fallback_source"] as? String,
            MosaicTopProcessMemorySource.residentSize.rawValue
        )
        XCTAssertEqual(
            sample["resident_memory_fallback_source"] as? String,
            MosaicTopProcessMemorySource.rusageResidentSize.rawValue
        )
        XCTAssertEqual(sample["mosaic_scope"] as? Bool, true)

        let unscopedSnapshot = MosaicTopProcessSnapshot(
            processes: [],
            sampledAt: Date(timeIntervalSince1970: 0),
            includesProcessDetails: false,
            includesMosaicScope: false
        )
        XCTAssertEqual(unscopedSnapshot.samplePayload()["mosaic_scope"] as? Bool, false)

        let fallbackSnapshot = MosaicTopProcessSnapshot(
            processes: [
                MosaicTopProcessInfo(
                    ...
                    path: nil,
                    ttyDevice: nil,
                    mosaicWorkspaceID: nil,
                    mosaicSurfaceID: nil,
                    mosaicAttributionReason: nil,
                    processGroupID: nil,
    ...
    func testUnavailableMemorySourcesAreExposedInAggregatePayloads() throws {
        ...
        let snapshot = MosaicTopProcessSnapshot(
            processes: [
                MosaicTopProcessInfo(
                    pid: unavailablePID,
                    parentPID: 0,
    ...
    func testKernProcArgsWorkspaceID() {
        ...
        let scope = MosaicTopProcessSnapshot.mosaicScope(fromKernProcArgs: bytes)
    ...
    func testKernProcArgsTabIDFallback() {
        ...
        let bytes = kernProcArgs(environment: [
            "MOSAIC_TAB_ID=\(tabID.uuidString)"
        ])
    ...
    func testKernProcArgsSurfaceID() {
        ...
        let bytes = kernProcArgs(environment: [
            "MOSAIC_SURFACE_ID=\(surfaceID.uuidString)"
        ])
    ...
    func testCodexMonitorArgumentsSupportJoinedUUIDOptions() throws {
        ...
        let scope = try XCTUnwrap(MosaicTopProcessSnapshot.mosaicScope(
            arguments: [
        ...
        XCTAssertEqual(scope.workspaceID, workspaceID)
        XCTAssertEqual(scope.surfaceID, surfaceID)
        XCTAssertEqual(scope.attributionReason, "mosaic-hook-arguments")
    }

    func testCodexMonitorArgumentsIgnorePathValuedSubcommandLookalikes() {
        let workspaceID = UUID(uuidString: "55555555-5555-5555-5555-555555555555")!

        let scope = MosaicTopProcessSnapshot.mosaicScope(
            arguments: [
                "/Applications/mosaic.app/Contents/Resources/bin/mosaic",
                "other",
                "/tmp/hooks",
    ...
    func testCodexMonitorArgumentsRequireMosaicExecutable() {
        let workspaceID = UUID(uuidString: "55555555-5555-5555-5555-555555555555")!
    ...
    func testLaunchdParentedCodexMonitorArgumentsAttachToOwningSurface() throws {
        ...
        let scope = try XCTUnwrap(MosaicTopProcessSnapshot.mosaicScope(fromKernProcArgs: bytes))
        XCTAssertEqual(scope.workspaceID, workspaceID)
        XCTAssertEqual(scope.surfaceID, surfaceID)
        ...
        let snapshot = MosaicTopProcessSnapshot(
            processes: [
                MosaicTopProcessInfo(
                    ...
                    parentPID: 1,
                    name: "mosaic",
                    path: "/Applications/mosaic.app/Contents/Resources/bin/mosaic",
                    ttyDevice: nil,
                    mosaicWorkspaceID: scope.workspaceID,
                    mosaicSurfaceID: scope.surfaceID,
                    mosaicAttributionReason: scope.attributionReason,
                    processGroupID: nil,
                    terminalProcessGroupID: nil,
                    cpuPercent: 0,
        ...
        XCTAssertEqual(monitorProcess["attribution_reason"] as? String, "mosaic-hook-arguments")
    ...
    private struct SpawnedProcessTree {
        ...
        static func start() throws -> SpawnedProcessTree {
            let directory = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
                .appendingPathComponent("mosaic-top-\(UUID().uuidString)", isDirectory: true)
