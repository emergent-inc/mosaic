...
struct WindowTitleTemplateTests {
    private let backupsDefaultsKey = "mosaic.settingsFile.backups.v1"
    ...
    @Test func resolvesWindowPlaceholdersAndPreservesUnknownPlaceholders() throws {
        ...
        let template = WindowTitleTemplate(
            rawValue: "[mosaic:{windowToken}] {activeWorkspace} {activeDirectory} {windowId} {defaultTitle} {appName} {unknown}"
        )
        ...
        #expect(resolved == "[mosaic:01234567] Build /tmp/project 01234567-89ab-cdef-0123-456789abcdef Fallback mosaic {unknown}")
    }
    ...
    @Test func resolverDoesNotExpandPlaceholdersInsideReplacementValues() throws {
        ...
        #expect(resolved == "{windowId} mosaic")
    }
    ...
    @Test func settingsFileStoreAppliesAppWindowTitleTemplate() throws {
        ...
        let directoryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-window-title-template-\(UUID().uuidString)", isDirectory: true)
        ...
        #expect(defaults.string(forKey: WindowTitleTemplate.userDefaultsKey) == "[mosaic:{windowToken}] {activeWorkspace}")
    }
    ...
    @Test func settingsFileStoreAppliesWorkspaceAutoNamingAutomationSetting() throws {
        ...
        let directoryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-workspace-auto-naming-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directoryURL) }
    ...
    @Test func settingsFileStoreAppliesAutoNamingAgentAutomationSetting() throws {
        ...
        let directoryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-auto-naming-agent-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directoryURL) }
    ...
    @Test func selectedWorkspaceDirectoryChangeRefreshesActiveDirectoryTitle() throws {
        ...
        defaults.set("[mosaic:{windowToken}] {activeDirectory}", forKey: WindowTitleTemplate.userDefaultsKey)
        ...
        #expect(window.title == "[mosaic:01234567] /tmp/old")
        ...
        #expect(window.title == "[mosaic:01234567] /tmp/new")
    }

    private func isolatedDefaults() throws -> UserDefaults {
        let suiteName = "mosaic.WindowTitleTemplateTests.\(UUID().uuidString)"
