...
private final class MosaicCLISentryTelemetryBundleToken {}

@Suite struct MosaicCLISentryTelemetryRegressionTests {
    ...
    @Test func staleSocketConnectRefusalDoesNotCaptureSentryTelemetry() throws {
        ...
        let root = URL(
            fileURLWithPath: "/tmp/mosaic-sr-\(UUID().uuidString.prefix(8))",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let socketPath = root.appendingPathComponent("mosaic.sock", isDirectory: false).path
    ...
    @Test func missingSocketDoesNotCaptureSentryTelemetry() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-cli-sentry-missing-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    @Test func unexpectedSocketTelemetryStoresWithoutBlockingForSentryFlush() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-cli-sentry-flush-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        environment["MOSAIC_CLI_SENTRY_STORE_PROBE_PATH"] = storeProbePath
    ...
    private func bundledCLIPath() throws -> String {
        try BundledCLITestSupport.bundledCLIPath(for: MosaicCLISentryTelemetryBundleToken.self)
    }
    ...
    private func sentryProbeEnvironment(socketPath: String, probePath: String) -> [String: String] {
        ...
        environment["MOSAIC_CLI_SENTRY_CAPTURE_PROBE_PATH"] = probePath
