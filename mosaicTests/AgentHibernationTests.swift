...
final class AgentHibernationTests: XCTestCase {
    ...
    func testSocketLifecycleAcceptsRegisteredCustomAgentKey() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-custom-lifecycle-\(UUID().uuidString)", isDirectory: true)
        let configDirectory = root.appendingPathComponent(".mosaic", isDirectory: true)
        ...
        """.write(to: configDirectory.appendingPathComponent("mosaic.json"), atomically: true, encoding: .utf8)
    ...
    func testSettingsDefaultToOptInAndNotifyOnChanges() throws {
        let suiteName = "mosaic-agent-hibernation-\(UUID().uuidString)"
    ...
    func testSessionIndexLoadsAgentLifecycleFromHookStore() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-agent-hibernation-index-\(UUID().uuidString)", isDirectory: true)
    ...
    func testSessionIndexUsesLiveHookPIDAsProcessID() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-agent-hibernation-live-hook-pid-\(UUID().uuidString)", isDirectory: true)
        let storeURL = RestorableAgentKind.codex.hookStoreFileURL(homeDirectory: home.path)
        try FileManager.default.createDirectory(at: storeURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: home) }
        ...
        let index = RestorableAgentSessionIndex.load(
            ...
            processArgumentsProvider: { requestedPID in
                requestedPID == pid
                    ? MosaicTopProcessArguments(
                        arguments: ["/bin/sleep", "30"],
                        environment: [
                            "MOSAIC_WORKSPACE_ID": workspaceId.uuidString,
                            "MOSAIC_SURFACE_ID": panelId.uuidString,
                            "MOSAIC_AGENT_LAUNCH_KIND": RestorableAgentKind.codex.rawValue,
                        ]
                    )
                    : nil
            }
    ...
    func testSessionIndexAcceptsNodeBackedClaudeProcessAsLiveHookPID() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-agent-hibernation-claude-node-pid-\(UUID().uuidString)", isDirectory: true)
                ...
                requestedPID == pid
                    ? MosaicTopProcessArguments(
                        ...
                        environment: [
                            "MOSAIC_WORKSPACE_ID": workspaceId.uuidString,
                            "MOSAIC_SURFACE_ID": panelId.uuidString,
                            "MOSAIC_AGENT_LAUNCH_KIND": RestorableAgentKind.claude.rawValue,
                        ]
                    )
                    : nil
            }
    ...
    func testLiveProcessScopeMatchingAcceptsLegacyEnvironmentKeys() throws {
        let workspaceId = UUID()
        let panelId = UUID()
        let process = MosaicTopProcessArguments(
            arguments: ["/usr/bin/codex"],
            environment: [
                "MOSAIC_TAB_ID": workspaceId.uuidString,
                "MOSAIC_PANEL_ID": panelId.uuidString,
            ]
        )

        XCTAssertTrue(process.matchesMosaicScope(workspaceId: workspaceId, surfaceId: panelId))
        XCTAssertFalse(process.matchesMosaicScope(workspaceId: UUID(), surfaceId: panelId))
        XCTAssertFalse(process.matchesMosaicScope(workspaceId: workspaceId, surfaceId: UUID()))
    }
    ...
    func testProcessDetectedSnapshotPreservesMatchingHookLifecycleWithoutRefreshingActivity() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-agent-hibernation-detected-\(UUID().uuidString)", isDirectory: true)
    ...
    func testProcessDetectedSnapshotPreservesMatchingHookLifecycleWhenHookPIDIsStale() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-agent-hibernation-stale-pid-\(UUID().uuidString)", isDirectory: true)
        let storeURL = RestorableAgentKind.opencode.hookStoreFileURL(homeDirectory: home.path)
        try FileManager.default.createDirectory(at: storeURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: home) }
    ...
    func testProcessDetectedSnapshotPreservesHookLifecycleWhenRestoredPanelIDsChange() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-agent-hibernation-remapped-panel-\(UUID().uuidString)", isDirectory: true)
        let storeURL = RestorableAgentKind.opencode.hookStoreFileURL(homeDirectory: home.path)
        try FileManager.default.createDirectory(at: storeURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: home) }
    ...
    func testProcessDetectedOnlySnapshotDoesNotUseScanTimeAsActivity() {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-agent-hibernation-empty-home-\(UUID().uuidString)", isDirectory: true)
    ...
    func testSupportedAgentSnapshotsHaveResumeCommandsForHibernation() {
        let cwd = "/tmp/mosaic-agent-hibernation"
    ...
    func testCustomRegisteredAgentSnapshotCanHibernateWhenResumeCommandExists() {
        let registration = MosaicVaultAgentRegistration(
            id: "local-agent",
            name: "Local Agent",
            detect: MosaicVaultAgentDetectRule(processName: "local-agent"),
    ...
    func testInvalidatedIndexedAgentSnapshotIsNotEligibleForHibernation() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-agent-hibernation-invalidated-index-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: home, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: home) }
        ...
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            workingDirectory: "/tmp/mosaic-agent-hibernation",
            launchCommand: launch("codex", "/usr/local/bin/codex", cwd: "/tmp/mosaic-agent-hibernation")
        )
