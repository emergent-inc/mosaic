...
struct DockControlDefinitionDecodingTests {
    ...
    private func makeTemporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent(
            "mosaic-dock-config-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func projectConfigIdentityUsesResolvedDockFile() throws {
        ...
        let mosaicDirectory = root.appendingPathComponent(".mosaic", isDirectory: true)
        try FileManager.default.createDirectory(at: mosaicDirectory, withIntermediateDirectories: true)
        let dockConfig = mosaicDirectory.appendingPathComponent("dock.json", isDirectory: false)
