...
@Suite struct SessionPersistenceResumeBindingTests {
    ...
    @Test func decodingAgentHookBindingRewritesPersistedPATHManagedAgentExecutable() throws {
        let executablePath = Self.homeManagedExecutablePath(
            ...
            "versions",
            "node",
            "mosaic-missing-\(UUID().uuidString)",
            "bin"
        )
        let json = """
        {
        ...
        #expect(startupInput.contains("MOSAIC_CLAUDE_WRAPPER_SHIM"), "\(startupInput)")
    ...
    @Test func agentHookBindingRewritesSupportedLocalManagedExecutablePaths() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-surface-resume-stale-managed-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let executablePaths = [
            Self.localManagedExecutablePath(root: root, executableName: "codex", ".fnm", "current", "bin"),
            "/tmp/mosaic-cli-shims/\(UUID().uuidString)/codex",
    ...
    @Test func agentHookBindingWithDirectEnvironmentAssignmentRewritesMovedExecutable() throws {
        ...
        let binding = SurfaceResumeBindingSnapshot(
            kind: "codex",
            command: "MOSAIC_TRACE=1 '\(staleExecutablePath)' 'resume' 'session-env-cli'",
        ...
        #expect(startupInput.contains("MOSAIC_TRACE=1 codex 'resume' 'session-env-cli'"), "\(startupInput)")
    ...
    @Test func agentHookBindingWithQuotedEnvAssignmentRewritesMovedExecutable() throws {
        ...
        let binding = SurfaceResumeBindingSnapshot(
            kind: "codex",
            command: "env 'MOSAIC_TRACE=1' '\(staleExecutablePath)' 'resume' 'session-quoted-env-cli'",
        ...
        #expect(startupInput.contains("env 'MOSAIC_TRACE=1' codex 'resume' 'session-quoted-env-cli'"), "\(startupInput)")
        #expect(!startupInput.contains(staleExecutablePath), "\(startupInput)")
    }
    ...
    @Test func agentHookClaudeBindingWithDirectEnvironmentAssignmentPreservesAssignmentSyntax() throws {
        ...
        let binding = SurfaceResumeBindingSnapshot(
            kind: "claude",
            command: "MOSAIC_TRACE='bar baz' '\(staleExecutablePath)' '--resume' 'session-env-cli'",
            checkpointId: "session-env-cli",
            source: "agent-hook",
            autoResume: true
        )
        ...
        #expect(startupInput.contains("/bin/sh -c"), "\(startupInput)")
        #expect(startupInput.contains("MOSAIC_CLAUDE_WRAPPER_SHIM"), "\(startupInput)")
        #expect(startupInput.contains("MOSAIC_TRACE="), "\(startupInput)")
    ...
    @Test func agentHookClaudeBindingWithShellOperatorKeepsOriginalCommandShape() throws {
        ...
        let redirection = "1>/tmp/mosaic-claude-resume.log"
        let binding = SurfaceResumeBindingSnapshot(
            kind: "claude",
    ...
    @Test @MainActor func remoteWorkspaceLocalTerminalResumeBindingUsesLocalRepair() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-local-resume-binding-\(UUID().uuidString)", isDirectory: true)
        ...
        let suiteName = "mosaic-session-resume-binding-\(UUID().uuidString)"
    ...
    @Test func agentHookSurfaceResumeStartupInputPreservesExistingPATHManagedAgentExecutable() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-surface-resume-existing-agent-\(UUID().uuidString)", isDirectory: true)
        let executable = root
    ...
    @Test func agentHookSurfaceResumeStartupInputFallsBackWhenRecordedAgentExecutableMoved() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("mosaic-surface-resume-moved-agent-\(UUID().uuidString)", isDirectory: true)
        ...
        process.environment = [
            ...
            "MOSAIC_FAKE_CODEX_OUTPUT": outputURL.path,
        ]
        let stderr = Pipe()
    ...
    private func makeOhMyZshLauncherEnvironment(
        ...
        return [
            ...
            "MOSAIC_ZSH_ZDOTDIR": userZdotdirURL.path,
            "MOSAIC_SHELL_INTEGRATION": "1",
            "MOSAIC_SHELL_INTEGRATION_DIR": integrationDir.path,
            "MOSAIC_ZSH_RESTORE_TERM": "xterm-256color",
            "MOSAIC_CODEX_WRAPPER_SHIM": codexShimURL.path,
            "MOSAIC_FAKE_CODEX_OUTPUT": codexOutputURL.path,
    ...
    private func writeOhMyZshFixture(at root: URL) throws {
        ...
        try """
        autoload -Uz add-zsh-hook
        _mosaic_test_autosuggest_precmd() { :; }
        _mosaic_test_autosuggest_preexec() { :; }
        add-zsh-hook precmd _mosaic_test_autosuggest_precmd
        add-zsh-hook preexec _mosaic_test_autosuggest_preexec
        _mosaic_test_autosuggest_self_insert() { zle .self-insert }
        zle -N self-insert _mosaic_test_autosuggest_self_insert
        """.write(
        ...
        try """
        _mosaic_test_syntax_highlighting_line_init() { :; }
        _mosaic_test_syntax_highlighting_line_finish() { :; }
        zle -N zle-line-init _mosaic_test_syntax_highlighting_line_init
        zle -N zle-line-finish _mosaic_test_syntax_highlighting_line_finish
        """.write(
