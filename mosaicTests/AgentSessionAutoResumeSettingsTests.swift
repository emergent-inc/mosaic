...
final class AgentSessionAutoResumeSettingsTests: XCTestCase {
    func testDefaultsKeyAndNotificationOnFlip() throws {
        let suiteName = "mosaic-agent-session-auto-resume-\(UUID().uuidString)"
    ...
    /// (wasAgentRunning == false), mosaic must NOT inject the resume command on restore.
    ...
    /// (wasAgentRunning == true), mosaic MUST auto-resume as before.
    @MainActor
    func testAgentRunningAtSnapshotAutoResumesWhenSettingEnabled() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            // Simulate: agent was still running when mosaic quit
    ...
    func testRemoteWorkspaceAutoResumeKeepsRemoteStartupCommand() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let source = Workspace()
            let remoteCommand = "ssh mosaic-macmini"
            let expectedRestoredRemoteCommand = "ssh -tt mosaic-macmini"
            source.configureRemoteConnection(
                WorkspaceRemoteConfiguration(
                    ...
                    localSocketPath: "/tmp/mosaic-auto-resume-remote.sock",
            ...
            XCTAssertFalse(input.contains("mosaic-agent-resume"), input)
        ...
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            source.configureRemoteConnection(
                WorkspaceRemoteConfiguration(
                    ...
                    localSocketPath: "/tmp/mosaic-auto-resume-long-remote.sock",
                    terminalStartupCommand: remoteCommand
                ),
                autoConnect: false
            )
            ...
            XCTAssertFalse(restoredInput.contains("mosaic-agent-resume"), restoredInput)
            XCTAssertEqual(
                restored.restoredAgentResumeStatesByPanelId[restoredPanelId],
                .awaitingAutoResumeCommand
            )
    ...
    func testResumeLauncherReturnsToLaunchCwdAfterAgentExits() {
        ...
        let exec = "exec -l \"$_mosaic_resume_shell\""
    ...
    private func assertAgentAutoResumeUsesStartupCommand(
        ...
        XCTAssertTrue(script.contains("MOSAIC_SHELL_INTEGRATION_DIR"), script, file: file, line: line)
        XCTAssertTrue(script.contains("MOSAIC_ZSH_ZDOTDIR"), script, file: file, line: line)
        XCTAssertTrue(script.contains("\"$_mosaic_resume_shell\" -lic"), script, file: file, line: line)
        XCTAssertTrue(script.contains("csh|tcsh) \"$_mosaic_resume_shell\" -c"), script, file: file, line: line)
        XCTAssertTrue(script.contains("exec -l \"$_mosaic_resume_shell\""), script, file: file, line: line)
    }
    ...
    private func makeRestorableAgentIndex(
        ...
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-agent-auto-resume-\(UUID().uuidString)", isDirectory: true)
        let previousHookStateDir = getenv("MOSAIC_AGENT_HOOK_STATE_DIR").map { String(cString: $0) }
        setenv("MOSAIC_AGENT_HOOK_STATE_DIR", home.appendingPathComponent("hook-state", isDirectory: true).path, 1)
        defer {
...
final class TerminalCopyOnSelectSettingsTests: XCTestCase {
    func testDefaultsNotificationAndGhosttyConfigMapping() throws {
        let suiteName = "mosaic-terminal-copy-on-select-\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
