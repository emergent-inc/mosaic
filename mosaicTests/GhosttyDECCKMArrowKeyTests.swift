...
struct GhosttyDECCKMArrowKeyTests {
    ...
    func windowKeyEquivalentTerminalArrowsEncodeDECCKMBytes() throws {
        ...
        let captureReadyMarker = "MOSAIC_DECCKM_READY_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        let captureMarker = "MOSAIC_DECCKM_HEX_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-decckm-arrow-capture-\(UUID().uuidString).py")
        let script = """
        import os
        import select
        import sys
