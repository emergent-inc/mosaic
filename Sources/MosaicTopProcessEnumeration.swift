...
private nonisolated let mosaicTopPIDPathBufferSize = 4096

nonisolated extension MosaicTopProcessSnapshot {
    static func allProcesses(includeProcessDetails: Bool, includeMosaicScope: Bool) -> [MosaicTopProcessInfo] {
        let sampledProcesses = allBSDProcesses()
        guard !sampledProcesses.isEmpty else { return [] }

        var scopeKeyByPID: [Int: MosaicTopProcessScopeCacheKey] = [:]
        ...
        var parentScopeKeys: [MosaicTopProcessScopeCacheKey: MosaicTopProcessScopeCacheKey] = [:]
        ...
        var currentCPUSamples: [MosaicTopProcessScopeCacheKey: MosaicTopProcessCPUSample] = [:]
        var processRecords: [(info: MosaicTopProcessInfo, cpuSampleKey: MosaicTopProcessScopeCacheKey?)] = []
        ...
        if includeMosaicScope {
            pruneMosaicScopeCache(activeKeys: activeScopeKeys)
        }
    ...
    private static func processInfo(
        ...
        includeMosaicScope: Bool,
        sampledAtNanoseconds: UInt64,
        currentCPUSamples: inout [MosaicTopProcessScopeCacheKey: MosaicTopProcessCPUSample]
    ) -> (info: MosaicTopProcessInfo, cpuSampleKey: MosaicTopProcessScopeCacheKey?)? {
        ...
        let mosaicScope = includeMosaicScope
            ? cachedMosaicScope(for: pid, cacheKey: cacheKey, nowNanoseconds: sampledAtNanoseconds)
            : nil
        ...
        let cpuSampleKey: MosaicTopProcessScopeCacheKey?
        if let taskInfo {
        ...
        return (MosaicTopProcessInfo(
            pid: pid,
            parentPID: Int(bsdInfo.pbi_ppid),
            name: name.isEmpty ? "pid-\(pid)" : name,
            path: path,
            ttyDevice: ttyDevice,
            mosaicWorkspaceID: mosaicScope?.workspaceID,
            mosaicSurfaceID: mosaicScope?.surfaceID,
            mosaicAttributionReason: mosaicScope?.attributionReason,
    ...
    private static func processPath(pid: Int) -> String? {
        var buffer = [CChar](repeating: 0, count: mosaicTopPIDPathBufferSize)
