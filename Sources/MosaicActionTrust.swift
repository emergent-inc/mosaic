...
struct MosaicActionTrustDescriptor: Codable, Sendable {
    var schemaVersion: Int = 1
    var actionID: String
    var kind: String
    var command: String?
    var target: String?
    var workspaceCommand: MosaicCommandDefinition?
...
final class MosaicActionTrust {
    static let shared = MosaicActionTrust()
    static let didChangeNotification = Notification.Name("mosaic.actionTrustDidChange")
    ...
    private init() {
        let appSupport = FileManager.default.urls(
            ...
            in: .userDomainMask
        ).first!.appendingPathComponent("mosaic")
    ...
    func isTrusted(_ descriptor: MosaicActionTrustDescriptor) -> Bool {
        trustedFingerprints.contains(descriptor.fingerprint)
    }

    func trust(_ descriptor: MosaicActionTrustDescriptor) {
