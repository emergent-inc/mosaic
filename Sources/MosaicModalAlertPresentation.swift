...
/// How ``runMosaicModalAlert(_:presentingWindow:willPresent:)`` ended up
/// presenting an alert.
///
...
enum MosaicModalAlertPresentation {
...
/// Returns whether `window` is one of mosaic's main windows.
///
/// Main windows carry the identifier `mosaic.main` or a `mosaic.main.<id>`
...
@MainActor
func isMosaicMainWindow(_ window: NSWindow) -> Bool {
    guard let raw = window.identifier?.rawValue else { return false }
    return raw == "mosaic.main" || raw.hasPrefix("mosaic.main.")
}

/// Returns the visible main mosaic window best suited to host a modal sheet.
///
...
/// when no main mosaic window is currently on screen, in which case callers
/// should fall back to an app-modal presentation.
///
/// - Parameter preferredWindow: A window to consider ahead of the
///   key/main/any search, used when it is visible and a mosaic main window
///   (e.g. a `TabManager`'s own owning window).
@MainActor
func mosaicMainWindowForModalPresentation(preferring preferredWindow: NSWindow? = nil) -> NSWindow? {
    if let preferredWindow, preferredWindow.isVisible, isMosaicMainWindow(preferredWindow) {
        return preferredWindow
    }
    if let keyWindow = NSApp.keyWindow, keyWindow.isVisible, isMosaicMainWindow(keyWindow) {
        return keyWindow
    }
    if let mainWindow = NSApp.mainWindow, mainWindow.isVisible, isMosaicMainWindow(mainWindow) {
        return mainWindow
    }
    return NSApp.windows.first { $0.isVisible && isMosaicMainWindow($0) }
}
...
/// alert as a sheet attached to the main mosaic window when one is available,
/// falling back to an app-modal `runModal()` only when there is no eligible
/// host window.
///
/// - Parameters:
///   - alert: The configured alert to present.
///   - presentingWindow: An explicit host window. When `nil`, the main mosaic
///     window is resolved via ``mosaicMainWindowForModalPresentation(preferring:)``.
...
@MainActor
func runMosaicModalAlert(
    _ alert: NSAlert,
    presentingWindow: NSWindow? = nil,
    willPresent: ((MosaicModalAlertPresentation) -> Void)? = nil
) -> NSApplication.ModalResponse {
    if NSApp.activationPolicy() == .regular {
        NSApp.activate(ignoringOtherApps: true)
    }

    let hostWindow = presentingWindow ?? mosaicMainWindowForModalPresentation()
    guard let hostWindow, hostWindow.attachedSheet == nil else {
        let presentation = MosaicModalAlertPresentation.appModal(hostWindowHadAttachedSheet: hostWindow?.attachedSheet != nil)
        MosaicModalAlertAnalytics.track(alert, presentation: presentation)
        willPresent?(presentation)
        return alert.runModal()
    }

    let presentation = MosaicModalAlertPresentation.sheet(hostWindow)
    MosaicModalAlertAnalytics.track(alert, presentation: presentation)
    willPresent?(presentation)
