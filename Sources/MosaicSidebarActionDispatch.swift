...
/// Serial lane for in-process `mosaic(...)` sidebar actions. Worker-lane methods
...
private let mosaicSidebarWorkerQueue = DispatchQueue(label: "com.mosaic.sidebar-action-worker")

// The custom-sidebar rendering, interpreter, JSON DSL, resizable split, and
// the file-watching model now live in the `MosaicSwiftRender` (logic) and
// `MosaicSwiftRenderUI` (SwiftUI) packages. The app target keeps only the
// mosaic-coupled action dispatch, the one piece that must reach
...
/// live mosaic command dispatcher.
///
/// `mosaic(...)` commands run in-process through
/// `TerminalController.handleSocketLine(_:)` (the same worker-aware surface the
/// socket CLI uses); `log` is a debug-only no-op for now.
@MainActor
func makeMosaicSidebarActionDispatch() -> SidebarActionDispatch {
    SidebarActionDispatch { action in
        ...
        mosaicSidebarWorkerQueue.async {
            for command in commands {
                switch command {
                case let .mosaic(method, params):
