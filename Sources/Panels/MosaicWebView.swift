...
extension WKWebView {
    nonisolated private static var mosaicSetPageMutedSelector: Selector {
        NSSelectorFromString("_setPageMuted:")
    }

    nonisolated private static var mosaicMediaMutedStateAudio: Int {
        1 << 0
    }

    @discardableResult
    func mosaicSetPageAudioMuted(_ muted: Bool) -> Bool {
        let selector = Self.mosaicSetPageMutedSelector
        ...
        function(self, selector, muted ? Self.mosaicMediaMutedStateAudio : 0)
        return true
    }

    var mosaicIsElementFullscreenActiveOrTransitioning: Bool {
    ...
    func mosaicIsManagedByExternalFullscreenWindow(relativeTo expectedWindow: NSWindow?) -> Bool {
        guard mosaicIsElementFullscreenActiveOrTransitioning else { return false }
...
/// Find-family shortcuts. The configured Find shortcut stays app-owned so mosaic can
/// choose browser find or right-sidebar file search from the current focus owner.
final class MosaicWebView: WKWebView {
    ...
    private static let pasteAsPlainTextFocusMessageHandlerName = "mosaicPasteAsPlainTextFocus"
    private static let browserFocusModeContextMenuItemIdentifier =
        NSUserInterfaceItemIdentifier("mosaic.browserFocusMode.toggle")
    ...
    const __mosaicPasteAsPlainTextHelpers = (() => {
      const existing = window.__mosaicPasteAsPlainTextHelpers;
      ...
      window.__mosaicPasteAsPlainTextHelpers = helpers;
      return helpers;
    })();
    """
    ...
    (() => {
      try {
        if (window.__mosaicPasteAsPlainTextFocusTrackerInstalled) return true;
        window.__mosaicPasteAsPlainTextFocusTrackerInstalled = true;
        ...
        const publish = (canPaste) => {
          ...
          window.__mosaicPasteAsPlainTextTargetAvailable = canPaste;
          try {
            handler?.postMessage({ canPaste });
          } catch (_) {}
        };

        window.__mosaicCanPasteAsPlainTextIntoCurrentFocus = () => {
          return __mosaicPasteAsPlainTextHelpers.canPasteAsPlainTextInto(document.activeElement);
        };

        const publishForElement = (el) => {
          publish(__mosaicPasteAsPlainTextHelpers.canPasteAsPlainTextInto(el));
        };
        ...
        document.addEventListener("mousedown", (ev) => {
          ...
          if (!__mosaicPasteAsPlainTextHelpers.canPasteAsPlainTextInto(target)) {
            publish(false);
          }
        }, true);
    ...
    private final class PasteAsPlainTextFocusMessageHandler: NSObject, WKScriptMessageHandler {
        func userContentController(
            ...
            guard let webView = message.webView as? MosaicWebView else {
                return
            }
    ...
    static func hasRecentMiddleClickIntent(for webView: WKWebView) -> Bool {
        guard let webView = webView as? MosaicWebView else { return false }
    ...
    private static func recordMiddleClickIntent(for webView: MosaicWebView) {
    ...
    private static var mosaicDownloadDelegateKey: UInt8 = 0
    ...
    var contextMenuLinkURLProvider: ((MosaicWebView, NSPoint, @escaping (URL?) -> Void) -> Void)?
    ...
    var mosaicDownloadDelegate: WKDownloadDelegate? {
        get {
            objc_getAssociatedObject(self, &Self.mosaicDownloadDelegateKey) as? WKDownloadDelegate
        }
        set {
            objc_setAssociatedObject(
                self,
                &Self.mosaicDownloadDelegateKey,
                newValue,
    ...
    private func pageCanAcceptPlainTextPaste() -> Bool {
        ...
        (() => {
            try {
                const fn = window.__mosaicCanPasteAsPlainTextIntoCurrentFocus;
    ...
    override func performKeyEquivalent(with event: NSEvent) -> Bool {
        ...
        if let decision = AppDelegate.shared?.handleBrowserFocusModeKeyEvent(
            ...
            case .forwardToWebView:
                ...
                // While focus mode is active, the page gets the shortcut once and mosaic/main-menu
    ...
    override func keyDown(with event: NSEvent) {
        ...
        // If this path reaches keyDown, forward it to WebKit instead of mosaic.
    ...
    private func handleMouseNavigationButton(_ event: NSEvent) -> Bool {
        ...
        case 3:
#if DEBUG
            mosaicDebugLog("browser.mouse.navigation web=\(ObjectIdentifier(self)) kind=back canGoBack=\(canGoBack ? 1 : 0)")
#endif
        ...
        case 4:
#if DEBUG
            mosaicDebugLog("browser.mouse.navigation web=\(ObjectIdentifier(self)) kind=forward canGoForward=\(canGoForward ? 1 : 0)")
#endif
    ...
    /// and its lifecycle live in `MosaicWebView+ContextMenuLinkCapture.swift`;
...
#if DEBUG
        mosaicDebugLog(Self.redactedContextDownloadDebugMessage(message()))
#endif
    }
    ...
    private static let blockedDragTypes: Set<NSPasteboard.PasteboardType> = [
        ...
        NSPasteboard.PasteboardType("com.mosaic.sidebar-tab-reorder"),
    ]
