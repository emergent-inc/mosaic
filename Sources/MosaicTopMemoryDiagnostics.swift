...
nonisolated let mosaicTopMemoryDiagnosticDefaultGroupLimit = 12

nonisolated struct MosaicTopProcessAttribution: Hashable, Sendable {
...
nonisolated extension MosaicTopProcessSnapshot {
    func memoryDiagnosticPayload(
        ...
        topGroupLimit: Int = mosaicTopMemoryDiagnosticDefaultGroupLimit,
        attributionByPID: [Int: MosaicTopProcessAttribution] = [:]
    ) -> [String: Any] {
        ...
        return [
            ...
            "app": [
                ...
                "memory_source": appProcess?.memorySource.rawValue ?? MosaicTopProcessMemorySource.unavailable.rawValue,
                "resident_memory_source": appProcess?.residentMemorySource.rawValue ?? MosaicTopProcessMemorySource.unavailable.rawValue
            ] as [String: Any],
            "children": [
                "root_pid": appPID,
    ...
    private struct MemoryDiagnosticGroupAccumulator {
        ...
        var processIDs: [Int] = []
        var attributions: [MosaicTopProcessAttribution: MemoryDiagnosticAttributionAccumulator] = [:]

        mutating func append(
            process: MosaicTopProcessInfo,
            attribution: MosaicTopProcessAttribution?
        ) {
            rssBytes = MosaicTopProcessSnapshot.clampedAdd(rssBytes, process.residentBytes)
    ...
    private struct MemoryDiagnosticAttributionAccumulator {
        ...
        mutating func append(process: MosaicTopProcessInfo) {
            rssBytes = MosaicTopProcessSnapshot.clampedAdd(rssBytes, process.residentBytes)
            processIDs.append(process.pid)
        }
    ...
    private func memoryDiagnosticGroups(
        ...
        for pid in pids.sorted() {
            ...
            groups[key]?.append(
                process: process,
                attribution: nearestMosaicAttribution(
                    for: pid,
                    attributionByPID: attributionByPID
                )
            )
    ...
    private func nearestMosaicAttribution(
        for pid: Int,
        attributionByPID: [Int: MosaicTopProcessAttribution]
        ...
        while currentPID > 0, visited.insert(currentPID).inserted {
            ...
            if process.mosaicWorkspaceID != nil || process.mosaicSurfaceID != nil {
                return MosaicTopProcessAttribution(
                    workspaceID: process.mosaicWorkspaceID,
                    workspaceRef: nil,
                    paneID: nil,
                    paneRef: nil,
                    surfaceID: process.mosaicSurfaceID,
                    surfaceRef: nil,
                    surfaceType: nil,
                    reason: process.mosaicAttributionReason ?? "mosaic-process-scope"
                )
            }
