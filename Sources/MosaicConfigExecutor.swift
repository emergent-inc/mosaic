...
@MainActor
struct MosaicConfigExecutor {

    @discardableResult
    static func execute(
        command: MosaicCommandDefinition,
        ...
        if let workspace = command.workspace {
            return authorizeProjectActionIfNeeded(
                ...
                PostHogAnalytics.shared.trackAction(
                    actionID: actionID ?? command.id,
                    surface: "mosaic_config",
                    entrypoint: "workspace_command",
                    source: "MosaicConfigExecutor.execute"
                )
                onExecuted?()
            }
    ...
    static func prepareShellInputIfAuthorized(
        ...
        confirm: Bool,
        actionID: String,
        target: MosaicConfigTerminalCommandTarget,
        configSourcePath: String?,
        globalConfigPath: String,
        displayTitle: String? = nil,
        icon: MosaicButtonIcon? = nil,
    ...
    static func authorizeProjectAutomationIfNeeded(
        descriptor: MosaicActionTrustDescriptor,
        confirm: Bool,
        configSourcePath: String?,
        globalConfigPath: String,
    ...
    private static func authorizeProjectActionIfNeeded(
        ...
        let isTrusted = MosaicActionTrust.shared.isTrusted(descriptor)
    ...
    private static func makeConfirmDialog(
        ...
        alert.messageText = (trimmedDisplayTitle?.isEmpty == false)
            ? trimmedDisplayTitle!
            : String(
                localized: "dialog.mosaicConfig.confirmCommand.title",
                defaultValue: "Run Project Action?"
            )
        let messageFormat = String(
            localized: "dialog.mosaicConfig.confirmCommand.messageWithCommand",
        ...
        alert.addButton(withTitle: String(
            localized: "dialog.mosaicConfig.confirmCommand.run",
            defaultValue: "Run Once"
        ))
        alert.addButton(withTitle: String(
            localized: "dialog.mosaicConfig.confirmCommand.trustAndRun",
            defaultValue: "Trust and Run"
        ))
        alert.addButton(withTitle: String(
            localized: "dialog.mosaicConfig.confirmCommand.cancel",
            defaultValue: "Cancel"
        ))
        return alert
    }
    ...
    private static func handleConfirmDialogResponse(
        ...
        case .alertSecondButtonReturn:
            MosaicActionTrust.shared.trust(descriptor)
            return true
        default:
            return false
        }
    ...
    private static func terminalTrustDescriptor(
        ...
        target: MosaicConfigTerminalCommandTarget,
        configSourcePath: String?,
        icon: MosaicButtonIcon?,
        iconSourcePath: String?,
        globalConfigPath: String
    ) -> MosaicActionTrustDescriptor {
        MosaicActionTrustDescriptor(
            actionID: actionID,
            ...
            projectRoot: configSourcePath.map { canonicalPath(MosaicButtonIcon.projectRoot(forConfigPath: $0)) },
    ...
    static func isTrustedSurfaceButton(
        ...
        workspaceCommand: MosaicResolvedCommand?,
        ...
        return MosaicActionTrust.shared.isTrusted(descriptor)
    }
    ...
    private static func executeWorkspaceCommand(
        command: MosaicCommandDefinition,
        workspace wsDef: MosaicWorkspaceDefinition,
        tabManager: TabManager,
        baseCwd: String
    ) -> Bool {
        ...
        if let existing = tabManager.tabs.first(where: { $0.customTitle == workspaceName }) {
            ...
            case .confirm:
                ...
                alert.messageText = String(
                    localized: "dialog.mosaicConfig.confirmRestart.title",
                    defaultValue: "Workspace Already Exists"
                )
                alert.informativeText = String(
                    localized: "dialog.mosaicConfig.confirmRestart.message",
                    defaultValue: "A workspace with this name already exists. Close it and create a new one?"
                )
                alert.alertStyle = .warning
                alert.addButton(withTitle: String(
                    localized: "dialog.mosaicConfig.confirmRestart.recreate",
                    defaultValue: "Recreate"
                ))
                alert.addButton(withTitle: String(
                    localized: "dialog.mosaicConfig.confirmRestart.cancel",
                    defaultValue: "Cancel"
                ))
        ...
        let resolvedCwd = MosaicConfigStore.resolveCwd(wsDef.cwd, relativeTo: baseCwd)
