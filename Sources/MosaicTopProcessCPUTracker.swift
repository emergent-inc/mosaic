...
nonisolated struct MosaicTopProcessCPUSample: Sendable {
    let totalTimeTicks: UInt64
    let sampledAtNanoseconds: UInt64
}

private nonisolated struct MosaicTopProcessCPUTrackerState: Sendable {
    var entries: [MosaicTopProcessScopeCacheKey: MosaicTopProcessCPUTrackerEntry] = [:]
    var latestPrunedAtNanoseconds: UInt64 = 0
}

private nonisolated struct MosaicTopProcessCPUTrackerEntry: Sendable {
    let sample: MosaicTopProcessCPUSample
    let cpuPercent: Double
    let parentKey: MosaicTopProcessScopeCacheKey?
}

private nonisolated final class MosaicTopProcessCPUTracker: @unchecked Sendable {
    private static let minimumSampleWindowNanoseconds: UInt64 = 1_000_000_000

    private let state = OSAllocatedUnfairLock(initialState: MosaicTopProcessCPUTrackerState())
    ...
    func cpuPercentages(
        for currentSamples: [MosaicTopProcessScopeCacheKey: MosaicTopProcessCPUSample],
        activeKeys: Set<MosaicTopProcessScopeCacheKey>,
        parentKeysByKey: [MosaicTopProcessScopeCacheKey: MosaicTopProcessScopeCacheKey],
        sampledAtNanoseconds: UInt64
    ) -> [MosaicTopProcessScopeCacheKey: Double] {
        state.withLock { state in
            var percentages: [MosaicTopProcessScopeCacheKey: Double] = [:]
            percentages.reserveCapacity(currentSamples.count)
            var heldKeys: Set<MosaicTopProcessScopeCacheKey> = []
            ...
            for (key, sample) in currentSamples {
                ...
                if let existing {
                    ...
                    guard elapsedNanoseconds >= Self.minimumSampleWindowNanoseconds else {
                        ...
                        if existing.parentKey != parentKey {
                            state.entries[key] = MosaicTopProcessCPUTrackerEntry(
                ...
                let cpuPercent = MosaicTopProcessSnapshot.cpuPercent(
                    current: sample,
...
private nonisolated let mosaicTopProcessCPUTracker = MosaicTopProcessCPUTracker()
private nonisolated let mosaicTopAbsoluteTimeNanosecondsRatio: Double? = {
...
nonisolated extension MosaicTopProcessSnapshot {
    ...
    static func cpuPercentages(
        for samples: [MosaicTopProcessScopeCacheKey: MosaicTopProcessCPUSample],
        activeKeys: Set<MosaicTopProcessScopeCacheKey>,
        parentKeysByKey: [MosaicTopProcessScopeCacheKey: MosaicTopProcessScopeCacheKey] = [:],
        sampledAtNanoseconds: UInt64
    ) -> [MosaicTopProcessScopeCacheKey: Double] {
        mosaicTopProcessCPUTracker.cpuPercentages(
            for: samples,
    ...
    static func cpuPercent(
        current: MosaicTopProcessCPUSample,
        previous: MosaicTopProcessCPUSample?
    ) -> Double {
        guard let previous,
    ...
    private static func absoluteTimeNanoseconds(_ ticks: UInt64) -> Double? {
        guard let ratio = mosaicTopAbsoluteTimeNanosecondsRatio else { return nil }
