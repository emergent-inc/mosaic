...
/// what `mosaic-presence-dev` verifies, so tagged dogfood builds get live
...
enum PresenceSettings {
    ...
    /// Base URL of the presence service (the mosaic-presence worker), e.g.
    /// "https://mosaic-presence.<account>.workers.dev". Empty means disabled.
    static let serviceURLKey = "presenceServiceURL"
    /// Env override for dev/tagged builds, mirroring MOSAIC_VM_API_BASE_URL.
    static let serviceURLEnvKey = "MOSAIC_PRESENCE_BASE_URL"
    /// The dev/staging worker (dev Stack project), the Debug-build default.
    /// See workers/presence/README.md.
    static let debugDefaultServiceURL = "https://mosaic-presence-dev.debussy.workers.dev"

    /// The production presence worker (prod Stack project), the Release-build
    /// default so stable mosaic can announce presence once the user enables mobile.
    /// See workers/presence/README.md.
    static let productionServiceURL = "https://presence.mosaic.dev"
