...
enum DefaultTerminalRegistration {
    ...
    static func currentStatus(
        ...
        let matchedURLSchemes = urlSchemes.filter { scheme in
            guard let url = URL(string: "\(scheme)://mosaic-default-terminal-check") else {
                return false
            }
...
enum DefaultTerminalUserAction {
...
#if DEBUG
        mosaicDebugLog("defaultTerminal.setAsDefault source=\(debugSource)")
#endif
        Task {
            do {
...
#if DEBUG
                mosaicDebugLog("defaultTerminal.setAsDefault.failed source=\(debugSource) error=\(error)")
#endif
    ...
    private static func presentSetAsDefaultError(_ error: Error) {
        ...
        alert.window.identifier = NSUserInterfaceItemIdentifier("mosaic.defaultTerminalRegistrationError")
        alert.runModal()
    }
}
...
struct TerminalDefaultFileOpenRequest: Equatable {
    ...
    init?(fileURL: URL, contentType: UTType? = nil, isExecutable: Bool? = nil) {
        ...
        guard !SessionPersistencePolicy.isMosaicCrashStorageURL(standardizedURL) else { return nil }
        guard !SessionPersistencePolicy.isMosaicCrashStorageURL(directoryCheckURL) else { return nil }
...
@MainActor
final class MosaicSSHURLProcessLauncher {
    static let shared = MosaicSSHURLProcessLauncher()
    ...
    @discardableResult
    func start(request: MosaicSSHURLRequest, preferredWindow: NSWindow?) -> Bool {
        ...
        process.terminationHandler = { [weak preferredWindow] terminatedProcess in
            ...
            Task { @MainActor in
                ...
                let format = String(
                    ...
                    defaultValue: "`mosaic ssh` exited with status %d."
                )
        ...
        do {
...
#if DEBUG
            mosaicDebugLog("sshURL.launchCLI pid=\(process.processIdentifier) socket=\(socketPath) targetLength=\(request.destination.count)")
#endif
            return true
        } catch {
            ...
            presentLaunchFailure(
                summary: String(
                    ...
                    defaultValue: "`mosaic ssh` could not be launched."
                ),
...
@MainActor
private final class MosaicSSHURLConfirmationGate: NSObject {
...
extension AppDelegate {
    ...
    func handleMosaicExternalURLs(from urls: [URL]) -> Bool {
        ...
        guard intentCounts.total == 1 else {
            if intentCounts.ssh > 1 && intentCounts.navigation == 0 && intentCounts.text == 0 {
                showMosaicSSHURLParseError(.multipleLinks)
            } else {
                showMosaicTextURLParseError(.multipleLinks)
            }
            return true
        }

        if handleMosaicSSHURLs(from: urls) {
            return true
        }
        if handleMosaicNavigationURLs(from: urls) {
            return true
        }
        if handleMosaicTextURLs(from: urls) {
            return true
        }
        return false
    }
    ...
    private func mosaicExternalURLIntentCounts(in urls: [URL]) -> MosaicExternalURLIntentCounts {
        urls.reduce(MosaicExternalURLIntentCounts()) { counts, url in
            var nextCounts = counts
            switch MosaicSSHURLRequest.parse(url) {
            case .success(.some), .failure:
                nextCounts.ssh += 1
            case .success(nil):
                break
            }
            switch MosaicNavigationURLRequest.parse(url) {
            case .success(.some), .failure:
                nextCounts.navigation += 1
            case .success(nil):
                break
            }
            switch MosaicTextURLRequest.parse(url) {
            case .success(.some), .failure:
                nextCounts.text += 1
            case .success(nil):
                break
    ...
    @discardableResult
    func handleMosaicNavigationURLs(from urls: [URL]) -> Bool {
        var navigationRequests: [MosaicNavigationURLRequest] = []
        var parseErrors: [(url: URL, error: MosaicNavigationURLParseError)] = []
...
#if DEBUG
            mosaicDebugLog("navigationURL.ignored reason=multipleLinks count=\(urls.count) intents=\(navigationIntentCount)")
#endif
            return true
        }
...
#if DEBUG
            mosaicDebugLog("navigationURL.blocked reason=\(parseError.error) url=\(parseError.url.absoluteString.prefix(160))")
#endif
            return true
        }
        ...
        if let request = navigationRequests.first {
            ...
            if handleMosaicNavigationURLRequest(request) {
    ...
    @discardableResult
    private func handleMosaicNavigationURLRequest(_ request: MosaicNavigationURLRequest) -> Bool {
...
#if DEBUG
            mosaicDebugLog("navigationURL.notFound workspace=\(workspaceId.uuidString.prefix(8))")
#endif
            return false
        }
    ...
    @discardableResult
    func handleMosaicSSHURLs(from urls: [URL]) -> Bool {
        var sshURLRequests: [MosaicSSHURLRequest] = []
        var sshURLParseErrors: [MosaicSSHURLParseError] = []
        for url in urls {
            switch MosaicSSHURLRequest.parse(url) {
            case .success(.some(let request)):
        ...
        if sshURLIntentCount > 1 {
            ...
            for error in sshURLParseErrors {
                ...
                showMosaicSSHURLParseError(error)
            }
            if let request = sshURLRequests.first {
                handleMosaicSSHURLRequest(request)
            }
        }
        return true
    }
    ...
    @discardableResult
    func handleMosaicTextURLs(from urls: [URL]) -> Bool {
        var textURLRequests: [MosaicTextURLRequest] = []
        var textURLParseErrors: [(deeplinkType: String, error: MosaicTextURLParseError)] = []
        for url in urls {
            switch MosaicTextURLRequest.parse(url) {
            case .success(.some(let request)):
        ...
        if textURLIntentCount > 1 {
            ...
            for parseError in textURLParseErrors {
                ...
                showMosaicTextURLParseError(parseError.error)
            }
            if let request = textURLRequests.first {
                handleMosaicTextURLRequest(request)
            }
        }
        return true
    }

    private func handleMosaicSSHURLRequest(_ request: MosaicSSHURLRequest) {
#if DEBUG
        let target = request.originalURL.host ?? request.originalURL.path
        mosaicDebugLog("sshURL.prompt target=\(target) destinationLength=\(request.destination.count) hasPort=\(request.port != nil)")
#endif

        trackDeeplinkReceived(type: "ssh")
        deferInitialMainWindowBootstrapForExternalConfirmation()
        guard confirmMosaicSSHURLRequest(request) else {
...
#if DEBUG
            mosaicDebugLog("sshURL.cancelled")
#endif
            return
        }
        ...
        let didStart = MosaicSSHURLProcessLauncher.shared.start(
            request: request,
    ...
    private func handleMosaicTextURLRequest(_ request: MosaicTextURLRequest) {
#if DEBUG
        let target = request.originalURL.host ?? request.originalURL.path
        mosaicDebugLog("textURL.prompt target=\(target) kind=\(request.kind.rawValue) textLength=\(request.text.count)")
#endif

        let deeplinkType = Self.deeplinkType(for: request)
        trackDeeplinkReceived(type: deeplinkType)
        deferInitialMainWindowBootstrapForExternalConfirmation()
        guard confirmMosaicTextURLRequest(request) else {
...
#if DEBUG
            mosaicDebugLog("textURL.cancelled")
#endif
            return
        }
        ...
        let didPaste = pasteTextInPreferredMainWindowFromExternalLink(
            ...
            onSendFailure: { [weak self] in
                self?.showMosaicTextURLPasteFailure(request)
            }
        )
        if !didPaste {
            showMosaicTextURLPasteFailure(request)
        }
        var analyticsProperties: [String: Any] = [
            "no_focus": request.noFocus,
        ]
    ...
    private static func deeplinkType(forNavigationTarget target: MosaicNavigationURLRequest.Target) -> String {
        switch target {
        case .workspace:
            return "workspace"
        case .pane:
    ...
    private static func analyticsLinkKind(for request: MosaicTextURLRequest) -> LinkingAnalyticsKind {
        switch request.kind {
        case .prompt:
            return .prompt
        case .rules:
    ...
    private static func analyticsLinkKind(for _: MosaicTextURLParseError) -> LinkingAnalyticsKind {
        .prompt
    }
    ...
    private static func deeplinkType(for request: MosaicTextURLRequest) -> String {
        switch request.kind {
        case .prompt:
            return "prompt"
        case .rules:
    ...
    private static func analyticsErrorKind(for error: MosaicSSHURLParseError) -> String {
        String(describing: error)
    }

    private static func analyticsErrorKind(for error: MosaicTextURLParseError) -> String {
        String(describing: error)
    }

    private func confirmMosaicSSHURLRequest(_ request: MosaicSSHURLRequest) -> Bool {
        let alert = NSAlert()
        alert.alertStyle = .warning
        alert.messageText = String(
        ...
        let gate = MosaicSSHURLConfirmationGate()
        if alert.buttons.count > 1 {
            gate.connectButton = alert.buttons[1]
        }
        alert.accessoryView = mosaicSSHURLAccessoryView(request: request, gate: gate)
    ...
    private func confirmMosaicTextURLRequest(_ request: MosaicTextURLRequest) -> Bool {
        let alert = NSAlert()
        alert.alertStyle = .warning
        ...
        alert.accessoryView = mosaicTextURLAccessoryView(request: request)
        return alert.runModal() == .alertSecondButtonReturn
    }

    private func mosaicSSHURLAccessoryView(
        request: MosaicSSHURLRequest,
        gate: MosaicSSHURLConfirmationGate
    ) -> NSView {
        ...
        let socketPath = MosaicSSHURLProcessLauncher.shared.resolvedSocketPath()
        let commandScrollView = mosaicSSHURLTextPreview(request.cliPreview(socketPath: socketPath), height: 80)
        ...
        let checkbox = NSButton(
            ...
            target: gate,
            action: #selector(MosaicSSHURLConfirmationGate.checkboxChanged(_:))
        )
    ...
    private func mosaicTextURLAccessoryView(request: MosaicTextURLRequest) -> NSView {
        let stack = NSStackView()
        stack.orientation = .vertical
        stack.alignment = .leading
        stack.spacing = 8
        ...
        let preview = mosaicSSHURLTextPreview(request.pasteText, height: 180)
    ...
    private func mosaicSSHURLTextPreview(_ text: String, height: CGFloat) -> NSScrollView {
    ...
    private func showMosaicSSHURLParseError(_ error: MosaicSSHURLParseError) {
        let alert = NSAlert()
        ...
        alert.informativeText = mosaicSSHURLParseErrorMessage(error)
        alert.addButton(withTitle: String(localized: "dialog.sshURL.blocked.ok", defaultValue: "OK"))
        alert.runModal()
    }

    private func showMosaicTextURLPasteFailure(_ request: MosaicTextURLRequest) {
        let alert = NSAlert()
        alert.alertStyle = .warning
        alert.messageText = request.kind == .prompt
    ...
    private func showMosaicTextURLParseError(_ error: MosaicTextURLParseError) {
        let alert = NSAlert()
        alert.alertStyle = .critical
        alert.messageText = String(
        ...
        alert.informativeText = mosaicTextURLParseErrorMessage(error)
        alert.addButton(withTitle: String(localized: "dialog.textURL.blocked.ok", defaultValue: "OK"))
        alert.runModal()
    }

    private func mosaicSSHURLParseErrorMessage(_ error: MosaicSSHURLParseError) -> String {
        switch error {
    ...
    private func mosaicTextURLParseErrorMessage(_ error: MosaicTextURLParseError) -> String {
        switch error {
        case .missingText:
            return String(
