...
nonisolated struct MosaicTopProcessScopeCacheKey: Hashable {
    let pid: Int
    let startSeconds: Int
    let startMicroseconds: Int
}

private nonisolated struct MosaicTopProcessScopeCacheValue {
    // nil means "this process was probed and has no mosaic scope". A negative entry
    // is honored as a hit only until `negativeExpiresAtNanos`, so a non-mosaic
    ...
    // ignored when `scope != nil`): a mosaic scope comes from inherited environment
    // or a stable argv and does not disappear for the process lifetime.
    let scope: MosaicTopProcessScope?
    let negativeExpiresAtNanos: UInt64
}

// How long a "no mosaic scope" result stays cached before the process is probed
...
// `mosaic hooks … monitor` later, must be re-probed eventually or it would never
// be attributed. The TTL bounds that attribution latency while still collapsing
// the per-poll sysctl storm for the steady-state majority of non-mosaic processes.
private nonisolated let mosaicTopNegativeScopeTTLNanoseconds: UInt64 = 15 * 1_000_000_000

// Result of probing a single process for its mosaic scope. `resolved` means the
...
nonisolated enum MosaicTopProcessScopeProbeResult: Equatable {
    case resolved(MosaicTopProcessScope?)
    case unavailable
}

// MosaicTopProcessSnapshot.capture is intentionally synchronous because it backs
...
private nonisolated let mosaicTopScopeCache = OSAllocatedUnfairLock(
    initialState: [MosaicTopProcessScopeCacheKey: MosaicTopProcessScopeCacheValue]()
)

nonisolated extension MosaicTopProcessSnapshot {
    static func scopeCacheKey(from kinfo: kinfo_proc) -> MosaicTopProcessScopeCacheKey {
        let startTime = kinfo.kp_proc.p_un.__p_starttime
        return MosaicTopProcessScopeCacheKey(
    ...
    static func scopeCacheKey(from bsdInfo: proc_bsdinfo) -> MosaicTopProcessScopeCacheKey {
        MosaicTopProcessScopeCacheKey(
    ...
    static func cachedMosaicScope(
        for pid: Int,
        cacheKey: MosaicTopProcessScopeCacheKey,
        nowNanoseconds: UInt64,
        probe: (Int, MosaicTopProcessScopeCacheKey) -> MosaicTopProcessScopeProbeResult = MosaicTopProcessSnapshot.mosaicScopeProbe
    ) -> MosaicTopProcessScope? {
        if let cached = mosaicTopScopeCache.withLock({ cache in cache[cacheKey] }) {
            if let scope = cached.scope {
                // Positive results never expire: a discovered mosaic scope is stable.
                return scope
            }
            ...
            // execed into a mosaic-scoped command since it was last sampled.
        }
        ...
        case .resolved(let scope):
            ...
            // return it.
            return mosaicTopScopeCache.withLock { cache -> MosaicTopProcessScope? in
                if scope == nil, let existing = cache[cacheKey], let existingScope = existing.scope {
                    return existingScope
                }
                cache[cacheKey] = MosaicTopProcessScopeCacheValue(
                    scope: scope,
                    negativeExpiresAtNanos: nowNanoseconds &+ mosaicTopNegativeScopeTTLNanoseconds
                )
                return scope
            }
        case .unavailable:
    ...
    static func pruneMosaicScopeCache(activeKeys: Set<MosaicTopProcessScopeCacheKey>) {
        mosaicTopScopeCache.withLock { cache in
            cache = cache.filter { activeKeys.contains($0.key) }
        }
    }

    // Probes a single process for its mosaic scope via sysctl.
    //
    ...
    // "no readable mosaic scope" — those processes are not mosaic-scoped and stay in
    ...
    static func mosaicScopeProbe(
        for pid: Int,
        expectedCacheKey: MosaicTopProcessScopeCacheKey
    ) -> MosaicTopProcessScopeProbeResult {
        ...
        return .resolved(mosaicScope(fromKernProcArgs: Array(buffer.prefix(Int(size)))))
    }

    // True when `pid` is still the same process (same start time) as the key.
    private static func processMatchesKey(
        _ pid: Int,
        _ expectedCacheKey: MosaicTopProcessScopeCacheKey
    ) -> Bool {
        guard let process = kinfoProc(for: pid) else { return false }
        return scopeCacheKey(from: process) == expectedCacheKey
    }

    static func mosaicScope(fromKernProcArgs bytes: [UInt8]) -> MosaicTopProcessScope? {
        guard let process = processArgumentsAndEnvironment(fromKernProcArgs: bytes) else {
            return nil
        }
        return mosaicScope(arguments: process.arguments, environment: process.environment)
    }

    static func mosaicScope(arguments: [String], environment: [String: String]) -> MosaicTopProcessScope? {
        if let environmentScope = mosaicScopeFromEnvironment(environment) {
            return environmentScope
        }
        if let hookScope = mosaicHookMonitorScope(arguments: arguments) {
            return hookScope
        }
        return nil
    }

    private static func mosaicScopeFromEnvironment(_ environment: [String: String]) -> MosaicTopProcessScope? {
        let workspaceID = uuidValue(in: environment, keys: ["MOSAIC_WORKSPACE_ID", "MOSAIC_TAB_ID"])
        let surfaceID = uuidValue(in: environment, keys: ["MOSAIC_SURFACE_ID", "MOSAIC_PANEL_ID"])
        guard workspaceID != nil || surfaceID != nil else { return nil }
        return MosaicTopProcessScope(
            workspaceID: workspaceID,
            surfaceID: surfaceID,
            attributionReason: "mosaic-environment"
        )
    }

    private static func mosaicHookMonitorScope(arguments: [String]) -> MosaicTopProcessScope? {
        ...
        return MosaicTopProcessScope(
            workspaceID: workspaceID,
            surfaceID: surfaceID,
            attributionReason: "mosaic-hook-arguments"
        )
    }
    ...
    private static func containsSubcommandPath(_ path: [String], in arguments: [String]) -> Bool {
        ...
        let executableName = URL(fileURLWithPath: arguments[0])
            .lastPathComponent
            .lowercased()
        guard executableName == "mosaic" else { return false }
...
nonisolated extension MosaicTopProcessArguments {
    func matchesMosaicScope(workspaceId: UUID, surfaceId: UUID) -> Bool {
        guard let scope = MosaicTopProcessSnapshot.mosaicScope(arguments: arguments, environment: environment) else {
            return false
        }
