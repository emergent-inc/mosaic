...
struct MosaicEventSubscriptionSnapshot {
    let subscription: MosaicEventSubscription
    let replay: [[String: Any]]
    let ack: [String: Any]
}

// Sendable safety: every mutable field is protected by `lock`; `semaphore` only wakes `next(timeout:)`.
final class MosaicEventSubscription: @unchecked Sendable {
    let id: UUID
...
// Sendable safety: event state is protected by `lock`; disk appends are delegated to `MosaicEventLogWriter`.
final class MosaicEventBus: @unchecked Sendable {
    static let shared = MosaicEventBus(eventLogURL: defaultEventLogURL())
    static let protocolName = "mosaic-events"
    ...
    static let defaultMaxPendingEventLogLines = MosaicEventLogWriter.defaultMaxPendingLines
    ...
    private let eventLogWriter: MosaicEventLogWriter?
    private let bootId = UUID().uuidString
    private var nextSequence: Int64 = 1
    private var retained: [[String: Any]] = []
    private var subscriptions: [UUID: MosaicEventSubscription] = [:]

    init(
        retainedEventLimit: Int = MosaicEventBus.defaultRetainedEventLimit,
        eventLogURL: URL? = nil,
        maxEventLogBytes: UInt64 = MosaicEventBus.defaultMaxEventLogBytes,
        maxEventLineBytes: Int = MosaicEventBus.defaultMaxEventLineBytes,
        maxPendingEventLogLines: Int = MosaicEventBus.defaultMaxPendingEventLogLines,
        maxPendingEventsPerSubscription: Int = MosaicEventBus.defaultMaxPendingEventsPerSubscription
    ) {
    ...
    func subscribe(
    ...
    ) -> MosaicEventSubscriptionSnapshot {
        let subscription = MosaicEventSubscription(
            names: names,
        ...
        let gapReason: String? = afterSequence.flatMap { after in
            ...
            if after > latestSequence {
                return "requested sequence is newer than this mosaic process; mosaic probably restarted"
            }
            return nil
        }
        ...
        return MosaicEventSubscriptionSnapshot(subscription: subscription, replay: replay, ack: ack)
    }

    func unsubscribe(_ subscription: MosaicEventSubscription) {
        lock.lock()
    ...
    private func removeSubscriptionIfStillActive(_ subscription: MosaicEventSubscription) {
        lock.lock()
    ...
    func heartbeat(subscription: MosaicEventSubscription) -> [String: Any] {
        [
            "type": "heartbeat",
            "protocol": Self.protocolName,
            "version": Self.protocolVersion,
    ...
    private static func sanitizedJSONValue(_ value: Any, depth: Int) -> Any {
        ...
        case let value as [String: Any]:
            ...
            if value.count > maxSanitizedObjectEntries {
                result["__mosaic_truncated_entries"] = value.count - maxSanitizedObjectEntries
            }
            return result
        case let value as [Any]:
            ...
            if value.count > maxSanitizedArrayItems {
                result.append(["__mosaic_truncated_items": value.count - maxSanitizedArrayItems])
            }
            return result
        default:
