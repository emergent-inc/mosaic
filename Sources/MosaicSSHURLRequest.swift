...
enum MosaicSSHURLParseError: Error, Equatable {
...
struct MosaicSSHURLRequest: Equatable {
    ...
    ) -> Result<MosaicSSHURLRequest?, MosaicSSHURLParseError> {
    ...
    private static func parseStandardSSHURL(_ url: URL) -> Result<MosaicSSHURLRequest?, MosaicSSHURLParseError> {
        guard let components = URLComponents(url: url, resolvingAgainstBaseURL: false) else {
            return .failure(.missingDestination)
        }
    ...
    private static func standardSSHURLPort(in components: URLComponents) -> Result<Int?, MosaicSSHURLParseError> {
    ...
    private static func structuredSSHOptions(from queryItems: [URLQueryItem]) -> Result<[String], MosaicSSHURLParseError> {
        var options: [String] = []
    ...
    private static func boundedInteger(_ value: String, parameter: String, range: ClosedRange<Int>) -> Result<Int, MosaicSSHURLParseError> {
        guard !containsUnsafeHiddenCharacter(value),
    ...
    private static func normalizedBooleanValue(named name: String, in queryItems: [URLQueryItem]) -> Result<Bool, MosaicSSHURLParseError> {
...
enum MosaicNavigationURLParseError: Error, Equatable {
    case unsupportedURLShape
    case invalidIdentifier(String)
}

struct MosaicNavigationURLRequest: Equatable {
    ...
    static func parse(
        _ url: URL,
        supportedSchemes: Set<String> = activeSupportedSchemes
    ) -> Result<MosaicNavigationURLRequest?, MosaicNavigationURLParseError> {
        guard isSupportedScheme(url.scheme, supportedSchemes: supportedSchemes) else {
            return .success(nil)
        }
        guard let components = URLComponents(url: url, resolvingAgainstBaseURL: false) else {
        ...
        if route.count == 2 {
            return .success(MosaicNavigationURLRequest(originalURL: url, target: .workspace(workspaceId)))
        }
...
enum MosaicTextURLParseError: Error, Equatable {
    case missingText
...
struct MosaicTextURLRequest: Equatable {
    ...
    static let maxTitleLength = 160
    static let supportedSchemes: Set<String> = MosaicSSHURLRequest.supportedSchemes
    static var activeSupportedSchemes: Set<String> {
    ...
    static func parse(
        _ url: URL,
        supportedSchemes: Set<String> = activeSupportedSchemes
    ) -> Result<MosaicTextURLRequest?, MosaicTextURLParseError> {
        guard isSupportedScheme(url.scheme, supportedSchemes: supportedSchemes) else {
            return .success(nil)
        }
        ...
        return .success(MosaicTextURLRequest(
            originalURL: url,
            kind: kind,
            text: text,
            name: name,
    ...
    private static func normalizedBooleanValue(named name: String, in queryItems: [ParsedQueryItem]) -> Result<Bool, MosaicTextURLParseError> {
        guard let item = queryItems.first(where: { $0.name.lowercased() == name }) else {
            return .success(false)
        }
        guard let rawValue = item.value else {
