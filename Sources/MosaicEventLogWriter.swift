...
nonisolated private let mosaicEventLogLogger = Logger(subsystem: "mosaic.com.emergent.app", category: "event-log")

// Sendable safety: pending state is protected by `lock`; file IO runs on `queue`.
final class MosaicEventLogWriter: @unchecked Sendable {
    static let defaultMaxPendingLines = 1_024

    private static let queue = DispatchQueue(label: "com.mosaicterm.event-log", qos: .utility)
    ...
    private func flushPendingLines() {
        while true {
            ...
            if pendingLines.isEmpty {
                ...
                lock.unlock()
                if droppedCount > 0 {
                    mosaicEventLogLogger.warning("Dropped \(droppedCount, privacy: .public) mosaic event log line(s) under disk backpressure")
                }
                return
            }
            lines = pendingLines
    ...
    private func append(_ lines: [String]) {
        ...
        do {
            ...
            mosaicEventLogLogger.error("Failed to append mosaic event log: \(String(describing: error), privacy: .private)")
        }
    }
