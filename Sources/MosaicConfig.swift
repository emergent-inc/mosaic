...
extension CodingUserInfoKey {
    static let mosaicWorkspaceColorDefaults = CodingUserInfoKey(rawValue: "mosaicWorkspaceColorDefaults")!
}

struct MosaicConfigFile: Codable, Sendable {
    var actions: [String: MosaicConfigActionDefinition]
    var ui: MosaicConfigUIDefinition?
    var notifications: MosaicNotificationConfigDefinition?
    var newWorkspaceCommand: String?
    var surfaceTabBarButtons: [MosaicSurfaceTabBarButton]?
    var commands: [MosaicCommandDefinition]
    var vault: MosaicVaultConfigDefinition?
    var workspaceGroups: MosaicConfigWorkspaceGroupsDefinition?
    ...
    init(
        actions: [String: MosaicConfigActionDefinition] = [:],
        ui: MosaicConfigUIDefinition? = nil,
        notifications: MosaicNotificationConfigDefinition? = nil,
        newWorkspaceCommand: String? = nil,
        surfaceTabBarButtons: [MosaicSurfaceTabBarButton]? = nil,
        commands: [MosaicCommandDefinition] = [],
        vault: MosaicVaultConfigDefinition? = nil,
        workspaceGroups: MosaicConfigWorkspaceGroupsDefinition? = nil
    ) {
    ...
    init(from decoder: Decoder) throws {
        ...
        let decodedActions = try container.decodeIfPresent(
            [String: MosaicConfigActionDefinition].self,
            forKey: .actions
        ) ?? [:]
        ...
        ui = try container.decodeIfPresent(MosaicConfigUIDefinition.self, forKey: .ui)
        notifications = try container.decodeIfPresent(MosaicNotificationConfigDefinition.self, forKey: .notifications)
        ...
        let rootSurfaceButtons = try container.decodeIfPresent(
            [MosaicSurfaceTabBarButton].self,
        ...
        commands = try container.decodeIfPresent([MosaicCommandDefinition].self, forKey: .commands) ?? []
        vault = try container.decodeIfPresent(MosaicVaultConfigDefinition.self, forKey: .vault)
        workspaceGroups = try container.decodeIfPresent(
            MosaicConfigWorkspaceGroupsDefinition.self,
            forKey: .workspaceGroups
        )
    }

    private static func normalizedActions(
        _ decodedActions: [String: MosaicConfigActionDefinition],
        codingPath: [CodingKey]
    ) throws -> [String: MosaicConfigActionDefinition] {
        var actions: [String: MosaicConfigActionDefinition] = [:]
        ...
        for (rawID, action) in decodedActions {
            ...
            let canonicalID = MosaicSurfaceTabBarBuiltInAction(configID: id)?.configID ?? id
    ...
    private static func validatedSurfaceTabBarButtons(
        ...
        codingPath: [CodingKey]
    ) throws -> [MosaicSurfaceTabBarButton] {
        var seen = Set<String>()
...
struct MosaicConfigWorkspaceGroupsDefinition: Codable, Sendable, Equatable {
    var byCwd: [String: MosaicConfigWorkspaceGroupEntry]?
...
struct MosaicConfigWorkspaceGroupEntry: Codable, Sendable, Equatable {
    var color: String?
    var icon: String?
    var contextMenu: [MosaicConfigContextMenuItem]?
...
struct MosaicResolvedWorkspaceGroupConfig: Sendable, Equatable {
    let originalKey: String
    let normalizedKey: String
    let isGlob: Bool
    let color: String?
    let iconSymbol: String?
    let contextMenuItems: [MosaicResolvedConfigContextMenuItem]
...
enum MosaicNotificationHooksMode: String, Codable, Sendable, Hashable {
    case append
    case replace
}

struct MosaicNotificationConfigDefinition: Codable, Sendable, Hashable {
    var hooks: [MosaicNotificationHookDefinition]?
    var hooksMode: MosaicNotificationHooksMode?
...
struct MosaicNotificationHookDefinition: Codable, Sendable, Hashable {
...
struct MosaicResolvedNotificationHook: Sendable, Hashable {
    let id: String
    let command: String
    let timeoutSeconds: TimeInterval
    let sourcePath: String?
    let cwd: String
    let trustDescriptor: MosaicActionTrustDescriptor?
    ...
    init(
        ...
        sourcePath: String?,
        cwd: String,
        trustDescriptor: MosaicActionTrustDescriptor? = nil
    ) {
        self.id = id
        self.command = command
        self.timeoutSeconds = timeoutSeconds
    ...
    static func == (lhs: MosaicResolvedNotificationHook, rhs: MosaicResolvedNotificationHook) -> Bool {
        lhs.id == rhs.id &&
...
enum MosaicConfigTerminalCommandTarget: String, Codable, Sendable, Hashable {
    case currentTerminal
    case newTabInCurrentPane

    static let defaultForActions: MosaicConfigTerminalCommandTarget = .newTabInCurrentPane
}

extension MosaicSurfaceTabBarBuiltInAction {
    init(from decoder: Decoder) throws {
...
enum MosaicConfigAgentKind: Sendable, Hashable {
    case codex
    case claudeCode
    ...
    var defaultIcon: MosaicButtonIcon {
        switch self {
        case .codex:
...
extension MosaicConfigAgentKind: Codable {
    init(from decoder: Decoder) throws {
        let container = try decoder.singleValueContainer()
        let value = try container.decode(String.self)
            .trimmingCharacters(in: .whitespacesAndNewlines)
...
enum MosaicButtonIcon: Codable, Sendable, Hashable {
    case symbol(String)
    ...
    func bonsplitIcon(
        ...
        case .imagePath(let path):
            guard let preparedImage = Self.preparedImageAsset(
                ...
                NSLog("[MosaicConfig] icon image path is not allowed: %@", path)
    ...
    func resolvingRelativeImagePath(relativeToConfig configSourcePath: String?) -> MosaicButtonIcon {
    ...
    private static func preparedImageAsset(
        ...
        guard let data = FileManager.default.contents(atPath: resolvedPath) else {
            NSLog("[MosaicConfig] icon image does not exist: %@", resolvedPath)
            return nil
        }
        guard data.count <= maxImageBytes else {
            NSLog("[MosaicConfig] icon image is too large: %@", resolvedPath)
            return nil
        }
        if looksLikeSVGPath(resolvedPath), !isSafeSVG(data: data) {
            NSLog("[MosaicConfig] icon SVG contains unsupported content: %@", resolvedPath)
            return nil
        }
    ...
    static func projectRoot(forConfigPath configPath: String) -> String {
        ...
        if (configDir as NSString).lastPathComponent == ".mosaic" {
...
struct MosaicConfigActionDefinition: Codable, Sendable, Hashable {
    var action: MosaicSurfaceTabBarButtonAction?
    var title: String?
    var subtitle: String?
    var keywords: [String]?
    var palette: Bool?
    var shortcut: StoredShortcut?
    var icon: MosaicButtonIcon?
    var tooltip: String?
    var confirm: Bool?
    var terminalCommandTarget: MosaicConfigTerminalCommandTarget?
    ...
    init(
        action: MosaicSurfaceTabBarButtonAction? = nil,
        title: String? = nil,
        subtitle: String? = nil,
        keywords: [String]? = nil,
        palette: Bool? = nil,
        shortcut: StoredShortcut? = nil,
        icon: MosaicButtonIcon? = nil,
        tooltip: String? = nil,
        confirm: Bool? = nil,
        terminalCommandTarget: MosaicConfigTerminalCommandTarget? = nil
    ) {
        self.action = action
        self.title = title
    ...
    init(from decoder: Decoder) throws {
        ...
        icon = try container.decodeIfPresent(MosaicButtonIcon.self, forKey: .icon)
        ...
        terminalCommandTarget = try container.decodeIfPresent(MosaicConfigTerminalCommandTarget.self, forKey: .target)
        ...
        case "builtin":
            ...
            guard let builtIn = MosaicSurfaceTabBarBuiltInAction(configID: raw) else {
                throw DecodingError.dataCorruptedError(
                    forKey: .builtin,
                    in: container,
        ...
        case "agent":
            let agent = try container.decode(MosaicConfigAgentKind.self, forKey: .agent)
...
enum MosaicSurfaceTabBarButtonAction: Sendable, Hashable {
    case builtIn(MosaicSurfaceTabBarBuiltInAction)
    case command(String)
    case agent(MosaicConfigAgentKind, args: String?)
    ...
    var defaultButtonIcon: MosaicButtonIcon {
        switch self {
        case .builtIn(let action):
...
struct MosaicSurfaceTabBarButton: Codable, Sendable, Hashable, Identifiable {
    var id: String
    var title: String?
    var icon: MosaicButtonIcon?
    var tooltip: String?
    ...
    static let newTerminal = actionReference(MosaicSurfaceTabBarBuiltInAction.newTerminal.configID)
    static let newBrowser = actionReference(MosaicSurfaceTabBarBuiltInAction.newBrowser.configID)
    static let splitRight = actionReference(MosaicSurfaceTabBarBuiltInAction.splitRight.configID)
    static let splitDown = actionReference(MosaicSurfaceTabBarBuiltInAction.splitDown.configID)

    static let defaults: [MosaicSurfaceTabBarButton] = [
        .newTerminal,
        .newBrowser,
        .splitRight,
        .splitDown
    ...
    static func builtIn(
        _ action: MosaicSurfaceTabBarBuiltInAction,
        id: String? = nil,
        title: String? = nil,
        icon: MosaicButtonIcon? = nil,
        tooltip: String? = nil
    ) -> MosaicSurfaceTabBarButton {
        MosaicSurfaceTabBarButton(
    ...
    var resolvedTerminalCommandTarget: MosaicConfigTerminalCommandTarget {
        terminalCommandTarget ?? MosaicConfigTerminalCommandTarget.defaultForActions
    }
    ...
    init(
        ...
        icon: MosaicButtonIcon? = nil,
        tooltip: String? = nil,
        action: MosaicSurfaceTabBarButtonAction,
        confirm: Bool? = nil,
        terminalCommandTarget: MosaicConfigTerminalCommandTarget? = nil,
    ...
    init(from decoder: Decoder) throws {
        if let legacy = try? decoder.singleValueContainer().decode(String.self) {
            ...
            self = MosaicSurfaceTabBarButton(
                id: MosaicSurfaceTabBarBuiltInAction(configID: trimmed)?.configID ?? trimmed,
                action: .actionReference(MosaicSurfaceTabBarBuiltInAction(configID: trimmed)?.configID ?? trimmed)
            )
            return
        }
        ...
        let explicitIcon = try container.decodeIfPresent(MosaicButtonIcon.self, forKey: .icon)
        ...
        let rawAgent = try container.decodeIfPresent(MosaicConfigAgentKind.self, forKey: .agent)
        ...
        if let rawType {
            ...
            guard let builtIn = MosaicSurfaceTabBarBuiltInAction(configID: rawBuiltin) else {
                throw DecodingError.dataCorruptedError(
                    forKey: .builtin,
                    in: container,
            ...
            action = .actionReference(rawAction)
                  ...
                  let builtIn = MosaicSurfaceTabBarBuiltInAction(configID: explicitId) {
            action = .builtIn(builtIn)
        } else {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
    ...
    func resolved(
        actions: [String: MosaicResolvedConfigAction],
        codingPath: [CodingKey]
    ) throws -> MosaicSurfaceTabBarButton {
        ...
        let resolvedIdentifier = MosaicSurfaceTabBarBuiltInAction(configID: identifier)?.configID ?? identifier
        if let definition = actions[resolvedIdentifier] {
            return MosaicSurfaceTabBarButton(
                id: id,
        ...
        if let builtIn = MosaicSurfaceTabBarBuiltInAction(configID: identifier) {
            return MosaicSurfaceTabBarButton(
                id: id,
                title: title,
                icon: icon,
...
struct MosaicResolvedConfigAction: Identifiable, Sendable, Hashable {
    var id: String
    var title: String
    var subtitle: String?
    var keywords: [String]
    ...
    func applying(
        _ definition: MosaicConfigActionDefinition,
        sourcePath: String?
    ...
    static func fromDefinition(
        id: String,
        definition: MosaicConfigActionDefinition,
        sourcePath: String?
    ) -> MosaicResolvedConfigAction? {
        ...
        return MosaicResolvedConfigAction(
            id: id,
            title: title,
    ...
    static func builtIn(_ builtIn: MosaicSurfaceTabBarBuiltInAction) -> MosaicResolvedConfigAction {
        let title: String
        let keywords: [String]
        switch builtIn {
        case .newWorkspace:
        ...
        return MosaicResolvedConfigAction(
            id: builtIn.configID,
            title: title,
            subtitle: String(localized: "command.mosaicConfig.builtInSubtitle", defaultValue: "mosaic"),
            keywords: keywords,
            palette: true,
            shortcut: nil,
    ...
    private static func defaultTitle(for id: String, action: MosaicSurfaceTabBarButtonAction) -> String {
        switch action {
        case .agent(let agent, _):
            switch agent {
            case .codex:
                return String(localized: "command.mosaicConfig.defaultCodexTitle", defaultValue: "Codex")
            case .claudeCode:
                return String(localized: "command.mosaicConfig.defaultClaudeCodeTitle", defaultValue: "Claude Code")
            }
        case .command:
            return id
        case .workspaceCommand(let commandName):
...
struct MosaicCommandDefinition: Codable, Sendable, Identifiable {
    var name: String
    var description: String?
    var keywords: [String]?
    var restart: MosaicRestartBehavior?
    var workspace: MosaicWorkspaceDefinition?
    var command: String?
    var confirm: Bool?

    var id: String {
        "mosaic.config.command." + (name.addingPercentEncoding(withAllowedCharacters: .alphanumerics) ?? name)
    }
    ...
    init(
        ...
        keywords: [String]? = nil,
        restart: MosaicRestartBehavior? = nil,
        workspace: MosaicWorkspaceDefinition? = nil,
        command: String? = nil,
        confirm: Bool? = nil
    ) {
        self.name = name
    ...
    init(from decoder: Decoder) throws {
        ...
        keywords = try container.decodeIfPresent([String].self, forKey: .keywords)
        restart = try container.decodeIfPresent(MosaicRestartBehavior.self, forKey: .restart)
        workspace = try container.decodeIfPresent(MosaicWorkspaceDefinition.self, forKey: .workspace)
...
indirect enum MosaicLayoutNode: Codable, Sendable {
    case pane(MosaicPaneDefinition)
    case split(MosaicSplitDefinition)
    ...
    init(from decoder: Decoder) throws {
        ...
        if hasPane && hasDirection {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
                    debugDescription: "MosaicLayoutNode must not contain both 'pane' and 'direction' keys"
                )
            )
        }

        if hasPane {
            let pane = try container.decode(MosaicPaneDefinition.self, forKey: .pane)
            self = .pane(pane)
        } else if hasDirection {
            let splitDef = try MosaicSplitDefinition(from: decoder)
            self = .split(splitDef)
        } else {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
                    debugDescription: "MosaicLayoutNode must contain either a 'pane' key or a 'direction' key"
                )
            )
        }
    }
...
struct MosaicSplitDefinition: Codable, Sendable {
    var direction: MosaicSplitDirection
    var split: Double?
    var children: [MosaicLayoutNode]

    init(direction: MosaicSplitDirection, split: Double? = nil, children: [MosaicLayoutNode]) {
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        direction = try container.decode(MosaicSplitDirection.self, forKey: .direction)
        split = try container.decodeIfPresent(Double.self, forKey: .split)
        children = try container.decode([MosaicLayoutNode].self, forKey: .children)
        if children.count != 2 {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
...
enum MosaicSplitDirection: String, Codable, Sendable {
    case horizontal
    case vertical
}

struct MosaicPaneDefinition: Codable, Sendable {
    var surfaces: [MosaicSurfaceDefinition]

    init(surfaces: [MosaicSurfaceDefinition]) {
        self.surfaces = surfaces
    }

    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        surfaces = try container.decode([MosaicSurfaceDefinition].self, forKey: .surfaces)
        if surfaces.isEmpty {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
...
struct MosaicSurfaceDefinition: Codable, Sendable {
    var type: MosaicSurfaceType
    var name: String?
    var command: String?
    var cwd: String?
...
enum MosaicSurfaceType: String, Codable, Sendable {
    case terminal
    case browser
    case project
}

struct MosaicResolvedCommand: Sendable {
    let command: MosaicCommandDefinition
    let sourcePath: String?
}

struct MosaicConfigIssue: Identifiable, Equatable, Sendable {
...
@MainActor
final class MosaicConfigStore: ObservableObject {
    private static let defaultNewWorkspaceContextMenu: [MosaicConfigContextMenuItem] = [
        .action(MosaicConfigContextMenuActionItem(action: MosaicSurfaceTabBarBuiltInAction.newWorkspace.configID)),
        .action(MosaicConfigContextMenuActionItem(action: MosaicSurfaceTabBarBuiltInAction.cloudVM.configID)),
    ]

    @Published private(set) var loadedCommands: [MosaicCommandDefinition] = []
    @Published private(set) var loadedActions: [MosaicResolvedConfigAction] = []
    @Published private(set) var newWorkspaceCommandName: String?
    @Published private(set) var newWorkspaceActionID: String?
    @Published private(set) var newWorkspaceContextMenuItems: [MosaicResolvedConfigContextMenuItem] = []
    ...
    /// configured.
    @Published private(set) var workspaceGroupConfigs: [MosaicResolvedWorkspaceGroupConfig] = []
    @Published private(set) var surfaceTabBarButtons: [MosaicSurfaceTabBarButton] = MosaicSurfaceTabBarButton.defaults
    @Published private(set) var notificationHooks: [MosaicResolvedNotificationHook] = []
    @Published private(set) var configurationIssues: [MosaicConfigIssue] = []
    ...
    private(set) var actionLookup: [String: MosaicResolvedConfigAction] = [:]
    ...
    private struct ActionEntry {
        let definition: MosaicConfigActionDefinition
        let sourcePath: String?
    }
    ...
    private struct ResolvedSurfaceTabBarButtons {
        let buttons: [MosaicSurfaceTabBarButton]
        let terminalCommandSourcePaths: [String: String]
    }

    private struct ResolvedContextMenuItems {
        let items: [MosaicResolvedConfigContextMenuItem]
        let issues: [MosaicConfigIssue]
    }

    private struct NewWorkspaceCommandResolution {
        let command: MosaicResolvedCommand?
        let issue: MosaicConfigIssue?
    }
    ...
    private var surfaceTabBarWorkspaceCommands: [String: MosaicResolvedCommand] = [:]
    private var resolvedNewWorkspaceCommandCache: MosaicResolvedCommand?
    private var resolvedNewWorkspaceActionCache: MosaicResolvedConfigAction?
    ...
    // The global config and hook files use MosaicFileWatch.FileWatcher.
    ...
    private let watchQueue = DispatchQueue(label: "com.mosaic.config-file-watch")
    ...
    private static func searchDirectoryForLocalConfigPath(_ path: String) -> String {
        ...
        if (configDirectory as NSString).lastPathComponent == ".mosaic" {
    ...
    init(
        globalConfigPath: String = MosaicConfigStore.defaultGlobalConfigPath(),
        ...
        NotificationCenter.default.publisher(for: MosaicActionTrust.didChangeNotification)
    ...
    func notificationHooks(startingFrom directory: String?) -> [MosaicResolvedNotificationHook] {
        let globalConfig = parseConfig(at: globalConfigPath).config
        let localConfigs: [(path: String, config: MosaicConfigFile)]
        if let directory, !directory.isEmpty {
            localConfigs = findMosaicConfigHierarchy(startingFrom: directory).compactMap { path in
    ...
    private func resolvedLocalConfigPath(startingFrom directory: String) -> String {
        findMosaicConfig(startingFrom: directory)
            ?? defaultLocalConfigPath(startingFrom: directory)
    }

    private func defaultLocalConfigPath(startingFrom directory: String) -> String {
        (((directory as NSString).appendingPathComponent(".mosaic") as NSString)
            .appendingPathComponent("mosaic.json"))
    }

    private func findMosaicConfig(startingFrom directory: String) -> String? {
        var current = directory
        let fs = FileManager.default
        while true {
            let candidates = [
                ((current as NSString).appendingPathComponent(".mosaic") as NSString)
                    .appendingPathComponent("mosaic.json"),
    ...
    private func findMosaicConfigHierarchy(startingFrom directory: String) -> [String] {
        var current = directory
        let fs = FileManager.default
        var paths: [String] = []
        while true {
    ...
    func loadAll() {
        ...
        var configuredNewWorkspaceContextMenu: [MosaicConfigContextMenuItem]?
        var configuredNewWorkspaceContextMenuSourcePath: String?
        var configuredSurfaceTabBarButtons: [MosaicSurfaceTabBarButton]?
        ...
        let configuredButtons = configuredSurfaceTabBarButtons ?? MosaicSurfaceTabBarButton.defaults
        let defaultResolvedButtons = (try? MosaicSurfaceTabBarButton.defaults.map {
    ...
    private func resolvedLocalNotificationHookPaths(fallbackLocalPath: String?) -> [String] {
        if let searchDirectory = localConfigSearchDirectory {
            var paths = findMosaicConfigHierarchy(startingFrom: searchDirectory)
    ...
    private func resolveNotificationHooks(
        globalConfig: MosaicConfigFile?,
        localConfigs: [(path: String, config: MosaicConfigFile)]
    ) -> [MosaicResolvedNotificationHook] {
        var hooks: [MosaicResolvedNotificationHook] = []
    ...
    private func resolvedNotificationHooks(
        _ definitions: [MosaicNotificationHookDefinition],
        sourcePath: String
    ) -> [MosaicResolvedNotificationHook] {
        let cwd = MosaicButtonIcon.projectRoot(forConfigPath: sourcePath)
        ...
        return definitions.compactMap { definition in
            ...
            if isGlobalHook {
                ...
                trustDescriptor = MosaicActionTrustDescriptor(
            ...
            return MosaicResolvedNotificationHook(
                id: definition.id,
                command: definition.command,
    ...
    private func actionEntries(
        from actions: [String: MosaicConfigActionDefinition],
        sourcePath: String?
    ) -> [String: ActionEntry] {
    ...
    private func resolvedActionRegistry(
        ...
        var registry = Dictionary(
            uniqueKeysWithValues: MosaicSurfaceTabBarBuiltInAction.allCases.map {
                ($0.configID, MosaicResolvedConfigAction.builtIn($0))
            }
        )

        func apply(_ entries: [String: ActionEntry]) {
            for (id, entry) in entries {
                let registryID = MosaicSurfaceTabBarBuiltInAction(configID: id)?.configID ?? id
                if let existing = registry[registryID] {
                ...
                } else if let resolved = MosaicResolvedConfigAction.fromDefinition(
                    id: id,
                    ...
                    NSLog("[MosaicConfig] action '%@' ignored because it does not define a runnable action", id)
                }
            }
        }
        ...
        for command in commands where registry[command.id] == nil {
            ...
            registry[command.id] = MosaicResolvedConfigAction(
                id: command.id,
                title: String(
                    localized: "command.mosaicConfig.customTitle",
                    defaultValue: "Custom: \(sanitizeConfigText(command.name))"
                ),
                subtitle: command.description.map { sanitizeConfigText($0) }
                    ?? String(localized: "command.mosaicConfig.subtitle", defaultValue: "mosaic.json"),
    ...
    private func resolvedSurfaceTabBarButtons(
        ...
        var resolvedButtons: [MosaicSurfaceTabBarButton] = []
        ...
        for button in buttons {
            do {
                ...
                NSLog("[MosaicConfig] %@ ignored: %@", settingName, String(describing: error))
                return nil
            }
        }
    ...
    private func resolvedSurfaceTabBarButton(
        _ button: MosaicSurfaceTabBarButton,
        actions: [String: MosaicResolvedConfigAction]
        ...
        if let entry = actions[resolvedIdentifier] {
            let resolvedButton = MosaicSurfaceTabBarButton(
                id: button.id,
        ...
        if let builtIn = MosaicSurfaceTabBarBuiltInAction(configID: identifier) {
            return ResolvedSurfaceTabBarButtonEntry(
                button: MosaicSurfaceTabBarButton(
                    id: button.id,
                    title: button.title,
                    icon: button.icon,
    ...
    private func resolvedSurfaceTabBarWorkspaceCommands(
    ...
    ) -> (buttons: [MosaicSurfaceTabBarButton], workspaceCommands: [String: MosaicResolvedCommand]) {
        var visibleButtons: [MosaicSurfaceTabBarButton] = []
        var workspaceCommands: [String: MosaicResolvedCommand] = [:]
        ...
        for button in buttons {
            ...
            guard let command = resolvedWorkspaceCommand(
                ...
                NSLog(
                    "[MosaicConfig] surfaceTabBarButtons action '%@' hidden because workspace command '%@' is unavailable",
                    button.id,
                    commandName
                )
                continue
    ...
    func resolvedNewWorkspaceCommand() -> MosaicResolvedCommand? {
        resolvedNewWorkspaceCommandCache
    }

    func resolvedNewWorkspaceAction() -> MosaicResolvedConfigAction? {
        resolvedNewWorkspaceActionCache
    }

    func resolvedAction(id: String) -> MosaicResolvedConfigAction? {
        actionLookup[canonicalActionID(id)]
    }

    func paletteCustomActions() -> [MosaicResolvedConfigAction] {
        let builtInIDs = Set(MosaicSurfaceTabBarBuiltInAction.allCases.map(\.configID))
        return loadedActions.filter { action in
            action.palette && !builtInIDs.contains(action.id)
        }
    }

    func shortcutActions() -> [MosaicResolvedConfigAction] {
        let builtInIDs = Set(MosaicSurfaceTabBarBuiltInAction.allCases.map(\.configID))
        return loadedActions.filter { action in
    ...
    private func resolvedConfiguredNewWorkspaceAction(
        ...
        if let actionID {
            ...
            guard let action = actions[resolvedActionID] else {
                ...
                NSLog("[MosaicConfig] %@", issue.logMessage)
        ...
        return NewWorkspaceActionResolution(
            action: MosaicResolvedConfigAction(
    ...
    func resolveWorkspaceGroupConfig(forCwd cwd: String?) -> MosaicResolvedWorkspaceGroupConfig? {
        guard let cwd, !cwd.isEmpty, !workspaceGroupConfigs.isEmpty else { return nil }
        let normalizedCwd = Self.normalizeAbsolutePath(cwd)
        var best: (MosaicResolvedWorkspaceGroupConfig, Int)?
    ...
    private static func cwdEntryMatches(
        _ entry: MosaicResolvedWorkspaceGroupConfig,
        cwd: String
    ) -> Bool {
    ...
    private func resolveWorkspaceGroupConfigsFromLayers(
        ...
        commands: [MosaicCommandDefinition],
        sourcePaths: [String: String],
        issues: inout [MosaicConfigIssue]
    ) -> [MosaicResolvedWorkspaceGroupConfig] {
        var resolved: [String: MosaicResolvedWorkspaceGroupConfig] = [:]
    ...
    private func resolveWorkspaceGroupConfigEntry(
        key: String,
        entry: MosaicConfigWorkspaceGroupEntry,
        sourcePath: String,
        actions: [String: MosaicResolvedConfigAction],
        commands: [MosaicCommandDefinition],
        sourcePaths: [String: String],
        ...
        return MosaicResolvedWorkspaceGroupConfig(
            originalKey: trimmed,
    ...
    private func resolvedConfigContextMenuItems(
        _ configuredItems: [MosaicConfigContextMenuItem]?,
        actions: [String: MosaicResolvedConfigAction],
        commands: [MosaicCommandDefinition],
        sourcePaths: [String: String],
        settingName: String,
        ...
        var resolvedItems: [MosaicResolvedConfigContextMenuItem] = []
        var issues: [MosaicConfigIssue] = []
        ...
        for (index, configuredItem) in configuredItems.enumerated() {
            ...
            case .action(let item):
                ...
                resolvedItems.append(
                    .action(
                        MosaicResolvedConfigMenuAction(
    ...
    private func canonicalActionID(_ id: String) -> String {
        MosaicSurfaceTabBarBuiltInAction(configID: id)?.configID ?? id
    }
    ...
    private func resolvedConfiguredNewWorkspaceCommand(
        ...
        return NewWorkspaceCommandResolution(
            command: MosaicResolvedCommand(command: command, sourcePath: sourcePaths[command.id]),
            issue: nil
        )
    }

    private func newWorkspaceResolutionIssue(
        kind: MosaicConfigIssue.Kind,
        settingName: String,
        commandName: String?,
        sourcePath: String?
    ) -> NewWorkspaceCommandResolution {
    ...
    private func resolvedWorkspaceCommand(
        ...
        guard let command = commands.first(where: { $0.name == commandName }) else {
            NSLog("[MosaicConfig] %@ '%@' does not match any loaded command", settingName, commandName)
            return nil
        }
        guard command.workspace != nil else {
            NSLog("[MosaicConfig] %@ '%@' must reference a workspace command", settingName, commandName)
            return nil
        }
        return MosaicResolvedCommand(command: command, sourcePath: sourcePaths[command.id])
    }
    ...
    private func parseConfig(at path: String) -> ParsedConfigResult {
        ...
        guard let data = fileManager.contents(atPath: path),
              !data.isEmpty else {
            let issue = schemaIssue(path: path, message: "mosaic.json is empty")
        ...
        do {
            ...
            NSLog("[MosaicConfig] JSONC preprocessing error at %@: %@", path, String(describing: error))
            return ParsedConfigResult(config: nil, issue: issue)
        }

        do {
            let config = try JSONDecoder().decode(MosaicConfigFile.self, from: sanitized)
            parsedConfigCache[path] = ParsedConfigCacheEntry(
                fileSize: fileSize,
                modificationDate: modificationDate,
                workspaceColorPaletteFingerprint: paletteFingerprint,
            ...
            NSLog("[MosaicConfig] parse error at %@: %@", path, String(describing: error))
            return ParsedConfigResult(config: nil, issue: issue)
        }
    }

    private func schemaIssue(path: String, message: String) -> MosaicConfigIssue {
        MosaicConfigIssue(
            kind: .schemaError,
    ...
    /// Watches the global config via ``MosaicFileWatch/FileWatcher``, which handles
