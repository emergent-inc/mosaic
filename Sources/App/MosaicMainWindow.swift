...
final class MainWindowHostingView<Content: View>: NSHostingView<Content> {
    ...
    override var fittingSize: NSSize { MosaicMainWindow.minimumContentSize }
    override var intrinsicContentSize: NSSize { MosaicMainWindow.minimumContentSize }
...
@MainActor
func configureMosaicMainWindowDragBehavior(_ window: NSWindow) {
    window.isMovableByWindowBackground = false
    window.isMovable = false
}

@MainActor
final class MosaicMainWindow: NSWindow {
    ...
    /// mosaic creates its main window programmatically (never from a nib), so it
    ...
    /// `.fullScreenNone`) so a mosaic main window can always enter a native
    ...
    /// mosaic owns main-window placement: it persists and restores window frames
    ...
    /// arrangements. Because mosaic never re-asserts the saved frame after wake,
