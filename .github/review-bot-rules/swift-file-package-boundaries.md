...
- More than 250 lines added to an existing production Swift file that is already over 800 lines, unless the PR is actively extracting code out of that file. Treat the exception as met only when the PR removes or moves one of the mixed responsibilities listed below out of the file, or documents that responsibility behind a new package boundary, and the file's total line count decreases by more than 200 lines, or when a new SwiftPM package target is introduced and the oversized file shrinks because code moved into that target.
