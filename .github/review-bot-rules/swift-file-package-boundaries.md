...
mosaic already has a checked-in Swift file length budget reference (`.github/swift-file-length-budget.tsv` and `scripts/swift_file_length_budget.py`). This rule is the enforcement layer for review: do not satisfy it by mechanically moving code around, and do not expand the TSV budget for a feature that should instead be split by responsibility or extracted into a package.
...
- A feature implemented directly in the app target/module's root `Sources/` path when its core logic is independent of mosaic app lifecycle and can compile/test without AppKit, SwiftUI view state, Ghostty globals, or process-wide singletons.
...
Line counting follows the existing budget script as a shared measurement convention, even if that script is not required as an active CI gate: count physical lines including blank lines; scan mosaic-owned Swift files under `Sources`, `CLI`, `Packages`, `mosaicTests`, and `mosaicUITests`; exclude whole path subtrees containing `/vendor/`, `/ghostty/`, `/homebrew-mosaic/`, `/SourcePackages/`, or `/.ci-source-packages/`; and use 500 lines as the tracked-file reference threshold from `.github/swift-file-length-budget.tsv`. For this LLM rule, use the post-change physical file length when visible; use PR added-line count only for the "more than 250 lines added" growth check.

Package-boundary signals:

- The code has a stable domain noun and public API that can be expressed without view types.
- The code needs tests that should run without launching mosaic or constructing app UI.
