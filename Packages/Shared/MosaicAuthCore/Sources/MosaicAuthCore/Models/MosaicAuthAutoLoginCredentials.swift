...
/// sign-in.
public struct MosaicAuthAutoLoginCredentials: Equatable, Sendable {
    /// The account email.
    ...
    /// (`MOSAIC_UITEST_STACK_EMAIL` / `MOSAIC_UITEST_STACK_PASSWORD`), or `nil`
    ...
    public init?(
        ...
        guard let email = environment["MOSAIC_UITEST_STACK_EMAIL"], !email.isEmpty else {
            return nil
        }
        guard let password = environment["MOSAIC_UITEST_STACK_PASSWORD"], !password.isEmpty else {
            return nil
        }
