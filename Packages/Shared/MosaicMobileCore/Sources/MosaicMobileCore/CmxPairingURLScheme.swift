...
/// The channel-specific URL scheme carried by mosaic pairing/attach deep links.
///
/// All builds used to register and emit one scheme (`mosaic-ios`), so scanning a
...
///
/// The iOS app's registered scheme comes from `MOSAIC_IOS_URL_SCHEME` in
...
public struct CmxPairingURLScheme {
    ...
    public static let release = "mosaic-ios"

    /// The scheme development (DEBUG/tagged) builds register and emit.
    public static let development = "mosaic-ios-dev"

    /// Every scheme any mosaic build may emit; parsers accept all of them.
    ...
    /// Whether `scheme` is a pairing scheme from any mosaic channel.
