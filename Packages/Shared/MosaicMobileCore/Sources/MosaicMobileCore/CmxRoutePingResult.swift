...
public enum CmxRoutePingResult: Sendable, Equatable {
    ...
    /// listening on the port (mosaic not running, or mobile pairing off).
    case refused
...
extension CmxRoutePingResult {
    ...
    /// listening on the port. Use ``isListening`` for "the mosaic port answered".
    ...
    /// Whether a service actually accepted the connection on the mosaic port (only
