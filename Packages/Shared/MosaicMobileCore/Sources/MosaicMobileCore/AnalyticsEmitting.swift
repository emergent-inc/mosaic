...
/// It lives in `MosaicMobileCore` (the universally-imported, dependency-free base
/// package) so the lowest fire-site — `MobileClientIDRepository` in
/// `MosaicMobileShellModel` — can depend on the seam without any upward edge. The
/// concrete `AnalyticsEmitter` actor lives in `MosaicMobileAnalytics`, is built
