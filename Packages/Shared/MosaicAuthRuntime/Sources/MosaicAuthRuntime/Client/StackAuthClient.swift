...
public struct StackAuthClient: AuthClient {
    ...
    public func currentUser(throwOnMissing: Bool) async throws -> MosaicAuthUser? {
    ...
    public func listTeams() async throws -> [MosaicAuthTeam] {
        ...
        var summaries: [MosaicAuthTeam] = []
        summaries.reserveCapacity(teams.count)
        for team in teams {
            summaries.append(MosaicAuthTeam(id: team.id, displayName: await team.displayName))
        }
        return summaries
    }
    ...
    private static func mapped(_ user: CurrentUser) async -> MosaicAuthUser {
        ...
        return MosaicAuthUser(id: id, primaryEmail: email, displayName: name, imageURL: imageURL)
    }
}
