...
///
/// Consolidates the Stack project credentials (from ``MosaicAuthConfig``) with the
/// magic-link callback URL and the mosaic web API base URL, so both apps build
...
public struct AuthConfig: Equatable, Sendable {
    ...
    public let stack: MosaicAuthConfig
    /// The auth callback URL the magic-link email should target.
    public let magicLinkCallbackURL: String
    /// The base URL of the mosaic web API (device-token registration, push).
    public let apiBaseURL: String

    /// Creates an auth configuration from its resolved parts.
    public init(stack: MosaicAuthConfig, magicLinkCallbackURL: String, apiBaseURL: String) {
        self.stack = stack
    ...
    public init(
        environment: MosaicAuthEnvironment,
        overrides: [String: String] = [:]
    ) {
        let stack = MosaicAuthConfig(
