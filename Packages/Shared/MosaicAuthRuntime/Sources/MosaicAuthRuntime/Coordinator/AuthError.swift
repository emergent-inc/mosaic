...
/// The shared, localized error vocabulary for mosaic authentication flows.
///
/// Lifted out of the iOS-only `MosaicMobileAuth` package so both the macOS and iOS
...
/// previously surfaced through `MosaicMobileCore.L10n`.
public enum AuthError: Error, LocalizedError, Equatable, Sendable {
    ...
    /// The OS/browser auth session closed before a valid callback reached mosaic.
