...
extension AuthCoordinator {
    ...
    private func accessTokenWithoutStateClear() async throws -> String {
        ...
        if launch.mockDataEnabled {
            return "mosaic-ui-test-stack-token"
        }
        #endif
    ...
    /// mosaic-owned backend endpoints (e.g. the cloud VM service) with the
    /// `Authorization: Bearer <access>` + `X-Stack-Refresh-Token: <refresh>`
    /// header pair.
    ///
    ...
    public func refreshCurrentUserIdentity() async throws -> MosaicAuthUser? {
        await awaitBootstrapped()
