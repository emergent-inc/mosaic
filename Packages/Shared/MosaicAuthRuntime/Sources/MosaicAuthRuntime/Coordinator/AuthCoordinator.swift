public import MosaicAuthCore
import Foundation
public import Observation
import OSLog

private let authLog = Logger(subsystem: "ai.emergent.inc.mosaic", category: "auth")

/// The shared, injected auth orchestrator for mosaic.
///
...
///     sessionCache: MosaicAuthSessionCache(keyValueStore: defaults, key: "auth_has_tokens"),
///     userCache: MosaicAuthIdentityStore(keyValueStore: defaults, key: "auth_cached_user"),
///     teamSelection: MosaicAuthTeamSelectionStore(keyValueStore: defaults, key: "auth_selected_team"),
...
public final class AuthCoordinator {
    ...
    public internal(set) var currentUser: MosaicAuthUser?
    ...
    public private(set) var availableTeams: [MosaicAuthTeam] = []
    /// The user's selected team id. Writes persist through the injected
    /// ``MosaicAuthCore/MosaicAuthTeamSelectionStore``.
    ...
    private let userCache: MosaicAuthIdentityStore
    private let teamSelection: MosaicAuthTeamSelectionStore
    ...
    var debugCredentials: MosaicAuthAutoLoginCredentials?
    ...
    public init(
        ...
        sessionCache: MosaicAuthSessionCache,
        userCache: MosaicAuthIdentityStore,
    ...
    public func sendCode(to email: String) async throws {
        ...
        if launch.includesDevAuth,
           email.trimmingCharacters(in: .whitespacesAndNewlines) == "42" {
            let creds = MosaicAuthAutoLoginCredentials(email: "l@l.com", password: "abc123")
    ...
    public func verifyCode(_ code: String) async throws {
        ...
        let fullCode = MosaicAuthMagicLinkCode(code: code, nonce: nonce).composed
        do {
            let client = self.client
    ...
    func applySignedInUser(_ user: MosaicAuthUser) async {
    ...
    func apply(_ state: MosaicAuthState) {
        currentUser = state.currentUser
        isAuthenticated = state.isAuthenticated
        isRestoringSession = state.isRestoringSession
    }

    func loadCachedUser() -> MosaicAuthUser? {
        do {
    ...
    func saveCachedUser(_ user: MosaicAuthUser) {
        do {
    ...
    var autoLoginCredentials: MosaicAuthAutoLoginCredentials? {
    ...
    var fixtureUser: MosaicAuthUser? {
        MosaicAuthUser(
    ...
    static let uiTestMockUser = MosaicAuthUser(
        id: "uitest_user",
        primaryEmail: "uitest@mosaic.local",
        displayName: "UI Test"
    )
}
