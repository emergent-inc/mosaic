...
/// DEBUG builds additionally append to `/tmp/mosaic-auth-debug.log` (0600) so a
...
public struct AuthDebugLog: Sendable {
    ...
    #if DEBUG && os(macOS)
    private static let debugLogPath = "/tmp/mosaic-auth-debug.log"
    ...
    static func debugLogPaths(environment: [String: String]) -> [String] {
        ...
        if let taggedPath = environment["MOSAIC_DEBUG_LOG"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !taggedPath.isEmpty,
    ...
    public static func redacted(_ message: String) -> String {
        ...
        let replacements: [(pattern: String, replacement: String)] = [
            ...
            (#"(?i)\b(authorization|x-mosaic-refresh-token|x-mosaic-refresh-token|x-stack-access-token|x-stack-refresh-token)\s*[:=]\s*(?:Bearer\s+)?([^\s,;)]+)"#, "$1=<redacted>"),
