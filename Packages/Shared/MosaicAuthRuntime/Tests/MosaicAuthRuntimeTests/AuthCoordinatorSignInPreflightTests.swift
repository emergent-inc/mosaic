...
@Suite struct AuthCoordinatorSignInPreflightTests {
    ...
    private func makeCoordinator(
        ...
        cachedUser: MosaicAuthUser? = nil,
        hasCachedTokens: Bool = false
    ) -> AuthCoordinator {
        let store = FakeKeyValueStore()
        let sessionCache = MosaicAuthSessionCache(keyValueStore: store, key: "has_tokens")
        let userCache = MosaicAuthIdentityStore(keyValueStore: store, key: "cached_user")
