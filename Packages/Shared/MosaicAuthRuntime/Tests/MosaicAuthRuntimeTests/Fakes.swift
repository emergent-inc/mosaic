...
final class FakeKeyValueStore: MosaicAuthKeyValueStore, @unchecked Sendable {
...
actor FakeAuthClient: AuthClient {
    ...
    var user: MosaicAuthUser?
    var teams: [MosaicAuthTeam] = []
    ...
    init(access: String? = nil, refresh: String? = nil, user: MosaicAuthUser? = nil) {
        self.access = access
        self.refresh = refresh
        self.user = user
    }

    func setUser(_ user: MosaicAuthUser?) { self.user = user }
    ...
    func setTeams(_ teams: [MosaicAuthTeam]) { self.teams = teams }
    ...
    func currentUser(throwOnMissing: Bool) async throws -> MosaicAuthUser? {
...
extension AuthConfig {
    static let test = AuthConfig(
        stack: MosaicAuthConfig(projectId: "test", publishableClientKey: "test"),
