...
struct MosaicMobileRootView: View {
    ...
    #if os(iOS)
    init(store: MosaicMobileShellStore, onboardingStore: MobileOnboardingStore) {
        self.store = store
        self.onboardingStore = onboardingStore
        _hasSeenOnboarding = State(initialValue: onboardingStore.hasSeenOnboarding)
    }
    #else
    init(store: MosaicMobileShellStore) {
        self.store = store
    }
    #endif
    ...
    /// authorization grounds (a different mosaic account, or a token it could not
    ...
    private func connectUITestAttachURLIfNeeded() -> Bool {
        ...
        //   - MOSAIC_DOGFOOD_ATTACH_URL (UITestConfig.dogfoodAttachURL): NOT gated on
        ...
        //     for real (MOSAIC_UITEST_STACK_* with MOSAIC_UITEST_MOCK_DATA=0) and wants
        ...
        //   - MOSAIC_UITEST_ATTACH_URL (UITestConfig.attachURL): gated on mock data,
