...
public struct MosaicMobileAppView: View {
    @State private var store: MosaicMobileShellStore
    ...
    public init(
        store: MosaicMobileShellStore = .preview(),
    ...
    public var body: some View {
        #if os(iOS)
        MosaicMobileRootView(store: store, onboardingStore: onboardingStore)
            .environment(browserStore)
        #else
        MosaicMobileRootView(store: store)
            .environment(browserStore)
        #endif
    }
}
