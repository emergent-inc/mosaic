...
/// ``MosaicAuthRuntime/PushRegistrationService``, and routes foreground
/// presentation + taps to the active ``MosaicMobileShellStore`` for "mirror macOS"
...
public final class MobilePushCoordinator {
    ...
    private static let enabledKey = "mosaic.notifications.pushEnabled"

    /// APNs `aps.category` the web sets on every mosaic terminal push (see
    /// `MOSAIC_APNS_CATEGORY` in `web/services/apns/payload.ts`). The matching
    ...
    public static let dismissSyncCategoryIdentifier = "mosaic.terminal"

    @ObservationIgnored private weak var store: MosaicMobileShellStore?
    ...
    public func bind(store: MosaicMobileShellStore) {
        self.store = store
    ...
    public func configure(delegate: any UNUserNotificationCenterDelegate) {
        ...
        // mosaic banner delivers `UNNotificationDismissActionIdentifier` to the
    ...
    ///     with `mosaic.notificationId` as a fallback.
    ///   - macDeviceId: The Mac that owns the notification, from the `mosaic`
