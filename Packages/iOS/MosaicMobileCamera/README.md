...
iOS camera service for mosaic mobile QR-code pairing.
...
- ``QRCodeScanStream`` — an `AsyncStream<String>` of decoded QR payloads, fed by
  ...
  injected predicate (mosaic pairing only accepts `mosaic-ios://` links).
...
stream.yield("mosaic-ios://example")
stream.finish()
var seen: [String] = []
for await code in stream.codes { seen.append(code) }
#expect(seen == ["mosaic-ios://example"])
```
