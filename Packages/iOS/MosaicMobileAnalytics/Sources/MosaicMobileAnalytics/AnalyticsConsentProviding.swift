...
///
/// The analytics package must not depend on the settings domain (`MosaicSettings`),
...
/// `MosaicSettings.catalog.app.sendAnonymousTelemetry`; tests provide a fixed
...
///
/// The iOS app cannot import the macOS-only `MosaicSettings` package, so this reads
/// the same backing key that `MosaicSettings.catalog.app.sendAnonymousTelemetry`
