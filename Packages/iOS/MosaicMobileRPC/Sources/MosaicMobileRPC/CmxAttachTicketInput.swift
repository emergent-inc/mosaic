...
/// Decodes a scanned or pasted `mosaic-ios://` pairing/attach URL into a
/// validated ``CmxAttachTicket``.
public struct CmxAttachTicketInput {
    private init() {}

    /// Decode and validate a `mosaic-ios://pair` or `mosaic-ios://attach` URL.
    ///
    ...
    /// by Stack account, not ticket age). Only the ancient `mosaic-ios://pair`
    ...
    public static func decode(_ rawValue: String) throws -> CmxAttachTicket {
        ...
        // Accept any channel's pairing scheme (mosaic-ios for release builds,
        // mosaic-ios-dev for development); cross-channel pairing still works when
        ...
        // A QR minted by a newer mosaic whose grammar version this build does not
