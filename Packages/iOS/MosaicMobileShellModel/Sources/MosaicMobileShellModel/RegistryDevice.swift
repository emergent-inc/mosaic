...
/// One tagged mosaic app instance running on a registered device, as returned by
...
public struct RegistryAppInstance: Equatable, Sendable, Identifiable {
    /// The mosaic build tag this instance runs (`"stable"`, a dev tag, or
...
/// registry, with its running mosaic app instances.
///
...
/// `deviceId` here is the mosaic-generated device UUID (the wire `deviceId`), not
...
public struct RegistryDevice: Equatable, Sendable, Identifiable {
    /// Stable, cross-platform mosaic device UUID (matches `MobileHostIdentity` /
    ...
    public var lastSeenAt: Date
    /// The device's running mosaic app instances (tags), newest-first.
