...
/// optional `X-Mosaic-Team-Id` so the server scopes to the chosen team (defaults to
...
public actor DeviceRegistryService: DeviceRegistryRefreshing {
    ...
    /// - Parameters:
    ///   - apiBaseURL: The mosaic web API base URL (no trailing slash).
    ...
    private static let deviceIDKey = "mosaic.deviceRegistry.iosDeviceID"

    /// This iOS device's stable mosaic identity for the device registry.
    ///
    /// A mosaic-GENERATED persisted UUID (NOT `identifierForVendor`, which resets
    /// when the last mosaic app is removed, and NOT a hardware fingerprint).
    ...
    private func makeRequest(method: String, path: String, body: [String: Any]?) async -> URLRequest? {
        ...
        if let teamID = await teamIDProvider(), !teamID.isEmpty {
            request.setValue(teamID, forHTTPHeaderField: "X-Mosaic-Team-Id")
        }
        if let body {
