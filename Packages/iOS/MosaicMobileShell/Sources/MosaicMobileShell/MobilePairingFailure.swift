...
internal import MosaicMobileRPC
internal import MosaicMobileSupport
internal import MosaicMobileTransport
import Foundation
...
public enum MobilePairingFailureCategory: Equatable, Sendable {
    ...
    /// The address was reachable but nothing accepted the connection: mosaic is not
    ...
    /// The Mac is signed in to a different mosaic account than this device.
    ...
    case ticketExpired
    /// The scanned/typed input was not a pairing QR mosaic recognizes (malformed,
    /// not a mosaic code, or an unreadable URL).
    case invalidCode
    /// The QR is a mosaic pairing code, but it uses a grammar version newer than
    /// this build understands (the Mac is on a newer mosaic). The fix is updating
