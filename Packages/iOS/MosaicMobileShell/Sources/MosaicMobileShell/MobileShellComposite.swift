public import MosaicMobileCore
public import MosaicAgentChat
internal import MosaicMobileDiagnostics
public import MosaicMobilePairedMac
public import MosaicMobileRPC
public import MosaicMobileShellModel
internal import MosaicMobileSupport
public import MosaicMobileTransport
public import Foundation
import Observation
internal import OSLog

private let mobileShellLog = Logger(
    subsystem: Bundle.main.bundleIdentifier ?? "dev.mosaic.ios",
    category: "mobile-shell"
)

/// Transitional alias for the decomposed shell facade.
///
/// The iOS views and push coordinator still bind to `MosaicMobileShellStore`;
...
public typealias MosaicMobileShellStore = MobileShellComposite
...
public final class MobileShellComposite: MobileTerminalOutputSinking {
    ...
    private static let hasKnownPairedMacDefaultsKey = "mosaic.mobile.hasKnownPairedMac"
    ...
    public static func preview(runtime: (any MobileSyncRuntime)? = nil) -> MosaicMobileShellStore {
        MosaicMobileShellStore(
            runtime: runtime,
    ...
    /// The team's registered devices and their mosaic app instances (tags), for the
    ...
    /// The mosaic device id of the Mac the live connection currently targets, or
    ...
    private func connectPairingURLResult(
        ...
        do {
            ...
            if case MobileSyncPairingPayloadError.loopbackRouteRejected = error {
                ...
                // A real mosaic QR whose grammar version this build predates: the
    ...
    private var multiMacAggregationEnabled: Bool {
        if let raw = ProcessInfo.processInfo.environment["MOSAIC_MULTI_MAC_AGGREGATION"]?
    ...
    static let foregroundAnonymousKey = "__mosaic_foreground__"
    ...
    func prepareTerminalReplayFailureRetry(
        ...
        guard retryCount < Self.maxTerminalReplayFailureRetries else {
            MobileDebugLog.anchormux(
                "MOSAIC_REPLAY retry_exhausted surface=\(surfaceID) attempts=\(retryCount)"
            )
            return nil
        }
        terminalReplayFailureRetryCountsBySurfaceID[surfaceID] = retryCount + 1
        MobileDebugLog.anchormux(
            "MOSAIC_REPLAY retry_after_failure surface=\(surfaceID) attempt=\(retryCount + 1)"
        )
    ...
    private func requestTerminalReplayForCurrentBarrier(
        ...
        MobileDebugLog.anchormux("MOSAIC_REPLAY retry_\(reason) surface=\(surfaceID)")
    ...
    private func registerTerminalOutput(
        ...
        pendingTerminalByteEndSeqBySurfaceID.removeValue(forKey: surfaceID)
        #if DEBUG
        mobileShellLog.info("MOSAIC_REPLAY register sink surface=\(surfaceID, privacy: .public) connected=\(self.connectionState == .connected, privacy: .public) hasClient=\(self.remoteClient != nil, privacy: .public) workspaceCount=\(self.workspaces.count, privacy: .public)")
        #endif
        requestTerminalReplay(surfaceID: surfaceID)
    }
    ...
    func requestTerminalReplay(
        ...
        guard let client = remoteClient else {
            ...
            #if DEBUG
            mobileShellLog.error("MOSAIC_REPLAY skip surface=\(surfaceID, privacy: .public) reason=no_remote_client")
            #endif
            return
        }
        guard let workspaceID = workspaceID(forTerminalID: surfaceID) else {
            ...
            #if DEBUG
            mobileShellLog.error("MOSAIC_REPLAY skip surface=\(surfaceID, privacy: .public) reason=workspace_not_found")
            #endif
            return
        }
        let remoteWorkspaceID = remoteWorkspaceID(for: workspaceID)
        if let replayBarrierTokenForRequest {
            guard terminalReplayBarrierTokensInFlightBySurfaceID[surfaceID] != replayBarrierTokenForRequest else {
                #if DEBUG
                mobileShellLog.info("MOSAIC_REPLAY skip surface=\(surfaceID, privacy: .public) reason=barrier_in_flight")
                #endif
                return
            }
        } else {
            guard !terminalReplaySurfaceIDsInFlight.contains(surfaceID) else {
                #if DEBUG
                mobileShellLog.info("MOSAIC_REPLAY skip surface=\(surfaceID, privacy: .public) reason=in_flight")
                #endif
                return
            }
        }
        ...
        let replayTask = Task { @MainActor [weak self] in
            ...
            case .success(let data):
                guard self.terminalReplayRequestIDsInFlightBySurfaceID[surfaceID] == replayRequestID else {
                    MobileDebugLog.anchormux("MOSAIC_REPLAY stale_request surface=\(surfaceID)")
                    return
                }
                ...
                if let replayBarrierTokenForRequest {
                    guard self.terminalReplayBarrierTokensBySurfaceID[surfaceID] == replayBarrierTokenForRequest else {
                        MobileDebugLog.anchormux("MOSAIC_REPLAY barrier_stale surface=\(surfaceID)")
                        return
                    }
                }
                #if DEBUG
                ...
                if let renderGrid {
                    deliverBytes = nil
                    MobileDebugLog.anchormux("MOSAIC_REPLAY render_grid surface=\(surfaceID) spans=\(renderGrid.rowSpans.count) seq=\(renderGrid.stateSeq)")
                } else if let snapshotBytes, !snapshotBytes.isEmpty {
                    deliverBytes = Self.terminalSnapshotReplacementBytes(snapshotBytes)
                    MobileDebugLog.anchormux("MOSAIC_REPLAY snapshot surface=\(surfaceID) bytes=\(snapshotBytes.count) seq=\(replaySeq ?? 0)")
                } else {
                    deliverBytes = bytes
                    MobileDebugLog.anchormux("MOSAIC_REPLAY raw_tail surface=\(surfaceID) bytes=\(bytes?.count ?? -1) seq=\(replaySeq ?? 0)")
                }
                if let renderGrid {
            ...
            case .failure(let error):
                guard self.terminalReplayRequestIDsInFlightBySurfaceID[surfaceID] == replayRequestID else {
                    MobileDebugLog.anchormux("MOSAIC_REPLAY stale_request_failed surface=\(surfaceID)")
                    return
                }
                mobileShellLog.error("MOSAIC_REPLAY failed surface=\(surfaceID, privacy: .public) error=\(String(describing: error), privacy: .private)")
                guard self.remoteClient === client else {
                    self.clearTerminalReplayInFlightIfCurrent(
                        surfaceID: surfaceID,
                        requestID: replayRequestID
    ...
    private func handleTerminalRenderGridEvent(_ event: MobileEventEnvelope) {
        ...
        #if DEBUG
        mobileShellLog.info("MOSAIC_REPLAY live render_grid surface=\(renderGrid.surfaceID, privacy: .public) full=\(renderGrid.full, privacy: .public) spans=\(renderGrid.rowSpans.count, privacy: .public) cleared=\(renderGrid.clearedRows.count, privacy: .public) seq=\(renderGrid.stateSeq, privacy: .public) hasSink=true")
        #endif
    ...
    private func handleTerminalBytesEvent(_ event: MobileEventEnvelope) {
        ...
        mobileShellLog.info("MOSAIC_REPLAY live bytes surface=\(surfaceID, privacy: .public) byteCount=\(bytes.count, privacy: .public) seq=\(debugSeq, privacy: .public) hasSink=\(self.hasTerminalOutputSink(surfaceID: surfaceID), privacy: .public)")
        #endif
