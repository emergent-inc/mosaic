...
/// authoritative `mosaic.notificationId` payload key (with the request
...
public struct SystemDeliveredNotificationClearer: DeliveredNotificationClearing {
    ...
    /// `mosaic.notificationId` payload key when present (authoritative — the Mac
    ...
    static func macNotificationID(for request: UNNotificationRequest) -> String {
        if let mosaic = request.content.userInfo["mosaic"] as? [String: Any],
