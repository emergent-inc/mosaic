...
private let log = Logger(subsystem: "ai.emergent.inc.mosaic.ios", category: "ghostty.runtime")
...
private func mosaicIOSRuntimeReadClipboardCallback(
...
public final class GhosttyRuntime {
    ...
    init() throws {
        ...
        runtimeConfig.read_clipboard_cb = mosaicIOSRuntimeReadClipboardCallback
    ...
    nonisolated static func iOSConfigURLs(
        ...
        #if os(iOS)
        var urls: [URL] = []
        if let overridePath = processInfo.environment["MOSAIC_GHOSTTY_CONFIG_PATH"], !overridePath.isEmpty {
