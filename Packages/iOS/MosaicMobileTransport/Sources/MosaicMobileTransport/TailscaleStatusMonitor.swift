...
///
/// Construct one at the composition root and hand it to views (mosaic injects it
/// through the `\.tailscaleStatusMonitor` SwiftUI environment key in
/// `MosaicMobileShellUI`). Inject a custom provider to drive tests.
@MainActor
@Observable
public final class TailscaleStatusMonitor {
    ...
    public init(
        ...
        monitor.start(queue: DispatchQueue(label: "dev.mosaic.tailscale-status", qos: .utility))
    }
