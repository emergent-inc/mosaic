...
@Suite struct UITestConfigTests {
    ...
    @Test func explicitEnableTurnsOnMockData() {
        let env = ["MOSAIC_UITEST_MOCK_DATA": "1"]
        #if DEBUG
    ...
    @Test func valueReturnsTrimmedNonEmptyWhenMockEnabled() {
        let env = [
            "MOSAIC_UITEST_MOCK_DATA": "1",
            "MOSAIC_UITEST_ADD_DEVICE_NAME": "  Work Mac  ",
        ]
        #if DEBUG
        #expect(UITestConfig.value(for: "MOSAIC_UITEST_ADD_DEVICE_NAME", env: env) == "Work Mac")
        #else
        #expect(UITestConfig.value(for: "MOSAIC_UITEST_ADD_DEVICE_NAME", env: env) == nil)
        #endif
    }

    @Test func valueIsNilWhenMockDisabled() {
        let env = ["MOSAIC_UITEST_ADD_DEVICE_NAME": "Work Mac"]
        #expect(UITestConfig.value(for: "MOSAIC_UITEST_ADD_DEVICE_NAME", env: env) == nil)
    }
    ...
    @Test func valueIsNilWhenBlank() {
        let env = [
            "MOSAIC_UITEST_MOCK_DATA": "1",
            "MOSAIC_UITEST_ADD_DEVICE_HOST": "   ",
        ]
        #expect(UITestConfig.value(for: "MOSAIC_UITEST_ADD_DEVICE_HOST", env: env) == nil)
    }
    ...
    @Test func dogfoodAttachURLReturnedWithMockDisabled() {
        let env = [
            "MOSAIC_UITEST_MOCK_DATA": "0",
            "MOSAIC_DOGFOOD_ATTACH_URL": "mosaic-ios://attach?v=1&payload=abc",
        ]
        #if DEBUG
        #expect(UITestConfig.dogfoodAttachURL(from: env) == "mosaic-ios://attach?v=1&payload=abc")
        #else
    ...
    /// (`MOSAIC_UITEST_ATTACH_URL`) stays nil, which is exactly why the dedicated
    ...
    @Test func legacyAttachURLStaysNilWithMockDisabledButDogfoodDoesNot() {
        let env = [
            "MOSAIC_UITEST_MOCK_DATA": "0",
            "MOSAIC_UITEST_ATTACH_URL": "mosaic-ios://attach?v=1&payload=legacy",
            "MOSAIC_DOGFOOD_ATTACH_URL": "mosaic-ios://attach?v=1&payload=dogfood",
        ]
        #expect(UITestConfig.value(for: "MOSAIC_UITEST_ATTACH_URL", env: env) == nil)
        #if DEBUG
        #expect(UITestConfig.dogfoodAttachURL(from: env) == "mosaic-ios://attach?v=1&payload=dogfood")
        #else
        #expect(UITestConfig.dogfoodAttachURL(from: env) == nil)
        #endif
    }
    ...
    @Test func dogfoodAttachURLIsTrimmed() {
        let env = ["MOSAIC_DOGFOOD_ATTACH_URL": "  mosaic-ios://attach?v=1&payload=zzz  "]
        #if DEBUG
        #expect(UITestConfig.dogfoodAttachURL(from: env) == "mosaic-ios://attach?v=1&payload=zzz")
        #else
        #expect(UITestConfig.dogfoodAttachURL(from: env) == nil)
        #endif
    }
    ...
    @Test func dogfoodAttachURLIsNilWhenBlank() {
        let env = ["MOSAIC_DOGFOOD_ATTACH_URL": "   "]
        #expect(UITestConfig.dogfoodAttachURL(from: env) == nil)
    }

    @Test func agentChatPreviewFlagIsDebugOnly() {
        let env = ["MOSAIC_UITEST_AGENT_CHAT_PREVIEW": "1"]
    ...
    @Test func agentChatPreviewFlagRequiresOne() {
        ...
        #expect(UITestEnvironmentConfig(
            environment: ["MOSAIC_UITEST_AGENT_CHAT_PREVIEW": "0"]
        ).agentChatPreviewEnabled == false)
    }

    @Test func agentChatInlinePreviewFlagIsDebugOnly() {
        let env = ["MOSAIC_UITEST_AGENT_CHAT_INLINE_PREVIEW": "1"]
        let config = UITestEnvironmentConfig(environment: env)
        #if DEBUG
    ...
    @Test func agentChatInlinePreviewFlagRequiresOne() {
        ...
        #expect(UITestEnvironmentConfig(
            environment: ["MOSAIC_UITEST_AGENT_CHAT_INLINE_PREVIEW": "0"]
