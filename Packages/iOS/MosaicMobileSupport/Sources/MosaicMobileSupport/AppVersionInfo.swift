...
/// (the `MosaicDevTag` / `MosaicGitSHA` Info.plist keys), e.g.
...
public struct AppVersionInfo: Sendable, Equatable {
    ...
    /// The dev `--tag` (`MosaicDevTag`); empty for release/TestFlight builds.
    public let devTag: String

    /// The short git SHA (`MosaicGitSHA`); empty for release/TestFlight builds.
    ///
    ...
    public init(infoDictionary: [String: Any]?, isDevBuild: Bool) {
        func trimmedString(_ key: String) -> String {
            ...
            // Unexpanded build-setting placeholders (e.g. "$(MOSAIC_GIT_SHA)" when
        ...
        self.devTag = trimmedString("MosaicDevTag")
        self.gitSHA = trimmedString("MosaicGitSHA")
