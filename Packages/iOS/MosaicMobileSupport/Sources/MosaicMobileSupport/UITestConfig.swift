...
public struct UITestConfig {
    ...
    /// fires under the XCUITest mock harness), this reads `MOSAIC_DOGFOOD_ATTACH_URL`
    /// *without* the mock gate. The dev-launch tooling
    /// (`scripts/mobile-dev-launch.sh`, `scripts/dev-setup.sh`) signs in for real
    /// against the live backend (`MOSAIC_UITEST_MOCK_DATA=0`) and wants the phone to
    ...
    /// - Returns: The trimmed value of `MOSAIC_DOGFOOD_ATTACH_URL` when present and
    ...
    public static func dogfoodAttachURL(from env: [String: String]) -> String? {
        ...
        // URL is returned with MOSAIC_UITEST_MOCK_DATA=0 (the real-backend
        // dev-launch path) and iOS auto-pair actually fires.
        let value = env["MOSAIC_DOGFOOD_ATTACH_URL"]?.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    ///
    /// When `MOSAIC_UITEST_TERMINAL_PREVIEW=1`, the root view renders a standalone
    ...
    public static var terminalLayoutPreviewEnabled: Bool {
        #if DEBUG
        return ProcessInfo.processInfo.environment["MOSAIC_UITEST_TERMINAL_PREVIEW"] == "1"
        #else
        return false
        #endif
    }
    ...
    ///
    /// When `MOSAIC_UITEST_WORKSPACE_LIST_PREVIEW=1`, the root view renders a
    ...
    public static var workspaceListLayoutPreviewEnabled: Bool {
        #if DEBUG
        return ProcessInfo.processInfo.environment["MOSAIC_UITEST_WORKSPACE_LIST_PREVIEW"] == "1"
            || ProcessInfo.processInfo.arguments.contains("MOSAIC_UITEST_WORKSPACE_LIST_PREVIEW=1")
        #else
        return false
        #endif
    }
    ...
    ///
    /// When `MOSAIC_UITEST_STREAMING_CHAT_PREVIEW=1`, the root view renders a
    ...
    public static var streamingChatPreviewEnabled: Bool {
        #if DEBUG
        return ProcessInfo.processInfo.environment["MOSAIC_UITEST_STREAMING_CHAT_PREVIEW"] == "1"
        #else
        return false
        #endif
    }
    ...
    ///
    /// When `MOSAIC_UITEST_AGENT_CHAT_PREVIEW=1`, the root view renders the
    ...
    ///
    /// When `MOSAIC_UITEST_AGENT_CHAT_INLINE_PREVIEW=1`, the root view renders the
    ...
    /// `MOSAIC_UITEST_MOCK_DATA` of `0`/`1` wins; otherwise the presence of
    ...
    public static func mockDataEnabled(from env: [String: String]) -> Bool {
        #if DEBUG
        if env["MOSAIC_UITEST_MOCK_DATA"] == "0" {
            return false
        }
        if env["MOSAIC_UITEST_MOCK_DATA"] == "1" {
            return true
        }
