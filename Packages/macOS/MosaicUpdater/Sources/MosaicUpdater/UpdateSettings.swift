...
/// Registers mosaic's Sparkle preference defaults and performs the one-time migration that
...
/// configuration, so this is a value type constructed with them (defaulting to mosaic's
...
public struct UpdateSettings: Sendable {
    ...
    /// mosaic's marker that the v2 automatic-checks migration already ran.
    public static let migrationKey = "mosaic.sparkle.automaticChecksMigration.v2"
    ...
    /// The scheduled-check interval mosaic registers (1h by default).
    public let scheduledCheckInterval: TimeInterval

    /// Creates the settings with mosaic's defaults.
    ///
