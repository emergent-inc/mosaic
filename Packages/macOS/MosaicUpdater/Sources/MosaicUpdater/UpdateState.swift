...
public enum UpdateState: Equatable {
    ...
    case idle
    /// Sparkle is asking whether to enable automatic checks (mosaic suppresses this UI).
    ...
    public enum ReleaseNotes {
        ...
        public init?(displayVersionString: String) {
            ...
            if let semver = Self.extractSemanticVersion(from: version) {
                ...
                if let url = URL(string: "https://github.com/emergent-inc/mosaic/releases/tag/\(tag)") {
                    self = .tagged(url)
                    return
                }
            }
            ...
            if let url = URL(string: "https://github.com/emergent-inc/mosaic/commit/\(newHash)") {
                self = .commit(url)
            } else {
                return nil
            }
