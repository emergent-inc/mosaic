...
/// Coordinates mosaic's custom Sparkle update flow: owns the `SPUUpdater` and its
...
public final class UpdateController {
    ...
    /// Whether the running build is a mosaic DEV/staging build that must never be compared against
    ...
    ///   - settings: The Sparkle defaults/migration configuration. Defaults to mosaic's hourly check.
    ...
    public init(log: any UpdateLogging,
        ...
        if isDevLikeBundle {
            ...
            // mosaic's launch/background probe is short-circuited by the `automaticallyChecksForUpdates`
    ...
    public func startUpdaterIfNeeded() {
        ...
        if ProcessInfo.processInfo.environment["MOSAIC_UI_TEST_RESET_SPARKLE_PERMISSION"] == "1" {
    ...
    private func recordUITestTimestamp(key: String) {
        ...
        guard let path = env["MOSAIC_UI_TEST_TIMING_PATH"] else { return }
...
extension UpdateController {
    /// Whether `bundleIdentifier` is a mosaic DEV (`mosaic.com.emergent.app.debug[.<tag>]`) or staging
    ...
    /// `isStagingBundleIdentifier` (in the MosaicSettings package). The classification is
    /// duplicated here deliberately to avoid introducing a `MosaicUpdater → MosaicSettings` package
