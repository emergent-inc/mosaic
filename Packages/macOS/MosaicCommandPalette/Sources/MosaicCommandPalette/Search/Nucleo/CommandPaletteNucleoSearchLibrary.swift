...
// search state.
/// Loaded `mosaic-nucleo-ffi` dylib handle with resolved C entry points.
///
...
final class CommandPaletteNucleoSearchLibrary: @unchecked Sendable {
    ...
    private static func load(path: String) -> CommandPaletteNucleoSearchLibrary? {
        ...
        guard let createIndex = symbol("mosaic_nucleo_index_create", from: handle, as: CreateIndex.self),
              let destroyIndex = symbol("mosaic_nucleo_index_destroy", from: handle, as: DestroyIndex.self),
              let searchIndexWithBoosts = symbol(
                "mosaic_nucleo_index_search_with_boosts",
                from: handle,
                as: SearchIndexWithBoosts.self
              ),
              let version = symbol("mosaic_nucleo_ffi_version", from: handle, as: Version.self) else {
            dlclose(handle)
            return nil
        }
    ...
    private static func defaultLibraryPaths() -> [String] {
        var paths: [String] = []
        let environmentPath = ProcessInfo.processInfo.environment["MOSAIC_NUCLEO_FFI_LIB"]
        ...
        // Packages/macOS/MosaicCommandPalette/Sources/MosaicCommandPalette/Search/Nucleo/.
        ...
        paths.append(
            sourceRoot
                .appendingPathComponent("Native/CommandPaletteNucleoFFI/target/mosaic-nucleo-ffi")
                .appendingPathComponent(Self.libraryFileName)
                .path
        )
        paths.append(
