...
public enum CommandPaletteRequestKind: String, Sendable, CaseIterable {
    ...
    ///
    /// These strings are byte-identical to the `mosaic.*` `Notification.Name`
    /// literals the app target previously posted inline, so observers keyed on
    /// the existing names are unaffected.
    public var notificationName: String {
        switch self {
        case .commands:
            return "mosaic.commandPaletteRequested"
        case .switcher:
            return "mosaic.commandPaletteSwitcherRequested"
        case .renameTab:
            return "mosaic.commandPaletteRenameTabRequested"
        case .renameWorkspace:
            return "mosaic.commandPaletteRenameWorkspaceRequested"
        case .editWorkspaceDescription:
            return "mosaic.commandPaletteEditWorkspaceDescriptionRequested"
        }
    }
