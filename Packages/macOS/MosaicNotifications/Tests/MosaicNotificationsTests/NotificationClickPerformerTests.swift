import Foundation
import Testing
@testable import MosaicNotifications
...
struct NotificationClickPerformerTests {
    ...
    func selectsExistingFile() {
        ...
        finder.existingPaths = ["/tmp/mosaic/report.txt"]
        let performer = NotificationClickPerformer(finder: finder)

        #expect(performer.perform(.revealInFinder(path: "/tmp/mosaic/report.txt")))
        #expect(finder.selected == ["/tmp/mosaic/report.txt"])
    ...
    func opensContainingDirectory() {
        ...
        finder.existingPaths = ["/tmp/mosaic"]
        let performer = NotificationClickPerformer(finder: finder)

        #expect(performer.perform(.revealInFinder(path: "/tmp/mosaic/missing.txt")))
        #expect(finder.selected.isEmpty)
        #expect(finder.opened == ["/tmp/mosaic"])
    }
    ...
    func failsWhenNothingExists() {
        ...
        #expect(performer.perform(.revealInFinder(path: "/tmp/mosaic/missing.txt")) == false)
        #expect(finder.selected.isEmpty)
        #expect(finder.opened.isEmpty)
    }
    ...
    func expandsTilde() {
        ...
        let expectedPath = expandedHome + "/mosaic-file.txt"
        finder.existingPaths = [expectedPath]
        let performer = NotificationClickPerformer(finder: finder)

        #expect(performer.perform(.revealInFinder(path: "~/mosaic-file.txt")))
