...
/// git/PR subsystem moved into `MosaicGit`. Exercises remote-slug derivation
...
@Suite struct GitConfigIncludeTests {
    ...
    @Test func prioritizesUpstreamThenOriginAndDeduplicates() {
        let output = """
        origin https://github.com/austinwang/mosaic.git (fetch)
        origin https://github.com/austinwang/mosaic.git (push)
        upstream git@github.com:emergent-inc/mosaic.git (fetch)
        upstream git@github.com:emergent-inc/mosaic.git (push)
        backup ssh://git@github.com/emergent-inc/mosaic.git (fetch)
        mirror https://gitlab.com/emergent-inc/mosaic.git (fetch)
        """
        #expect(
            GitMetadataService.githubRepositorySlugs(fromGitRemoteVOutput: output)
                == ["emergent-inc/mosaic", "austinwang/mosaic"]
        )
    }
    ...
    @Test func ignoresInlineComments() {
        ...
        [remote "origin"] ; user's main fork
            url = git@github.com:austinwang/mosaic.git # main origin
            fetch = +refs/heads/*:refs/remotes/origin/*
        [remote "upstream"] # canonical repo
            url = https://github.com/emergent-inc/mosaic.git ; upstream source
            fetch = +refs/heads/*:refs/remotes/upstream/*
        """
        #expect(slugs(fromConfig: config) == ["emergent-inc/mosaic", "austinwang/mosaic"])
    }
    ...
    @Test func usesLastRemoteURLValue() {
        ...
        """
        #expect(slugs(fromConfig: config) == ["emergent-inc/mosaic"])
    }
    ...
    @Test func readsIncludedConfigFiles() throws {
        ...
        try """
        [remote "origin"]
            url = "git@github.com:austinwang/mosaic.git" # user's main fork
        ...
        #expect(slugs(forDirectory: fixture.root.path) == ["emergent-inc/mosaic", "austinwang/mosaic"])
    }
    ...
    @Test func appliesIncludesInPlace() throws {
        ...
        #expect(slugs(forDirectory: fixture.root.path) == ["emergent-inc/mosaic"])
    }
    ...
    @Test func treatsTrailingSlashGitdirAsRecursive() throws {
        ...
        let parent = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaicgit-recursive-\(UUID().uuidString)", isDirectory: true)
        let repoRoot = parent.appendingPathComponent("teams/mosaic", isDirectory: true)
        ...
        #expect(slugs(forDirectory: repoRoot.path) == ["emergent-inc/mosaic"])
    }
