...
@Suite struct TerminalQuicklookPathResolutionTests {
    @Test func fallsBackToStrippedPathWhenLiteralPathIsMissing() {
        let strippedPath = "/tmp/mosaic-cmdclick-path.md"
        #expect(
    ...
    @Test func prefersLiteralPathThatReallyEndsWithDot() {
        let literalPath = "/tmp/mosaic-cmdclick-literal-dot.md."
        let strippedPath = "/tmp/mosaic-cmdclick-literal-dot.md"
        #expect(
    ...
    @Test func prefersLiteralPathThatReallyEndsWithParen() {
        let literalPath = "/tmp/mosaic-cmdclick-literal-paren)"
        let strippedPath = "/tmp/mosaic-cmdclick-literal-paren"
        #expect(
            TerminalPathResolver(fileExists: existsIn([literalPath, strippedPath])).resolveQuicklookPath(
                literalPath,
                cwd: "/tmp"
    ...
    @Test func unquotesShellQuotedToken() {
        let existingFile = "/tmp/mosaic quicklook spaced.md"
        #expect(
            TerminalPathResolver(fileExists: existsIn([existingFile])).resolveQuicklookPath(
                "\"\(existingFile)\"",
                cwd: "/tmp"
    ...
    @Test func unescapesBackslashEscapedSpaces() {
        let existingFile = "/tmp/mosaic quicklook escaped.md"
        #expect(
            TerminalPathResolver(fileExists: existsIn([existingFile])).resolveQuicklookPath(
                "/tmp/mosaic\\ quicklook\\ escaped.md",
                cwd: "/tmp"
            ) == existingFile
        )
    }
...
@Suite struct TerminalVisibleLineResolutionTests {
    ...
    @Test func resolvesRawSegmentUnderColumn() throws {
        let existingFile = "/tmp/mosaic-visible-line.md"
        let line = "open /tmp/mosaic-visible-line.md now"
        ...
        #expect(resolution.rawToken == "/tmp/mosaic-visible-line.md")
    }

    @Test func resolvesShellEscapedTokenSpanningSpaces() throws {
        let existingFile = "/tmp/mosaic visible escaped.md"
        let line = "cat /tmp/mosaic\\ visible\\ escaped.md"
        let resolution = try #require(
            TerminalPathResolver(fileExists: existsIn([existingFile])).resolveVisibleLinePath(
                line,
                column: 6,
