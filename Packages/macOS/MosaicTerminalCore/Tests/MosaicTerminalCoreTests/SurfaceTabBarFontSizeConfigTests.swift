...
struct SurfaceTabBarFontSizeConfigTests {
    ...
    @Test func editorParsesLastSurfaceTabBarValueAndClamps() {
        ...
        #expect(MosaicGhosttyConfigSettingEditor().parsedSurfaceTabBarFontSize(in: contents)
            == MosaicGhosttyConfigSettingEditor.maxSurfaceTabBarFontSize)
    }

    @Test func editorReturnsNilWhenSurfaceTabBarValueAbsent() {
        #expect(MosaicGhosttyConfigSettingEditor().parsedSurfaceTabBarFontSize(in: "sidebar-font-size = 14") == nil)
    }

    @Test func editorFormatsSurfaceTabBarValueTrimmingTrailingZeros() {
        #expect(MosaicGhosttyConfigSettingEditor().formattedSurfaceTabBarFontSize(12) == "12")
        #expect(MosaicGhosttyConfigSettingEditor().formattedSurfaceTabBarFontSize(12.5) == "12.5")
    }

    @Test func editorWriteSettingRoundTripsSurfaceTabBarValue() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-surface-tab-bar-\(UUID().uuidString)", isDirectory: true)
        ...
        #expect(MosaicGhosttyConfigSettingEditor().parsedSurfaceTabBarFontSize(in: contents) == 13)
    }
}
