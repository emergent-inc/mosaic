...
@MainActor
private final class FakeConfigStore: MosaicConfigStoreReloading {
    private(set) var loadAllCount = 0
    func loadAll() { loadAllCount += 1 }
}

@MainActor
private final class FakeEnvironment: MosaicConfigStoreReloadEnvironment {
    var stores: [FakeConfigStore]
    private(set) var titleRefreshCount = 0

    init(stores: [FakeConfigStore]) { self.stores = stores }

    var reloadableConfigStores: [any MosaicConfigStoreReloading] { stores }
    func refreshWindowTitlesAfterConfigReload() { titleRefreshCount += 1 }
}

@MainActor
@Suite struct MosaicConfigStoreReloadCoordinatorTests {
    @Test func reloadsEachDistinctStoreOnceAndRefreshesTitles() {
        ...
        let coordinator = MosaicConfigStoreReloadCoordinator(environment: env) { source, count in
    ...
    @Test func reloadWithNoStoresStillRefreshesTitles() {
        ...
        var reportedCount: Int?
        let coordinator = MosaicConfigStoreReloadCoordinator(environment: env) { _, count in
            reportedCount = count
        }
