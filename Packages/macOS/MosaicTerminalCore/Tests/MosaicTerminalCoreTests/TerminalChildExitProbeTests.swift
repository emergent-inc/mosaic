...
@Suite struct TerminalChildExitProbeTests {
    @Test func loadReturnsEmptyPayloadForMissingFile() {
        #expect(TerminalChildExitProbe().load(at: "/tmp/mosaic-termcore-missing-\(UUID().uuidString)") == [:])
    }

    @Test func loadRoundTripsJSONPayload() throws {
        let path = NSTemporaryDirectory() + "mosaic-termcore-probe-\(UUID().uuidString).json"
    ...
    @Test func writeIsInertWithoutEnvironmentOptIn() {
        // The test process does not set MOSAIC_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP,
