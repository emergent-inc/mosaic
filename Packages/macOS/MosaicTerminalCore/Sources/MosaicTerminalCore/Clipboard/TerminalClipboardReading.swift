...
///
/// Implemented by `TerminalPasteboardService` in `MosaicTerminalServices` and
...
public protocol TerminalClipboardReading: AnyObject, Sendable {
    ...
    /// locations mosaic does not support.
    ...
    /// applying mosaic's flavor-priority rules (file URLs, image-only guard,
