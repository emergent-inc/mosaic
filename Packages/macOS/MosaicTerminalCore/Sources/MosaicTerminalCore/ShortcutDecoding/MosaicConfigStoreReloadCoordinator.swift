/// Orchestrates reloading every per-window mosaic configuration store and refreshing
/// window titles, as the app delegate's `reloadMosaicConfigStores(source:)` did.
///
/// Owned by the app delegate and given a weak reference to it through
/// `MosaicConfigStoreReloadEnvironment`, so the delegate retaining the coordinator
...
@MainActor
public final class MosaicConfigStoreReloadCoordinator {
    private weak var environment: (any MosaicConfigStoreReloadEnvironment)?
    ...
    public init(
        environment: any MosaicConfigStoreReloadEnvironment,
