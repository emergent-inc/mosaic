...
/// Resolves where mosaic's Ghostty configuration lives and what it should inject,
...
/// includes) for the font and appearance directives that drive mosaic's
...
public struct GhosttyConfigDiscovery {
    ...
    /// own identifier.
    public static let releaseBundleIdentifier = MosaicGhosttyConfigPathResolver.releaseBundleIdentifier

    private let fileReader: any GhosttyConfigFileReading
    private let fontProbe: any GhosttyFontProbing
    private let pathResolver: MosaicGhosttyConfigPathResolver
    ...
    public init(
        ...
        pathResolver: MosaicGhosttyConfigPathResolver = MosaicGhosttyConfigPathResolver()
    ) {
    ...
    /// already covers the ranges mosaic would otherwise auto-map.
    ...
    /// Returns only the CJK mappings mosaic should auto-inject after respecting
    ...
    /// application-support config paths mosaic may load at runtime.
    ...
    /// Whether mosaic should inject its managed CJK `font-codepoint-map` fallback.
    ...
    /// Whether mosaic should apply its managed default appearance across the
    ...
    /// Computes the resolved plain-theme override mosaic must inject when the
    ...
    public func conditionalThemeOverrideConfigContents(
        ...
        // syntax, even when both sides resolve to the same theme. `mosaic themes
        ...
        // https://github.com/emergent-inc/mosaic/issues/3459. Only override sides the
    ...
    /// summarizes the font directives relevant to mosaic's injected CJK fallback.
    ...
    /// `config-file` processing, including native Ghostty, legacy, and mosaic
    ...
    public func loadedGhosttyConfigScanPaths(
        ...
        let appSupportConfigURLs = mosaicAppSupportConfigURLs(
    ...
    /// Whether mosaic should load the legacy `config` file because the new
    ...
    /// Resolves the mosaic Application Support Ghostty config file URLs for the
    /// running build, via the shared ``MosaicGhosttyConfigPathResolver``.
    public func mosaicAppSupportConfigURLs(
