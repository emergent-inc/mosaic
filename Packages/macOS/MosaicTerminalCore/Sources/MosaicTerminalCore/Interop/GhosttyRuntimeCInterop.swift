...
private func mosaic_ghostty_surface_clear_selection(_ surface: ghostty_surface_t) -> Bool

/// The one sanctioned seam for libghostty symbols that are linked by name
/// rather than imported through the GhosttyKit header.
///
/// mosaic's libghostty fork exports a small number of symbols that are not part
...
public struct GhosttyRuntimeCInterop {
    ...
    ///
    /// Mirrors `ghostty_surface_clear_selection` from the mosaic libghostty
    ...
    public static func clearSelection(_ surface: ghostty_surface_t) -> Bool {
        mosaic_ghostty_surface_clear_selection(surface)
    }
