...
/// resolves to the mosaic wrapper; both paths are exported to the shell as
/// `MOSAIC_CLAUDE_WRAPPER_SHIM` / `MOSAIC_CLAUDE_WRAPPER_SHIM_ROOT`.
public struct TerminalSurfaceClaudeCommandShim: Equatable, Sendable {
    ...
    /// directory, if the bundled `mosaic-codex-wrapper` was available. Carried
    /// alongside the claude shim so the runtime surface creation can export
    /// `MOSAIC_CODEX_WRAPPER_SHIM` into the managed terminal environment (the
    /// same way it exports `MOSAIC_CLAUDE_WRAPPER_SHIM`), which a resumed codex
    /// session needs to route its `codex resume` through the wrapper and keep
    /// mosaic hooks. `nil` when the codex wrapper was absent.
