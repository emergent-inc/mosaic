...
struct AgentSpawnIdentityTests {
    ...
    // focused surface A (which would desync from MOSAIC_PANEL_ID and restore into the wrong surface).
    ...
    func orphanOwnSurfaceIsNotPairedWithFocusedWorkspace() {
        // Only MOSAIC_SURFACE_ID was inherited (no MOSAIC_WORKSPACE_ID), e.g. partial env scrubbing, while
