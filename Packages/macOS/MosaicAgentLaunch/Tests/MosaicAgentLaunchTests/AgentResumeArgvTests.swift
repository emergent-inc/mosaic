...
struct AgentResumeArgvTests {
    ...
    func opencodeResumeDropsInternalTUISettingsSelector() {
        ...
        #expect(
            ...
            ) == .resolved(["mosaic", "omo", "--session", "SID", "--model", "anthropic/claude-sonnet-4-6"])
        )
        #expect(
            AgentResumeArgv().builtInKind(
                kind: "opencode",
    ...
    @Test("mosaic wrapper launchers resolve before per-kind verbs")
    func launcherWrappers() {
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "claudeTeams", sessionId: "SID", executablePath: nil, arguments: ["mosaic", "claude-teams"]
            ) == .resolved(["mosaic", "claude-teams", "--resume", "SID"])
        )
        #expect(
            AgentResumeArgv().launcherResolution(
        ...
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "codexTeams", sessionId: "SID", executablePath: nil, arguments: ["mosaic", "codex-teams"]
            ) == .resolved(["mosaic", "codex-teams", "resume", "SID"])
        )
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "omo", sessionId: "SID", executablePath: nil, arguments: ["mosaic", "omo"]
            ) == .resolved(["mosaic", "omo", "--session", "SID"])
        )
        // One-shot wrappers have no resumable form (omx and omc share an arm; exercise each).
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "omx", sessionId: "SID", executablePath: nil, arguments: ["mosaic", "omx"]
            ) == .resolved(nil)
        )
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "omc", sessionId: "SID", executablePath: nil, arguments: ["mosaic", "omc"]
            ) == .resolved(nil)
        )
    ...
    func renderedPortableClaudeResumeShellCommand() {
        ...
        // Launcher resumes that resolve to mosaic's own CLI emit no bare `claude`:
        // already-portable quoted words stay unwrapped.
        #expect(
            AgentResumeArgv.renderedPortableClaudeResumeShellCommand(
                parts: ["/Applications/mosaic.app/Contents/Resources/bin/mosaic", "claude-teams", "--resume", "SID"],
                quote: quote
            ) == "'/Applications/mosaic.app/Contents/Resources/bin/mosaic' 'claude-teams' '--resume' 'SID'"
        )
    }

    @Test("Codex wrapper token resolves MOSAIC_CODEX_WRAPPER_SHIM, degrading to bare codex")
    ...
    func renderedPortableCodexResumeShellCommand() {
        ...
        #expect(
            AgentResumeArgv.renderedPortableCodexResumeShellCommand(
                parts: ["/Applications/mosaic.app/Contents/Resources/bin/mosaic", "codex-teams", "resume", "SID"],
                quote: quote
            ) == "'/Applications/mosaic.app/Contents/Resources/bin/mosaic' 'codex-teams' 'resume' 'SID'"
        )
    }
}
