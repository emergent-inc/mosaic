...
/// Regression coverage for https://github.com/emergent-inc/mosaic/issues/5427.
///
/// mosaic injects Claude Code's hooks from the `Resources/bin/claude` wrapper,
/// which is first on `PATH` inside mosaic terminals and re-injects the hook
/// `--settings` whenever it sees `--resume`. The captured launch executable,
/// however, is the *real* claude binary (`MOSAIC_AGENT_LAUNCH_EXECUTABLE`).
...
@Suite("Claude resume routes through the mosaic wrapper")
struct ClaudeResumeHookSettingsTests {
    ...
    func staleHookSettingsStripped() throws {
        ...
        // dropped; the wrapper re-applies mosaic's current hooks at exec time.
        let argv = try #require(
            AgentResumeArgv().builtInKind(
                kind: "claude",
                sessionId: "s",
                ...
                arguments: [
                    ...
                    "opus",
                    "--settings",
                    "/tmp/mosaic-claude-hook-x/settings.json"
                ]
            )
        )
        #expect(argv == ["claude", "--resume", "s", "--model", "opus"])
    ...
    func mergedHookSettingsKeepsUserKeysOnResume() throws {
        let mergedSettings = #"{"effortLevel":"max","preferredNotifChannel":"notifications_disabled","hooks":{"SessionStart":[{"matcher":"","hooks":[{"type":"command","command":"mosaic hooks claude session-start","timeout":10}]}]}}"#
