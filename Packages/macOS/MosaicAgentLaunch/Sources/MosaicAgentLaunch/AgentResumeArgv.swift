...
/// (`AgentResumeCommandBuilder` in the app target) and the standalone `mosaic-cli` surface-restore
...
///
/// Resolution order mirrors the historical app builder: a mosaic wrapper launcher
...
public struct AgentResumeArgv: Sendable, Equatable {
    ...
    /// The shell token that resolves mosaic's `claude` wrapper at exec time.
    ///
    /// The claude resume/fork argv emits a bare `claude` executable and relies on
    /// mosaic's `claude` wrapper (which re-injects the hook `--settings` on `--resume`)
    /// being reachable. Inside mosaic terminals the wrapper is reached two ways: a
    ...
    ///
    /// `MOSAIC_CLAUDE_WRAPPER_SHIM` is a *managed* terminal environment variable (set by
    ...
    /// `-lic` launcher. https://github.com/emergent-inc/mosaic/issues/5639
    ///
    ...
    /// fallback used when the variable is unset outside mosaic.
    ///
    ...
    public static let claudeWrapperShellExecutableToken =
        "\"$([ -x \"${MOSAIC_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$MOSAIC_CLAUDE_WRAPPER_SHIM\" || printf claude)\""

    /// The shell token that resolves mosaic's `codex` wrapper at exec time.
    ///
    ...
    /// `mosaic-codex-wrapper` and dropping every mosaic hook (no `SessionStart`, the
    ...
    /// environment variable `MOSAIC_CODEX_WRAPPER_SHIM` (set by
    ...
    /// the hooks fire. https://github.com/emergent-inc/mosaic/issues/5639
    ///
    ...
    /// variable is unset outside mosaic.
    ///
    ...
    public static let codexWrapperShellExecutableToken =
        "\"$([ -x \"${MOSAIC_CODEX_WRAPPER_SHIM:-}\" ] && printf '%s' \"$MOSAIC_CODEX_WRAPPER_SHIM\" || printf codex)\""
    ...
    /// `"$_mosaic_resume_shell" -c <command>` for its `csh|tcsh` and `*` branches), and the
    ...
    /// apply), `sh` inherits `MOSAIC_CLAUDE_WRAPPER_SHIM` from the managed terminal
    /// environment and resolves the token, and outside mosaic the unset variable still
    ...
    /// composing on the outside. https://github.com/emergent-inc/mosaic/issues/5639
    ...
    ///
    /// Claude-launcher resumes that resolve to mosaic's own CLI (`mosaic claude-teams --resume …`)
    /// emit no bare `claude` executable: they were already portable quoted words, the mosaic
    ...
    /// layer lets the app resume builder and the mosaic-cli surface-restore publisher share one
    /// wrapper-routing seam. https://github.com/emergent-inc/mosaic/issues/5639
    ...
    /// identically while `sh` still inherits `MOSAIC_CODEX_WRAPPER_SHIM` from the
    ...
    /// Call only for the codex kind. https://github.com/emergent-inc/mosaic/issues/5639
    ...
    /// The result of resolving a mosaic wrapper launcher (the `claude-teams` / `codex-teams` / `omo`
    /// style launchers mosaic injects), checked before the per-kind verb.
    public enum LauncherResolution: Sendable, Equatable {
        /// The launcher is a mosaic wrapper; the associated value is its resume argv, or `nil` when the
    ...
    /// Resolves a resume argv from a mosaic wrapper launcher, or ``LauncherResolution/passthrough`` when
    ...
    public func launcherResolution(
        ...
        switch launcher {
        case "claudeTeams":
            let parts = commandParts(executablePath: executablePath, arguments: arguments, fallbackExecutable: "mosaic")
            var tail = parts.tail
    ...
    /// Builds the claude resume argv, routing it through mosaic's `claude` wrapper
    /// so mosaic hooks fire on the resumed session.
    ///
    /// mosaic injects Claude Code's hook `--settings` from the `mosaic-claude-wrapper`,
    ...
    /// *real* claude binary (`MOSAIC_AGENT_LAUNCH_EXECUTABLE`), so resuming with it
    ...
    /// the `MOSAIC_CLAUDE_WRAPPER_SHIM` managed env var), because relying on the wrapper
    /// being first on `PATH` does not hold inside the `$SHELL -lic` restore launcher
    /// (https://github.com/emergent-inc/mosaic/issues/5639). The captured executable is
    /// intentionally ignored for claude; the wrapper resolves the real binary
    /// (honouring `MOSAIC_CUSTOM_CLAUDE_PATH`).
