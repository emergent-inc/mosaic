...
private final class FakeHistoryProvider: BrowserProfileHistoryProviding {
    ...
    var defaultURL: URL? = URL(fileURLWithPath: "/tmp/mosaic-test-default/browser_history.json")
...
private func makeRepository(
    suiteName: String = "mosaic.browserprofiles.test.\(UUID().uuidString)",
    ...
    bundleIdentifier: String = "ai.emergent.inc.mosaic.test"
...
struct BrowserProfileRepositoryTests {
    ...
    @Test func historyFileURLDefaultAndPerProfileShape() {
        let history = FakeHistoryProvider()
        let (repo, _) = makeRepository(history: history, bundleIdentifier: "ai.emergent.inc.mosaic.test")
        ...
        #expect(url.path.contains("ns-ai.emergent.inc.mosaic.test"))
    ...
    @Test func loadDedupesPersistedBuiltInDefault() {
        let suite = "mosaic.browserprofiles.test.dedup.\(UUID().uuidString)"
