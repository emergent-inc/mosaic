...
public struct BrowserFindScript: Sendable, Equatable {
    ...
    public static func search(query: String) -> BrowserFindScript {
        ...
        (() => {
          const MARK_CLASS = '__mosaic-find';
          const CURRENT_CLASS = '__mosaic-find-current';
          ...
          window.__mosaicFindMatches = matches;
          window.__mosaicFindIndex = 0;
          ...
          if (!document.getElementById('__mosaic-find-style')) {
            const style = document.createElement('style');
            style.id = '__mosaic-find-style';
            style.textContent = `
              mark.__mosaic-find { background: #facc15; color: #000; border-radius: 2px; }
              mark.__mosaic-find.__mosaic-find-current { background: #f97316; color: #fff; }
            `;
    ...
    public static func next() -> BrowserFindScript {
        ...
        (() => {
          const matches = window.__mosaicFindMatches || [];
          if (matches.length === 0) return JSON.stringify({ total: 0, current: 0 });
          let idx = window.__mosaicFindIndex || 0;
          if (!matches[idx] || !matches[idx].isConnected) {
            window.__mosaicFindMatches = [];
            window.__mosaicFindIndex = 0;
            return JSON.stringify({ total: 0, current: 0 });
          }
          matches[idx].classList.remove('__mosaic-find-current');
          ...
          matches[idx].classList.add('__mosaic-find-current');
          matches[idx].scrollIntoView({ block: 'center', behavior: 'smooth' });
          window.__mosaicFindIndex = idx;
    ...
    public static func clear() -> BrowserFindScript {
        ...
        (() => {
          ...
          window.__mosaicFindMatches = [];
          window.__mosaicFindIndex = 0;
          const style = document.getElementById('__mosaic-find-style');
    ...
    document.querySelectorAll('mark.__mosaic-find').forEach(mark => {
