...
extension BrowserControlService {
    ...
    public func findScript(finderBody: String) -> String {
        ...
        \(finderBody)
          ...
          if (!__mosaicFound) return { ok: false, error: 'not_found' };
          const selector = __mosaicCssPath(__mosaicFound);
          if (!selector) return { ok: false, error: 'not_found' };
          return {
            ok: true,
            selector,
            tag: String(__mosaicFound.tagName || '').toLowerCase(),
            text: String(__mosaicFound.textContent || '').trim()
          };
        })()
        """
    }
