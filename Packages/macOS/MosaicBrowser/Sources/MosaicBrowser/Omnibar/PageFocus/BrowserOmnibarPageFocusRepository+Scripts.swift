...
extension BrowserOmnibarPageFocusRepository {
    ...
    /// `window.__mosaicAddressBarFocusState` (mirrored to the top frame) and tags
    /// the element with a stable `data-mosaic-addressbar-focus-id`. Returns a
    /// `captured:<id>` / `cleared:*` / `error` status string.
    static let captureScript = """
    (() => {
      try {
        const syncState = (state) => {
          window.__mosaicAddressBarFocusState = state;
          try {
            if (window.top && window.top !== window) {
              window.top.postMessage({ mosaicAddressBarFocusState: state }, "*");
            } else if (window.top) {
              window.top.__mosaicAddressBarFocusState = state;
            }
          } catch (_) {}
        };
        ...
        let id = active.getAttribute("data-mosaic-addressbar-focus-id");
        if (!id) {
          id = "mosaic-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2, 8);
          active.setAttribute("data-mosaic-addressbar-focus-id", id);
        }
    ...
    (() => {
      try {
        if (window.__mosaicAddressBarFocusTrackerInstalled) return true;
        ...
        if (window.top === window && !window.__mosaicAddressBarFocusMessageBridgeInstalled) {
          window.__mosaicAddressBarFocusMessageBridgeInstalled = true;
          window.addEventListener("message", (ev) => {
            try {
              const data = ev ? ev.data : null;
              if (!data || !Object.prototype.hasOwnProperty.call(data, "mosaicAddressBarFocusState")) return;
              window.__mosaicAddressBarFocusState = data.mosaicAddressBarFocusState || null;
            } catch (_) {}
          }, true);
        }
        ...
        const ensureFocusId = (el) => {
          let id = el.getAttribute("data-mosaic-addressbar-focus-id");
          if (!id) {
            id = "mosaic-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2, 8);
            el.setAttribute("data-mosaic-addressbar-focus-id", id);
          }
          return id;
        };
    ...
    (() => {
      try {
        const readState = () => {
          let state = window.__mosaicAddressBarFocusState;
          try {
            if ((!state || typeof state.id !== "string" || !state.id) &&
                window.top && window.top.__mosaicAddressBarFocusState) {
              state = window.top.__mosaicAddressBarFocusState;
            }
          } catch (_) {}
          return state;
        };
        ...
        const clearState = () => {
          window.__mosaicAddressBarFocusState = null;
          try {
            if (window.top && window.top !== window) {
              window.top.postMessage({ mosaicAddressBarFocusState: null }, "*");
            } else if (window.top) {
              window.top.__mosaicAddressBarFocusState = null;
            }
          } catch (_) {}
        };
        ...
        const selector = '[data-mosaic-addressbar-focus-id="' + state.id + '"]';
