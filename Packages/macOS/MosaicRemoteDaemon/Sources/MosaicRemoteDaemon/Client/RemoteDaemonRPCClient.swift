...
/// Synchronous JSON-RPC client for a mosaicd-remote daemon over one of three
...
public final class RemoteDaemonRPCClient: @unchecked Sendable {
    ...
    static let bakedVMDaemonSocketPath = "/run/mosaicd-remote.sock"
    ...
    let writeQueue = DispatchQueue(label: "com.mosaic.remote-ssh.daemon-rpc.write.\(UUID().uuidString)")
    let stateQueue = DispatchQueue(label: "com.mosaic.remote-ssh.daemon-rpc.state.\(UUID().uuidString)")
    ...
    ///   - remotePath: Remote path of the mosaicd-remote binary (stdio
    ///     transport only).
    ...
    public func start() throws {
        ...
        do {
            ...
            guard missingCapabilities.isEmpty else {
                throw NSError(domain: "mosaic.remote.daemon.rpc", code: 2, userInfo: [
