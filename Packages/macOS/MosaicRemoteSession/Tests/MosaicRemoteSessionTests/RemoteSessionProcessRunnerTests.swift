...
// `mosaic.remote.process` error codes 1 and 2.
//
...
struct RemoteSessionProcessRunnerTests {
    ...
    @Test("Launch failure throws the pinned mosaic.remote.process code 1")
    func launchFailurePinsErrorCode() {
        ...
        #expect {
            try runner.run(
                RemoteProcessRequest(
                    executable: "/nonexistent/mosaic-no-such-binary",
                    arguments: [],
                    timeout: 2
                ),
                operation: nil
            ...
            return nsError.domain == "mosaic.remote.process"
                && nsError.code == 1
                && nsError.localizedDescription.hasPrefix("Failed to launch mosaic-no-such-binary:")
        }
    }
