...
// mosaicd-remote serve-stdio children reparented to launchd after a crash or
...
extension RemoteSessionCoordinator {
    ...
    /// orphaned (PPID 1) mosaic-owned ssh transports for `destination`,
    ...
    public static func orphanedMosaicRemoteSSHPIDs(
        psOutput: String,
        destination: String,
        relayPort: Int? = nil,
        ...
        return psOutput
            ...
            .compactMap { line -> Int? in
                ...
                guard isOrphanedMosaicRemoteSSHCommand(
                    parsed.command,
    ...
    static func killOrphanedRemoteSSHProcesses(
        ...
        for pid in orphanedMosaicRemoteSSHPIDs(
            psOutput: output,
    ...
    private static func isOrphanedMosaicRemoteSSHCommand(
        _ command: String,
        destination: String,
        relayPort: Int?,
        ...
        if let relayPort {
            ...
            return isMosaicRemotePersistentDaemonServeStdioCommand(
                trimmed,
        ...
        if let trimmedPersistentDaemonSlot {
            if isMosaicRemotePersistentDaemonServeStdioCommand(
                trimmed,
                slot: trimmedPersistentDaemonSlot
            ) {
                return true
            }
            return isMosaicRemoteNonPersistentDaemonServeStdioCommand(trimmed)
        }
        if isMosaicRemoteDaemonServeStdioCommand(trimmed) {
            return true
        }
        return false
    }

    private static func isMosaicRemoteDaemonServeStdioCommand(_ command: String) -> Bool {
        guard command.contains("mosaicd-remote") else { return false }
    ...
    private static func isMosaicRemoteNonPersistentDaemonServeStdioCommand(_ command: String) -> Bool {
        guard isMosaicRemoteDaemonServeStdioCommand(command) else { return false }
        let normalized = command
            .replacingOccurrences(of: "'", with: " ")
            .replacingOccurrences(of: "\"", with: " ")
    ...
    private static func isMosaicRemotePersistentDaemonServeStdioCommand(
        _ command: String,
        slot: String
    ) -> Bool {
        guard isMosaicRemoteDaemonServeStdioCommand(command) else { return false }
