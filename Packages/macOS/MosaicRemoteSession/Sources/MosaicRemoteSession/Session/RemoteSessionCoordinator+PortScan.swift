...
extension RemoteSessionCoordinator {
    ...
    private func scanRemotePortsByPanelLocked(ttyNamesByPanel: [UUID: String]) throws -> [UUID: [Int]] {
        ...
        guard result.status == 0 else {
            ...
            throw NSError(domain: "mosaic.remote.ports", code: 90, userInfo: [
    ...
    private func shouldUseFallbackRemotePortPollingLocked() -> Bool {
        // `mosaic ssh` owns the remote shell bootstrap and can report the remote
    ...
    private func shouldUseTTYFallbackRemotePortPollingLocked() -> Bool {
        // `mosaic ssh` can still land in shells without our command hooks, such as
    ...
    static func remotePortScanScript(ttyNames: [String], excluding ports: Set<Int>) -> String {
        ...
        return """
        set -eu
        mosaic_tracked_ttys=" \(ttySet) "
        mosaic_tty_csv='\(ttyCSV)'
        mosaic_excluded_ports=" \(excludedPorts) "

        mosaic_emit_port() {
          mosaic_tty="$1"
          mosaic_port="$2"
          case "$mosaic_tracked_ttys" in
            *" $mosaic_tty "*) ;;
            *) return 0 ;;
          esac
          case "$mosaic_excluded_ports" in
            *" $mosaic_port "*) return 0 ;;
          esac
          [ "$mosaic_port" -ge 1024 ] && [ "$mosaic_port" -le 65535 ] || return 0
          printf '%s\\t%s\\n' "$mosaic_tty" "$mosaic_port"
        }

        mosaic_used_ss=0
        if [ -d /proc ] && command -v ss >/dev/null 2>&1; then
          mosaic_ss_output="$(ss -ltnpH 2>/dev/null || true)"
          case "$mosaic_ss_output" in
            *pid=*)
              mosaic_used_ss=1
              printf '%s\\n' "$mosaic_ss_output" | while IFS= read -r mosaic_line; do
                [ -n "$mosaic_line" ] || continue
                mosaic_port="$(printf '%s\\n' "$mosaic_line" | awk '{print $4}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ { print $1; exit }')"
                [ -n "$mosaic_port" ] || continue
                printf '%s\\n' "$mosaic_line" | awk '
                  {
                    line = $0
                ...
                ' | while IFS= read -r mosaic_pid; do
                  [ -n "$mosaic_pid" ] || continue
                  mosaic_tty_path="$(readlink "/proc/$mosaic_pid/fd/0" 2>/dev/null || true)"
                  [ -n "$mosaic_tty_path" ] || continue
                  mosaic_tty="${mosaic_tty_path##*/}"
                  [ -n "$mosaic_tty" ] || continue
                  mosaic_emit_port "$mosaic_tty" "$mosaic_port"
                done
              done
              ;;
          esac
        ...
        if [ "$mosaic_used_ss" -eq 0 ] && command -v lsof >/dev/null 2>&1 && [ -n "$mosaic_tty_csv" ]; then
          mosaic_tmpdir="$(mktemp -d 2>/dev/null || mktemp -d -t mosaic-ports)"
          trap 'rm -rf "$mosaic_tmpdir"' EXIT INT TERM
          mosaic_pid_tty_map="$mosaic_tmpdir/pid_tty"
          ps -t "$mosaic_tty_csv" -o pid=,tty= 2>/dev/null | awk '
            NF >= 2 {
              tty = $2
              sub(/^.*\\//, "", tty)
              print $1 "\\t" tty
          ...
          ' > "$mosaic_pid_tty_map"
          [ -s "$mosaic_pid_tty_map" ] || exit 0
          mosaic_pid_csv="$(awk '{print $1}' "$mosaic_pid_tty_map" | paste -sd, -)"
          [ -n "$mosaic_pid_csv" ] || exit 0
          lsof -nP -a -p "$mosaic_pid_csv" -iTCP -sTCP:LISTEN -Fpn 2>/dev/null | awk -v map="$mosaic_pid_tty_map" '
            BEGIN {
              while ((getline < map) > 0) {
                pid_to_tty[$1] = $2
              }
          ...
          ' | while IFS=$'\\t' read -r mosaic_tty mosaic_port; do
            [ -n "$mosaic_tty" ] || continue
            [ -n "$mosaic_port" ] || continue
            mosaic_emit_port "$mosaic_tty" "$mosaic_port"
          done
    ...
    static func remoteAllPortsScanScript(excluding ports: Set<Int>) -> String {
        ...
        if command -v ss >/dev/null 2>&1; then
          ss -ltnH 2>/dev/null | awk '{print $4}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ {print $1}' | while IFS= read -r mosaic_port; do
            [ -n "$mosaic_port" ] || continue
            mosaic_emit_port "$mosaic_port"
          done
        elif command -v netstat >/dev/null 2>&1; then
          netstat -lnt 2>/dev/null | awk 'NR > 2 {print $4}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ {print $1}' | while IFS= read -r mosaic_port; do
            [ -n "$mosaic_port" ] || continue
            mosaic_emit_port "$mosaic_port"
          done
        elif command -v lsof >/dev/null 2>&1; then
          lsof -nP -iTCP -sTCP:LISTEN 2>/dev/null | awk 'NR > 1 {print $9}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ {print $1}' | while IFS= read -r mosaic_port; do
            [ -n "$mosaic_port" ] || continue
            mosaic_emit_port "$mosaic_port"
          done
        fi
