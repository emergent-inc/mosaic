...
public import MosaicRemoteDaemon
...
/// orchestration, mosaicd-remote bootstrap and install, the reverse CLI relay,
...
public final class RemoteSessionCoordinator: @unchecked Sendable {
    ...
    let queue = DispatchQueue(label: "com.mosaic.remote-ssh.\(UUID().uuidString)", qos: .utility)
    ...
    ///   - manifestRepository: mosaicd-remote manifest/binary-cache repository.
    ...
    /// The capabilities advertised by the mosaicd-remote baked into the Freestyle snapshot
    ...
    static func bakedVMDaemonHello() -> DaemonHello {
        DaemonHello(
            ...
            remotePath: "/usr/local/bin/mosaicd-remote"
        )
    }
    ...
    func beginConnectionAttemptLocked() {
        ...
        do {
            ...
            if configuration.skipDaemonBootstrap {
                // Cloud-VM path: mosaicd-remote is pre-baked in the image and exposed via
                // systemd socket activation at /run/mosaicd-remote.sock. We skip the probe,
            ...
            guard missingCapabilities.isEmpty else {
                throw NSError(domain: "mosaic.remote.daemon", code: 43, userInfo: [
