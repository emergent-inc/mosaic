...
// composition, the `/tmp/mosaic-drop-<uuid>` path shape, cleanup script text,
...
extension RemoteSessionCoordinator {
    /// Uploads dropped local files to `/tmp/mosaic-drop-*` paths on the remote
    ...
    /// The `/tmp/mosaic-drop-<uuid>[.ext]` remote path a dropped local file
    ...
    public static func remoteDropPath(for fileURL: URL, uuid: UUID = UUID()) -> String {
        ...
        return "/tmp/mosaic-drop-\(uuid.uuidString.lowercased())\(lowercasedSuffix)"
    }
