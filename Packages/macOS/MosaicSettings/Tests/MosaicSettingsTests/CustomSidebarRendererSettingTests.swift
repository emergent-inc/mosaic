...
/// store: the on-disk strings users put in `~/.config/mosaic/mosaic.json` must
...
struct CustomSidebarRendererSettingTests {
    private func makeStore() -> (JSONConfigStore, URL) {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-renderer-tests-\(UUID().uuidString)", isDirectory: true)
        try? FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        let fileURL = tempDir.appendingPathComponent("mosaic.json", isDirectory: false)
