...
@Suite(.serialized) struct SocketControlPasswordStoreTests {
    private func tempFileURL() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-pw-test-\(UUID().uuidString)")
    ...
    @Test func environmentPasswordTakesPriority() {
        let store = SocketControlPasswordStore(
            environment: ["MOSAIC_SOCKET_PASSWORD": "fromenv"],
            fileURL: tempFileURL()
        )
    ...
    @Test func migrateMovesKeychainPasswordIntoFileOnce() throws {
        let url = tempFileURL()
        let deleted = Counter()
        let defaults = UserDefaults(suiteName: "mosaic-pw-test-\(UUID().uuidString)")!
        let store = SocketControlPasswordStore(
            environment: [:],
            fileURL: url,
