...
/// without touching the real Application Support, keychain, or `mosaic.json`.
@Suite struct SocketPasswordStoreConvergenceTests {
    private let secretKey = SettingCatalog().automation.socketPassword

    /// A throwaway control directory standing in for ``MosaicStateDirectory``.
    private func tempBaseDirectory() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-convergence-\(UUID().uuidString)", isDirectory: true)
    }
    ...
    /// the password file's parent directory. This mirrors `mosaicApp.init`, which
    ...
    @Test func bothStoresResolveIdenticalFileURL() async throws {
        ...
        // Exactly the derivation mosaicApp.init performs.
