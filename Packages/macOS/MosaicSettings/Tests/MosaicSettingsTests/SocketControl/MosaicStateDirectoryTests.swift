...
@Suite struct MosaicStateDirectoryTests {
    ...
    @Test func socketPathStaysWellWithinUnixPathLimit() {
        ...
        let socket = MosaicStateDirectory.url(homeDirectory: home)
    ...
    @Test func legacyApplicationSupportURLPointsAtOldLocation() {
        let legacy = MosaicStateDirectory.legacyApplicationSupportURL(fileManager: .default)
        // Only used for one-time migration; it must still address the old folder.
        #expect(legacy?.lastPathComponent == "mosaic")
        #expect(legacy?.path.contains("/Application Support/mosaic") == true)
    }
}
...
@Suite(.serialized) struct SocketControlPasswordMigrationTests {
    private func tempDir() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-pw-migrate-\(UUID().uuidString)", isDirectory: true)
    }
    ...
    /// TCC-protected directory (https://github.com/emergent-inc/mosaic/issues/5146).
