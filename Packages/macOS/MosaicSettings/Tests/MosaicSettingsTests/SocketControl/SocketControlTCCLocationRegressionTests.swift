...
/// Regression coverage for https://github.com/emergent-inc/mosaic/issues/5146.
///
/// The control socket directory and the socket password file are read by the
/// separately code-signed `mosaic` CLI on every agent session-start/stop hook. On
...
@Suite struct SocketControlTCCLocationRegressionTests {
    ...
    @Test func controlSocketDirectoryIsOutsideTCCProtectedAppData() throws {
        ...
        #expect(directory.hasSuffix("/.local/state/mosaic"))
    }
    ...
    @Test func socketPasswordFileIsOutsideTCCProtectedAppData() throws {
        ...
        #expect(path.hasSuffix("/.local/state/mosaic/socket-control-password"))
    }
}
