...
Strongly-typed, migratable settings storage for mosaic. Depends only on
Foundation and `MosaicFileWatch` (for config-file reload watching). Modern
...
- **`JSONConfigStore`** — wraps `~/.config/mosaic/mosaic.json`. Structured config
...
- **JSON config** lives at `~/.config/mosaic/mosaic.json` by default
  (`MosaicConfigLocation().userConfigFile`). The store creates the file (and
  parent directory) on first write. File is missing on first launch, in
  which case every read returns the key's default value.

A populated mosaic.json looks like this — pretty-printed, sorted keys, JSONC
...
let jsonConfigStore = JSONConfigStore(
    fileURL: MosaicConfigLocation().userConfigFile
)
...
let tempDir = FileManager.default.temporaryDirectory
    .appending(path: "mosaic-readme-\(UUID().uuidString)")
try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
let fileURL = tempDir.appending(path: "mosaic.json")
...
1. Pick the catalog section under `Sources/MosaicSettings/Keys/` (e.g.
...
1. Create `Sources/MosaicSettings/Keys/FooCatalogSection.swift`:
   ```swift
...
- **`JSONConfigStore`** — `actor`. Async reads/writes/observe. Owns one
  `MosaicFileWatch.FileWatcher` and fans out file-change events to per-subscriber
  bounded signal streams (no `N × parse` work under burst changes). File
  watching itself lives in the `MosaicFileWatch` package.
...
is the production registry. See `Tests/MosaicSettingsTests/` for patterns.
...
comparisons. See `Tests/MosaicSettingsTests/ShortcutWhenClauseTests.swift`.

## Concurrency

The mosaic repo enforces a strict modern-concurrency policy
(see the mosaic `CLAUDE.md` "Modern Swift concurrency" section): no locks,
