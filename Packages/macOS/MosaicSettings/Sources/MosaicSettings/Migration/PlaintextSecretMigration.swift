...
/// `mosaic.json` config file and into a secure secret sink, then removes the
/// plaintext from `mosaic.json`.
///
/// The socket-control password used to be storable as a plaintext string in
/// `mosaic.json` (an editable, copyable, versionable file). Convergence makes the
...
/// very start of launch, before any managed-config layer reads `mosaic.json`, so
...
public enum PlaintextSecretMigration {
    ...
    public enum Outcome: Equatable, Sendable {
        /// `mosaic.json` does not exist or could not be read; nothing to do.
        case noConfigFile
        ...
        /// Copying the plaintext into the secret sink failed, so `mosaic.json` was
        ...
        /// removed from `mosaic.json`. Also covers an empty/whitespace plaintext.
        case scrubbedWithoutCopy
    }

    /// Lifts a plaintext secret at `plaintextKeyPath` out of `mosaic.json` into the
    /// secret sink (only when the sink is empty), then removes the key from
    /// `mosaic.json` after backing the file up.
    ///
    ...
    ///   - configURL: The `mosaic.json` location.
    ...
    ///     plaintext is left in `mosaic.json` (``Outcome/saveFailedLeftIntact``) so
        ...
        if let plaintext, !plaintext.isEmpty {
            ...
            if existing == nil || existing?.isEmpty == true {
                do {
                    ...
                    // The secure write failed; leave the plaintext in mosaic.json so a
    ...
    /// that appear outside of string literals, so a JSONC `mosaic.json` can be
