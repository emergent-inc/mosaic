...
/// signed in to the mosaic backend.
///
/// The sign-in flow, token storage, and `MosaicAuthUser` model itself
/// live in the host app's auth package and reach the package's
/// ``AccountSection`` via an injected delegate (see ``AccountFlow``
/// in MosaicSettingsUI), not through this catalog.
