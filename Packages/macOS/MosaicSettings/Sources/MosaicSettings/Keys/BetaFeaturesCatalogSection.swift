...
/// pattern `<feature-domain>.beta.<flag-name>` so the mosaic.json view
...
public struct BetaFeaturesCatalogSection: SettingCatalogSection {
    ...
    /// JSON) discovered from `~/.config/mosaic/sidebars/` and selectable in the
    ...
    /// Remote tmux: mirror a remote host's tmux sessions in the mosaic sidebar
    ...
    /// create/close propagate to the remote, while quitting mosaic leaves the
