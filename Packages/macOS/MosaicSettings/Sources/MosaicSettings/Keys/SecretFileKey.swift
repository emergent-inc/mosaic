...
///
/// `SecretFileKey` is the third key flavor in ``MosaicSettings``, alongside
/// ``DefaultsKey`` (UserDefaults) and ``JSONKey`` (the mosaic JSON config). It
/// exists for values that must not live in the shared, user-editable
/// `mosaic.json` (for example, the socket-control password): each secret is
...
/// store (the app uses `~/.config/mosaic`, the same directory as `mosaic.json`).
///
/// ```swift
