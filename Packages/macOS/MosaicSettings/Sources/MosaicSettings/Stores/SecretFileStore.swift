...
/// `mosaic.json`: each secret is written to its own file with owner-only
...
/// store with `~/.config/mosaic` (the same directory as `mosaic.json`); tests pass
...
public actor SecretFileStore {
    ...
    public static let didChangeNotification = Notification.Name("mosaic.secretFileStoreDidChange")
    ...
    ///   (created on first write if absent). The app uses `~/.config/mosaic`.
