...
/// Typed read/write/observe access to settings persisted in the mosaic JSON config file.
///
...
///
/// Observation uses a single ``MosaicFileWatch/FileWatcher`` owned by the store
...
/// let store = JSONConfigStore(fileURL: MosaicConfigLocation().userConfigFile)
...
public actor JSONConfigStore {
    ...
    ///     ``MosaicConfigLocation/userConfigFile`` for the standard mosaic path.
    ...
    /// drains the ``MosaicFileWatch/FileWatcher`` events and fans out to every
