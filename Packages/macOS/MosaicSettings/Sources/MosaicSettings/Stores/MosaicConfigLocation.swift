...
/// Conventional on-disk locations for the mosaic JSON config.
///
...
///
/// ```swift
/// let locations = MosaicConfigLocation()
/// let store = JSONConfigStore(fileURL: locations.userConfigFile)
/// ```
public struct MosaicConfigLocation: Sendable, Hashable {
    /// The primary mosaic config file: `<home>/.config/mosaic/mosaic.json`.
    public let userConfigFile: URL

    /// The legacy fallback: `<home>/.config/mosaic/settings.json`. The app's
    ...
    public init(home: URL = FileManager.default.homeDirectoryForCurrentUser) {
        ...
        self.userConfigFile = home.appending(path: ".config/mosaic/mosaic.json")
        self.legacyFallbackFile = home.appending(path: ".config/mosaic/settings.json")
    }
}
