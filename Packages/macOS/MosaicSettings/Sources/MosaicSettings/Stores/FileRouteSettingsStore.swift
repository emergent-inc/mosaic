...
/// `UserDefaults` under the catalog's `app.openMarkdownInMosaicViewer` and
/// `app.openSupportedFilesInMosaic` keys.
///
...
public struct FileRouteSettingsStore: FileRouteSettingsReading {
    ...
    public static let markdownRouteDidChange = Notification.Name("mosaic.cmdClickMarkdownRouteDidChange")

    /// Posted after the supported-file route toggle changes.
    public static let supportedFileRouteDidChange = Notification.Name("mosaic.cmdClickSupportedFileRouteDidChange")
    ...
    public func setSupportedFileRouteEnabled(_ enabled: Bool) {
        keys.openSupportedFilesInMosaic.set(enabled, in: defaults)
