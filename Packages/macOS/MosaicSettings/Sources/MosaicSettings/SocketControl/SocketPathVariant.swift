/// A flavor of the mosaic app, used to derive per-flavor socket and marker file paths.
///
...
/// nightly/staging/dev builds that carry a tag in their bundle identifier or `MOSAIC_TAG`.
public enum SocketPathVariant: Equatable, Sendable {
    ...
    /// The marker file name (within ``MosaicStateDirectory``) that records this
    ...
    public var tmpPath: String {
        ...
        case .nightly(let slug):
            if let slug = Self.sanitizedSlug(slug) {
                return "/tmp/mosaic-nightly-\(slug)-last-socket-path"
            }
            return "/tmp/mosaic-nightly-last-socket-path"
        case .staging(let slug):
            if let slug = Self.sanitizedSlug(slug) {
                return "/tmp/mosaic-staging-\(slug)-last-socket-path"
            }
            return "/tmp/mosaic-staging-last-socket-path"
        case .dev(let slug):
            if let slug = Self.sanitizedSlug(slug) {
                return "/tmp/mosaic-dev-\(slug)-last-socket-path"
            }
            return "/tmp/mosaic-dev-last-socket-path"
        }
    }
