...
/// The per-user directory that holds mosaic's control-plane runtime state: the
...
/// the mosaic app (bundle id `mosaic.com.emergent.app`) and the standalone `mosaic` CLI
/// installed at `/usr/local/bin/mosaic`. On macOS Sequoia, a non-sandboxed process
...
///
/// This directory therefore resolves to `~/.local/state/mosaic`, a plain dotfolder
/// macOS does **not** treat as protected app data. It is the sibling of the
/// existing `~/.local/state/mosaic/crash` breadcrumb directory.
///
/// ```swift
...
/// let socket = MosaicStateDirectory.url(homeDirectory: home).appendingPathComponent("mosaic.sock")
/// ```
public enum MosaicStateDirectory {
    /// The directory name segment under `~/.local/state` (and the legacy name
    /// under `~/Library/Application Support`).
    public static let directoryName = "mosaic"

    /// The mosaic state directory: `<home>/.local/state/mosaic`.
    ///
