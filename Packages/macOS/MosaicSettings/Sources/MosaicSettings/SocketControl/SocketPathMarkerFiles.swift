...
public enum SocketPathMarkerFiles {
    ...
    public static let stableTmpPath = "/tmp/mosaic-last-socket-path"
    ...
    public static let defaultDebugSocketPath = "/tmp/mosaic-debug.sock"
    public static let defaultNightlySocketPath = "/tmp/mosaic-nightly.sock"
    public static let defaultStagingSocketPath = "/tmp/mosaic-staging.sock"
    ...
    public static func variant(
        ...
        if bundleId == baseDebugBundleIdentifier {
            if let tag = normalized(environment["MOSAIC_TAG"]),
    ...
    public static func defaultSocketPath(
        ...
        case .nightly(let slug):
            if let slug {
                return "/tmp/mosaic-nightly-\(slug).sock"
            }
            return nightlySocketPath
        case .staging(let slug):
            if let slug {
                return "/tmp/mosaic-staging-\(slug).sock"
            }
