...
///
/// The password sources, highest priority first, are the `MOSAIC_SOCKET_PASSWORD`
...
public struct SocketControlPasswordStore: Sendable {
    ...
    public static let didChangeNotification = Notification.Name("mosaic.socketControlPasswordDidChange")
    ...
    /// - Parameters:
    ///   - environment: The process environment used to read `MOSAIC_SOCKET_PASSWORD`.
    ...
    /// The default password file URL within the mosaic state directory.
    ///
    /// Resolves to `<directory>/socket-control-password`, where `directory`
    /// defaults to ``MosaicStateDirectory`` (`~/.local/state/mosaic`). The file lives
    /// outside Application Support so the separately-signed `mosaic` CLI can read it
    ...
    ///     ``MosaicStateDirectory`` location is resolved via `fileManager`.
    ...
    /// (`~/Library/Application Support/mosaic/socket-control-password`) into the
    /// current ``MosaicStateDirectory`` location, once.
    ///
    ...
    public static func migrateLegacyApplicationSupportPasswordFileIfNeeded(
        ...
        guard let destination = defaultPasswordFileURL(fileManager: fileManager),
              let legacyDirectory = MosaicStateDirectory.legacyApplicationSupportURL(fileManager: fileManager) else {
            return
        }
        migratePasswordFile(
