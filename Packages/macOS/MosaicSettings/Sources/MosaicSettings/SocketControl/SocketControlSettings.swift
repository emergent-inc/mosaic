...
/// Pure resolution policy for the mosaic control socket: where its path lives, which
...
public struct SocketControlSettings {
    ...
    /// Environment key that, when truthy, permits honoring `MOSAIC_SOCKET_PATH` overrides.
    public static let allowSocketPathOverrideKey = "MOSAIC_ALLOW_SOCKET_OVERRIDE"
    /// Environment key carrying the socket password (highest-priority password source).
    public static let socketPasswordEnvKey = "MOSAIC_SOCKET_PASSWORD"
    /// Environment key carrying the dev build's launch tag.
    public static let launchTagEnvKey = "MOSAIC_TAG"
    ...
    public static let legacyStableDefaultSocketPath = "/tmp/mosaic.sock"

    /// The stable build's default socket path (within ``MosaicStateDirectory``, falling back to `/tmp`).
    ...
    public static func shouldBlockUntaggedDebugLaunch(
        ...
        // so isRunningUnderXCTest() misses it. Check for any MOSAIC_UI_TEST_ env var.
    ...
    /// The control-socket path to use, honoring `MOSAIC_SOCKET_PATH` overrides only when safe.
    public static func socketPath(
        ...
        guard let override = environment["MOSAIC_SOCKET_PATH"], !override.isEmpty else {
            return fallback
        }
    ...
    /// The per-user stable socket path (`mosaic-<uid>.sock` in ``MosaicStateDirectory``, `/tmp` fallback).
    public static func userScopedStableSocketPath(currentUserID: uid_t = getuid()) -> String {
        stableSocketDirectoryURL()?
            ...
            .path ?? "/tmp/mosaic-\(currentUserID).sock"
    }
    ...
    /// Whether a `MOSAIC_SOCKET_PATH` override should be honored for this build.
    ...
    private static func inheritedBundleIdentifierConflicts(
        ...
        guard let inheritedBundleIdentifier = normalizedBundleIdentifier(environment["MOSAIC_BUNDLE_ID"]),
    ...
    ///
    /// Resolves to ``MosaicStateDirectory`` (`~/.local/state/mosaic`) rather than
    /// Application Support: the separately-signed `mosaic` CLI connects to this
    ...
    /// from other apps" prompt (https://github.com/emergent-inc/mosaic/issues/5146).
    public static func stableSocketDirectoryURL(fileManager: FileManager = .default) -> URL? {
        MosaicStateDirectory.url(homeDirectory: fileManager.homeDirectoryForCurrentUser)
    }

    /// The stable control socket file URL (within ``MosaicStateDirectory``), if it can be resolved.
    ...
    /// The `MOSAIC_SOCKET_ENABLE` override as a tri-state (`true`/`false`/`nil` when unset/invalid).
    public static func envOverrideEnabled(
        environment: [String: String] = ProcessInfo.processInfo.environment
    ) -> Bool? {
        guard let raw = environment["MOSAIC_SOCKET_ENABLE"], !raw.isEmpty else {
            return nil
        }
    ...
    /// The `MOSAIC_SOCKET_MODE` override as a ``SocketControlMode``, or `nil` when unset/invalid.
    public static func envOverrideMode(
        environment: [String: String] = ProcessInfo.processInfo.environment
    ) -> SocketControlMode? {
        guard let raw = environment["MOSAIC_SOCKET_MODE"], !raw.isEmpty else {
            return nil
        }
        return parseMode(raw)
    }

    /// The effective mode after applying `MOSAIC_SOCKET_ENABLE`/`MOSAIC_SOCKET_MODE` overrides.
    /// - Parameters:
    ...
    public static func effectiveMode(
        ...
        if let overrideEnabled = envOverrideEnabled(environment: environment) {
            ...
            return userMode == .off ? .mosaicOnly : userMode
        }
