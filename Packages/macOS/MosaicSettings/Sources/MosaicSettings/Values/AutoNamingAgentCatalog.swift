...
/// power user can name a custom agent in `~/.config/mosaic/mosaic.json` even if it
...
public struct AutoNamingAgentOption: Sendable, Hashable {
    ...
    /// `<agent>` segment of `mosaic hooks <agent> auto-name`.
    public let slug: String
    /// Brand display name shown in the picker (proper noun — not localized).
    public let displayName: String
    /// Whether mosaic currently knows how to drive this agent as a summarizer.
...
///
/// Lives in `MosaicSettings` (imported by both the app's Settings UI and the
/// bundled `mosaic` CLI) so the picker and the summarizer dispatch share one
...
public enum AutoNamingAgentCatalog {
    ...
    /// Agents whose binary mosaic knows how to invoke for a one-shot
    ...
    /// agent until mosaic learns to drive them).
    ...
    /// True only when mosaic can drive `slug` as a summarizer. Custom/unknown
