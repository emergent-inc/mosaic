...
/// Controls which clients may connect to the mosaic automation socket.
///
/// Stored under the catalog entry ``SettingCatalog/automationSocketControlMode``.
/// The cases mirror the on-disk strings used by `~/.config/mosaic/mosaic.json` and
...
public enum SocketControlMode: String, CaseIterable, Sendable, SettingCodable {
    ...
    case off
    /// Only the bundled `mosaic` CLI may connect.
    case mosaicOnly
