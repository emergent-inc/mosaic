...
extension NSColor {
    ...
    func mosaicCanvasLighten(by amount: CGFloat) -> NSColor {
        blended(withFraction: amount, of: .white) ?? self
    }

    /// Darkens toward black by `amount` (0...1).
    func mosaicCanvasDarken(by amount: CGFloat) -> NSColor {
        blended(withFraction: amount, of: .black) ?? self
    }

    /// The active (selected) tab fill for a bar of this background color.
    var mosaicCanvasActiveTabFill: NSColor {
        mosaicCanvasIsLight ? mosaicCanvasDarken(by: 0.065) : mosaicCanvasLighten(by: 0.12)
    }

    /// The hovered (unselected) tab fill for a bar of this background color.
    var mosaicCanvasHoverTabFill: NSColor {
        let adjusted = mosaicCanvasIsLight ? mosaicCanvasDarken(by: 0.03) : mosaicCanvasLighten(by: 0.07)
