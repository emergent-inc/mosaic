...
/// stateless `MosaicGit` ``MosaicGit/PullRequestProbeService`` pipeline.
///
...
public final class PullRequestPollService: PullRequestProbing {
    ...
    // Resolves slugs for candidate seeds (stateless MosaicGit reader).
    let gitMetadataService: GitMetadataService
    // Fetches and matches GitHub PRs (stateless MosaicGit pipeline).
