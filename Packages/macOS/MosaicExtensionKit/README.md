# MOSAIC Extension Kit

`MosaicExtensionKit` is the zero-dependency public SDK for MOSAIC sidebar extensions.
...
Host-side lifecycle, discovery, and display belong in `Packages/macOS/MosaicExtensionHostSupport`.
Internal mosaic-owned sidebar provider/render models live in `Packages/macOS/MosaicSidebarProviderKit`.
They are separate from the public extension-author SDK.

## Five-Minute Sidebar Extension

Sidebar extensions are ExtensionKit app extensions. `MosaicExtensionKit` and the reference projects target macOS 14+, matching MOSAIC.
...
4. Keep the extension point identifier as `mosaic.com.emergent.app.mosaic.sidebar`.
5. Build and launch the containing app once so macOS registers the embedded extension.
6. In MOSAIC, open Sidebar Extensions from the puzzle button next to the sidebar help button and enable your extension.
...
<dict>
  ...
  <string>mosaic.com.emergent.app.mosaic.sidebar</string>
</dict>
```

Define your ExtensionKit entrypoint by conforming directly to `MosaicSidebarExtension`.
...
@Observable
@MainActor
final class ExampleSidebarExtension: MosaicSidebarExtension {
...
`MosaicSidebarExtension` is the public extension protocol. It refines `AppExtension`,
requires a manifest and SwiftUI `body`, and delivers `MosaicSidebarContext`, which
contains the filtered `MosaicSidebarSnapshot` and a typed `MosaicSidebarHost` command
channel.

The lower-level transport lives behind MOSAIC host SPI. New sidebar extensions should
conform to `MosaicSidebarExtension` and should not handle XPC directly.
...
host-side callbacks are SPI for MOSAIC's own host implementation.
Creating or splitting a browser surface with a URL requires both the surface
action scope and `openURL`.

## Permissions

List every scope and action your extension needs in its manifest. MOSAIC filters the
...
If your extension does not appear, confirm the containing app has been launched, the embedded appex is signed by your team, the extension point identifier is unchanged, and MOSAIC's Sidebar Extensions browser shows the extension as enabled.
