...
public struct MosaicSidebarWorkspace: Codable, Equatable, Identifiable, Sendable {
    public var id: UUID
    ...
    public var surfaces: [MosaicSidebarSurface]
    ...
    public init(from decoder: Decoder) throws {
        ...
        surfaces = try container.decodeIfPresent([MosaicSidebarSurface].self, forKey: .surfaces) ?? []
    }

    @_spi(MosaicHostTransport)
    public func filtered(for scopes: some Sequence<MosaicExtensionScope>) -> MosaicSidebarWorkspace {
        let scopeSet = Set(scopes)
        return MosaicSidebarWorkspace(
            id: id,
            title: title,
            detail: detail,
            isPinned: isPinned,
