...
/// Public identifiers for the MOSAIC sidebar ExtensionKit surface.
@_spi(MosaicHostTransport)
public enum MosaicSidebarExtensionPoint {
    ...
    /// per build tag (e.g. `mosaic.com.emergent.app.debug.my-tag.mosaic.sidebar`) so that concurrent debug
    ...
    /// committed to source.
    public static let baseIdentifier = "mosaic.com.emergent.app.mosaic.sidebar"

    /// Info.plist key a bundle may declare to override the extension point identifier.
    ///
    /// Populated at build time from the `MOSAIC_SIDEBAR_EXTENSION_POINT_ID` build setting
    ...
    public static let identifierInfoPlistKey = "MosaicSidebarExtensionPointIdentifier"
    ...
    /// Default ExtensionKit scene identifier hosted inside the mosaic sidebar.
