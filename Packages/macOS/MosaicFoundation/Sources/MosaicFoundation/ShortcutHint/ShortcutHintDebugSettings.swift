...
public struct ShortcutHintDebugSettings {
    ...
    /// `shortcuts.showModifierHoldHints` toggle. `MosaicFoundation` is a leaf
    /// module and cannot import `MosaicSettings`, so the key is duplicated here;
    ...
    public var alwaysShowHints: Bool {
        Self.defaultAlwaysShowHints || environment["MOSAIC_UI_TEST_SHORTCUT_HINTS_ALWAYS_SHOW"] == "1"
    }
