...
/// from Sentry's relay `@common` scrubbers, plus mosaic's own provider-key / JWT /
...
public struct ScrubberDenylists: Sendable {
    ...
    /// sentry-python itself credits as "stolen from relay". mosaic's substring
    ...
    static let sensitiveKeyMarkers: [String] = [
        ...
        // mosaic-original markers retained (not in sentry-python's list) so no
    ...
    /// (PII keys added because mosaic runs with `sendDefaultPii = false`), with the
    ...
    static let sensitiveKeyExactMarkers: Set<String> = [
        ...
        // sentry-python DEFAULT_PII_DENYLIST (mosaic sends sendDefaultPii = false).
    ...
    ///
    /// The first seven entries are mosaic-original and partly exceed relay (relay
    ...
    /// value rules mosaic was missing, ported with no capture group so mosaic's
    ...
    /// members are intentionally NOT duplicated here: mosaic already covers email /
    ...
    static let valuePatterns: [SentryRegexPattern] = [
        // --- mosaic-original rules (kept verbatim; partly exceed relay) ---
        ...
        // (relay-pii/src/regexes.rs:341-345), hand-tuned for mosaic's free text.
        //
        ...
        // prefixes. No relay equivalent — mosaic-specific add for dev secrets.
        ...
        // --- relay @common value rules mosaic was missing (ported, no group) ---
        // @pemkey — a PEM private/public key block. relay captures group 1 (the
        // body) and replaces only the body; mosaic redacts the WHOLE block (header,
        ...
        // dotall flag (mosaic's SentryRegexPattern sets only .caseInsensitive).
