...
/// App-wide magnification for mosaic-owned chrome and terminal configuration.
///
/// Stored as an integer percent (100 = default, 150 = 1.5x, 200 = 2x).
/// SwiftUI call sites should use ``View/mosaicFont(size:weight:design:monospacedDigit:)``
/// or ``View/mosaicFont(_:weight:design:)``. AppKit call sites should use the
...
public struct GlobalFontMagnification {
    ...
    /// Capped at 200% so mosaic fixed-size chrome does not clip or overflow.
    ...
    public static let didChangeNotification = Notification.Name("mosaic.globalFontMagnification.didChange")
