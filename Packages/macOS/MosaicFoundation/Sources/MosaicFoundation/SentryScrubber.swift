...
public struct SentryScrubber: Sendable {
    ...
    ///
    /// This is mosaic's equivalent of relay's `@common` `@urlauth` rule
    ...
    /// ``ScrubberDenylists/valuePatterns`` because mosaic's redaction loop treats
    ...
    /// `@common` ports and mosaic's own provider-key / JWT / AWS rules live in one
    ...
    /// sentry-python `DEFAULT_DENYLIST`). mosaic's substring-contains semantics
