    ...
    ForEach(needsAttention) { ws in
      ...
      HStack(alignment: .top, spacing: 9) {
        ...
        VStack(alignment: .leading, spacing: 7) {
          ...
          HStack(spacing: 6) {
            Button { mosaic("workspace.select", workspace_id: ws.id) } label: {
              HStack(spacing: 4) {
            ...
            Button { mosaic("workspace.markRead", workspace_id: ws.id) } label: {
            ...
            if ws.pr.number > 0 {
              Menu("PR") {
                Button("Open #\(ws.pr.number)") { mosaic("pr.open", url: ws.pr.url) }
                Button("Copy branch") { mosaic("workspace.copyBranch", workspace_id: ws.id) }
              }
              .font(.system(size: 10))
            }
          }
      ...
      .onTapGesture { mosaic("workspace.select", workspace_id: ws.id) }
      .contextMenu {
        Button("Triage") { mosaic("workspace.select", workspace_id: ws.id) }
        Button("Mark read") { mosaic("workspace.markRead", workspace_id: ws.id) }
        Button(ws.pinned ? "Unpin" : "Pin") { mosaic("workspace.togglePin", workspace_id: ws.id) }
      }
    }
