    ...
    VStack(alignment: .leading, spacing: 14) {
                ...
                ForEach(Array(reviewable.enumerated()), id: \.offset) { idx, ws in
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        HStack(alignment: .top, spacing: 8) {
                            ...
                            // actions menu
                            Menu("") {
                                Button("Open PR #\(pr.number)") { mosaic("openURL", param: pr.url) }
                                Button("Focus workspace") { mosaic("selectWorkspace", param: ws.id) }
                                Button("Approve") { mosaic("prApprove", param: pr.number) }
                                Button("Request changes") { mosaic("prRequestChanges", param: pr.number) }
                                Button("Refresh PR") { mosaic("prRefresh", param: pr.number) }
                                if pr.status == "approved" {
                                    Button("Merge") { mosaic("prMerge", param: pr.number) }
                                }
                            }
                        }
                    ...
                    .contextMenu {
                        Button("Open PR #\(pr.number)") { mosaic("openURL", param: pr.url) }
                        Button("Focus workspace") { mosaic("selectWorkspace", param: ws.id) }
                        Button("Refresh") { mosaic("prRefresh", param: pr.number) }
                    }
                    .onTapGesture { mosaic("selectWorkspace", param: ws.id) }
        ...
        ViewThatFits {
            HStack(spacing: 10) {
                ...
                Spacer()
                Button {
                    mosaic("prRefreshAll", param: total)
                } label: {
                    HStack(spacing: 4) {
