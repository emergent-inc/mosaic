...
func portChip(_ p: Int, _ wsId) -> some View {
    ...
    .onTapGesture { mosaic("workspace.select", workspace_id: wsId) }
}
...
func cell(_ ws) -> some View {
    ...
    .contextMenu {
        Button(action: { mosaic("workspace.select", workspace_id: ws.id) }) {
            Label("Focus workspace", systemName: "scope")
        }
        if let pr = ws.pr {
            Button(action: { mosaic("open.url", url: pr.url) }) {
