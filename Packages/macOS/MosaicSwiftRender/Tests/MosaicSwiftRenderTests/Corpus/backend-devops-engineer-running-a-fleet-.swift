HSplitView {
    ...
    VStack(alignment: .leading, spacing: 10) {
        ...
        ForEach(services) { svc in
            HStack(spacing: 8) {
                ...
                // restart
                Button(action: { mosaic("service.restart", service_id: svc.id) }) {
                    Image(systemName: "arrow.clockwise")
                }
                // tail logs (focuses/opens the service's log surface)
                Button(action: { mosaic("surface.focus", surface_id: svc.logSurfaceId) }) {
                    Image(systemName: "text.alignleft")
                }
            }
            .padding(4)
            .onTapGesture { mosaic("service.open", service_id: svc.id) }
        }
        ...
        ForEach(ports) { p in
            HStack {
                ...
                Spacer()
                Button(action: { mosaic("port.kill", port: p.port) }) {
        ...
        HStack(spacing: 8) {
            Button("Restart all") { mosaic("service.restartAll") }
                .foregroundColor("#FF9F0A")
            Button("Stop all") { mosaic("service.stopAll") }
    ...
    VStack(alignment: .leading, spacing: 10) {
        ...
        HStack(spacing: 8) {
            ...
            Spacer()
            Button(action: { mosaic("ci.rerun", run_id: build.runId) }) {
                Image(systemName: "arrow.clockwise")
            }
            Button(action: { mosaic("ci.open", run_id: build.runId) }) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                HStack {
                    ...
                    Button(d.env == "prod" ? "Ship prod" : "Deploy") {
                        mosaic("deploy.start", env: d.env, sha: build.sha)
                    }
