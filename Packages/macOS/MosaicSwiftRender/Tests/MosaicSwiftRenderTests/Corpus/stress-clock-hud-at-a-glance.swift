    ...
    if busy.count == 0 {
      ...
      ForEach(busy.prefix(6)) { w in
        ...
        HStack(spacing: 8) {
          ...
          Spacer()
          Button { mosaic("workspace.markRead", workspace_id: w.id) } label: {
        ...
        .contextMenu {
          Button("Open") { mosaic("workspace.select", workspace_id: w.id) }
          Button("Mark read") { mosaic("workspace.markRead", workspace_id: w.id) }
        }
      }
    }
  }
