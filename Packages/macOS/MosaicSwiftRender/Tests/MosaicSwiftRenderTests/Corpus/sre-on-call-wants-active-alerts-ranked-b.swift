    ...
    VStack(spacing: 10) {
        ...
        if incident.active {
            VStack(spacing: 4) {
                ...
                Text(incident.title)
                    .font(.caption)
                HStack {
                    Button("Ack") { mosaic("incident.ack", incident_id: incident.id) }
                        .padding(4)
                        .foregroundColor("#0A84FF")
                    Button("Bridge") { mosaic("link.open", url: incident.bridgeUrl) }
                        .padding(4)
                        .foregroundColor("#0A84FF")
                    Spacer()
        ...
        ForEach(alerts) { a in
            VStack(spacing: 3) {
                ...
                HStack {
                    Button("Ack") { mosaic("alert.ack", alert_id: a.id) }
                        .font(.caption)
                        .padding(3)
                        .foregroundColor("#0A84FF")
                    Button("Runbook") { mosaic("link.open", url: a.runbookUrl) }
                        .font(.caption)
                        .padding(3)
                        .foregroundColor("#0A84FF")
                    Button("Logs") { mosaic("sre.tail", service: a.service) }
                        .font(.caption)
                        .padding(3)
                        .foregroundColor("#0A84FF")
                    Spacer()
            ...
            .padding(6)
            .onTapGesture { mosaic("sre.tail", service: a.service) }
            Divider()
        }
        ...
        ForEach(runbooks) { r in
            ...
            .padding(4)
            .onTapGesture { mosaic("link.open", url: r.url) }
        }
    }
    .padding(8)
    ...
    VStack(spacing: 4) {
        HStack {
            ...
            Text("LOGS · \(tailService)")
                .font(.caption).fontWeight(.semibold)
            Spacer()
            Button(action: { mosaic("sre.tail.terminal", service: tailService) }) {
                HStack {
