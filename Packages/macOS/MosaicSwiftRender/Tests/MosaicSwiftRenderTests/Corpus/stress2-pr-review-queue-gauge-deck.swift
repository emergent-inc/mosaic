    ...
    VStack(alignment: .leading, spacing: 14) {
                    ...
                    if let pr = w.pr {
                        ...
                        VStack(alignment: .leading, spacing: 9) {
                            ...
                            HStack(alignment: .top, spacing: 8) {
                                ...
                                Menu {
                                    Button("Open #\(pr.number)") { mosaic("pr.open", url: pr.url) }
                                    Button("Focus workspace") { mosaic("workspace.select", workspace_id: w.id) }
                                    Divider()
                                    switch pr.status {
                                    case "review_required":
                                        Button("Approve") { mosaic("pr.approve", number: pr.number) }
                                        Button("Request changes") { mosaic("pr.requestChanges", number: pr.number) }
                                    case "changes_requested":
                                        Button("Re-review") { mosaic("pr.review", number: pr.number) }
                                    case "approved":
                                        Button("Merge") { mosaic("pr.merge", number: pr.number) }
                                    default:
                                        Button("View checks") { mosaic("pr.checks", number: pr.number) }
                                    }
                                    if pr.stale {
                                        Button("Refresh (stale)") { mosaic("pr.refresh", number: pr.number) }
                                    }
                                } label: {
                        ...
                        .contextMenu {
                            Button("Open #\(pr.number)") { mosaic("pr.open", url: pr.url) }
                            Button("Focus workspace") { mosaic("workspace.select", workspace_id: w.id) }
                            Button("Refresh") { mosaic("pr.refresh", number: pr.number) }
                        }
                        .onTapGesture {
                            mosaic(pr.stale ? "pr.refresh" : "workspace.select", workspace_id: w.id)
                        }
        ...
        ViewThatFits {
            HStack(spacing: 10) {
                ...
                Spacer()
                Button {
                    mosaic("pr.refreshAll", count: total)
                } label: {
                    HStack(spacing: 4) {
            ...
            HStack {
                Text("\(total) in queue")
                    ...
                    .foregroundColor(.secondary)
                Spacer()
                Button("Refresh") { mosaic("pr.refreshAll", count: total) }
                    .font(.caption)
                    .fontWeight(.semibold)
            }
        }
