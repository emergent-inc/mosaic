...
func row(_ w, _ i) -> some View {
  ...
  .opacity(g == "R" ? 0.6 : 1.0)
  .onTapGesture { mosaic("workspace.select", workspace_id: w.id) }
  .keyboardShortcut(.return)
  .contextMenu {
    Button("Select") { mosaic("workspace.select", workspace_id: w.id) }
    Button("Stop agent") { mosaic("workspace.stop", workspace_id: w.id) }
    Button(w.pinned == true ? "Unpin" : "Pin") { mosaic("workspace.pin", workspace_id: w.id) }
    if w.pr != nil {
      Button("Open PR") { mosaic("workspace.openPR", workspace_id: w.id) }
    }
  }
