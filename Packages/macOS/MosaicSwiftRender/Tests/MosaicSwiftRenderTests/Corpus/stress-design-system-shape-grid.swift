...
func swatch(_ hex: String, _ kind: Int, _ label: String, _ angle: Double) -> some View {
  ...
  .help("\(label) — \(hex)")
  .contextMenu {
    Button("Apply \(hex)") { mosaic("noop", value: hex) }
    Button("Copy name") { mosaic("noop", value: label) }
  }
}
  ...
  VStack(alignment: .leading) {
    ...
    Menu("Palette actions") {
      ForEach(Array(palette().prefix(4).enumerated()), id: \.offset) { i, hex in
        Button("Set accent \(shadeName(i))") { mosaic("noop", value: hex) }
      }
      Button("Reset") { mosaic("noop", value: "reset") }
    }
    .padding(.top, 4)
