...
func prPill(_ ws: Any) -> some View {
    return Group {
        if ws.pr != nil {
            ...
            .help("PR \(ws.pr.label)")
            .onTapGesture { mosaic("openURL", param: ws.pr.url) }
        } else {
...
func railRow(_ ws: Any) -> some View {
    ...
    .contextMenu {
        Button("Focus") { mosaic("workspace.focus", param: ws.id) }
        Button(ws.pinned ? "Unpin" : "Pin") { mosaic("workspace.togglePin", param: ws.id) }
        Button("Mark read") { mosaic("workspace.markRead", param: ws.id) }
    }
    .help(ws.directory)
    .onTapGesture { mosaic("workspace.select", param: ws.id) }
}
...
func tabCard(_ tab: Any) -> some View {
    ...
    .contextMenu {
        Button("Focus tab") { mosaic("tab.focus", param: tab.id) }
        Button(tab.pinned ? "Unpin tab" : "Pin tab") { mosaic("tab.togglePin", param: tab.id) }
        Button("Close tab") { mosaic("tab.close", param: tab.id) }
    }
    .onTapGesture { mosaic("tab.focus", param: tab.id) }
}
...
func cockpitHeader() -> some View {
    return HStack(spacing: 8) {
        ...
        Spacer()
        Menu("•••") {
            Button("Collapse all reads") { mosaic("workspace.markAllRead") }
            Button("New workspace") { mosaic("workspace.create") }
            Section("Sort") {
                Button("By unread") { mosaic("workspace.sort", param: "unread") }
                Button("By recent") { mosaic("workspace.sort", param: "recent") }
            }
        }
        .foregroundColor(.secondary)
        Text(clockLabel())
    ...
    HSplitView {
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 6) {
                ...
                Button("+") { mosaic("tab.create", param: selectedId) }
                    .foregroundColor(.secondary)
