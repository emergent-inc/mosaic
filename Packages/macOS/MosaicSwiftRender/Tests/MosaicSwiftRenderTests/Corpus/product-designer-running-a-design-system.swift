...
HSplitView(spacing: 0) {
  ...
  VStack(alignment: .leading, spacing: 14) {
      ...
      VStack(alignment: .leading, spacing: 6) {
        ...
        if ws.selected {
          ForEach(ws.tabs) { tab in
            ...
            .onTapGesture { mosaic("surface.focus", surface_id: tab.id) }
          }
        }
      }
    }
    ...
    Divider()
    Button(action: { mosaic("workspace.create", template: "design-export") }) {
      HStack(spacing: 6) {
  ...
  VStack(alignment: .leading, spacing: 16) {
    ...
    VStack(alignment: .leading, spacing: 8) {
      ForEach(palette) { c in
        ...
        .onTapGesture { mosaic("clipboard.write", text: c["hex"]) } // clipboard verb -> missing
      }
    }
    ...
    VStack(alignment: .leading, spacing: 8) {
      ForEach(recentAssets) { a in
        ...
        .onTapGesture { mosaic("file.reveal", path: "exports/\(a["file"])") }
      }
    }
    ...
    ForEach(spacing) { s in
      ...
      .onTapGesture { mosaic("clipboard.write", text: s["v"]) }
    }
    ...
    Divider()
    Button(action: { mosaic("surface.run", command: "npm run tokens:build") }) {
      HStack(spacing: 6) {
