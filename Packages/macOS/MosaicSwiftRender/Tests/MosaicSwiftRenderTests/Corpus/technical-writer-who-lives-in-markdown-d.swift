HSplitView {
    ...
    VStack(alignment: .leading, spacing: 10) {
        ...
        .padding(4)
        .onTapGesture { mosaic("writer.sprint.toggle", minutes: 25) }
        ...
        HStack(spacing: 8) {
            Button("＋ Section") { mosaic("writer.insert", snippet: "## ") }
                .font(.caption).foregroundColor("#7AA2F7")
            Button("Frontmatter") { mosaic("writer.insert", snippet: "frontmatter") }
                .font(.caption).foregroundColor("#7AA2F7")
        }
        .padding(4)
    }
    ...
    VStack(alignment: .leading, spacing: 12) {
        ...
        Button(action: { mosaic("writer.format") }) {
            HStack(spacing: 6) {
        ...
        Button(action: { mosaic("writer.preview") }) {
            HStack(spacing: 6) {
        ...
        Button(action: { mosaic("writer.commit", message: "docs: writing session") }) {
            HStack(spacing: 6) {
        ...
        if lintClean {
            Button(action: { mosaic("writer.publish") }) {
                HStack(spacing: 6) {
