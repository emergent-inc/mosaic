...
/// Loopback TCP relay that lets the remote host's `mosaic` CLI reach the local
/// mosaic control socket: it listens on `127.0.0.1`, authenticates each
...
///
/// Wire behavior is pinned: the `mosaic-relay-auth` challenge JSON (protocol,
...
public final class RemoteCLIRelayServer: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.mosaic.remote-ssh.cli-relay.\(UUID().uuidString)", qos: .utility)
    ...
    ///
    /// - Parameters:
    ///   - localSocketPath: Path of the local mosaic control unix socket.
    ///   - relayID: Public identifier echoed in the auth challenge.
    ///   - relayTokenHex: Hex-encoded shared secret; throws
    ///     `mosaic.remote.relay` code 7 when invalid or empty.
    ...
    public init(
        ...
        guard let relayToken = Session.hexData(from: relayTokenHex), !relayToken.isEmpty else {
            throw NSError(domain: "mosaic.remote.relay", code: 7, userInfo: [
    ...
    public func start() throws -> Int {
        ...
        if waitResult != .success {
            ...
            listener.cancel()
            throw NSError(domain: "mosaic.remote.relay", code: 8, userInfo: [
