...
private func mosaicCGSDefaultConnectionForThread() -> UnsafeMutableRawPointer?
...
@discardableResult
private func mosaicCGSSetWindowBackgroundBlurRadius(
...
///
/// Lifted faithfully from the `mosaicResetCompositorBackgroundBlur` free function
...
/// the caller's isolation domain (the legacy `mosaicResetCompositorBackgroundBlur`
...
public struct CompositorBlurController: Sendable {
    ...
    /// `mosaicResetCompositorBackgroundBlur(on:)`. The caller resolves
    /// `window.windowNumber` in its own (main-actor) isolation domain.
    public func resetBackgroundBlur(windowNumber: Int) {
        _ = mosaicCGSSetWindowBackgroundBlurRadius(
            mosaicCGSDefaultConnectionForThread(),
            UInt(windowNumber),
            0
        )
    }
