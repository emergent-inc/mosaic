import Foundation
import Testing
@testable import MosaicWorkspaces
...
struct SessionSnapshotRepositoryTests {
    ...
    @Test("snapshot file paths derive from the sanitized bundle identifier under mosaic/")
    func snapshotFileURLShape() throws {
        let dir = try makeTempDirectory()
        defer { try? FileManager.default.removeItem(at: dir) }
        let repository = makeRepository(appSupport: dir, bundleIdentifier: "com.mosaic odd/id")

        let primary = try #require(repository.defaultSnapshotFileURL())
        let backup = try #require(repository.manualRestoreSnapshotFileURL())
        #expect(primary.path == dir.appendingPathComponent("mosaic/session-com.mosaic_odd_id.json").path)
        #expect(backup.path == dir.appendingPathComponent("mosaic/session-com.mosaic_odd_id-previous.json").path)
    }
