...
public extension SocketTransport {
    ...
    ///
    /// Used with ``isProcessDescendant(_:of:)`` to enforce the `mosaicOnly`
    ...
    ///     // refuse: client was not started inside mosaic
    /// }
    /// ```
    ...
    ///
    /// Pairs with ``peerProcessID(of:)`` for the `mosaicOnly` ancestry check;
    /// see that symbol for a usage example.
    /// - Parameters:
    ///   - pid: The process to test.
    ///   - ancestorPid: The candidate ancestor (the mosaic process).
