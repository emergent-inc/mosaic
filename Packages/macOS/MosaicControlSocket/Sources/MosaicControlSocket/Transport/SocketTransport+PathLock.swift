...
extension SocketTransport {
    ...
    /// reusable lock marker, based on the well-known mosaic socket filenames
    ...
    func canReplaceUnmarkedRefusedSocket(at path: String) -> Bool {
        let filename = URL(fileURLWithPath: (path as NSString).standardizingPath)
            .lastPathComponent
            .lowercased()
        let recoverablePrefixes = ["mosaic-debug-", "mosaic-nightly-", "mosaic-staging-"]
        return filename == "mosaic-debug.sock" ||
            filename == "mosaic-nightly.sock" ||
            filename == "mosaic-staging.sock" ||
