...
/// localized row text, the mosaic.json paths owned by that row, and a
...
/// `[CuratedSettingEntry].mosaicDefault` constant.
public struct CuratedSettingEntry: Sendable, Hashable {
    ...
    /// Dotted mosaic.json paths that should scroll to this search entry's
    ...
    /// Dotted mosaic.json path used as the scroll/highlight anchor.
    ///
