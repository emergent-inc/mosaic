...
/// opening the user's editor on mosaic.json, sending feedback, posting
...
public protocol SettingsHostActions: AnyObject {
    ...
    /// Opens the mosaic JSON config file in the user's preferred
    ...
    /// Opens System Settings to the mosaic notifications pane so the
    /// user can grant / revoke OS-level notification permission.
    func openSystemNotificationSettings()

    /// Restarts the mosaic app. Used after the user changes the
    ...
    /// Opens the mosaic terminal config preview window (the legacy
