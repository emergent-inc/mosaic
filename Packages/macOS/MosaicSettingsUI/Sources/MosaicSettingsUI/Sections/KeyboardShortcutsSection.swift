...
public struct KeyboardShortcutsSection: View {
    ...
    private var chordsRow: some View {
        SettingsCardRow(
            ...
            subtitle: String(localized: "settings.shortcuts.chords.subtitle", defaultValue: "Add tmux-style multi-step shortcuts in mosaic.json, for example [\"ctrl+b\", \"c\"].")
        ) {
            HStack(spacing: 8) {
                Link(
                ...
                Button(String(localized: "settings.app.settingsFile.openButton", defaultValue: "Open mosaic.json")) {
    ...
    private func streamBindings() async {
        ...
        let task = Task {
            for await dictionary in jsonStore.values(for: catalog.shortcuts.bindings) {
                ...
                // non-unbound (e.g. the user edited mosaic.json directly to
                ...
                // shortcut changes. Externally-edited mosaic.json should
    ...
    /// external `mosaic.json` edit or another settings surface restoring a usable
    ...
    private func pruneConflictRejections() {
        ...
        // (mosaic.json reload) dismisses the validation banner too.
