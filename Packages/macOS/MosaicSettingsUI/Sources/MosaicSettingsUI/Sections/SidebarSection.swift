...
public struct SidebarSection: View {
    ...
    public init(defaultsStore: UserDefaultsSettingsStore, catalog: SettingCatalog, hostActions: SettingsHostActions) {
        ...
        _prLinks = State(initialValue: DefaultsValueModel(store: defaultsStore, key: catalog.sidebar.openPullRequestLinksInMosaicBrowser))
        _portLinks = State(initialValue: DefaultsValueModel(store: defaultsStore, key: catalog.sidebar.openPortLinksInMosaicBrowser))
    ...
    private var mainCard: some View {
        SettingsCard {
                ...
                VStack(alignment: .trailing, spacing: 4) {
                    HStack(spacing: 8) {
                        ...
                        Text(String.localizedStringWithFormat(String(localized: "settings.fontSize.valuePoints", defaultValue: "%@ pt"), hostActions.formattedFontSize(sidebarFont.points)))
                            .mosaicFont(size: 12, weight: .medium, design: .rounded)
                            .monospacedDigit()
            ...
            SettingsCardRow(
                configurationReview: .json("sidebar.openPullRequestLinksInMosaicBrowser"),
                String(localized: "settings.app.openSidebarPRLinks", defaultValue: "Open Sidebar PR Links in mosaic Browser"),
                subtitle: prLinksSubtitle(prVisible: showPR.current, prClickable: prClickable.current, openInMosaic: prLinks.current)
            ) {
            ...
            SettingsCardRow(
                configurationReview: .json("sidebar.openPortLinksInMosaicBrowser"),
    ...
    private func prLinksSubtitle(prVisible: Bool, prClickable: Bool, openInMosaic: Bool) -> String {
        if !prVisible {
