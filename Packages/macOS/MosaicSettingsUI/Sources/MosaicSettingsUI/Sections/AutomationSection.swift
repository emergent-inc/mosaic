...
public struct AutomationSection: View {
    ...
    private var socketControlCard: some View {
        ...
        SettingsCard {
            ...
            SettingsCardNote(String(localized: "settings.automation.socketOverrides.note", defaultValue: "Overrides: MOSAIC_SOCKET_ENABLE, MOSAIC_SOCKET_MODE, and MOSAIC_SOCKET_PATH (set MOSAIC_ALLOW_SOCKET_OVERRIDE=1 for stable/nightly builds)."))
        }
    }
    ...
    private var ampCard: some View {
        SettingsCard {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.amp.note", defaultValue: "Hooks must be installed with `mosaic hooks amp install`. They no-op outside mosaic terminals. When disabled, the installed Amp plugin stays inactive without needing to be removed."))
        }
    }
    ...
    private var cursorCard: some View {
        SettingsCard {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.cursor.note", defaultValue: "Hooks must be installed with `mosaic hooks cursor install`. They no-op outside mosaic terminals."))
        }
    }
    ...
    private var geminiCard: some View {
        SettingsCard {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.gemini.note", defaultValue: "Hooks must be installed with `mosaic hooks gemini install`. They no-op outside mosaic terminals."))
        }
    }
    ...
    private var kiroCard: some View {
        SettingsCard {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.kiro.note", defaultValue: "Hooks must be installed with `mosaic hooks kiro install`, then run Kiro with `kiro-cli chat --agent mosaic` (or set it as your default agent). They no-op outside mosaic terminals."))
        }
    }
    ...
    private var portCard: some View {
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: String(localized: "settings.automation.portBase.subtitle", defaultValue: "Starting port for MOSAIC_PORT env var."),
                controlWidth: Self.columnWidth
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.port.note", defaultValue: "Each workspace gets MOSAIC_PORT and MOSAIC_PORT_END env vars with a dedicated port range. New terminals inherit these values."))
        }
    }
