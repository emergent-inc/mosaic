...
/// underlying mosaic.json path next to the row when the host enables
/// "show config paths".
///
...
public struct SettingsCardRow<Trailing: View>: View {
    ...
    // map the mosaic.json path(s) it declares via `configurationReview`
    ...
    /// custom-control rows that don't write a single mosaic.json key);
