...
public struct TerminalSurfaceSpawnPolicy: Sendable {
    ...
    /// Whether Cursor hooks are enabled (`MOSAIC_CURSOR_HOOKS_DISABLED`).
    public var cursorHooksEnabled: Bool

    /// Whether Gemini hooks are enabled (`MOSAIC_GEMINI_HOOKS_DISABLED`).
    public var geminiHooksEnabled: Bool

    /// Whether Kiro hooks are enabled (`MOSAIC_KIRO_HOOKS_DISABLED`).
    public var kiroHooksEnabled: Bool

    /// The Kiro notification level raw value
    /// (`MOSAIC_KIRO_NOTIFICATION_LEVEL`).
    public var kiroNotificationLevel: String

    /// Whether Amp hooks are enabled (`MOSAIC_AMP_HOOKS_DISABLED`).
    public var ampHooksEnabled: Bool

    /// Whether mosaic shell integration is enabled (the
    /// `sidebarShellIntegration` setting).
    public var shellIntegrationEnabled: Bool

    /// Whether sidebar git-status watching is enabled (`MOSAIC_NO_GIT_WATCH`).
    public var watchGitStatusEnabled: Bool

    /// Whether sidebar pull-request watching is enabled (`MOSAIC_NO_PR_WATCH`).
