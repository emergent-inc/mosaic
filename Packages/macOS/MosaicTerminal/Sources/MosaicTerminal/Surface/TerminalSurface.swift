...
public final class TerminalSurface: Identifiable, ObservableObject {
    ...
    // The surface value DTOs live in MosaicTerminalCore; these aliases keep the
    // nested TerminalSurface.NamedKeySendResult/.InputSendResult names that
    // other files use.
    public typealias NamedKeySendResult = MosaicTerminalCore.NamedKeySendResult
    public typealias InputSendResult = MosaicTerminalCore.InputSendResult
    ...
    public typealias MosaicContextEnvironment = TerminalSurfaceMosaicContextEnvironment
    ...
    /// Port ordinal base/range for MOSAIC_PORT assignment, snapshotted by the app composition root.
    ...
    /// mosaic renderer reclamation: whether the current runtime surface's GPU
    ...
    /// Port ordinal for MOSAIC_PORT range assignment. Captured at construction so
    /// every runtime startup path uses the same immutable workspace port range.
    let portOrdinal: Int
    let surfaceContext: ghostty_surface_context_e
    let configTemplate: MosaicSurfaceConfigTemplate?
    ...
    /// The retained byte-tee lease for the libghostty PTY tee callback (mosaic
    ...
    static func mosaicContextEnvironment(
        workspaceId: UUID,
        surfaceId: UUID,
        socketPath: String
    ) -> MosaicContextEnvironment {
        MosaicContextEnvironment(
