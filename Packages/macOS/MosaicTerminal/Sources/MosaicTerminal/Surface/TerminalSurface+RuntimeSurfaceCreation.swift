...
extension TerminalSurface {
    ...
    func createNativeRuntimeSurface(
        ...
        var baseConfig = configTemplate ?? MosaicSurfaceConfigTemplate()
        var surfaceConfig = ghostty_surface_config_new()
        let magnificationPercent = globalFontMagnificationPercent()
        surfaceConfig.font_size = MosaicSurfaceConfigTemplate.runtimeFontSize(
        ...
        Self.applyManagedMosaicContextEnvironment(
            Self.mosaicContextEnvironment(
                workspaceId: tabId,
                surfaceId: id,
        ...
        setManagedEnvironmentValue("MOSAIC_SOCKET", "")
        ...
        if let bundledCLIURL = Bundle.main.resourceURL?.appendingPathComponent("bin/mosaic"),
           runtimeFilesystem.isExecutableFile(bundledCLIURL.path) {
            setManagedEnvironmentValue("MOSAIC_BUNDLED_CLI_PATH", bundledCLIURL.path)
        }
        if let bundleId = Bundle.main.bundleIdentifier, !bundleId.isEmpty {
            setManagedEnvironmentValue("MOSAIC_BUNDLE_ID", bundleId)
        }
        ...
        do {
            ...
            setManagedEnvironmentValue("MOSAIC_PORT", String(startPort))
            setManagedEnvironmentValue("MOSAIC_PORT_END", String(startPort + sessionPortRangeSize - 1))
            setManagedEnvironmentValue("MOSAIC_PORT_RANGE", String(sessionPortRangeSize))
        }

        let spawnPolicy = spawnPolicyProvider.currentSpawnPolicy()
        let claudeHooksEnabled = spawnPolicy.claudeHooksEnabled
        if !claudeHooksEnabled {
            setManagedEnvironmentValue("MOSAIC_CLAUDE_HOOKS_DISABLED", "1")
        }
        ...
        if !spawnPolicy.codexHooksEnabled {
            setManagedEnvironmentValue("MOSAIC_CODEX_HOOKS_DISABLED", "1")
        }
        if let customClaudePath = spawnPolicy.customClaudePath {
            setManagedEnvironmentValue("MOSAIC_CUSTOM_CLAUDE_PATH", customClaudePath)
        }
        ...
        if !spawnPolicy.cursorHooksEnabled {
            setManagedEnvironmentValue("MOSAIC_CURSOR_HOOKS_DISABLED", "1")
        }
        if !spawnPolicy.geminiHooksEnabled {
            setManagedEnvironmentValue("MOSAIC_GEMINI_HOOKS_DISABLED", "1")
        }
        if !spawnPolicy.kiroHooksEnabled {
            setManagedEnvironmentValue("MOSAIC_KIRO_HOOKS_DISABLED", "1")
        }
        setManagedEnvironmentValue("MOSAIC_KIRO_NOTIFICATION_LEVEL", spawnPolicy.kiroNotificationLevel)
        if !spawnPolicy.ampHooksEnabled {
            setManagedEnvironmentValue("MOSAIC_AMP_HOOKS_DISABLED", "1")
        }
        ...
        if let claudeShim {
            setManagedEnvironmentValue("MOSAIC_CLAUDE_WRAPPER_SHIM", claudeShim.executablePath)
            setManagedEnvironmentValue("MOSAIC_CLAUDE_WRAPPER_SHIM_ROOT", claudeShim.directoryPath)
            ...
            // session resolves the codex executable through MOSAIC_CODEX_WRAPPER_SHIM
            // (see AgentResumeArgv.codexWrapperShellExecutableToken), so without
            // this the resumed codex bypasses mosaic-codex-wrapper and loses its
            ...
            if let codexShim = claudeShim.codexCommandShim {
                setManagedEnvironmentValue("MOSAIC_CODEX_WRAPPER_SHIM", codexShim.executablePath)
                setManagedEnvironmentValue("MOSAIC_CODEX_WRAPPER_SHIM_ROOT", codexShim.directoryPath)
            }
            let currentPath = env["PATH"]
                ?? getenv("PATH").map { String(cString: $0) }
                ?? ProcessInfo.processInfo.environment["PATH"]
        ...
        if spawnPolicy.shellIntegrationEnabled,
           ...
           Self.shellIntegrationDirectoryExists(integrationDir) {
            setManagedEnvironmentValue("MOSAIC_SHELL_INTEGRATION", "1")
            setManagedEnvironmentValue("MOSAIC_SHELL_INTEGRATION_DIR", integrationDir)
