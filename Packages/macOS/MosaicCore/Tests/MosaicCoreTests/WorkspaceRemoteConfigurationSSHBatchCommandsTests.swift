...
struct WorkspaceRemoteConfigurationSSHBatchCommandsTests {
    ...
    func daemonTransportArgumentsWithoutSlot() {
        let arguments = configuration().daemonTransportArguments(remotePath: "/remote/mosaicd-remote")
        let expectedCommand = #"sh -c 'exec '"'"'/remote/mosaicd-remote'"'"' '"'"'serve'"'"' '"'"'--stdio'"'"''"#
        #expect(
            arguments == ["-T"]
                + expectedBatchArguments
                + ["-o", "RequestTTY=no", "mosaic-macmini", expectedCommand]
        )
    }
    ...
    func daemonTransportArgumentsWithSlot() {
        ...
        ).daemonTransportArguments(remotePath: "/remote/mosaicd-remote")
        let expectedCommand = #"sh -c 'exec '"'"'/remote/mosaicd-remote'"'"' '"'"'serve'"'"' '"'"'--stdio'"'"' '"'"'--persistent'"'"' '"'"'--slot'"'"' '"'"'ws-1'"'"''"#
        #expect(
            arguments == ["-T"]
                + expectedBatchArguments
                + ["-o", "RequestTTY=no", "mosaic-macmini", expectedCommand]
        ...
        #expect(
            arguments == ["-N", "-T", "-S", "none"]
                ...
                + [
                    ...
                    "-o", "RequestTTY=no",
                    "-L", "127.0.0.1:64123:/run/mosaicd-remote.sock",
                    "mosaic-macmini",
                ]
        )
    }
    ...
    func reverseRelayControlMasterArguments() throws {
        ...
        #expect(
            arguments == expectedBatchArguments
                + ["-O", "forward", "-R", "127.0.0.1:64007:127.0.0.1:54321", "mosaic-macmini"]
        )
    }
    ...
    func reverseRelayControlMasterCancelArguments() throws {
        ...
        #expect(
            arguments == expectedBatchArguments
                + ["-O", "cancel", "-R", "127.0.0.1:64007", "mosaic-macmini"]
        )
    }
