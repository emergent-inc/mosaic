...
public struct WorkspaceRemoteConfiguration: Equatable, Sendable {
    ...
    /// CLI relay port for remote `mosaic` command forwarding.
    ...
    /// True for cloud-VM remotes (Freestyle snapshots) where mosaicd-remote is pre-baked in
    ...
    /// the baked daemon through an SSH local forward to `/run/mosaicd-remote.sock`.
