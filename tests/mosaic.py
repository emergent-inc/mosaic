#!/usr/bin/env python3
"""
mosaic Python Client

A client library for programmatically controlling mosaic via Unix socket.
...
# The control socket, markers, and password live in the non-TCC mosaic state
# directory (~/.local/state/mosaic), not Application Support, so the separately
# signed mosaic CLI can touch them without the macOS "access data from other apps"
# prompt (https://github.com/emergent-inc/mosaic/issues/5146).
_STATE_DIR = os.path.expanduser("~/.local/state/mosaic")
_STABLE_SOCKET_PATH = os.path.join(_STATE_DIR, "mosaic.sock")
...
def _is_known_mosaic_bundle_id(bundle_id: str) -> bool:
    return (
...
def _default_bundle_id() -> str:
    override = (os.environ.get("MOSAIC_BUNDLE_ID") or "").strip()
    if override and _is_known_mosaic_bundle_id(override):
        return override

    tag = os.environ.get("MOSAIC_TAG")
...
def _socket_variant() -> Tuple[str, Optional[str]]:
    ...
    if bundle_id == _DEFAULT_DEBUG_BUNDLE_ID:
        tag = os.environ.get("MOSAIC_TAG", "").strip()
...
def _last_socket_path_files() -> List[str]:
    ...
    if variant == "nightly":
        ...
        tmp_marker = f"/tmp/mosaic-nightly-{slug}-last-socket-path" if slug else "/tmp/mosaic-nightly-last-socket-path"
    elif variant == "staging":
        marker = f"staging-{slug}-last-socket-path" if slug else "staging-last-socket-path"
        tmp_marker = f"/tmp/mosaic-staging-{slug}-last-socket-path" if slug else "/tmp/mosaic-staging-last-socket-path"
    elif variant == "dev":
        marker = f"dev-{slug}-last-socket-path" if slug else "dev-last-socket-path"
        tmp_marker = f"/tmp/mosaic-dev-{slug}-last-socket-path" if slug else "/tmp/mosaic-dev-last-socket-path"
...
def _variant_socket_candidates() -> List[str]:
    variant, slug = _socket_variant()
    if variant == "nightly":
        return [f"/tmp/mosaic-nightly-{slug}.sock"] if slug else ["/tmp/mosaic-nightly.sock"]
    if variant == "staging":
        return [f"/tmp/mosaic-staging-{slug}.sock"] if slug else ["/tmp/mosaic-staging.sock"]
    if variant == "dev":
        if slug:
            return [f"/tmp/mosaic-debug-{slug}.sock"]
        return ["/tmp/mosaic-debug.sock"]
...
def _default_socket_path() -> str:
    ...
    if bundle_id == _DEFAULT_DEBUG_BUNDLE_ID:
        tagged = glob.glob("/tmp/mosaic-debug-*.sock")
        tagged.extend(glob.glob(os.path.join(_STATE_DIR, "mosaic*.sock")))
        tagged = [
            p for p in tagged
            if os.path.exists(p)
            and os.path.basename(p).startswith("mosaic-debug-")
        ]
        if tagged:
...
class mosaic:
    """Client for controlling mosaic via Unix socket"""
    ...
    def _send_command(self, command: str) -> str:
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if time.time() - start >= 5.0:
                        raise mosaicError("Command timed out")
                    continue
                if not chunk:
                    break
                data += chunk.decode()
            ...
            raise mosaicError(f"Socket error: {e}")
    ...
    def seed_drag_pasteboard_types(self, types: List[str]) -> None:
        ...
        if not types:
            raise mosaicError("seed_drag_pasteboard_types requires at least one type")
        payload = ",".join(t.strip() for t in types if t and t.strip())
        if not payload:
            raise mosaicError("seed_drag_pasteboard_types requires at least one non-empty type")
    ...
    def layout_debug(self) -> dict:
        ...
        try:
            ...
            raise mosaicError(f"layout_debug JSON decode failed: {e}: {payload[:200]}")
    ...
    def render_stats(self, panel: Union[str, int, None] = None) -> dict:
        ...
        try:
            ...
            raise mosaicError(f"render_stats JSON decode failed: {e}: {payload[:200]}")
    ...
    def panel_snapshot(self, panel: Union[str, int], label: str = "") -> dict:
        ...
        if len(parts) != 5:
            raise mosaicError(f"panel_snapshot parse failed: {response}")
...
def main():
    """CLI interface for mosaic"""
    import sys
    import argparse

    parser = argparse.ArgumentParser(description="mosaic CLI")
    ...
    try:
        with mosaic(args.socket) as client:
            if not args.command:
                # Interactive mode
                print("mosaic CLI (type 'help' for commands, 'quit' to exit)")
                while True:
                    try:
                        line = input("> ").strip()
