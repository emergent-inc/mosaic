...
SCRIPT="$ROOT_DIR/Resources/bin/start-mosaic-profiling"
TMP_DIR="$(mktemp -d)"
...
EOF
chmod +x "$plist_buddy"
export MOSAIC_PROFILE_PLIST_BUDDY="$plist_buddy"
...
export MOSAIC_PROFILE_DEFAULTS="$defaults_bin"
...
export MOSAIC_PROFILE_SYSTEM_PROFILER="$system_profiler_bin"
...
if "$SCRIPT" --dry-run --test-ps-file "$ps_file" --out "$TMP_DIR/ambiguous" >/tmp/mosaic-profile-ambiguous.log 2>&1; then
  echo "FAIL: unqualified selection should reject multiple mosaic processes" >&2
  exit 1
fi
if ! grep -Fq "multiple mosaic processes are running" /tmp/mosaic-profile-ambiguous.log; then
  echo "FAIL: ambiguous selection did not explain how to discriminate instances" >&2
  cat /tmp/mosaic-profile-ambiguous.log >&2
  exit 1
fi
...
HOME="$TMP_DIR" PATH="$fake_bin:$PATH" MOSAIC_PROFILE_TOC_TIMEOUT_SECONDS=1 "$SCRIPT" \
...
PATH="$fake_bin:$PATH" MOSAIC_PROFILE_SYSTEM_PROFILER="$failing_system_profiler" "$SCRIPT" \
  --test-ps-file "$ps_file" \
  --channel dev \
  --tag dog \
  --duration 1 \
...
PATH="$fake_bin:$PATH" MOSAIC_PROFILE_SYSTEM_PROFILER="$sleep_system_profiler" MOSAIC_PROFILE_SYSTEM_PROFILER_TIMEOUT_SECONDS=1 "$SCRIPT" \
  --test-ps-file "$ps_file" \
  --channel dev \
  --tag dog \
  --duration 1 \
...
if PATH="$fail_bin:$PATH" "$SCRIPT" \
  ...
  --template "Time Profiler" \
  --no-submit \
  --out "$all_failed_out" >/tmp/mosaic-profile-all-failed.log 2>&1; then
  echo "FAIL: all-failed profiling run should exit nonzero" >&2
  exit 1
fi
if ! grep -Fq "all profiling templates failed" /tmp/mosaic-profile-all-failed.log ||
  ...
  cat /tmp/mosaic-profile-all-failed.log >&2
  cat "$all_failed_out/summary.md" >&2
  exit 1
fi

submit_output="$("$ROOT_DIR/Resources/bin/submit-mosaic-profile" --dry-run --profile "$timeout_out" --target-name "Mosaic DEV dog" --target-pid 303 --channel dev --bundle-id mosaic.com.emergent.app.debug.dog --reply-to "user@example.com")"
if [[ "$submit_output" != *"Recipient: contact@emergent.inc"* ]] ||
   [[ "$submit_output" != *"Reply-to: user@example.com"* ]] ||
   [[ "$submit_output" != *"Subject: mosaic profiling capture: Mosaic DEV dog"* ]]; then
...
"$ROOT_DIR/Resources/bin/submit-mosaic-profile" --dry-run --profile "$timeout_out" --target-name "Mosaic DEV dog" >/dev/null
...
EOF
chmod +x "$sleep_ditto"
MOSAIC_PROFILE_DITTO="$sleep_ditto" "$ROOT_DIR/Resources/bin/submit-mosaic-profile" \
  --profile "$timeout_out" \
  --package-only &
...
package_output="$(MOSAIC_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-mosaic-profile" --profile "$timeout_out" --package-only)"
...
MOSAIC_PROFILE_OSASCRIPT="$cancel_bin" MOSAIC_PROFILE_OPEN="$open_bin" MOSAIC_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-mosaic-profile" \
  --profile "$timeout_out" \
...
MOSAIC_PROFILE_OSASCRIPT="$sleep_osascript" MOSAIC_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-mosaic-profile" \
  --profile "$timeout_out" \
  --target-name "Mosaic DEV dog" \
  --target-pid 303 \
  --channel dev \
...
if MOSAIC_PROFILE_OSASCRIPT="$cancel_bin" MOSAIC_PROFILE_OPEN="$open_bin" MOSAIC_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-mosaic-profile" \
  --profile "$timeout_out" \
  --target-name "Mosaic DEV dog" \
  --target-pid 303 \
  --channel dev \
  --bundle-id mosaic.com.emergent.app.debug.dog \
  --send >/tmp/mosaic-profile-send-cancel.log 2>&1; then
  echo "FAIL: submit helper send mode should fail when Mail send is canceled" >&2
  exit 1
fi
if ! grep -Fq "User canceled" /tmp/mosaic-profile-send-cancel.log; then
  echo "FAIL: submit helper send cancellation did not preserve the AppleScript error" >&2
  cat /tmp/mosaic-profile-send-cancel.log >&2
  exit 1
fi
...
HOME="$TMP_DIR" MOSAIC_PROFILE_FEEDBACK_EMAIL=wrong@example.com MOSAIC_PROFILE_LOCALE=ja_JP MOSAIC_PROFILE_OSASCRIPT="$capture_osascript" MOSAIC_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-mosaic-profile" \
  --profile "$timeout_out" \
  --target-name "Mosaic DEV dog" \
  --target-pid 303 \
  --channel dev \
  ...
  --note-file "$note_file" \
  --send
if ! grep -Fq "mosaicプロファイルを送信" "$captured_args" ||
   ! grep -Fq "下書きを開く" "$captured_args" ||
   ! grep -Fq "mosaicプロファイリングキャプチャ" "$captured_args" ||
...
echo "PASS: start-mosaic-profiling target selection and default templates"
