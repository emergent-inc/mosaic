#!/usr/bin/env python3
"""Behavioral guards for mosaicTests CI sharding."""
...
HELPER = ROOT / "scripts" / "ci" / "mosaic_unit_test_shard.py"
...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_root = Path(tmp)
        test_root = tmp_root / "mosaicTests"
        test_root.mkdir()
    ...
    extension_selector = "-only-testing:mosaicTests/LargeSuiteTests/testExtensionRegression"
    ...
    suite_selector = "-only-testing:mosaicTests/LargeSuiteTests"
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for shard in range(1, 5):
            ...
            for focused_selector in (
                "-only-testing:mosaicTests/BrowserSystemProxyMirrorTests",
                "-only-testing:mosaicTests/GhosttyOptionAsAltModsTests",
            ):
    ...
    print("PASS: mosaicTests sharding covers extension methods and leaves focused gates explicit")
    return 0
