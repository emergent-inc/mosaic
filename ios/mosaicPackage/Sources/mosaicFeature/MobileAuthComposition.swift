...
/// ``MosaicAuthRuntime/AuthConfig`` from the environment + an injected
/// `LocalConfig.plist` override table, builds the ``MosaicAuthRuntime/AuthCoordinator``
/// (with a `StackAuthClient`, persistence caches over an injected `UserDefaults`,
/// and an ``MosaicAuthRuntime/AuthPresentationContextProvider``), and the
/// ``MosaicAuthRuntime/PushRegistrationService``. Replaces `AuthManager.shared`,
...
public struct MobileAuthComposition {
    ...
    public init(
        ...
        let teamSelection = MosaicAuthTeamSelectionStore(
            keyValueStore: defaults,
            key: "auth_selected_team"
        )
        let launch = AuthLaunchOptions(
            clearAuthRequested: environment["MOSAIC_UITEST_CLEAR_AUTH"] == "1",
