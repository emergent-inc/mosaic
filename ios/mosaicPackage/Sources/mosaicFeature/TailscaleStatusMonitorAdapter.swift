import MosaicMobileShellUI
import MosaicMobileTransport

/// Adapts the transport layer's ``MosaicMobileTransport/TailscaleStatusMonitor``
/// onto the shell UI's read-only ``MosaicMobileShellUI/TailscaleStatusObserving``
/// port, so `MosaicMobileShellUI` stays decoupled from the concrete system
