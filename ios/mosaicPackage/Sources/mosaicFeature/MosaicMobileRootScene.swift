...
import MosaicMobileAnalytics
import MosaicMobilePairedMac
import MosaicMobileShell
import MosaicMobileShellModel
import MosaicMobileSupport
@_exported import MosaicMobileShellUI
...
private let mobileRootSceneLog = Logger(subsystem: "dev.mosaic.ios", category: "mobile-root-scene")

/// Top-level mobile scene root.
///
/// Renders the live mosaic mobile UI: a ``MosaicMobileAppView`` backed by a fresh
/// ``MosaicMobileShellStore`` and the injected ``AuthCoordinator``. In DEBUG
/// builds, setting the environment variable `MOSAIC_ZOOM_STRESS=1` instead mounts
/// the terminal zoom-stress repro harness (`MobileZoomStressView`).
///
/// The composition root (`mosaicApp`) builds the ``MosaicMobileRuntime`` and the
...
public struct MosaicMobileRootScene: View {
    private let runtime: MosaicMobileRuntime
    ...
    private var content: some View {
        ...
        } else if ProcessInfo.processInfo.environment["MOSAIC_ZOOM_STRESS"] == "1" {
            MobileZoomStressView()
        } else {
            MosaicMobileAppView(store: makeStore(), onboardingStore: onboardingStore)
        }
        #else
        MosaicMobileAppView(store: makeStore(), onboardingStore: onboardingStore)
        #endif
        #else
        MosaicMobileAppView(store: makeStore())
        #endif
    }

    @MainActor
    private func makeStore() -> MosaicMobileShellStore {
        let coordinator = auth.coordinator
